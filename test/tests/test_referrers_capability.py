...
def test_sign_writes_capability_cache_with_fresh_expiry(
    ...
    result = _sign(ocx, sigstore_stack, identity_token, published_package)
...
def test_second_sign_reuses_cached_capability_without_reprobing(
    ...
    wall-clock time.
    """
    first = _sign(ocx, sigstore_stack, identity_token, published_package)
    assert first.returncode == 0, first.stderr

    cache_path = _cache_path(ocx)
    first_probed_at = json.loads(cache_path.read_text())["probed_at"]

    second = _sign(ocx, sigstore_stack, identity_token, published_package)
