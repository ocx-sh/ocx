...
* Flags are ``--records-dir DIR`` / ``--records-name TEMPLATE`` and exist on
  ``ocx run`` and ``ocx package exec`` only. ``ocx launcher exec`` takes none —
...
Test inventory
--------------
 1. test_run_frame_emits_record                                  — three frames
...
def _package_root(ocx: OcxRunner, short: str) -> Path:
    ...
    location = result.get(short) if isinstance(result, dict) else None
    assert location is not None, f"ocx package which must report a path for {short!r}"
    return Path(location)


def _project_with_tool(ocx: OcxRunner, tmp_path: Path, pkg: PackageInfo) -> Path:
    """Create a locked single-binding project directory for ``ocx run``."""
...
# ---------------------------------------------------------------------------
# 1-3. Three recording frames — run, package exec, launcher exec
# ---------------------------------------------------------------------------


def test_run_frame_emits_record(
    ocx: OcxRunner, published_package: PackageInfo, tmp_path: Path
) -> None:
    """``ocx run --records-dir`` writes one record for the project tier.
    ...
    sink = _sink(tmp_path, "run")

    result = _run_in(ocx, project, "run", "--records-dir", str(sink), "--", "hello")
    ...
    assert record["frame"]["command"] == "run", (
        f"the run frame reports frame.command='run'; got {record['frame']}"
    )
    assert record["scope"]["tier"] == "project", (
        f"ocx run records the project tier; got scope={record['scope']}"
    )
...
def test_root_digest_is_the_platform_leaf_never_the_index(
    ...
    make_package(ocx, unique_repo, "1.0.0", tmp_path / "host", platform=host_platform, new=True)
    make_package(ocx, unique_repo, "1.0.0", tmp_path / "foreign", platform=foreign, new=False)
...
def test_registries_name_the_content_host_not_the_logical_namespace(
    ...
    """
    package = make_package(ocx, unique_repo, "1.0.0", tmp_path, new=True, index=False)
