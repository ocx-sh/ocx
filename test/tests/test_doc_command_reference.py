...
* ``lock`` — references ``ocx.lock`` and ``declaration_hash``, includes
  an exit-code table.
* ``hook-env`` — references the ``prompt-hook`` flow and the
  ``OCX_PROMPT_HASH`` fingerprint.
...
NEW_COMMAND_ANCHORS = [
    ...
    ("{#shell-profile-generate}", "shell profile generate"),
    ("{#generate}", "generate"),
...
def test_hook_env_section_references_prompt_hook(cli_ref_text: str) -> None:
    """`hook-env` must reference the `prompt-hook` flow it serves."""
    section = _slice_section_by_anchor(cli_ref_text, "{#hook-env}")
    assert "prompt-hook" in section.lower() or "prompt hook" in section.lower(), (
        "`hook-env` section must reference the `prompt-hook` flow "
        "(it's the prompt-side machinery that calls hook-env)"
    )


def test_hook_env_section_references_prompt_hash_fingerprint(
    cli_ref_text: str,
) -> None:
    ...
    assert "_OCX_APPLIED" in section, (
        "`hook-env` section must mention the prompt-hash fingerprint env "
        "var `_OCX_APPLIED`"
    )
