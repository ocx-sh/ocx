...
def _bundle(ocx: OcxRunner, layer_dir: Path, tmp_path: Path, name: str, platform: str) -> Path:
    ...
    ocx.plain(
        "package", "create", "-m", str(metadata_path), "-p", platform,
        "-o", str(bundle), str(layer_dir),
    )
    return bundle
...
def test_package_push_mount_cross_repository_reuse(
    ...
    bundle = _bundle(ocx, layer_dir, tmp_path, "shared-layer", plat)
