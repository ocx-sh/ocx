...
Covers ``ocx shell-hook``, ``ocx hook-env``, ``ocx shell init`` per
...
---------------------------------------
The Phase 7 stubs at ``crates/ocx_cli/src/command/{shell_hook,hook_env,shell_init}.rs``
...
Test inventory
--------------
1.  ``test_shell_hook_emits_bash_exports``
2.  ``test_shell_hook_skips_uninstalled_tool_with_stderr_note``
3.  ``test_shell_hook_warns_on_stale_lock``
4.  ``test_shell_hook_no_project_emits_nothing``
5.  ``test_hook_env_unchanged_fingerprint_emits_empty``
6.  ``test_hook_env_changed_fingerprint_emits_unset_and_reexport``
7.  ``test_hook_env_no_prior_applied_emits_full_set``
8.  ``test_hook_env_v2_payload_treated_as_changed`` (architect-mandated)
...
13. ``test_remote_flag_does_not_force_network_for_shell_hook``
14. ``test_remote_flag_does_not_force_network_for_hook_env``
15. ``test_hook_env_empty_applied_treated_as_no_prior_state`` (Round 2)
16. ``test_shell_hook_missing_lock_present_toml_emits_stderr_and_exits_zero`` (Round 2)
"""
...
# ---------------------------------------------------------------------------
# 1. shell-hook emits valid bash exports for installed tools
# ---------------------------------------------------------------------------


def test_shell_hook_emits_bash_exports(ocx: OcxRunner, tmp_path: Path) -> None:
...
""",
    ...
    result = _run(ocx, project, "shell-hook", "--shell", "bash")

    assert result.returncode == EXIT_SUCCESS, (
        f"shell-hook must succeed; rc={result.returncode}\nstderr:\n{result.stderr}"
    )
    assert "export" in result.stdout, (
        f"expected at least one export line; got stdout:\n{result.stdout}"
    )
    ok, syn_err = _bash_syntax_ok(result.stdout)
    assert ok, f"shell-hook output failed bash -n: {syn_err}\nstdout:\n{result.stdout}"


# ---------------------------------------------------------------------------
# 2. shell-hook skips uninstalled tool with a stderr note
# ---------------------------------------------------------------------------


def test_shell_hook_skips_uninstalled_tool_with_stderr_note(
...
# ---------------------------------------------------------------------------
# 3. shell-hook with stale lock → stderr warning, exports use stale digests
# ---------------------------------------------------------------------------


def test_shell_hook_warns_on_stale_lock(ocx: OcxRunner, tmp_path: Path) -> None:
    """Plan §7 line 770: stale lock → stderr warning, continue with stale digests.

    Distinct from ``ocx exec`` which exits 65 on stale lock. ``shell-hook``
...
""",
    ...
    assert result.returncode == EXIT_SUCCESS, (
        f"shell-hook with stale lock must NOT fail (unlike ocx exec); "
...
def test_shell_hook_no_project_emits_nothing(ocx: OcxRunner, tmp_path: Path) -> None:
...
# ---------------------------------------------------------------------------
# 5. hook-env fast path: unchanged fingerprint → empty output
# ---------------------------------------------------------------------------


def test_hook_env_unchanged_fingerprint_emits_empty(
    ocx: OcxRunner, tmp_path: Path
) -> None:
...
""",
    ...
    first = _run(ocx, project, "hook-env", "--shell", "bash")
    assert first.returncode == EXIT_SUCCESS, first.stderr
    applied = _extract_applied(first.stdout)
    assert applied is not None, (
        f"first hook-env must export _OCX_APPLIED; got:\n{first.stdout}"
    )

    second = _run(
        ocx, project, "hook-env", "--shell", "bash",
...
# ---------------------------------------------------------------------------
# 6. hook-env diff path: change → unset old + new export + new applied
# ---------------------------------------------------------------------------


def test_hook_env_changed_fingerprint_emits_unset_and_reexport(
    ocx: OcxRunner, tmp_path: Path
) -> None:
...
# ---------------------------------------------------------------------------
# 7. hook-env with no prior applied → full export set, no unset
# ---------------------------------------------------------------------------


def test_hook_env_no_prior_applied_emits_full_set(
    ocx: OcxRunner, tmp_path: Path
) -> None:
...
""",
    ...
    cmd = _ocx_cmd(ocx, "hook-env", "--shell", "bash")
    ...
    assert result.returncode == EXIT_SUCCESS, result.stderr
    assert "export" in result.stdout, (
        f"first hook-env run must emit exports; got stdout:\n{result.stdout}"
    )
    assert _extract_applied(result.stdout) is not None, (
        f"first hook-env run must set _OCX_APPLIED; got stdout:\n{result.stdout}"
    )
...
def test_hook_env_v2_payload_treated_as_changed(
    ocx: OcxRunner, tmp_path: Path
) -> None:
...
""",
    ...
    assert "export" in result.stdout, (
        f"hook must still emit exports under --remote; got stdout:\n{result.stdout}"
    )


def test_remote_flag_does_not_force_network_for_hook_env(
    ocx: OcxRunner, tmp_path: Path
) -> None:
    """Architect: ``--remote hook-env`` with ``OCX_OFFLINE=1`` → still succeeds."""
...
""",
    ...
    result = _run(
        ocx, project, "--remote", "hook-env", "--shell", "bash",
        extra_env={"OCX_OFFLINE": "1"},
    )

    assert result.returncode == EXIT_SUCCESS, (
        f"--remote + OCX_OFFLINE=1 must still succeed for hook-env; "
        f"rc={result.returncode}\nstderr:\n{result.stderr}"
    )
    assert _extract_applied(result.stdout) is not None, (
        f"hook-env must still emit _OCX_APPLIED under --remote; "
        f"got stdout:\n{result.stdout}"
    )
...
def test_hook_env_empty_applied_treated_as_no_prior_state(
    ocx: OcxRunner, tmp_path: Path
) -> None:
...
# ---------------------------------------------------------------------------
# 16. (Round 2 W4) shell-hook with ocx.toml but no ocx.lock
# ---------------------------------------------------------------------------


def test_shell_hook_missing_lock_present_toml_emits_stderr_and_exits_zero(
    ocx: OcxRunner, tmp_path: Path
) -> None:
    """Project has ``ocx.toml`` but no ``ocx.lock`` → exit 0, stderr mentions lock.

    Mirrors the prompt-hook contract from plan §7 line 770: the shell-hook
...
""",
    ...
    assert result.returncode == EXIT_SUCCESS, (
        f"missing lock must NOT fail shell-hook; rc={result.returncode}\n"
        f"stderr:\n{result.stderr}"
    )
