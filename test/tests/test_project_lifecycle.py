...
def test_init_add_update_pull_remove_roundtrip(
    ...
    3. ``ocx upgrade``        — whole-file re-resolve of every declared tag
    ...
    # Step 3: ocx upgrade (whole-file re-resolve)
    update_r = _run(ocx, project_dir, "upgrade")
    assert update_r.returncode == EXIT_SUCCESS, (
        f"ocx upgrade failed: rc={update_r.returncode}, stderr={update_r.stderr!r}"
    )
