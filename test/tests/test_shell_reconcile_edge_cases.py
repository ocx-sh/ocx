...
def test_ec_fp_001_same_size_same_second_write_is_invisible(arena: Arena) -> None:
    ...
    output = matrix.pty_session(
        ...
        [
            ...
            f"printf '%s' '{edited}' > '{project / 'ocx.toml'}'",
            f"touch -d @{original_stat.st_mtime!r} '{project / 'ocx.toml'}'",
            "true",
...
def test_ec_scope_005a_a_git_boundary_inside_the_project_tree_is_retained_as_indeterminate(arena: Arena) -> None:
    ...
    reading ``activate.rs::walk_is_indeterminate``), a walk miss while CWD is
    still an ancestor-or-self of the previously-applied project's directory
    AND that project's ``ocx.toml`` is still a regular file is classified
...
def test_ec_scope_006_a_transient_git_read_failure_below_the_project_boundary(arena: Arena) -> None:
    ...
    — the real implementation uses PascalCase (``Walk::Indeterminate``, in
    ``crates/ocx_cli/src/command/self_group/activate.rs::walk_is_indeterminate``),
    which the grep silently missed. Confirmed shipped by direct code read and
    by this test's own green result below.
    """
    ...
    assert _read(result, "during_chmod") == "v1", (
        f"A-11 IS shipped (confirmed via activate.rs::walk_is_indeterminate, Walk::Indeterminate/Determinate/"
...
_UNCOVERED_ROWS = frozenset(
    {
        ...
        "EC-HOOK-009",  # Windows PowerShell 5.1 — Windows runner leg (WP-18)
        "EC-PATH-013",  # real cmd.exe — Windows runner leg (WP-18)
        "EC-QUOTE-004",  # Batch FOR /F — rust-unit half unwritten, Windows leg WP-18
        "EC-QUOTE-010",  # Batch growth proof — rust-unit half unwritten, Windows leg WP-18
        "EC-QUOTE-011",  # Batch precondition — rust-unit half unwritten, Windows leg WP-18
        "EC-SIZE-003",  # 32767-char block cap is enforced only on Windows (WP-18)
...
def test_traceability_every_pytest_and_manual_row_names_a_real_covering_test() -> None:
    """Every register row whose Test tier names ``pytest-hostshell``/``pytest-shellzoo`` (even combined), or is exactly ``manual-only``, must have a Coverage cell naming something real in THIS module."""
    if _REGISTER_PATH is None:
        pytest.skip(
            "the edge-case register is not reachable from "
            f"{Path(__file__).resolve()} — no ancestor holds "
    ...
    assert len(register) == 223, f"the register must still parse to exactly 223 rows; got {len(register)}"
    test_to_ids = _this_modules_test_to_ids()
    known_test_names = set(test_to_ids.keys())
...
def test_summary_is_silent_when_the_prompt_changes_nothing(arena: Arena) -> None:
    ...
    spoken = [line for line in settled.stdout.splitlines() if "'ocx: " in line]
