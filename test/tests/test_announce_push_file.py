...
def test_push_reports_keep_tags_and_keeps_them_out_of_the_tags_file(
    ocx: OcxRunner, unique_repo: str, tmp_path: Path
) -> None:
    """`PushReport.keep_tags_written` names every `__ocx.keep.<algorithm>-<hex>`
    tag the push wrote, and the `--tags-file` it wrote alongside contains
    none of them.

    Two halves of one rule (D7). A keep tag names a manifest by its own digest,
    not a version: the push genuinely writes it (so the report must say so —
    the report states what reached the registry), and announce would drop it
    (so the file that feeds `announce --tags-file` must not carry it in
    the first place). Reporting is not publishing.

    Non-vacuity: the report's keep list must be non-empty. `push` writes
    one keep tag per distinct platform manifest unless `--no-keep-tag` is
    given, so an empty list here means the push path changed and the file
    ...
    keep_tags = report["keep_tags_written"]
    assert keep_tags, "a default push must write at least one keep tag, or this test excludes nothing"
    ...
    for tag in keep_tags:
        assert tag not in file_tags, (
            f"the tags file feeds `announce --tags-file`; a keep tag is not a version "
            f"and must never enter it: {tag} in {file_tags}"
        )


def test_push_still_reports_when_the_tags_file_cannot_be_written(
    ocx: OcxRunner, unique_repo: str, tmp_path: Path
) -> None:
    """The tags file is a scratch artifact written AFTER the registry push
    ...
    assert result.returncode != 0, "a tags file the caller asked for and did not get is a failure"
