...
def _find_blob_data_for_digest(shared_cache: Path, digest: str) -> Path:
    """Locate the CAS ``data`` file for an ImageIndex manifest ``digest``.
...
def test_retain_all_when_peer_blob_absent_on_cleaner(
    ...
    Scenario:
    1. Runner A pins pkg v1.0.0 via a project ``ocx.lock`` (OCX_SHARED_STORE=true)
       and pulls it: the package dir AND the ImageIndex manifest blob land in the
       shared cache; A's shared-roots ledger records the ImageIndex digest.
    2. We DELETE the ImageIndex manifest blob from the shared cache — simulating
       the per-instance-cache deployment where the cleaner lacks the blob even
       though the peer's package directory is present on the shared volume.
    ...
    # The shared-roots ledger records the ImageIndex manifest digest. Read it
    # from A's lock file (the `pinned` field) so the deletion targets exactly the
    # blob the peer digest references.
    lock_text = (proj_a / "ocx.lock").read_text(encoding="utf-8")
    digest_match = re.search(r"@(sha256:[0-9a-f]{64})", lock_text)
    assert digest_match is not None, f"no pinned digest found in lock:\n{lock_text}"
    image_index_digest = digest_match.group(1)
    ...
    blob_data = _find_blob_data_for_digest(shared_store.shared_cache, image_index_digest)
    blob_data.unlink()
    assert not blob_data.exists(), "precondition: ImageIndex manifest blob removed"
