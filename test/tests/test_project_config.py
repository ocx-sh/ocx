...
tests here assert that the CLI accepts ``--project`` / ``OCX_PROJECT_FILE``,
...
# ---------------------------------------------------------------------------
# OCX_PROJECT_FILE env var
# ---------------------------------------------------------------------------


def test_project_env_var_accepts_valid_file(ocx: OcxRunner, tmp_path: Path) -> None:
    """``OCX_PROJECT_FILE=<valid>`` reaches the command without a config error."""
    project_file = tmp_path / "ocx.toml"
    project_file.write_text("")
    ...
    result = _run_with_env(
        ...
        "index",
        "catalog",
        extra_env={"OCX_PROJECT_FILE": str(project_file)},
    )

    assert result.returncode == 0, (
        f"OCX_PROJECT_FILE=<valid> should not abort the command; "
        f"rc={result.returncode}, stderr={result.stderr!r}"
    )


def test_project_env_var_missing_file_exits_79(ocx: OcxRunner) -> None:
    """``OCX_PROJECT_FILE=<missing>`` exits 79 (NotFound).
    ...
    result = _run_with_env(
        ...
        "index",
        "catalog",
        extra_env={"OCX_PROJECT_FILE": missing},
    )

    assert result.returncode == 79, (
        f"OCX_PROJECT_FILE=<missing> should exit 79 (NotFound), got "
...
def test_project_config_loads_via_env_var(
    ...
    """``OCX_PROJECT_FILE=fixture/ocx.toml`` parses without erroring.
    ...
    assert result.returncode == 0, (
        f"valid ocx.toml via OCX_PROJECT_FILE should be accepted; "
        f"rc={result.returncode}, stderr={result.stderr!r}"
    )
