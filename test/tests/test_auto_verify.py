...
Identity and issuer come from the stack fixture rather than literals — the local
dex mints exactly one identity, and restating it here would be a second source
of truth for it. Sign and install target the host platform so the signed
...
def _write_operator_policy(ocx: OcxRunner, scope: str, identity: str, issuer: str) -> None:
...
def test_no_verify_skips_with_single_warn(
    ...
    _write_operator_policy(ocx, _policy_scope(ocx, v1), sigstore_stack.identity, sigstore_stack.issuer)
...
def test_verify_flag_overrides_env_opt_out(
    ...
    pkg = published_package
    _sign(ocx, sigstore_stack, identity_token, pkg)
    _write_operator_policy(ocx, _policy_scope(ocx, pkg), BAD_IDENTITY, sigstore_stack.issuer)  # would fail if verified
    ...
    forced = _run(ocx, "install", pkg.short, flags=("--verify",), extra_env={"OCX_NO_VERIFY": "1", **_tuf(sigstore_stack)})
...
def test_pull_is_policy_gated(
    ...
    result = _run(ocx, "pull", pkg.short, extra_env=_tuf(sigstore_stack))
    assert result.returncode == 77, (
...
def test_offline_auto_verify_with_pinned_material(
    ...
    Install carries no `--rekor-url`, so any key fetch would go to the DEFAULT
    public endpoint — which `OCX_OFFLINE=1` forbids, and whose key never signed
    this SET. A pass therefore proves the pinned key was used and no
    Sigstore-services request was made.
    """
    pkg = published_package
    _sign(ocx, sigstore_stack, identity_token, pkg)
    _write_operator_policy(ocx, _policy_scope(ocx, pkg), sigstore_stack.identity, sigstore_stack.issuer)

    online = _run(ocx, "install", pkg.short, extra_env=_tuf(sigstore_stack))
    assert online.returncode == 0, f"online install (cache warm) failed: {online.stderr.strip()}"

    offline = _run(ocx, "install", pkg.short, extra_env={"OCX_OFFLINE": "1", **_tuf(sigstore_stack)})
...
def test_offline_auto_verify_bad_policy_still_enforced(
    ...
    policy to a wrong identity, then re-install OFFLINE. The signature verifies
    cryptographically from the pinned key (no fetch — see that test), but the
    ...
    # Flip the policy to a wrong signer: the crypto still passes from the pinned
    # key, so only the identity check can produce the failure below.
    _write_operator_policy(ocx, _policy_scope(ocx, pkg), BAD_IDENTITY, sigstore_stack.issuer)
...
def test_run_is_policy_gated(
    ocx: OcxRunner, published_package: PackageInfo, sigstore_stack: SigstoreStack, tmp_path,
) -> None:
