...
def test_setup_no_modify_path_via_env_var(ocx: OcxRunner, tmp_path: Path) -> None:
    ...
    leaving the profile untouched - exactly like the flag.
    """
    _seed_candidate(ocx)
    profile = tmp_path / "profile"
    profile.write_text("# pristine\n")
    ...
    assert result.returncode == 0, f"OCX_NO_MODIFY_PATH=1 setup must exit 0; stderr:\n{result.stderr}"

    payload = json.loads(result.stdout)
    assert payload["profiles"] == [], (
        f"truthy OCX_NO_MODIFY_PATH must skip profile edits; got: {payload!r}"
    )
    assert profile.read_text() == "# pristine\n", "OCX_NO_MODIFY_PATH=1 must leave the profile untouched"
