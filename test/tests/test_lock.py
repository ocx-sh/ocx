...
def _tool_leaf_digests(lock_text: str, name: str) -> list[str]:
    ...
    ``[tool.platforms]`` table. A lock entry has no single ``pinned`` digest —
...
def test_lock_two_tools_produces_valid_lock_file(
    ...
    a correctly shaped V3 ``ocx.lock`` (``[[tool]]`` entries with a bare
    ``repository`` + ``[tool.platforms]`` leaf map, sorted by ``(group,
    name)``, valid declaration hash, ``lock_version = 3``).
    """
...
""",
    ...
    # Metadata header: required fields present. The writer only emits V3.
    assert "lock_version = 3" in lock_text, "lock_version = 3 missing"
    ...
    for name, repo in [("alpha", repo_a), ("beta", repo_b)]:
        ...
        assert f'repository = "{ocx.registry}/{repo}@' not in lock_text, (
            f"repository for {name} must not carry a digest"
        )
        assert f'repository = "{ocx.registry}/{repo}:' not in lock_text, (
            f"repository for {name} must not carry a tag"
        )
    ...
    assert "[tool.platforms]" in lock_text, "lock must carry a [tool.platforms] table"
    assert len(_leaf_digests(lock_text)) >= 2, (
        f"expected at least one leaf digest per tool; full lock:\n{lock_text}"
    )
    assert "pinned =" not in lock_text, "lock must NOT carry a legacy `pinned` line"
...
def test_lock_rejects_upgrade_flag(
    ...
    """``ocx lock --upgrade`` is rejected by clap (exit 64): there is no
    migration flag — a lock written by an unsupported version is rejected
    outright (regenerate with ``ocx lock``). Use ``ocx update`` to force a
    re-resolve of every tag.
    """
    short = uuid4().hex[:8]
...
# ---------------------------------------------------------------------------
# V1/V2 locks are rejected outright — no migration, no bridged read (D3/D5)
# ---------------------------------------------------------------------------


def test_lock_rejects_committed_v1_lock_with_regenerate_hint(
    ocx: OcxRunner, tmp_path: Path
) -> None:
    """A committed **V1** lock (``lock_version = 1`` with a single ``pinned``
    index digest per tool) is rejected outright by any ``ocx`` invocation
    that reads it — never bridged, never silently upgraded. The error names
    the found version and the ``ocx lock`` regenerate remedy (the entire
    migration story per the pre-1.0 no-back-compat policy).
    """
    short = uuid4().hex[:8]
...
"""
    ...
    result = _run_lock(ocx, project)
    assert result.returncode == EXIT_CONFIG, (
        f"a V1 lock must be rejected (exit {EXIT_CONFIG}); "
        f"got rc={result.returncode}\nstderr:\n{result.stderr}"
    )
    assert "ocx lock" in result.stderr, (
        f"the error must name the `ocx lock` regenerate remedy; stderr:\n{result.stderr}"
    )
    # The rejected lock is left untouched on disk — no partial rewrite.
    lock_after = (project / "ocx.lock").read_text()
    assert "lock_version = 1" in lock_after, (
        f"a rejected lock must not be rewritten; lock after:\n{lock_after}"
    )


def test_lock_rejects_committed_v2_lock_with_regenerate_hint(
    ocx: OcxRunner, tmp_path: Path
) -> None:
    """A committed **V2** lock (canonical-key-free per-platform leaf map, the
    format V3 superseded) is likewise rejected outright — V3 is the only
    version this build reads or writes.
    """
    short = uuid4().hex[:8]
...
[tool.platforms]
"linux/amd64" = "sha256:{fake_leaf_digest}"
"""
    )

    result = _run_lock(ocx, project)
    assert result.returncode == EXIT_CONFIG, (
        f"a V2 lock must be rejected (exit {EXIT_CONFIG}); "
        f"got rc={result.returncode}\nstderr:\n{result.stderr}"
    )
    assert "ocx lock" in result.stderr, (
        f"the error must name the `ocx lock` regenerate remedy; stderr:\n{result.stderr}"
...
def test_fault_injected_add_rolls_back_committed_lock_without_panic(
    ocx: OcxRunner, tmp_path: Path
) -> None:
    """A fault-injected ``ocx add`` rolls the lock back to its
    **byte-identical** predecessor on a failure after the lock rename — the
    mutation guard captures the predecessor's raw bytes at acquisition time
    and restores them verbatim, regardless of what the predecessor's shape
    is (a V1/`LegacyIndex` predecessor was the historical regression class
    this guards; that shape no longer exists at all, but the general rollback
    contract remains load-bearing).
...
""",
    ...
    predecessor_bytes = _read_lock_bytes(project)

    # Fault-injected add of the SECOND package: resolution succeeds, the new
    # lock is renamed into place, then the fault fires before the manifest
    # rewrite → the commit rolls the lock back to the predecessor bytes.
    ...
    rolled_back_bytes = _read_lock_bytes(project)
    assert rolled_back_bytes == predecessor_bytes, (
        "rollback must restore the committed lock byte-for-byte; "
        f"before={predecessor_bytes!r}\nafter={rolled_back_bytes!r}"
    )
...
def test_lock_offline_uncached_dirty_exits_non_zero(
    ...
    """Bare ``ocx --offline lock`` against a dirty V3 lock whose leaf digest
    ...
    ``TagNotFound`` (79); both prove no network call succeeded. The fake
    declaration_hash makes the lock dirty, so bare ``ocx lock`` re-resolves
    the declared tag — refused offline rather than silently fetched.

    Setup: hand-author an ``ocx.toml`` pointing at a fake registry path + a
    V3 lock with a stale declaration_hash and a fake (never-fetched) leaf
    ...
    - ``ocx.lock`` on disk is unchanged (no partial rewrite)
    """
    short = uuid4().hex[:8]
...
""",
    ...
    # Hand-author a V3 lock with a stale declaration_hash + fake leaf digest.
    fake_leaf_digest = "a" * 64
    v3_lock_text = f"""\
[metadata]
lock_version = 3
declaration_hash_version = 1
...
"linux/amd64" = "sha256:{fake_leaf_digest}"
"""
    (project / "ocx.lock").write_text(v3_lock_text)
    ...
    # The lock on disk must be unchanged — no partial rewrite was committed.
    lock_after = (project / "ocx.lock").read_text()
    assert lock_after == v3_lock_text, (
        f"a failed lock must not rewrite the lock; lock after:\n{lock_after}"
    )
