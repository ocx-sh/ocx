...
ocx package push -n -m base-metadata.json -i acme/mytool:1.0.0 -p linux/amd64 base.tar.xz tool-v1.tar.xz
BASE_DIGEST=$(sha256sum base.tar.xz | awk '{print $1}')
ocx package create build-v2 -m metadata.json -o tool-v2.tar.xz -p linux/amd64
ocx package push -i acme/mytool:1.0.1 "sha256:${BASE_DIGEST}.tar.xz" tool-v2.tar.xz
# endregion cast
