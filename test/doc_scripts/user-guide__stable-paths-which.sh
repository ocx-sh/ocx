...
# region cast
ocx package install --select "$PKG_KITWARE_CMAKE"
ocx package which --current "$PKG_KITWARE_CMAKE"
ocx package which --candidate "$PKG_KITWARE_CMAKE"
# endregion cast

current_path="$(ocx package which --current "$REPO_KITWARE_CMAKE")"
[[ -n "$current_path" ]] || {
    echo "ERROR: ocx package which --current returned empty output" >&2
    exit 1
}

candidate_path="$(ocx package which --candidate "$PKG_KITWARE_CMAKE")"
[[ -n "$candidate_path" ]] || {
