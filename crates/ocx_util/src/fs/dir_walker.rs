    ...
    F: Fn(&Path, usize) -> WalkDecision<T> + Send + Sync,
{
    ...
    if depth >= max_depth {
        log::warn!("Directory walk hit max depth at '{}'", dir.display());
        return Ok(ExploreResult {
            items,
            children: Vec::new(),
        });
    ...
    let mut entries = tokio::fs::read_dir(&dir).await.map_err(|e| FileError::new(&dir, e))?;
    let mut children = Vec::new();
    while let Some(entry) = entries.next_entry().await.map_err(|e| FileError::new(&dir, e))? {
        let path = entry.path();
        if !entry.file_type().await.map_err(|e| FileError::new(&path, e))?.is_dir() {
            continue;
        }
