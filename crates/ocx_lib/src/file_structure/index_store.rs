...
impl IndexStore {
    ...
    pub async fn read_root(
        ...
        // Absent or stale — self-heal under the source's transaction lock. A
        // concurrent writer can commit a FRESHER root + catalog entry between
        // our pre-lock read above and the lock acquisition below; re-read the
        // root bytes AFTER the lock is held and derive the recovered entry
        // from THOSE bytes. Deriving from the pre-lock read would silently
        // clobber the concurrent writer's newer entry back to stale
        // (lost-update) — see `read_root_recovery_never_clobbers_a_fresher_concurrently_committed_entry`.
        ...
        let (bytes, root) = match self.read_root_inner(source, repository, &repository_check).await? {
            Some(fresh) => fresh,
            // Root vanished between the pre-lock read and the lock acquisition
            // — no fresher bytes to derive from; fall back to the pre-lock read.
            None => (bytes, root),
        };
