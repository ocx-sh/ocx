...
/// fully-qualified [`Identifier`] strings (`registry/repo:tag` —
/// optionally `@digest`); bare-tag forms are rejected with
...
impl ProjectConfig {
    ...
    ///
    /// Precedence: explicit > `OCX_PROJECT_FILE` > CWD walk >
...
"#;
    ...
    fn parse_accepts_full_identifier_forms() {
        ...
        //
        // Each value must round-trip through `Identifier::Display`
        // verbatim — no normalization, no auto-injection of `latest`.
        let toml_str = r#"
[tools]
cmake = "ocx.sh/cmake:3.28"
