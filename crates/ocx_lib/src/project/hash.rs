...
mod tests {
    ...
    fn hash_deterministic_same_input() {
        let config = ProjectConfig::from_parts(
            tools(&[("cmake", "ocx.sh/cmake:3.28"), ("ninja", "ocx.sh/ninja:1.11")]),
            BTreeMap::new(),
        );
    ...
    fn hash_changes_on_tool_added() {
        let base = ProjectConfig::from_parts(tools(&[("cmake", "ocx.sh/cmake:3.28")]), BTreeMap::new());
        let added = ProjectConfig::from_parts(
            tools(&[("cmake", "ocx.sh/cmake:3.28"), ("ninja", "ocx.sh/ninja:1.11")]),
            BTreeMap::new(),
        );
    ...
    fn hash_changes_on_group_added() {
        ...
        let added = ProjectConfig::from_parts(tools(&[("cmake", "ocx.sh/cmake:3.28")]), groups);
        let h1 = declaration_hash(&base);
        let h2 = declaration_hash(&added);
        assert_ne!(h1, h2);
    }
    ...
    fn hash_stable_with_unicode_tags() {
        ...
        let config = ProjectConfig::from_parts(
            tools(&[("tool", "ocx.sh/tool:v1.0-\u{03b1}")]), // α
            BTreeMap::new(),
        );
    ...
    fn hash_corpus_case_2_single_tool() {
        ...
        let config = ProjectConfig::from_parts(tools(&[("cmake", "ocx.sh/cmake:3.28")]), BTreeMap::new());
        let got = declaration_hash(&config);
    ...
    fn hash_corpus_case_4_digest_suffixed_version() {
        ...
        let config2 = ProjectConfig::from_parts(
            tools(&[(
                "cmake",
    ...
    fn hash_independent_of_tool_insertion_order() {
        ...
        let config_forward = ProjectConfig::from_parts(forward, groups_forward);
        let config_reversed = ProjectConfig::from_parts(reversed, groups_reversed);
    ...
    fn hash_differs_for_nfc_vs_nfd_tag() {
        ...
        let nfc = ProjectConfig::from_parts(tools(&[("tool", "ocx.sh/tool:v1.0-\u{00e9}")]), BTreeMap::new());
        // NFD: "e" + combining acute U+0301.
        let nfd = ProjectConfig::from_parts(tools(&[("tool", "ocx.sh/tool:v1.0-e\u{0301}")]), BTreeMap::new());
    ...
    fn hash_accepts_special_char_tags() {
        ...
        let config = ProjectConfig::from_parts(tools(special), BTreeMap::new());
        let a = declaration_hash(&config);
        let b = declaration_hash(&config);
