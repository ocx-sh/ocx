...
/// constant (module-level "Cache lifecycle" note). One hour keeps "I just installed
/// musl-tools" from being a workday-long puzzle while still removing the probe
/// from essentially every invocation: a full re-detect costs ~2.7 ms, so one per
/// hour per shell is unmeasurable beside the ~3.6 ms an `ocx` process costs to
/// start at all.
///
/// Deliberately shorter than the referrers (6 h) and trust-root (24 h) caches:
/// those key on a remote whose answer cannot change under the user's hands
/// between two commands, this one keys on the local machine, where a package
/// manager can change it between two prompts.
#[cfg(target_os = "linux")]
