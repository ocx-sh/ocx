...
//! (`decode_snapshot_manifest` in `local_index`) dispatches on the per-source
...
fn build_index_http_client() -> reqwest::Client {
    ...
    builder.build().unwrap_or_else(|error| {
        ...
        // construction stays infallible, logging so it is not silent.
        log::warn!("index HTTP client build with bundled roots failed ({error}); using reqwest defaults");
        reqwest::Client::new()
    })
}
...
/// uses — it is not the committed snapshot.
#[derive(Default)]
struct SourceCacheInner {
    ...
    /// Never set on a served-but-unsupported version — an unsupported index
    /// stays a genuinely re-checked hard error rather than a remembered
    /// steady state. Config-driven construction means there is no probe
    /// outcome to soften a transport failure into — that always propagates.
...
impl OcxIndex {
    ...
    /// `config.json` (404) is a clean pass, not an outcome to remember. A
    /// transport failure reaching `config.json` propagates as a hard error on
    /// every call — there is no soft "maybe not an index yet" state left to
    /// absorb it.
    ///
    /// # Errors
    ///
    ...
    async fn check_format_version(&self) -> Result<()> {
        ...
        match self.transport.get(&url, None).await? {
            ...
            // Absent config: not (yet) an OCX index at this base URL — inert.
            IndexFetch::NotFound | IndexFetch::NotModified => {}
        }
...
///
/// Shared with [`LocalIndex`](super::LocalIndex)'s snapshot read-back so an
/// offline resolve through a persisted observation presents the identical
...
impl index_impl::IndexImpl for OcxIndex {
    ...
    async fn fetch_manifest_raw_bytes(
        ...
        // whose bytes hash to the leaf digest — A3-valid for a snapshot write.
        if identifier.digest().is_some() {
            let Some(physical) = self.physical_identifier(identifier).await? else {
                return Ok(None);
            };
...
mod tests {
    ...
    fn local_index(dir: &tempfile::TempDir) -> super::super::LocalIndex {
        super::super::LocalIndex::new(super::super::LocalConfig {
            snapshot_store: crate::file_structure::IndexStore::new(dir.path().join("index")),
        })
    }
