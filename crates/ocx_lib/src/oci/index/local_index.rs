...
/// File-backed snapshot of registry metadata, rooted at the index home.
///
...
impl LocalIndex {
    pub fn new(config: Config) -> Self {
        Self {
            snapshot: config.snapshot_store,
            allow_yanked: false,
        }
    }
    ...
    /// The snapshot store backing this local index — the effective index home
    ...
    pub fn snapshot(&self) -> &IndexStore {
        &self.snapshot
    }
    ...
    async fn refresh_derived(&self, identifier: &oci::Identifier, source: &super::Index) -> Result<()> {
        ...
        // Author the derived root's tag pointers. `commit_root_tag` merges each
        // upsert under the root-file lock, preserving every other tag.
        for (tag, content) in fetched {
            self.commit_root_tag(&identifier.clone_with_tag(&tag), &content).await?;
        }
        Ok(())
    }
    ...
    pub async fn sync_catalog(&self, source: &super::OcxIndex) -> Result<super::CatalogSyncOutcome> {
        ...
        let previous = self.snapshot.read_source_catalog(namespace).await?.unwrap_or_default();
        let previous_etag = self.snapshot.read_source_catalog_etag(namespace).await?;
        ...
        for repository in &outcome.moved {
            if !crate::utility::fs::path_exists_lossy(&self.snapshot.root_document_path(namespace, repository)).await {
                continue;
            }
        ...
        if !outcome.unchanged {
            let mut transaction = self.snapshot.begin_catalog_transaction(namespace).await?;
    ...
    fn validate_catalog_key(namespace: &str, key: &str) -> Result<()> {
        ...
        let identifier =
            oci::Identifier::parse(&format!("{namespace}/{key}")).map_err(|source| malformed(source.to_string()))?;
    ...
    ///   `index.ocx.sh` source, which [`decode_snapshot_manifest`] presents as a
    ...
    pub(super) async fn commit_root_tag(&self, identifier: &oci::Identifier, content: &oci::Digest) -> Result<()> {
        ...
        // `commit_root_tag` for distinct tags of one repository), so the
        ...
        let mut doc = match self.snapshot.read_root_document_bytes(source, repository).await? {
        ...
        // Upsert this tag, preserving every other tag's pointer and stamp.
        doc.tags.insert(
            tag.to_owned(),
            DerivedTag {
        ...
        self.snapshot.write_root_document(source, repository, &bytes).await?;
        Ok(())
    }
    ...
    ///   [`decode_snapshot_manifest`]); absent ⇒
    ...
    pub(super) async fn resolve_dispatch(
        ...
        // recovery).
        match self.snapshot.read_dispatch_object(source, repository, &content).await? {
            Some(bytes) => match decode_snapshot_manifest(&bytes)? {
    ...
    async fn read_root_by_kind(
        ...
        match kind {
            SourceKind::Published => self.snapshot.read_root(source, repository, repository_check).await,
            SourceKind::Derived => {
                self.snapshot
    ...
    pub(super) async fn list_local_repositories(&self, source: &str, kind: SourceKind) -> Result<Vec<String>> {
        match kind {
            SourceKind::Published => Ok(self
                ...
                .unwrap_or_default()),
            SourceKind::Derived => self.snapshot.list_wire_repositories(source).await,
        }
    }
    ...
    pub(super) async fn persist_published_root(&self, identifier: &oci::Identifier, bytes: &[u8]) -> Result<()> {
        ...
        let mut transaction = self.snapshot.begin_catalog_transaction(source).await?;
        transaction
...
/// Decodes a verified snapshot object into an [`oci::Manifest`], dispatching on
/// the per-source payload codec (`adr_index_indirection.md` A2).
///
/// Exactly two object kinds are ever written to the snapshot CAS: verbatim OCI
/// manifests (registry sources) and `index.ocx.sh` observation objects (native
/// codec). The untagged [`oci::Manifest`] requires `schemaVersion`, which an
/// observation object lacks, so the OCI parse fails cleanly for an observation
/// and the fall-through is unambiguous. An observation is decoded to the same
/// synthetic image index a live [`super::OcxIndex`] presents, so an offline
/// resolve walks snapshot-first without re-fetching.
///
...
fn decode_snapshot_manifest(bytes: &[u8]) -> Result<Option<oci::Manifest>> {
...
impl index_impl::IndexImpl for LocalIndex {
    ...
    // cross-check applies. Absent any external kind context, these
    // trait-level implementations default to `SourceKind::Derived` — the
    // uncatalogued read shares the exact root-document path with the
    // catalogued one and only skips the catalog cross-check/self-heal, never
    // resolution correctness (`adr_index_indirection.md` A2/H).
...
mod tests {
    ...
    /// `fetch_manifest_raw_bytes` with matching `(bytes, digest)`, the snapshot
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn chained_fetch_manifest_persists_object_into_snapshot() {
        let dir = TempDir::new().unwrap();
    ...
    /// (`test_snapshot_selfcontained.py::test_tampered_snapshot_object_
    ...
    fn decode_snapshot_manifest_dispatches_oci_and_observation() {
        ...
        assert!(decode_snapshot_manifest(b"not a manifest at all").unwrap().is_none());
    }
    ...
    async fn observation_chain_persists_and_resolves_offline() {
        ...
        // Fresh index resolves the tag offline through the snapshot: the obs
    ...
    async fn sync_catalog_refuses_traversing_catalog_keys_and_writes_nothing() {
        ...
        for key in malicious_keys {
            ...
            let local = LocalIndex::new(Config {
                snapshot_store: IndexStore::new(&home),
            });
