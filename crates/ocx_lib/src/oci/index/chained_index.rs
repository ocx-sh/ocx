...
/// Whether `err` reports a present-but-corrupt snapshot object — its on-disk
...
fn is_corrupt_snapshot_object(err: &crate::Error) -> bool {
    matches!(
        err,
        ...
        let local = if is_digest_addressed || self.mode != ChainMode::Remote {
            match self.local_index.resolve_dispatch(identifier, kind).await {
                ...
                Err(e) => {
                    ...
                    // the object.
                    let corrupt = is_corrupt_snapshot_object(&e);
...
mod chain_refs_tests {
    ...
    /// The snapshot store `make_local_index` reads/writes (the default
    ...
    async fn default_mode_cache_miss_walks_source_and_persists_chain_on_disk() {
        let cache_dir = TempDir::new().unwrap();
        let cache = make_local_index(&cache_dir);
        let blob_store = snapshot(&cache_dir);
    ...
    async fn remote_mode_digest_addressed_lookup_uses_cache() {
        ...
        // Pre-write the dispatch object directly into the snapshot CAS so the
        // cache has it. The bytes must hash to `digest_a` (A3 verify on read).
        snapshot(&cache_dir)
    ...
    async fn op_query_never_writes_local_index_in_any_mode() {
        ...
        for mode in [ChainMode::Default, ChainMode::Offline, ChainMode::Frozen] {
            ...
            assert!(
                !snapshot(&cache_dir).root_document_path(REGISTRY, REPO).exists(),
    ...
    async fn pinned_id_pull_skips_tag_pointer_commit() {
        ...
        let root_path = snapshot(&cache_dir).root_document_path(REGISTRY, REPO);
        assert!(
            !root_path.exists(),
...
mod tests {
    ...
    /// Build a real `LocalIndex` backed by a temp directory's snapshot home.
    ///
    ...
    // variant is not `is_corrupt_snapshot_object` (which matches only
    ...
    async fn corrupted_cache_read_falls_back_to_chain() {
        ...
        // first local read errors. The path is the snapshot store's
        // `<home>/<source>/p/<ns>/<pkg>.json`.
        let root_file = snapshot(&cache_dir).root_document_path(REGISTRY, REPO);
