...
/// ocx does define: without it, "expected `path` or `constant`" reads as a typo
/// report for what is really a version gap.
#[derive(Debug, Clone, PartialEq, Eq, thiserror::Error)]
#[error("unknown modifier type '{found}'; expected `path` or `constant` (a newer ocx may support it)")]
...
mod tests {
    ...
    /// "expected `path` or `constant`" as "you made a typo" — the wrong repair.
    /// Surfaces as 78 from `ocx.toml` and 64 from `--env`; both carry this text.
    #[test]
    fn modifier_kind_unknown_spelling_offers_the_upgrade_remedy() {
        let message = "list"
            .parse::<ModifierKind>()
            .expect_err("'list' names no modifier in this release")
            .to_string();
        assert!(
    ...
    fn unknown_type_tag_parses_and_keeps_its_name() {
        let json = r#"{"key":"GODEBUG","type":"list","separator":",","value":"gctrace=1"}"#;
        let var: Var = serde_json::from_str(json).expect("an unknown type must not break parsing");
        assert_eq!(var.key, "GODEBUG");
        match &var.modifier {
            Modifier::Unknown { type_name } => assert_eq!(type_name, "list"),
