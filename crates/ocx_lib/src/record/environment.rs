...
/// The launching process id, so a call tree (`make` → `ocx run` → tool)
...
mod tests {
    ...
    fn the_user_block_survives_either_half_going_missing() {
        ...
        let user = process(1, &PathBuf::from("/bin/true"))
            .user
            .expect("a determinable id alone still records the user");
        assert!(user.name.is_none());
        assert_eq!(
            user.id.is_some(),
            cfg!(unix),
            "the effective uid is determinable exactly where the platform has one"
        );
    }
