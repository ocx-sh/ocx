...
//! `autoInstalled`) is camelCase to match the provenance cluster it borrows
//! from, while the `process`, `host` and `os` blocks keep the flat lowercase
//! spelling of the vocabularies they were lifted from — including
//! `process.working_directory`, which is snake.
//!
...
pub struct ExecutionRecord {
    ...
    /// not. `ocx run -- bash …` picking up the *system* bash is a fact this
    ...
    /// `sh.ocx.role: root`, the transitive closure `dependency`.
...
pub struct RecordInputs<'a> {
    ...
    pub admitted: &'a AdmittedBinaries,
...
fn frame_for(scope: &Scope) -> Frame {
    let (command, identity) = match scope {
        Scope::Project { .. } => (FrameCommand::Run, FrameIdentity::Complete),
...
fn resolution_block(inputs: &RecordInputs<'_>) -> Resolution {
    ...
    Resolution {
        ...
        registries: composed.then(|| registries(inputs.packages)),
...
/// not. `ocx run -- bash` picking up the system bash lands in the second case,
...
fn executable_block(inputs: &RecordInputs<'_>) -> BTreeMap<String, String> {
    ...
    if !inputs.executable.starts_with(inputs.store_root) {
    ...
    if let Some(kind) = executable_kind(inputs.executable, inputs.store_root) {
...
fn owning_root<'a>(inputs: &'a RecordInputs<'_>) -> Option<&'a Arc<InstallInfo>> {
    inputs
        .packages
        .iter()
        .find(|info| inputs.executable.starts_with(info.dir().root()))
}
...
/// in. Deduplicated by content identity, so a package reachable twice appears
/// once and keeps its first-seen role.
...
fn project(
    ...
    let mut annotations = BTreeMap::from([("sh.ocx.role".to_string(), Value::from(role))]);
    let digest = digest_map(&identifier.digest());

    let Some(uri) = package_url(identifier, platform) else {
    ...
    annotations.insert("sh.ocx.visibility".to_string(), Value::from(visibility.to_string()));
...
fn binding_for<'a>(identifier: &PinnedIdentifier, scope: &'a Scope) -> Option<&'a PackageBinding> {
    match scope {
        ...
        Scope::Package { .. } | Scope::Launcher => None,
    }
}
...
impl AdmittedIndex {
    fn build(admitted: &AdmittedBinaries) -> Self {
        Self {
...
mod tests {
    ...
    impl Frame {
        /// The `ocx run` frame of the design record's first exemplary record:
    ...
    fn a_name_containing_the_join_separator_stays_its_own_element() {
        let cmake = pinned("ocx/cmake", "index.ocx.sh", None, LEAF_HEX);
        let mut frame = Frame::project();
        frame.admitted = AdmittedBinaries {
            binaries: ["a,b", "c"]
                .into_iter()
    ...
    fn frame_is_derived_from_the_scope() {
        assert_eq!(frame_for(&project_scope()).command, FrameCommand::Run);
    ...
    fn digests_are_bare_lowercase_hex_and_the_prefixed_form_lives_only_in_the_purl() {
        ...
        for descriptor in value["packages"].as_array().expect("packages") {
            ...
            assert!(
                descriptor["uri"].as_str().expect("uri").contains("@sha256:"),
                "the prefixed form survives inside the purl",
            );
        }
    ...
    fn build_assembles_every_block_of_a_launching_frame() {
        ...
        assert_eq!(record.frame.command, FrameCommand::Run);
