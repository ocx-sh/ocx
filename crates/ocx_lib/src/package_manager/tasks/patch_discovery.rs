...
impl PackageManager {
    ...
    /// 2. On `NeverLooked`: call [`fetch_and_persist_descriptor`]; on `None`
    ///    result, record `LookedNoDescriptor`; on `Some`, call
    ///    [`persist_patch_descriptor`] and record `LookedHasDescriptor`.
    /// 3. On `LookedHasDescriptor`: the descriptor blob is already in the CAS;
    ///    load it for companion collection (re-use of already-persisted state).
    /// 4. On `LookedNoDescriptor`: skip.
    ///
    /// After both descriptors are resolved, collect companions, then install
    /// each via [`install_companion`].
    ///
    /// ## Recursion guard
    ///
                ...
                PatchDiscoveryState::LookedHasDescriptor { manifest_digest } => {
                    ...
                    let digest = match crate::oci::Digest::try_from(manifest_digest.as_str()) {
                        ...
                        Err(error) => {
                            ...
                            // - re-fetch succeeds → fetch_and_persist_descriptor writes
                            //   LookedHasDescriptor with the new digest.
                            ...
                            if let Some(descriptor) =
                                fetch_and_persist_descriptor(self, descriptor_id, &tags_path).await?
                            {
                                descriptors.push(descriptor);
                            }
                            continue;
            ...
            match self.install_companion(&companion_id, platforms_vec).await {
                ...
                Err(kind) => {
                    if companion.required {
                        // Fail-closed: required companion failed → abort.
...
///
/// Returns `Some(PatchDescriptor)` when a descriptor was found and
/// successfully persisted; `None` when the patch tag does not exist at the
...
/// - Calls [`PatchTagMap::write_has_descriptor`] or
///   [`PatchTagMap::write_no_descriptor`] to record the three-state.
async fn fetch_and_persist_descriptor(
    ...
    match fetched {
        ...
        Some(FetchedDescriptorBlobs {
            ...
            // Record "looked, has descriptor" with the manifest digest.
            PatchTagMap::write_has_descriptor(tags_path, &persisted.manifest_digest.to_string())
                .await
                .map_err(PackageErrorKind::Internal)?;
...
mod tests {
    ...
    fn offline_view_preserves_patch_config() {
        ...
        let local_index = LocalIndex::new(LocalConfig {
            ...
            blob_store: BlobStore::new(tmp.path().join("blobs")),
        });
