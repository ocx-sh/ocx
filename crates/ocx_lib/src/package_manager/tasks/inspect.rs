...
pub struct Surface {
    ...
    /// package.
    pub binaries: Vec<(oci::PinnedIdentifier, metadata::BinaryName)>,
    /// Entrypoint names from every admitted node, attributed likewise.
    pub entrypoints: Vec<(oci::PinnedIdentifier, metadata::EntrypointName)>,
...
mod spec_tests {
    ...
    async fn walk_closure_interface_env_gates_by_node_and_entry_visibility() {
        ...
        assert_eq!(
            root_path_var.kind,
            crate::package::metadata::env::modifier::ModifierKind::Path,
