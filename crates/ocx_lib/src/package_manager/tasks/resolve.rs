    ...
    async fn build_site_patch_set(
        ...
        for (index, admitted_id) in admitted.iter().enumerate() {
            ...
            descriptor_load_tasks.spawn(async move {
                let _permit = package_manager::concurrency::acquire_permit(&semaphore).await;
    ...
    async fn find_companion_local(&self, companion_id: &oci::Identifier) -> crate::Result<Option<InstallInfo>> {
        ...
        let local_index = oci::index::Index::from_chained(
            oci::index::LocalIndex::new(oci::index::LocalConfig {
                snapshot_store: self.effective_index_snapshot(),
            }),
            Vec::new(),
...
mod spec_tests {
    ...
    fn snapshot_store(dir: &TempDir) -> IndexStore {
...
mod project_config_isolation_gate {
    ...
    const CONFIG_SOURCE: &str = include_str!("../../project/config.rs");
    ...
    fn config_does_not_reference_resolver_types() {
        ...
        let module_token = concat!("package_manager::tasks::", "resolve");
