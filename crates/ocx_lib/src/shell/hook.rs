...
fn fish_registration(binary: &str, watch_paths: &[PathBuf]) -> String {
    ...
    format!(
        ...
        check = fish_reconcile(&quoted, watch_paths)
    )
}
...
fn power_shell_registration(binary: &str, watch_paths: &[PathBuf]) -> String {
    ...
    format!(
        "if (-not (Test-Path variable:global:__ocxPrevPrompt)) {{\n\
        ...
        check = power_shell_reconcile(&quoted, watch_paths),
        call = PWSH_HOOK_CALL
    )
}
...
mod tests {
    ...
    /// fish's `functions -q` and pwsh's `Test-Path function:`/`variable:` were
    /// already function- and variable-scoped; they are asserted here so a future
    /// widening of either reds.
    #[test]
    fn every_existence_probe_is_scoped_to_the_namespace_it_asks_about() {
        for (shell, probes) in [
            ...
            (
                Shell::PowerShell,
                vec!["if (-not (Test-Path variable:global:__ocxPrevPrompt))"],
            ),
        ] {
...
mod live_shell_tests {
    ...
    fn run(argv: &[&str], script: &str) -> Option<String> {
        ...
        let go = |body: &str| match Command::new(bin).args(head).arg(body).output() {
