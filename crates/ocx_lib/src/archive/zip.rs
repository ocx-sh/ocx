...
pub(super) fn extract(archive: &Path, output: &Path, strip_components: usize, decompressed_cap: u64) -> Result<()> {
    ...
    for i in 0..zip.len() {
        ...
        if total_written > decompressed_cap {
            return Err(Error::ExtractionCapExceeded { cap: decompressed_cap }.into());
        }
        ...
        let Some(enclosed_name) = entry.enclosed_name().map(|p| p.to_path_buf()) else {
            return Err(Error::EntryEscape(PathBuf::from(entry.name())).into());
        };
        ...
        crate::utility::fs::refuse_if_symlink_in_path_sync(&out, Some(&canonical_root)).map_err(|e| match e {
            crate::utility::fs::SymlinkWalkError::Ancestor { .. } => Error::EntryEscape(enclosed_name.clone()),
        ...
        if entry.is_dir() {
            ...
            crate::symlink::validate_target(&canonical_root, &dst, Path::new(&target))?;
            crate::symlink::create(Path::new(&target), &dst)?;
        } else {
...
mod tests {
    ...
    fn a_zip_of_empty_entries_is_bounded_by_the_byte_cap() {
        ...
        assert!(
            matches!(
                error,
                crate::Error::Archive(crate::archive::Error::ExtractionCapExceeded { cap }) if cap == too_small
            ),
    ...
    fn extract_rejects_output_exceeding_the_cap() {
        ...
        assert!(
            matches!(
                error,
                crate::Error::Archive(crate::archive::Error::ExtractionCapExceeded { cap: 16 })
            ),
    ...
    fn windows_in_root_directory_symlink_extracts_as_a_junction() {
        ...
        assert!(
            crate::symlink::is_link(&output.join("link")),
