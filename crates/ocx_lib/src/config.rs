...
mod tests {
    ...
    /// `RecordsOptions` deliberately carries no `deny_unknown_fields`: a
    /// `[records]` block written for a newer ocx must not brick an older fleet
    /// binary reading the same file.
    #[test]
