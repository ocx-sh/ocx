...
//! ([`crate::setup::shell_config::set`]) and the `OCX_EXTRA_CA_CERTS`
...
use crate::config::loader::MAX_CONFIG_SIZE;
use crate::utility::fs::{BoundedReadError, lock_scoped, read_bounded, write_bytes_atomic};
...
/// `[managed]` fence is a comment-delimited region [`crate::setup::rc_block`]
...
fn lock_error(config_path: &Path, error: crate::Error) -> EditError {
    match error {
        crate::Error::InternalFile(_, source) if source.kind() == std::io::ErrorKind::TimedOut => EditError::Locked {
            path: config_path.to_path_buf(),
        },
        crate::Error::InternalFile(path, source) => EditError::Io { path, source },
        other => EditError::Io {
            path: config_path.to_path_buf(),
    ...
    F: FnOnce(&Path, &str) -> Result<String, EditError>,
{
    ...
    #[cfg(any(test, feature = "__testing"))]
    if let Some(millis) = crate::env::var(HOLD_OVERRIDE).and_then(|value| value.parse().ok()) {
...
mod tests {
    use super::*;
    use crate::setup::shell_config::HAND_WRITTEN;
