...
//! `crates/ocx_index/src/local_index.rs` — a `serde_json::to_vec` in
...
fn parse_image_index(bytes: &[u8], label: &str) -> ocx_oci::ImageIndex {
    match serde_json::from_slice::<ocx_oci::Manifest>(bytes) {
        Ok(ocx_oci::Manifest::ImageIndex(index)) => index,
        Ok(ocx_oci::Manifest::Image(_)) => panic!("{label} decoded as a bare image manifest, not an image index"),
...
/// [`ocx_oci::Platform::candidate_from_descriptor`] that `Index::fetch_candidates`
/// uses. Platforms are compared as their canonical grammar strings.
fn derive_candidates(index: &ocx_oci::ImageIndex) -> Vec<(String, String)> {
    index
        .manifests
        .iter()
        .filter_map(|entry| {
            let platform = ocx_oci::Platform::candidate_from_descriptor(entry)?;
...
fn every_vector_is_named_by_the_sha256_of_its_own_bytes() {
    ...
    for vector in &vectors {
        ...
        let actual = ocx_oci::Algorithm::Sha256.hash(&bytes).to_string();
        assert_eq!(
            actual,
...
fn every_vector_decodes_to_the_vendored_platform_candidate_list() {
    for vector in &expected_vectors() {
        ...
        let listed: Vec<String> = ocx_oci::Platform::from_image_index(&index)
            .iter()
...
fn attestation_and_platform_less_descriptors_are_never_candidates() {
    ...
    let unrepresentable: Vec<&ocx_oci::ImageIndexEntry> = index
        .manifests
        .iter()
        .filter(|entry| {
