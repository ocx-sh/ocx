...
use crate::{CatalogDocument, CatalogIndex, IndexFormatConfig, serialize_catalog, serialize_config};
use ocx_oci::Digest;
...
impl IndexStore {
    ...
    fn ensure_repository_contained(repository: &str) -> Result<()> {
        ocx_util::fs::path::join_under_root(Path::new("/ocx-index-home"), Path::new(repository))
            .map(|_| ())
            .map_err(|source| {
                ocx_store::file_structure::error::Error::RepositoryEscapesIndexHome {
    ...
    async fn write_verified_object(&self, target: PathBuf, claimed_digest: &Digest, bytes: &[u8]) -> Result<()> {
        ...
        let parent = target
            .parent()
            .ok_or_else(|| super::error::file_error(&target, std::io::Error::other("path has no parent")))?
            .to_path_buf();
        tokio::fs::create_dir_all(&parent)
            .await
            .map_err(|e| super::error::file_error(&parent, e))?;
        ...
        .await
        .map_err(|join_err| super::error::file_error(&target, std::io::Error::other(join_err)))?
        .map_err(|io_err| super::error::file_error(&target, io_err))?;
        Ok(())
    }
    ...
    async fn read_verified_object(&self, target: PathBuf, digest: &Digest) -> Result<Option<Vec<u8>>> {
        let bytes = match tokio::fs::read(&target).await {
            ...
            Err(e) => return Err(super::error::file_error(&target, e)),
        };
...
impl IndexStore {
    ...
    pub async fn lock_source(
        ...
        tokio::fs::create_dir_all(&source_dir)
            .await
            .map_err(|e| super::error::file_error(&source_dir, e))?;
        ocx_util::fs::lock_scoped(&self.locks_root, scope, &source_dir, discriminator, timeout)
            .await
            .map_err(Into::into)
    }
    ...
    pub fn root_document_path(&self, source: &str, repository: &str) -> PathBuf {
        self.wire_source_dir(source)
            .join("p")
            .join(ocx_store::file_structure::repository_path(repository))
    ...
    pub async fn read_root(
        ...
        source: &str,
        repository: &str,
        repository_check: impl Fn(&crate::IndexRoot) -> Result<()>,
    ...
    async fn persist_recovered_catalog_entry(
        ...
        repository_check: impl Fn(&crate::IndexRoot) -> Result<()>,
        prelock_bytes: &[u8],
        prelock_root: &crate::IndexRoot,
    ) -> Result<(Vec<u8>, crate::IndexRoot, String)> {
    ...
    async fn read_root_inner(
        ...
        repository: &str,
        repository_check: impl Fn(&crate::IndexRoot) -> Result<()>,
    ) -> Result<Option<(Vec<u8>, crate::IndexRoot)>> {
        Self::ensure_source_contained(source)?;
        Self::ensure_repository_contained(repository)?;
        ...
        let root: crate::IndexRoot = serde_json::from_slice(&bytes).map_err(|cause| {
    ...
    pub async fn read_source_config(&self, source: &str) -> Result<Option<IndexFormatConfig>> {
        ...
        let config = serde_json::from_slice(&bytes).map_err(|cause| crate::error::Error::MalformedIndexDocument {
    ...
    pub(crate) async fn ensure_source_config(&self, source: &str) -> Result<()> {
        ...
        let bytes = serialize_config(&IndexFormatConfig {
            format_version: crate::SUPPORTED_FORMAT_VERSION,
            name_segments: None,
        });
    ...
    pub async fn read_root_document_bytes(&self, source: &str, repository: &str) -> Result<Option<Vec<u8>>> {
        ...
        match tokio::fs::read(&target).await {
            ...
            Err(e) => Err(super::error::file_error(&target, e)),
        }
    }
    ...
    async fn write_bytes_atomic(target: &Path, bytes: Vec<u8>) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || -> std::io::Result<()> {
            ...
            tmp.as_file().sync_data()?;
            ocx_util::fs::persist_temp_file(tmp, &target_owned)
        })
        .await
        .map_err(|join_err| super::error::file_error(target, std::io::Error::other(join_err)))?
        .map_err(|io_err| super::error::file_error(target, io_err))?;
...
async fn is_dispatch_object_cas_dir(dir: &std::path::Path) -> bool {
    ...
    for algorithm_dir in algorithm_dirs {
        ...
        let Some(algorithm) = name
            .to_str()
            .and_then(|name| ocx_oci::Algorithm::ALL.iter().find(|a| a.prefix() == name))
        else {
            continue;
        };
...
impl CatalogTransaction<'_> {
    ...
    pub async fn write_root(
        ...
        repository: &str,
        bytes: &[u8],
        repository_check: impl FnOnce(&crate::IndexRoot) -> Result<()>,
    ) -> Result<()> {
...
mod wire_grammar_tests {
    ...
    async fn wire_writes_refuse_a_traversing_repository_and_write_nothing() {
        ...
        let digest = ocx_oci::Algorithm::Sha256.hash(b"payload");
    ...
    async fn wire_paths_accept_a_normal_nested_repository() {
        ...
        let digest = ocx_oci::Algorithm::Sha256.hash(&payload);
    ...
    //
    // [`IndexStore::wire_source_dir`] joins [`ocx_store::file_structure::slugify`]'s output as a
    ...
    fn root_catalog_entry_is_sha256_of_the_bytes() {
        ...
        let expected = ocx_oci::Algorithm::Sha256.hash(bytes).to_string();
    ...
    async fn write_order_dispatch_then_root_then_catalog_all_cohere_after_commit() {
        ...
        let dispatch_digest = ocx_oci::Algorithm::Sha256.hash(&dispatch_payload);
    ...
    async fn read_root_hard_fails_on_unparseable_root_document() {
        ...
        assert!(
            matches!(
                err,
                crate::error::Error::Store(ocx_store::file_structure::error::Error::MalformedRootDocument { .. })
            ),
    ...
    async fn read_root_propagates_repository_check_failure_as_data_error() {
        ...
        let result = s
            .read_root("ocx.sh", "kitware/cmake", |root: &IndexRoot| {
                ...
                Err(ocx_store::file_structure::error::Error::DigestMismatch {
        ...
        let _err = result.expect_err("a repository_check rejection must propagate, never be swallowed");
    }
    ...
    async fn read_root_uncatalogued_propagates_repository_check_failure() {
        ...
        let result = s
            ...
            .await;
        let _err = result.expect_err("a repository_check rejection must propagate on the derived path too");
    }
    ...
    async fn list_wire_repositories_collects_nested_root_documents_sorted_and_deduped() {
        ...
        let dispatch = s.dispatch_object_path("ocx.sh", "kitware/cmake", &ocx_oci::Algorithm::Sha256.hash(b"payload"));
    ...
    async fn list_wire_repositories_prunes_a_dispatch_dir_that_has_no_root_document() {
        ...
        let dispatch = s.dispatch_object_path("ocx.sh", "ca-bundle", &ocx_oci::Algorithm::Sha256.hash(b"payload"));
        tokio::fs::create_dir_all(dispatch.parent().unwrap()).await.unwrap();
        tokio::fs::write(&dispatch, b"{}").await.unwrap();
    ...
    async fn read_source_config_rejects_an_unparseable_document() {
        ...
        assert!(
            matches!(err, crate::error::Error::MalformedIndexDocument { .. }),
    ...
    async fn read_source_config_propagates_an_unreadable_document() {
        ...
        assert!(
            matches!(err, crate::error::Error::File(_)),
    ...
    async fn ensure_source_config_writes_the_wire_form_when_absent() {
        ...
        assert_eq!(
            on_disk,
            crate::serialize_config(&IndexFormatConfig {
                format_version: crate::SUPPORTED_FORMAT_VERSION,
                name_segments: None,
            }),
    ...
    async fn ensure_source_config_propagates_a_write_failure() {
        ...
        let _err = result.expect_err("an I/O failure on the config write must propagate, never be swallowed");
    }

    // ── 12. c/index.json goes through the wire formatter (C-025) ──────────

    /// Pins **the switch**: [`CatalogTransaction::commit`] must write
    /// `c/index.json` through [`crate::serialize_catalog`] rather
    ...
    async fn commit_writes_the_catalog_through_the_wire_formatter() {
        ...
        assert_eq!(
            on_disk,
            crate::serialize_catalog(&CatalogDocument::new(expected)),
    ...
    fn config_json_has_at_most_one_production_writer_and_it_is_the_update_path() {
        ...
        // Every crate's `src`, not just this one's. `ensure_source_config` is
        // `pub(crate)`, so its call sites cannot leave this crate and scanning
        // here alone would bound it — but `source_config_path` is `pub`, and the
        // extraction put a crate boundary where there was none. A walk rooted at
        // this crate would stay green while a second namer appeared in `ocx_lib`
        // or `ocx`, which is the whole property being guarded.
        let crates_root = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .expect("CARGO_MANIFEST_DIR is crates/<name>, so it has a parent");
        let mut files = Vec::new();
        for crate_dir in std::fs::read_dir(crates_root).expect("a readable crates/ directory") {
            let src = crate_dir.expect("a readable crate entry").path().join("src");
            if src.is_dir() {
        ...
        // Floor on the reader, not only its subject: `path_users` is checked
        // with `all()`, which a walk that read nothing satisfies trivially. A
        // wrong root, a renamed layout or a `read_dir` that silently yielded
        // little would otherwise be indistinguishable from a clean workspace.
        assert!(
            files.len() > 200,
            "the workspace scan read only {} files — too few for this to mean anything",
            files.len()
        );
        ...
        // Non-empty before `all()`: the three names below exist, so finding none
        // means the scan missed them, not that nobody names the path.
        assert!(
            !path_users.is_empty(),
            "no site names the config.json path at all — the scan found nothing to judge"
        );
        assert!(
            path_users
                .iter()
