...
pub(crate) fn encode_patches(patches: Option<&ResolvedPatchConfig>) -> Option<String> {
    ...
    let object = serde_json::json!({
        ...
        "no_patches":       config.no_patches.iter().collect::<Vec<_>>(),
    });
    match serde_json::to_string(&object) {
        Ok(json) => Some(json),
        Err(error) => {
            log::warn!("failed to encode OCX_PATCHES: {error}");
            None
        }
    }
}
...
mod tests {
    ...
    fn patch_config_toml_full_block_parses() {
        let toml_str = r#"
            ...
            required = false
        "#;
        let config: crate::Config = toml::from_str(toml_str).expect("valid [patches] TOML must parse");
    ...
    fn patch_config_toml_empty_section_yields_default() {
        ...
        let config: crate::Config = toml::from_str(toml_str).expect("empty [patches] section must parse");
        let patches = config.patches.expect("[patches] section must be present");
    ...
    fn patch_config_toml_unknown_fields_silently_ignored() {
        let toml_str = "[patches]\nregistry = \"r\"\nunknown_field = \"x\"\n";
        let result = toml::from_str::<crate::Config>(toml_str);
        assert!(
            result.is_ok(),
    ...
    /// OCX_PATCHES wire format round-trips the `no_patches` project opt-out
    /// across the process boundary (C5 — the launcher forwarding). A forwarded
    /// value with no `no_patches` key decodes to an empty set (backward
    ...
    fn ocx_patches_round_trip_no_patches() {
        ...
        let original = ResolvedPatchConfig {
            ...
            no_patches: ["ghcr.io/acme/cli".to_string(), "ocx.sh/java".to_string()]
                .into_iter()
                .collect(),
        };
        let json = encode_patches(Some(&original)).expect("encode");
        ...
        assert_eq!(
            ...
            "the no_patches opt-out set must survive the round-trip"
        );
        assert_eq!(parsed, original);
        ...
        let legacy = patches_from_env().expect("valid").expect("Some");
        assert!(
            legacy.no_patches.is_empty(),
            "absent no_patches key must decode to an empty set"
        );
    }
    ...
    fn no_patches_section_yields_none() {
        ...
        let config: crate::Config = toml::from_str(toml_str).expect("config without [patches] must parse");
        assert!(
    ...
    fn resolve_patch_config_returns_none_when_patches_absent() {
        ...
        let result = crate::patch::resolve_patch_config(&config);
        assert!(
    ...
    fn resolve_patch_config_applies_default_path_template() {
        ...
        let resolved = crate::patch::resolve_patch_config(&config)
    ...
    fn encode_patches_some_returns_json_with_three_fields() {
        ...
        assert!(parsed.contains_key("no_patches"), "JSON must have 'no_patches' key");
        ...
        assert_eq!(
            parsed["no_patches"].as_array().map(Vec::len),
            Some(0),
            "an empty opt-out must encode as an empty JSON array"
        );
    }
