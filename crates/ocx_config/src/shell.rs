...
pub enum ConsentPatternError {
    ...
    /// organisation. Clause 2 quantifies over `project::consent::verified_sources` — the
...
/// `ocx_trust::pattern_matches` returns `true` for an empty pattern, so without
