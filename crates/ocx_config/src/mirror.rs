...
fn resolve_mirror_role(
    ...
    if parsed.protocol == "http" && !ocx_oci::ssrf::allows_plain_http(insecure_hosts, &parsed.host) {
...
mod tests {
    ...
    fn mirror_config_toml_roundtrip() {
        ...
        let config: crate::Config = toml::from_str(toml_str).expect("valid TOML must parse");
    ...
    fn mirror_config_unknown_field_drops_the_entry() {
        ...
        let config =
            toml::from_str::<crate::Config>(toml_str).expect("an unrecognized field must not fail the whole config");
