...
pub async fn read_pause(paths: &ManagedConfigPaths) -> Option<ManagedConfigPause> {
    let bytes = tokio::fs::read(paths.pause_file()).await.ok()?;
    let pause: ManagedConfigPause = match serde_json::from_slice(&bytes) {
        Ok(pause) => pause,
        Err(error) => {
            log::debug!("managed-config pause file is corrupt, treating as absent: {error}");
            return None;
        }
    };
    let until = match chrono::DateTime::parse_from_rfc3339(&pause.paused_until) {
        Ok(until) => until,
        Err(error) => {
            log::debug!("managed-config pause timestamp is unparsable, treating as absent: {error}");
            return None;
        }
    };
...
pub async fn write_pause(paths: &ManagedConfigPaths, pause: &ManagedConfigPause) -> std::io::Result<()> {
    let dir = paths.dir();
    ...
    tokio::task::spawn_blocking(move || -> std::io::Result<()> {
        ...
        ocx_util::fs::write_bytes_atomic(&path, &bytes)
    })
    .await
...
pub async fn clear_pause(paths: &ManagedConfigPaths) -> std::io::Result<()> {
    match tokio::fs::remove_file(paths.pause_file()).await {
        Ok(()) => Ok(()),
...
mod tests {
    ...
    fn paths(dir: &tempfile::TempDir) -> ManagedConfigPaths {
        ManagedConfigPaths::new(dir.path())
    }
    ...
    async fn pause_round_trips_through_write_and_read() {
        ...
        write_pause(&paths, &pause).await.expect("write must succeed");
        let read = read_pause(&paths).await.expect("an in-force pause must read back");
    ...
    async fn absent_pause_reads_as_none() {
        let dir = tempfile::tempdir().unwrap();
        assert!(read_pause(&paths(&dir)).await.is_none());
    }
    ...
    async fn expired_pause_reads_as_none() {
        ...
        write_pause(&paths, &expired).await.unwrap();
        assert!(
            read_pause(&paths).await.is_none(),
    ...
    async fn corrupt_pause_reads_as_none() {
        ...
        std::fs::write(paths.pause_file(), b"{not json").unwrap();
        assert!(read_pause(&paths).await.is_none(), "corrupt JSON must read as absent");

        std::fs::write(paths.pause_file(), b"{\"paused_until\":\"not-a-timestamp\"}").unwrap();
        assert!(
            read_pause(&paths).await.is_none(),
    ...
    async fn read_pause_rejects_window_beyond_max_as_corrupt() {
        ...
        write_pause(&paths, &tampered).await.unwrap();
        assert!(
            read_pause(&paths).await.is_none(),
    ...
    async fn clear_pause_is_idempotent() {
        let dir = tempfile::tempdir().unwrap();
        let paths = paths(&dir);
        clear_pause(&paths).await.expect("clearing an absent pause succeeds");

        let pause = ManagedConfigPause::for_duration(std::time::Duration::from_secs(60), None);
        write_pause(&paths, &pause).await.unwrap();
        clear_pause(&paths).await.expect("clear must succeed");
        assert!(read_pause(&paths).await.is_none());
        clear_pause(&paths).await.expect("double clear must succeed");
    }
}
