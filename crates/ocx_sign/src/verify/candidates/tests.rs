...
use crate::verify::simplesigning_read::{SidecarKind, sidecar_tag};
...
use ocx_oci::referrer::media_types::{COSIGN_SBOM_ARTIFACT_TYPE, SIGSTORE_BUNDLE_V03};
use ocx_oci::{Algorithm, Descriptor, Digest, OCI_IMAGE_MEDIA_TYPE};
...
fn image() -> ocx_oci::native::Reference {
...
fn fallback_index(entries: Vec<ocx_oci::ImageIndexEntry>) -> Vec<u8> {
...
fn index_entry(digest: String, size: i64, artifact_type: Option<&str>) -> ocx_oci::ImageIndexEntry {
...
async fn the_same_signature_through_the_fallback_tag_reports_fallback_tag() {
    ...
    seed_tag(
        &data,
        ocx_oci::tag::referrer_fallback_tag(&subject()),
...
fn native_transport_is_nameable_with_the_ratified_signature() {
    const _FACTORY: fn(ocx_oci::native::Client, ocx_oci::auth::Auth) -> Box<dyn OciTransport> =
        ocx_oci::client::native_transport;
