...
/// registry. [`crate::verify::VerifyPipeline`] remains the only answer
...
impl SignerCandidate {
    ...
    /// must not break `ocx-mirror`, which takes these crates as path
    /// dependencies), and it makes a downstream struct literal a compile error.
