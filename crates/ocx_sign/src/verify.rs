...
// `DiscoveryMethod` is populated by the client's referrer listing and lives
// with it (`ocx_oci::referrer::discovery`, ADR 1.18); verify only reports it, on
// `VerifyResult::discovery_method`. Re-exported here because that field is the
// value's whole public surface — the same in-crate re-export C-040 keeps on
// `publisher.rs` for `LayerRef`.
pub use dsse::VerifiedAttestation;
pub use ocx_oci::referrer::DiscoveryMethod;
// `TrustRootLoadReason` is one arm of `VerifyErrorKind::TrustRootLoad`, and the
// binary's ladder gives each reason its own exit code.
