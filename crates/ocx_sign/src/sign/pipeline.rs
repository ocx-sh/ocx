...
use crate::attest::COSIGN_SIGN_PREDICATE_TYPE;
use crate::attest::statement;
use crate::sign::bundle::BUNDLE_V03_MEDIA_TYPE;
use crate::sign::format::SignatureFormat;
use crate::sign::referrers::{attach_referrer, map_client_error, referrers_capability};
use crate::sign::simplesigning_write;
...
struct SignedIdentity {
    ...
    key_backend: ocx_trust::key_ref::KeyBackendKind,
...
pub(crate) async fn guard_dialed_endpoints(
    ...
    for (dialed, url, flag) in endpoints {
        ...
        ocx_oci::endpoint::resolve_sigstore_url(url, trusted)
            .await
...
mod platform_narrowing_tests {
    ...
    use super::*;
    use ocx_oci::{INDEX_SCHEMA_VERSION, ImageIndex, ImageIndexEntry, ImageManifest};
    ...
    /// A resolution onto a bare image manifest — the shape a test picks when
    /// the axis under test is what the reference RESOLVED to, not how it was
    /// written.
    fn bare_manifest() -> (Digest, ocx_oci::Manifest) {
        (digest(0xaa), ocx_oci::Manifest::Image(ImageManifest::default()))
    }
    ...
    fn image_index_manifest() -> ocx_oci::Manifest {
        ocx_oci::Manifest::ImageIndex(ImageIndex {
    ...
    /// The same two-child index, as a resolution.
    fn image_index() -> (Digest, ocx_oci::Manifest) {
        (digest(0xaa), image_index_manifest())
    }
    ...
    /// A resolution onto an index whose one `linux/arm64` child is addressed by
    /// `child_digest`.
    fn index_listing(child_digest: &Digest) -> (Digest, ocx_oci::Manifest) {
    ...
    #[test]
    fn a_non_sha256_subject_is_refused_before_anything_is_written() {
        ...
        let accepted = sign_target_from_resolution(Some(&index_listing(&digest(0x02))), Some(&platform("linux/arm64")))
            .expect("a sha256 child is signable");
        assert_eq!(accepted.subject_digest, digest(0x02));
        assert_eq!(accepted.subject_digest.algorithm(), ocx_oci::Algorithm::Sha256);

        let narrowed = sign_target_from_resolution(Some(&index_listing(&sha384(0x02))), Some(&platform("linux/arm64")))
        ...
        // is the same refusal.
        let bare = (sha384(0xaa), ocx_oci::Manifest::Image(ImageManifest::default()));
        let _resolved = sign_target_from_resolution(Some(&bare), None)
            .expect_err("the resolved object is the subject, and it is sha384");
    }

    /// The absent flag on a BARE MANIFEST: sign what resolved.
    #[test]
    fn no_platform_against_a_bare_manifest_acts_on_the_resolved_object() {
        let target =
            sign_target_from_resolution(Some(&bare_manifest()), None).expect("a bare manifest needs no narrowing");
    ...
    #[test]
    fn no_platform_against_an_index_signs_the_index_itself() {
        let target = sign_target_from_resolution(Some(&image_index()), None).expect("an index is a legal subject");
        assert_eq!(
    ...
    #[test]
    fn a_platform_against_an_index_narrows_to_that_child() {
        let target =
            sign_target_from_resolution(Some(&image_index()), Some(&platform("linux/arm64"))).expect("arm64 is listed");
    ...
    #[test]
    fn a_platform_against_a_bare_manifest_is_refused_as_not_an_index() {
        let error = sign_target_from_resolution(Some(&bare_manifest()), Some(&platform("linux/amd64")))
    ...
    #[test]
    fn a_platform_the_index_does_not_list_is_still_target_not_found() {
        let _error = sign_target_from_resolution(Some(&image_index()), Some(&platform("windows/amd64")))
    ...
    /// an empty tail.
    #[test]
    fn an_unresolvable_reference_labels_the_absent_platform_as_any() {
        let error = sign_target_from_resolution(None, None).expect_err("nothing resolved");
        assert!(
            matches!(&error, SignErrorKind::TargetNotFound { platform } if platform == "any"),
            "expected the `any` label, got {error:?}"
        );
        let named = sign_target_from_resolution(None, Some(&platform("linux/amd64"))).expect_err("nothing resolved");
        assert!(
...
mod tests {
    ...
    use ocx_oci::native;
    use ocx_oci::referrer::capability::ReferrersApiCapability;
    ...
    fn map_client_error_keeps_invalid_image_index_internal_and_still_exits_65() {
        ...
        let invalid = ocx_oci::manifest::validate_image_index(&index).expect_err("schemaVersion 1 is refused");
    ...
    fn indirection_subject_digest() -> Digest {
        ocx_oci::Algorithm::Sha256.hash(b"indirected subject manifest")
    }
    ...
    fn indirecting_resolver<'a>(physical: PackageRef) -> Box<SubjectResolver<'a>> {
        Box::new(move |_identifier, platform| {
            ...
            Box::pin(async move {
                ...
                let target = sign_target_from_resolution(Some(&resolved), platform)?;
                Ok(ResolvedSubject {
                    target,
    ...
    /// index and the cosign sidecar) are empty until sign writes them.
    ...
    /// The sidecar a rival writer clobbers ours with: a well-formed
    /// simplesigning manifest carrying someone else's signature layer.
    fn rival_sidecar() -> Vec<u8> {
        serde_json::to_vec(&serde_json::json!({
            "schemaVersion": 2,
            "mediaType": OCI_IMAGE_MEDIA_TYPE,
            "config": {
                "mediaType": "application/vnd.oci.empty.v1+json",
                "digest": ocx_oci::referrer::media_types::EMPTY_CONFIG_DIGEST,
                "size": 2,
            },
            "layers": [{
                "mediaType": ocx_oci::referrer::media_types::SIMPLESIGNING_MEDIA_TYPE,
    ...
    impl Signer for FixedSigner {
        ...
        async fn sign_dsse(
            ...
            _: &Url,
            _: &Url,
        ) -> Result<crate::sign::bundle::SignedBundle, SignErrorKind> {
            ...
            *self.statement.lock().expect("statement lock") = Some(statement_bytes.to_vec());
            ...
            Ok(crate::sign::bundle::SignedBundle {
                key_backend: ocx_trust::key_ref::KeyBackendKind::Keyless,
                public_key_hint: None,
    ...
    async fn run_recorded_sign(mirrors: ocx_oci::client::MirrorMap) -> (Vec<String>, tempfile::TempDir) {
        let (result, calls, temp, _signer) = drive_sign(mirrors, recording_transport()).await;
    ...
    async fn drive_format(format: SignatureFormat) -> (SignResult, RecordingTransport, FixedSigner) {
        ...
        let (result, _calls, _temp, signer) = drive_sign_at(
            ...
            ocx_oci::client::MirrorMap::default(),
            transport.clone(),
            format,
        )
        .await;
    ...
    async fn a_simplesigning_sidecar_carries_the_claim_and_its_cosign_annotations() {
        ...
        assert_eq!(
            claim["critical"]["type"],
            crate::simplesigning::SIMPLESIGNING_CLAIM_TYPE
        );
        ...
        assert_eq!(
            layers[0]["mediaType"],
            ocx_oci::referrer::media_types::SIMPLESIGNING_MEDIA_TYPE,
        );
        assert_eq!(
            layers[0]["digest"],
            ocx_oci::Algorithm::Sha256.hash(&payload).to_string(),
    ...
    fn harness_claim_digest() -> Digest {
        ...
        ocx_oci::Algorithm::Sha256.hash(claim.to_signing_bytes().expect("claim serializes"))
    }
    ...
    async fn a_sidecar_already_holding_this_signature_is_left_alone() {
        ...
        let signed = crate::sign::SignedBlob {
            signature: b"test-signature".to_vec(),
            certificate_pem: None,
            chain_pem: None,
            rekor_bundle: None,
    ...
    async fn a_clobbered_sidecar_write_is_caught_by_the_read_back_and_retried() {
        let transport = recording_transport().clobbering_first_sidecar_put(&rival_sidecar());
    ...
    async fn signature_format_both_emits_each_shape() {
        ...
        assert_eq!(
            transport.referrer_manifests().len(),
            1,
    ...
    async fn signed_statement() -> serde_json::Value {
        let (result, _calls, _temp, signer) =
            drive_sign(ocx_oci::client::MirrorMap::default(), recording_transport()).await;
    ...
    async fn the_signature_referrer_announces_a_dsse_envelope_and_the_cosign_predicate_type() {
        ...
        let (result, _calls, _temp, _signer) =
            drive_sign(ocx_oci::client::MirrorMap::default(), transport.clone()).await;
        result.expect("sign must complete");

        let pushed = transport.referrer_manifests();
    ...
    async fn a_registry_with_the_referrers_api_gets_no_fallback_index() {
        let (calls, _state_dir) = run_recorded_sign(ocx_oci::client::MirrorMap::default()).await;
        assert!(
    ...
    async fn sign_reads_from_the_mirror_but_writes_to_the_canonical_host() {
        ...
        assert!(
            ReferrersApiCapability::from_cache("8.8.8.8", &state.referrers_capability_file("8.8.8.8"))
                .await
                .expect("cache read")
                .is_some(),
        ...
        assert!(
            ReferrersApiCapability::from_cache("mirror.example", &state.referrers_capability_file("mirror.example"))
                .await
                .expect("cache read")
                .is_none(),
    ...
    async fn sign_refuses_a_subject_manifest_the_read_host_served_under_a_different_digest() {
        ...
        let transport = recording_transport()
            .serving_subject_digest(ocx_oci::Algorithm::Sha256.hash(b"a different manifest").to_string());
        let (result, calls, _state_dir, _signer) = drive_sign(ocx_oci::client::MirrorMap::default(), transport).await;
