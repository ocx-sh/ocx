...
"#;
        ...
        let standalone = crate::declaration_hash(&config);
...
"#,
        ...
        let super::super::Error::Project(pe) = &err else {
...
"#;
        ...
        assert_eq!(
            crate::declaration_hash(&base_config),
            crate::declaration_hash(&policy_config),
...
"#;
        ...
        assert_eq!(
            crate::declaration_hash(&config_without),
            crate::declaration_hash(&config_with),
...
"#;
        ...
        assert_eq!(group.env.get("CI"), Some(&crate::EnvValue::constant("1")));
    }
...
"#;
        ...
        let crate::Error::Project(ref pe) = err else {
            panic!("expected a project-tier error, got {err:?}");
        };
        assert!(
...
"#;
    ...
    fn project_env_cannot_declare_trust_sensitive_ocx_keys() {
        for key in [
            ...
            for scope in ["env", "group.ci.env"] {
                ...
                #[allow(irrefutable_let_patterns)]
                let crate::Error::Project(project_error) = err else {
                    panic!("expected Error::Project");
                };
    ...
    async fn resolve_explicit_missing_returns_file_not_found_error() {
        ...
        assert!(
            matches!(
                err,
                ocx_config::error::Error::FileNotFound {
                    path: ref p,
                    tier: ocx_config::error::ConfigSource::Project,
                } if *p == missing,
            ),
...
"#;
    ...
    fn refuse_ocx_toml(toml: &str) -> crate::error::ProjectError {
    ...
    fn c015_the_folded_refusal_keeps_each_keywords_own_shipped_hint() {
        let default_err = refuse_ocx_toml("[group.Default]\n");
        ...
        let all_err = refuse_ocx_toml("[group.ALL]\n");
    ...
    fn no_name_is_both_reserved_and_off_charset_so_the_two_refusals_cannot_race() {
        ...
        let pe = refuse_ocx_toml("[group.\"Default \"]\n");
        assert!(
    ...
    fn ordering_the_reserved_default_guard_precedes_the_group_name_validator() {
        let pe = refuse_ocx_toml("[group.\"a b\"]\n\n[group.default]\n");
        assert!(
            matches!(&pe.kind, ProjectErrorKind::ReservedGroupName { name, .. } if name == "default"),
    ...
    fn c012_an_unknown_activate_value_refuses_in_the_file_while_the_env_tier_falls_through() {
        ...
        let pe = refuse_ocx_toml("activate = \"\"\n");
        assert!(
            matches!(&pe.kind, ProjectErrorKind::TomlParse(_)),
    ...
    async fn c012_the_global_ocx_toml_is_parsed_by_the_same_loader_with_the_same_refusal() {
        ...
        let _err = ProjectConfig::from_path(&path)
            .await
    ...
    fn c012_declaration_hash_is_unchanged_by_an_activate_or_a_pinned_flip() {
        ...
        let expected = crate::declaration_hash(&base_config);
        assert_eq!(
            crate::declaration_hash(&activate_config),
            expected,
            "an `activate` flip must not invalidate ocx.lock"
        );
        assert_eq!(
            crate::declaration_hash(&pinned_config),
            expected,
    ...
    fn c012_activate_and_pinned_do_not_short_circuit_the_name_validator() {
        let pe = refuse_ocx_toml("activate = \"bin\"\npinned = true\n\n[tools]\n\"a b\" = \"ocx.sh/x:1\"\n");
        assert!(
    ...
    // ── C-001 / C-033 — keys an `ocx.toml` must refuse ──────────────────────

    /// A real self-signed CA (the test stack's Fulcio root), so the inline form
    /// the refusal below is fed is material a fleet consumer could actually
    /// seed a client from — not a placeholder that would be refused by spelling
    /// anyway.
    ...
    /// neither `extra_ca_certs` key — a cloned repository can never add a
    /// trust root. Refused as an unknown key: `ProjectErrorKind::TomlParse`,
    /// exit 78.
    ///
    /// The two keys it refuses are `config.toml`'s, and the config loader has
    /// its own tests that they are *accepted* there; this is the other half of
    /// that pair, and it belongs to the type that does the refusing.
    #[test]
    fn ocx_toml_carrying_an_extra_ca_certs_key_is_refused() {
        for payload in [
            ...
            let err = crate::ProjectConfig::from_toml_str(payload)
                .expect_err("ocx.toml must refuse either extra_ca_certs spelling");
            let crate::Error::Project(project_error) = &err else {
                panic!("expected a project-tier error, got {err:?}");
            };
            assert!(
                matches!(project_error.kind, crate::ProjectErrorKind::TomlParse(_)),
    ...
    /// security property in a different file rests on it. The explicit strip in
    /// `ConfigLoader::fold_project_tier` — pinned by that module's own
    /// `c033_a_project_tier_fold_cannot_contribute_shell_consent` — is what
    /// makes the property structural; this pins the second door shut.
    #[test]
    fn c033_shell_in_a_project_file_is_refused() {
        let refused = toml::from_str::<crate::ProjectConfig>("[shell]\nhook = true\n");
        assert!(
            refused.is_err(),
