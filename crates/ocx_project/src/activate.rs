...
mod tests {
    ...
    // here:** "warns" versus "does not warn". The `log` crate is a process-wide
    // facade, and the crate's only capture (`oci/index/local_index.rs`)
    ...
    fn the_ladder_is_constructed_only_inside_the_two_shared_resolvers() {
        ...
        const ALLOWED: &[&str] = &[
            "ocx_project/src/ladder.rs",
            "ocx_project/src/activate.rs",
            "ocx_package_manager/src/activation.rs",
            "ocx_package_manager/src/composer.rs",
        ];

        let crates = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .expect("crates/ocx_project has a parent")
            .to_path_buf();
        let mut offenders: Vec<String> = Vec::new();
        let mut scanned = 0usize;
        // Both trees asserted present, AND the walk asserted non-empty below: a
        // scan that finds nothing because it looked nowhere is the green that
        // never ran, and `is_dir` alone cannot tell an emptied tree from a full
        // one (C-048).
        // Was `["ocx_lib/src", "ocx_cli/src"]`. WP-37 deleted `ocx_lib`, and
        // re-pointing the pair at the two crates that received the ladder would
        // have narrowed the scan to the files `ALLOWED` already exempts — a
        // walk that can only ever find what it is told to ignore. Every member
        // instead: strictly wider than the pair, and self-maintaining across
        // the remaining extractions.
        let mut roots = 0usize;
        for entry in std::fs::read_dir(&crates).expect("crates/ is readable") {
            let root = entry.expect("a crates/ entry is readable").path().join("src");
            if root.is_dir() {
                roots += 1;
                scan_for_ladder_constructions(&root, &crates, ALLOWED, &mut offenders, &mut scanned);
            }
        }
        assert!(
            roots > 10,
            "only {roots} crate(s) with a `src/` were found under {} — the oracle for this scan has \
             itself collapsed, and a narrow walk finds no offender for the wrong reason",
            crates.display()
        );
        assert!(
            scanned > 100,
            "the walk read only {scanned} source files under {} — it scanned almost nothing, \
             so the refusal below would pass for the wrong reason",
            crates.display()
        );
    ...
    fn scan_for_ladder_constructions(
        ...
        for entry in std::fs::read_dir(dir).expect("the source tree is readable") {
            ...
            if path.is_dir() {
                scan_for_ladder_constructions(&path, crates, allowed, offenders, scanned);
                continue;
            }
