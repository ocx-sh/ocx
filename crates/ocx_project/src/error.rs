...
pub enum Error {
    ...
    /// An OCI registry client call made while resolving a lock failed.
    ///
    /// Carried rather than flattened, and classified by delegation, because
    /// `ClientError` distinguishes transient from terminal and the exit code
    /// follows that distinction. Flattening it here would collapse 69 and 74
    /// into one (E1, DEC-23).
    #[error(transparent)]
    OciClient(#[from] ocx_oci::client::error::ClientError),
    /// An index-tier failure surfaced through lock resolution.
    // Deliberately NOT `#[from]`: the conversion below flattens two of the
    // index tier's variants into this tier's own, and a derived `From` would
    // wrap them instead. `resolve::classify_client_error` depends on that
    // flattening — `#[error(transparent)]` makes `source()` step straight over
    // a `ClientError`, so the classifier special-cases `Error::OciClient` and
    // can only see a client fault that arrives unwrapped.
    #[error(transparent)]
    ...
    /// An I/O failure with the offending path attached, from a helper that
    /// has no project-tier context to add.
    ///
    /// The same shape, and the same `IoError` (74) classification, that
    /// `ocx_lib::Error::InternalFile` gave before the move.
    #[error("{0}: {1}")]
...
///
/// Mirrors `ocx_lib`'s conversion for the two variants `ocx_project::Error`
/// has a destination for. It is not cosmetic: a derived `#[from]` yields
/// `OciIndex(OciClient(Registry(..)))`, and because both wrappers are
/// `#[error(transparent)]` a `source()` walk delegates past the `ClientError`
/// without ever yielding it. `resolve::classify_client_error` then reads a
/// transient 503 as `ClientFailure::Other`, the retry budget is never spent,
/// and `ProjectErrorKind::RegistryUnreachable`'s own `downcast_ref::<ClientError>`
/// misses for the same reason — so a transient fault exits 69 instead of 75.
/// The classifier's premise lives here, not in `resolve.rs`.
...
pub enum ProjectErrorKind {
    ...
    /// format-preserving mutation (see `crate::document`).
    ...
    /// the lossless, injective canonical [`ocx_oci::Platform`] grammar.
    ...
    /// canonical grammar spelling of the [`ocx_oci::Platform`] it parses
    ...
    /// [`ocx_config::env::Env::apply_ocx_config`] would forward the result to
    /// every child process.
    ...
    /// (`[A-Za-z_][A-Za-z0-9_]*`), per [`ocx_util::env::is_valid_env_key`].
    ...
    /// vocabulary. Message text mirrors [`ocx_package::metadata::env::modifier::ParseModifierKindError`]
    ...
    /// [`separator_is_valid`](ocx_package::metadata::env::list::separator_is_valid).
    #[error(
    ...
    /// Mirrors the ambient config loader's [`ocx_config::ConfigErrorKind::FileTooLarge`]
    ...
    /// (`crate::ProjectConfig::from_toml_str`); surfacing the
    ...
    ///
    /// Classifies as `ExitCode::TempFail` (75):
    /// a deadline on a registry
    ...
    /// [`crate::compose_tool_set`] as defense-in-depth when the
    /// CLI layer's pre-load check is bypassed by a non-CLI consumer.
    #[error("ocx.lock is missing; run `ocx lock`")]
    LockMissing,

    /// A binding name passed to [`crate::resolve_lock_touched`]
    ...
    /// [`crate::lock::LockMetadata::declaration_hash`] does not
    /// match the **pre-mutation** declaration hash of `ocx.toml`.
    ///
    /// Surfaced from [`crate::resolve_lock_touched`] (the
    ...
    ///
    /// Surfaced from the resolver ([`crate::resolve`]) when the
    /// index layer returns [`ocx_index::error::Error::PolicyResolutionBlocked`]:
