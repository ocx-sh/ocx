...
mod tests {
    ...
    fn link_name(project_dir: &Path) -> String {
        ocx_store::reference_manager::ReferenceManager::name_for_path(project_dir)
    }
    ...
    async fn a30_best_effort_registration_keys_off_the_shared_canonicalizer() {
        ...
        let expected = crate::consent::canonical_project_dir(&config_path).expect("the shared canonicalizer");
    ...
    ///
    /// No log-capture facility exists in `ocx_lib` (the `log` crate is a bare
    /// process-global facade), so the no-WARN expectation cannot be
