...
async fn fan_out_candidates(
    ...
    let candidates = index
        ...
        .await
        .map_err(|err| candidate_fetch_error(err.into(), identifier.clone()))?;
...
fn classify_index_error(err: &ocx_index::error::Error) -> ClientFailure {
    ...
    if let IndexError::SourceWalkFailed(arc) = err {
        return classify_index_error(arc.as_error());
    }
    let mut cause: Option<&(dyn std::error::Error + 'static)> = std::error::Error::source(err);
    while let Some(c) = cause {
        ...
        let manifests = children
            ...
            .map(|(digest, plat)| ImageIndexEntry {
                ...
                platform: plat.map(|p| {
                    ...
                    ocx_oci::native::Platform::from(&ocx)
                }),
                artifact_type: None,
                annotations: None,
            })
...
"#,
    ...
    type ScriptedResponse = ocx_index::error::Result<Option<Digest>>;
    ...
    async fn resolve_lock_retry_exhausted_on_transient_fault_classifies_as_temp_fail() {
        ...
        let _err = resolve_lock(&config, &index, &[], fast_options())
            .await
            .expect_err("retry exhaustion must surface as an error");
    ...
    async fn resolve_lock_timeout_classifies_as_temp_fail() {
        ...
        let _err = resolve_lock(&config, &index, &[], options)
            .await
