...
//! gate ([`LockCurrency::Stale`](crate::LockCurrency)) fires on every subsequent
...
impl MutationGuard {
    ...
    /// to choose between [`crate::resolve_lock_touched`]
    /// (predecessor present — carries untouched bindings forward,
    /// fail-closed on drift) and [`crate::resolve_lock`]
