...
fn platform(spec: &str) -> ocx_oci::Platform {
...
/// [`merge_platform_into_index`](ocx_oci::Client::merge_platform_into_index) per
...
impl Registry {
    ...
    fn drop_entry(&self, tag: &str, platform_spec: &str) {
        ...
        let ocx_oci::Manifest::ImageIndex(mut index) =
            serde_json::from_slice::<ocx_oci::Manifest>(&bytes).expect("a replayed body parses")
        else {
        ...
        let body = serde_json::to_vec(&ocx_oci::Manifest::ImageIndex(index)).expect("an index serializes");
        let digest = ocx_oci::Algorithm::Sha256.hash(&body).to_string();
...
async fn h9_a_swallowed_blocker_fetch_leaves_drift_that_check_finds_and_repair_closes() {
    ...
    let served: BTreeSet<String> = observation
        ...
        .filter_map(|tag| match &tag.manifest {
            ocx_oci::Manifest::ImageIndex(index) => Some(index.manifests.iter().map(|entry| entry.digest.clone())),
            ocx_oci::Manifest::Image(_) => None,
        })
        .flatten()
        .collect();
