...
use crate::version;
use ocx_index::{Index, IndexRoot, OcxIndex};
...
mod tests {
    ...
    fn client(data: &StubTransportData) -> ocx_oci::Client {
    ...
    fn seed_raw(data: &StubTransportData, tag: &str, bytes: Vec<u8>) {
        let digest = ocx_oci::Algorithm::Sha256.hash(&bytes).to_string();
    ...
    fn index_bytes(tag: &str, platforms: &[&str], schema_version: u8) -> Vec<u8> {
        let manifests = platforms
            .iter()
            .map(|platform| {
                let parsed: ocx_oci::Platform = platform.parse().expect("test platform must parse");
    ...
    fn seed_index(data: &StubTransportData, tag: &str, platforms: &[&str]) {
        seed_raw(data, tag, index_bytes(tag, platforms, ocx_oci::INDEX_SCHEMA_VERSION));
    }
    ...
    /// `source()`.
    fn chain(error: &PackageError) -> String {
    ...
    /// `ocx_index::ocx_index`'s tests: a present entry is a `200`, an absent
    ...
    fn seeded_index_source(committed: &ocx_oci::Digest) -> (OcxIndex, StubIndexTransport) {
    ...
    fn some_digest() -> ocx_oci::Digest {
        ocx_oci::Algorithm::Sha256.hash(b"committed index")
    }
    ...
    async fn f6_overlapping_fetches_keep_every_tag_paired_with_its_own_body() {
        ...
        let expected: Vec<(String, String)> = tags
            .iter()
            .map(|tag| {
                let digest = ocx_oci::Algorithm::Sha256
                    .hash(index_bytes(tag, &["linux/amd64"], ocx_oci::INDEX_SCHEMA_VERSION))
                    .to_string();
                (tag.clone(), digest)
            })
            .collect();
    ...
    async fn f7_a_bare_image_manifest_is_kept_as_a_bare_manifest() {
        ...
        assert!(
            matches!(latest.manifest, ocx_oci::Manifest::Image(_)),
        ...
        assert!(matches!(leaf.manifest, ocx_oci::Manifest::ImageIndex(_)));
        assert_eq!(
            leaf.size,
            i64::try_from(index_bytes("3.28.1", &["linux/amd64"], ocx_oci::INDEX_SCHEMA_VERSION).len())
    ...
    async fn f10_an_oversized_manifest_body_aborts() {
        ...
        let oversized = vec![b'{'; ocx_oci::client::MAX_INDEX_DOCUMENT_BYTES + 1];
    ...
    async fn f12_reads_authenticate_for_pull_only() {
        ...
        assert!(
            auth_calls
                .iter()
                .all(|(_, operation)| matches!(operation, ocx_oci::RegistryOperation::Pull)),
    ...
    async fn f16_a_logical_package_with_no_published_root_gathers_without_the_index_layer() {
        ...
        let expected = crate::cascade::graph::fold_expected(&observation);
        let scope =
            crate::cascade::graph::scope_filter(&observation.versions(), &[None]).expect("the whole graph is in scope");
        let report = crate::cascade::graph::diff(&observation, &expected, &scope);
        assert!(
