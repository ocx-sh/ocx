...
mod tests {
    ...
    fn child_key(digest: &str) -> String {
        test_identifier()
            .without_tag()
            .clone_with_digest(ocx_oci::Digest::try_from(digest).expect("well-formed digest"))
            .canonical_reference()
            .to_string()
    }
    ...
    fn planned_write(tag: AliasTag, children: &[String]) -> PlannedWrite {
        let manifests = children
            ...
            .enumerate()
            .map(|(position, digest)| {
                let platform: ocx_oci::Platform =
                    PLATFORMS[position % PLATFORMS.len()].parse().expect("valid platform");
    ...
    fn reason(tag: &AliasTag, position: usize, status: SlotStatus, observed: &str) -> crate::cascade::graph::SlotRow {
        let platform: ocx_oci::Platform = PLATFORMS[position % PLATFORMS.len()].parse().expect("valid platform");
        crate::cascade::graph::SlotRow {
            tag: tag.clone(),
    ...
    async fn post_write_digest_mismatch_warns_instead_of_failing() {
        ...
        write.observed_digest = Some(ocx_oci::Digest::try_from(child_digest(0xEE).as_str()).expect("well-formed"));
    ...
    async fn a_dead_orphan_is_dropped_and_its_alias_is_still_written() {
        ...
        let written: ocx_oci::ImageIndex = serde_json::from_slice(&pushed.0).expect("a published index parses");
        assert_eq!(
    ...
    async fn g15_an_alias_that_moved_since_the_gather_is_not_written() {
        ...
        moved.observed_digest = Some(ocx_oci::Digest::try_from(child_digest(0x11).as_str()).expect("well-formed"));
