...
use super::*;
use ocx_index::{IndexRoot, RootTag};
...
///
/// Deliberately not [`ocx_oci::Platform`]'s grammar: the fold keys on what
...
fn entry(platform_spec: &str, digest: &str) -> ocx_oci::ImageIndexEntry {
...
fn image_index(entries: Vec<ocx_oci::ImageIndexEntry>) -> ocx_oci::ImageIndex {
    ocx_oci::ImageIndex {
...
fn tag_digest(tag: &str) -> ocx_oci::Digest {
...
#[derive(Default)]
struct Graph {
    seeds: Vec<(String, Option<ocx_oci::ImageIndex>)>,
    root: Option<Vec<(String, ocx_oci::Digest)>>,
}
...
impl Graph {
    ...
    fn index(mut self, tag: &str, entries: Vec<ocx_oci::ImageIndexEntry>) -> Self {
    ...
    fn raw(mut self, tag: &str, index: ocx_oci::ImageIndex) -> Self {
    ...
    fn root(mut self, commits: &[(&str, ocx_oci::Digest)]) -> Self {
        self.root = Some(
            commits
                .iter()
    ...
    fn root_agreeing(self, tags: &[&str]) -> Self {
        let commits: Vec<(&str, ocx_oci::Digest)> = tags.iter().map(|tag| (*tag, tag_digest(tag))).collect();
        self.root(&commits)
    }

    fn build(self) -> TagGraphObservation {
        let names: Vec<&str> = self.seeds.iter().map(|(tag, _)| tag.as_str()).collect();
        let variants = crate::version::variant_names(names.iter().copied());
        ...
        for (tag, body) in &self.seeds {
            ...
            let manifest = match body {
                Some(index) => ocx_oci::Manifest::ImageIndex(index.clone()),
                None => ocx_oci::Manifest::Image(ocx_oci::ImageManifest::default()),
            };
            tags.insert(
                alias,
                ObservedTag {
...
fn findings_for_alias_entry(alias_entry: ocx_oci::ImageIndexEntry) -> Vec<String> {
    let observation = Graph::new()
        .index("1.0.0", vec![entry("linux/amd64", "image")])
...
fn c22_an_index_committing_a_different_digest_is_stale() {
    ...
    let observation = Graph::new()
        ...
        .index("latest", content())
        .root(&[
            ("latest", ocx_oci::Digest::Sha256("older".to_string())),
            ("1.0", tag_digest("1.0")),
            ("1", tag_digest("1")),
        ])
        .build();
    ...
    assert_eq!(
        ...
        [IndexFinding::Stale {
            ...
            committed: ocx_oci::Digest::Sha256("older".to_string()),
...
fn d9_a_digest_reference_names_one_manifest_not_a_graph() {
    let identifier = identifier().clone_with_digest(ocx_oci::Digest::Sha256("a".repeat(64)));
...
fn e11_the_plan_does_not_depend_on_tag_order() {
    let build = |reversed: bool| {
        ...
        let seeds: Vec<(&str, Vec<ocx_oci::ImageIndexEntry>)> = vec![
...
fn e13_every_planned_index_pins_the_schema_version() {
    ...
    for write in plan_of(&observation) {
        assert_eq!(write.index.schema_version, ocx_oci::INDEX_SCHEMA_VERSION);
...
fn e15_applying_a_plan_makes_the_graph_clean_and_the_next_plan_empty() {
    ...
    for write in &writes {
        ...
        applied.tags.insert(
            write.tag.clone(),
            ObservedTag {
                digest: ocx_oci::Algorithm::Sha256.hash(&body),
                size: i64::try_from(body.len()).expect("test body fits in i64"),
                manifest: ocx_oci::Manifest::ImageIndex(write.index.clone()),
            },
        );
    }
