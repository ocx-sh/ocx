...
pub(super) fn validate_env_list_entries(metadata: &Metadata) -> Result<(), PackageError> {
    ...
    for var in env {
        ...
        let Some(separator) = declared.separator.as_deref() else {
            return Err(Error::MissingListSeparator { key: var.key.clone() });
        };
...
/// # Errors
///
/// [`crate::error::Error::IntegrationInterpolation`] for the first
/// offending namespace, in lexicographic order.
pub(super) fn validate_integration_tokens(metadata: &Metadata) -> Result<(), PackageError> {
    use super::super::error::Error;
...
mod tests {
    ...
    fn unknown_env_modifier_type_exits_with_data_error() {
        ...
        let _error = ValidMetadata::try_from(meta).expect_err("unknown type must fail");
    }
    ...
    fn a_reserved_env_key_exits_with_data_error() {
        ...
        let _error = validate_for_publish(meta).expect_err("a reserved env key must fail");
    }
    ...
    fn a_list_without_a_separator_exits_with_data_error() {
        ...
        let _error = ValidMetadata::try_from(meta).expect_err("missing separator must fail");
    }
    ...
    fn an_unrecognised_token_exits_with_data_error_at_publish() {
        let _error = validate_for_publish(unrecognised_token_document()).expect_err("an unrecognised token must fail");
    }
    ...
    fn entrypoint_arg_deps_token_rejected() {
        ...
        let package_error = &err;
        let PackageError::EntrypointArgInterpolation { entrypoint, source, .. } = package_error else {
    ...
    fn entrypoint_arg_escaped_then_real_deps_token_still_rejected() {
        ...
        let package_error = err;
        let crate::error::Error::EntrypointArgInterpolation {
    ...
    fn a_forward_self_env_reference_is_refused_at_publish() {
        ...
        let PackageError::EnvVarInterpolation { var_key, source } = package_error else {
    ...
    fn a_forward_self_env_reference_exits_with_data_error() {
        let _error = validate_for_publish(self_env_pair(true)).expect_err("a forward reference must fail");
    }
    ...
    fn integrations_of_exact_bytes(target: usize) -> serde_json::Value {
        use crate::metadata::integrations::MAX_INTEGRATION_NAMESPACE_BYTES;
    ...
    fn an_invalid_namespace_key_is_refused_at_ingress() {
        ...
        let package_error = &error;
        let PackageError::IntegrationNamespaceInvalid { namespace, reason } = package_error else {
    ...
    fn over_cap_namespace_payload_exits_with_data_error() {
        ...
        let _error = ValidMetadata::try_from(meta).expect_err("over cap must fail");
    }

    #[test]
    fn total_integrations_map_over_the_per_package_cap_is_rejected_even_when_every_namespace_is_within_its_own_cap() {
        use crate::error::Error as PackageError;
        use crate::metadata::integrations::{MAX_INTEGRATION_NAMESPACE_BYTES, MAX_INTEGRATIONS_BYTES};
        ...
        let package_error = &error;
        let PackageError::IntegrationsTooLarge { size, max } = package_error else {
    ...
    fn a_integrations_map_at_the_per_package_cap_boundary_is_accepted() {
        use crate::metadata::integrations::MAX_INTEGRATIONS_BYTES;
    ...
    fn a_self_env_token_in_a_payload_is_refused_as_disallowed() {
        ...
        let package_error = &error;
        let PackageError::IntegrationInterpolation { namespace, source } = package_error else {
