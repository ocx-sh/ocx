...
mod tests {
    ...
    fn resolved(key: &str, value: &str) -> Entry {
        Entry {
            ...
            kind: crate::metadata::env::modifier::ModifierKind::Constant,
            separator: None,
        }
    }
    ...
    /// The two ways a `${self.env.KEY}` reference is refused, each pinned to
    /// the variant it produces: a key that was never declared, and a key
    /// declared twice before the reference so neither candidate is privileged.
    ///
    /// Both exit 65, and that half is asserted beside the impl in
    /// `ocx::exit::ocx_package` — the resolver that mints these values is
    /// private to this crate, and the ladder that reads them is in the binary,
    /// so neither half can hold both ends.
    #[test]
    fn self_env_refusals_name_their_own_variant() {
        let dir = TempDir::new().unwrap();
        let contexts: HashMap<DependencyName, DependencyContext> = HashMap::new();
        let scope = scope_of([resolved("A", "first"), resolved("A", "second")]);
        let resolver = TemplateResolver::new(dir.path(), &contexts).with_self_env(&scope);

        let undefined = resolver.resolve("${self.env.MISSING}").unwrap_err();
        assert!(
            matches!(&undefined, TemplateError::UndefinedSelfEnvRef { key, .. } if key == "MISSING"),
            "an undeclared key must be refused as undefined, naming it: {undefined}"
        );

        let ambiguous = resolver.resolve("${self.env.A}").unwrap_err();
        assert!(
            matches!(&ambiguous, TemplateError::AmbiguousSelfEnvRef { key } if key == "A"),
            "a twice-declared key must be refused as ambiguous, not resolved to one of them: {ambiguous}"
        );
    }
