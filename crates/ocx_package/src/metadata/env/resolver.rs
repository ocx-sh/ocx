...
impl<'a> EnvResolver<'a> {
    ...
    fn resolve_inner(
    ...
    ) -> Result<Option<Entry>, PackageError> {
        ...
        let mut value = resolved.map_err(|source| crate::error::Error::EnvVarInterpolation {
        ...
        if let Modifier::Path(path_modifier) = &var.modifier {
            ...
            if emit_assertions
                ...
                && !path.exists()
            {
                return Err(crate::error::Error::RequiredPathMissing(path));
            }
...
mod tests {
    ...
    use crate::metadata::env::var::Var;
    use crate::metadata::template::TemplateError;
    use std::collections::HashMap;
    ...
    fn list_var(key: &str, value: &str, separator: &str) -> Var {
        Var {
            ...
            visibility: crate::metadata::visibility::Visibility::PRIVATE,
        }
    }
    ...
    fn a_value_resolving_to_a_separator_edged_one_is_refused() {
        ...
        // `crate::error::Error::SeparatorEdgedListValue`), so the needle is
    ...
    fn a_missing_required_path_is_asserted_only_where_the_value_is_emitted() {
        ...
        assert!(
            matches!(&error, PackageError::RequiredPathMissing(_)),
