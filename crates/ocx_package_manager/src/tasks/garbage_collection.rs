...
mod tests {
    ...
    fn seed_companion_pkg_dir(root: &std::path::Path, pinned: &ocx_oci::PinnedIdentifier) -> std::path::PathBuf {
        ...
        let resolved_json = serde_json::to_string(&ocx_package::resolved_package::ResolvedPackage::new()).unwrap();
    ...
    fn seed_blob_dir(root: &std::path::Path, registry: &str, digest: &ocx_oci::Digest) -> std::path::PathBuf {
        let store = ocx_store::file_structure::BlobStore::new(root.join("blobs"));
    ...
    async fn patch_roots_retains_companion_pkg_and_descriptor_blob() {
        ...
        let fs = ocx_store::file_structure::FileStructure::with_root(root.to_path_buf());
