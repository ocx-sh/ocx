...
use ocx_store::file_structure::{CasTier, FileStructure, ShimDir};
...
async fn index_retention_pairs(data_path: &Path, blob_dir: &Path, registry_root: &Path) -> Vec<(PathBuf, PathBuf)> {
    ...
    let Ok(ocx_oci::Manifest::ImageIndex(index)) = serde_json::from_slice::<ocx_oci::Manifest>(&bytes) else {
        return Vec::new();
    };
    ...
    for entry in &index.manifests {
        let Ok(child_digest) = ocx_oci::Digest::try_from(entry.digest.as_str()) else {
            continue;
        };
        let child_dir =
            canonicalize_or_keep(&registry_root.join(ocx_store::file_structure::cas_shard_path(&child_digest)));
...
async fn has_live_refs(pkg_dir: &Path) -> bool {
    ...
    while let Ok(Some(entry)) = entries.next_entry().await {
        ...
        let path = entry.path();
        if ocx_util::fs::symlink::is_link(&path)
            && let Ok(target) = tokio::fs::read_link(&path).await
            && ocx_util::fs::path_exists_lossy(&target).await
        {
            return true;
        }
    }
...
pub mod tests {
    ...
    use ocx_store::file_structure::CasTier;
    ...
    async fn large_index_manifest_still_retains_via_child_edge() {
        use ocx_store::file_structure::{BlobDir, cas_shard_path};
        ...
        let child_digest = ocx_oci::Digest::Sha256(child_hex.clone());
        ...
        let mut manifests = vec![ocx_oci::ImageIndexEntry {
        ...
        for i in 0..200 {
            manifests.push(ocx_oci::ImageIndexEntry {
                media_type: "application/vnd.oci.image.manifest.v1+json".to_string(),
        ...
        let index_digest = ocx_oci::Digest::Sha256("c".repeat(64));
    ...
    /// 32 hex characters.
    fn digest_of(nibble: char) -> ocx_oci::Digest {
    ...
    async fn seed_blob(file_structure: &FileStructure, registry: &str, digest: &ocx_oci::Digest) -> PathBuf {
    ...
    async fn seed_shim(
        ...
        for digest in config_blobs {
            ...
            ocx_util::fs::symlink::update(
                file_structure.blobs.data(pinned.registry(), digest),
                shim.refs_blobs_dir()
                    .join(ocx_store::file_structure::cas_ref_name(digest)),
            )
            .unwrap();
        }
        ...
        for blob in blob_dirs {
            ...
            for entry in &index.manifests {
                ...
                let child_dir = super::canonicalize_or_keep(
                    &registry_root.join(ocx_store::file_structure::cas_shard_path(&child_digest)),
                );
    ...
    async fn build_synthetic_store(
        ...
        noise_count: usize,
    ) -> Vec<ocx_store::file_structure::BlobDir> {
        use ocx_store::file_structure::{BlobDir, cas_shard_path};
        ...
        let unique = |prefix: u8, counter: usize| -> ocx_oci::Digest {
            ocx_oci::Digest::Sha256(format!("{prefix:02x}{counter:030x}{:032x}", 0u64))
        };
        ...
        for index_number in 0..index_count {
            let manifests = (0..children_per_index)
                .map(|child_number| ocx_oci::ImageIndexEntry {
                    media_type: "application/vnd.oci.image.manifest.v1+json".to_string(),
