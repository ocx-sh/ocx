...
//! 3. [`ToolchainHome::ensure_gitignore`](ocx_store::file_structure::ToolchainHome::ensure_gitignore)
...
//! 4. **One [`lock_scoped`](ocx_util::fs::lock_scoped) over the home, for
...
//! [`ShimBinStore`](ocx_store::file_structure::ShimBinStore) blob — plus
//! `<name>.exec`, the sidecar carrying the baked selector and, on its second
//! line, the absolute `ocx` this render resolved (C-031, V-9).
//! **Both get their own
//! [`bin_fingerprint`](ocx_store::file_structure::RenderStamp::bin_fingerprint)
//! entry, keyed by the full on-disk file name** (RUL-26), so both appear in
//! [`RenderStamp::names`](ocx_store::file_structure::RenderStamp::names) and both
...
//! [`BinEntryStamp::file_id`](ocx_store::file_structure::BinEntryStamp::file_id),
...
//! [`move_dir`](ocx_util::fs::move_dir), which calls `remove_dir_all` on
...
//! [`symlink::replace_atomic`](ocx_util::fs::symlink::replace_atomic), never through
//! [`ReferenceManager`](ocx_store::reference_manager::ReferenceManager) — see the
//! carve-out section in
//! [`toolchain_store`](ocx_store::file_structure::ToolchainHome) for why the
...
/// [`project::mutation`](ocx_project::mutation) — one probe at render
...
pub struct RenderRequest<'a> {
    ...
    /// [`resolve_toolchain_home`](ocx_project::resolve_toolchain_home)'s
    /// answer for a project.
    ///
    /// Its root may not exist yet:
    /// [`ToolchainRoot::resolve`](ocx_config::ToolchainRoot::resolve)
    ...
    ///
    /// [`StateStore::set_render_stamp`](ocx_store::file_structure::StateStore::set_render_stamp)
    /// takes a [`RenderStampTarget`](ocx_store::file_structure::RenderStampTarget),
    ...
    ///   `RenderStampTarget::Project(&`[`ReferenceManager::name_for_path`](ocx_store::reference_manager::ReferenceManager::name_for_path)`(dir))`.
    ///
    /// **`dir` must already be the canonical project directory** —
    /// `project::consent::canonical_project_dir`'s answer, the same value
    /// [`ConsentStamp`](ocx_project::consent::ConsentStamp) records — not
    ...
    /// `bin/` covers [`DEFAULT_GROUP`](ocx_project::DEFAULT_GROUP) and
    ...
    ///
    /// `bin/` still covers [`DEFAULT_GROUP`](ocx_project::DEFAULT_GROUP) and
...
/// since it can match no [`BinaryName`](ocx_package::metadata::BinaryName)
...
pub enum RenderedArtifact {
    ...
    /// [`BinaryName`](ocx_package::metadata::BinaryName). Writes still go
...
pub struct RenderReport {
    ...
    ///
    /// `true` when [`DEFAULT_GROUP`](ocx_project::DEFAULT_GROUP) was among
    ...
    /// [`RenderStamp::names`](ocx_store::file_structure::RenderStamp::names)
    /// documents itself as "the on-disk entry set" and
    /// [`RenderStamp::new`](ocx_store::file_structure::RenderStamp::new) derives
...
impl PackageManager {
    ...
    /// [`ComposeRoots::roots`](crate::composer::ComposeRoots)
    ...
    /// [`DEFAULT_GROUP`](ocx_project::DEFAULT_GROUP) and nothing else
    /// (C-045).
    ///
    /// # Errors
    ///
    ...
    /// [`LockedTool::platforms`](ocx_project::LockedTool) through the shared
    /// [`select_best`](ocx_oci::platform::select_best) /
    /// [`is_compatible`](ocx_oci::platform::is_compatible) helper —
    ...
    ///
    /// A [`Selection::None`](ocx_oci::platform::Selection) — no compatible
    ...
    ///   [`BinaryName`](ocx_package::metadata::BinaryName), so no
...
impl ScopedLockParameters {
    ...
    /// # Errors
    ///
    /// [`lock_scoped`](ocx_util::fs::lock_scoped)'s own — the lock file
...
/// (C-072).
///
/// [`ToolchainHome::links_group`](ocx_store::file_structure::ToolchainHome::links_group)
...
/// [`ocx_util::env::var`], never by `std::env::var_os`, so an env-only seam would
...
/// [`BinEntryStamp::from_metadata`](ocx_store::file_structure::BinEntryStamp::from_metadata),
...
/// [`replace_atomic`](ocx_util::fs::symlink::replace_atomic)'s POSIX arm is a `rename`
...
/// [`skipped_render_warnings`](crate::skipped_render_warnings)
...
/// [`BinEntryStamp::of_file`](ocx_store::file_structure::BinEntryStamp::of_file) —
/// the one derivation this producer shares with the prompt gate that checks its
/// output (`crate::activation::bin_stamp_matches`, RUL-89). Spelling the read and the
...
/// and a [`lock_scoped`](ocx_util::fs::lock_scoped) repoint on
...
/// [`select_best`](ocx_oci::platform::select_best) /
/// [`is_compatible`](ocx_oci::platform::is_compatible) helper the render uses
...
///   `-g`, else [`DEFAULT_GROUP`](ocx_project::DEFAULT_GROUP) — before it
...
/// (`ToolchainLinks`' composer and `crate::activation::bin_mode_entry`) go on to
...
/// selects which of a [`LockedTool`](ocx_project::LockedTool)'s
...
///
/// If [`ToolchainHome::root`](ocx_store::file_structure::ToolchainHome::root),
/// `links/`, `shells/`, `shells/<shell>/` or
/// [`ToolchainHome::shell_bin`](ocx_store::file_structure::ToolchainHome::shell_bin)
...
/// [`ToolchainHome::active_is_valid`](ocx_store::file_structure::ToolchainHome::active_is_valid). The renderer routes the refusal to C-050's skip
...
/// [`ToolchainHome::ensure_gitignore`](ocx_store::file_structure::ToolchainHome::ensure_gitignore)
...
/// `crate::activation::project_contribution`, before it widens the per-prompt
...
/// link, so its rule is
/// [`ToolchainHome::active_is_valid`](ocx_store::file_structure::ToolchainHome::active_is_valid)
...
fn heal_active(home: &ToolchainHome, shell: &str) -> crate::Result<()> {
    ...
    // `ocx_util::fs::symlink::is_link`, never `Path::is_symlink` or `symlink_metadata`
...
///   [`ToolchainHome::entry`](ocx_store::file_structure::ToolchainHome::entry)'s
...
/// [`ocx_util::fs::symlink::remove`], which on Unix is `std::fs::remove_file` — it
...
fn prune_within(home: &ToolchainHome, artifact: &RenderedArtifact) -> crate::Result<()> {
    ...
    match artifact {
        ...
        // `ocx_util::fs::symlink::remove` is `std::fs::remove_file` on Unix
...
/// [`persist_temp_file`](ocx_util::fs::persist_temp_file) — which is
...
///
/// Not [`write_bytes_atomic`](ocx_util::fs::write_bytes_atomic), which
...
/// [`hardlink::create`](ocx_store::hardlink::create) links it, so `<name>.exe` is
...
/// [`read_bounded`](ocx_util::fs::read_bounded) closes the third, which no
