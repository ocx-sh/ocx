...
use ocx_util::archive;

use crate::error::PackageErrorKind;
use ocx_package::{install_info::InstallInfo, metadata};
...
impl PullCoordinator {
    pub(crate) fn new() -> Self {
        Self {
            write_group: ocx_util::singleflight::Group::new(
                /* max_entries */ 1024,
            ...
            ocx_oci::layer_ref::LayerRef::File { path, layout, .. } => {
                ...
                let media_type = ocx_oci::media_type::media_type_from_path(path).ok_or_else(|| {
                    ...
                    PackageErrorKind::Internal(
                        ocx_oci::client::error::ClientError::InvalidManifest(format!(
...
async fn pull_digest_layer_to_temp(
    ...
    let layer_desc = ocx_oci::Descriptor {
        media_type: media_type.as_media_type().to_string(),
        digest: digest.to_string(),
        size: blob_size,
        urls: None,
...
mod tests {
    ...
    async fn extract_archive_to_temp_writes_verbatim_tree() {
        ...
        let digest = ocx_oci::Digest::Sha256("a".repeat(64));
    ...
    async fn digest_layer_descriptor_size_matches_head_blob() {
        use ocx_oci::layer_ref::ArchiveMediaType;
        use ocx_oci::{
            Algorithm, Client,
        ...
        let archive_path = ocx_test_support::data::archive_xz();
        ...
        let layers = [ocx_oci::layer_ref::LayerRef::Digest {
            digest: digest.clone(),
    ...
    async fn file_layer_exceeding_size_cap_errors() {
        ...
        let layers = [ocx_oci::layer_ref::LayerRef::File {
            path: archive_path,
            layout: ocx_oci::LayerLayoutSpec::default(),
            mount_from: None,
        }];
    ...
    async fn file_layer_with_unknown_extension_rejected() {
        ...
        let mut archive = ocx_util::archive::Archive::create(&archive_path).await.unwrap();
