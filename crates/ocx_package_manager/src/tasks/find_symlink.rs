...
impl PackageManager {
    ...
    pub async fn find_symlink_all(
        ...
        if !errors.is_empty() {
            return Err(crate::error::Error::FindFailed(errors));
        }
...
mod tests {
    ...
    use ocx_store::{file_structure::FileStructure, file_structure::write_digest_file};
    ...
    async fn installed_current_digest_healthy_install_returns_digest() {
        ...
        let expected_digest: ocx_oci::Digest =
            ocx_oci::Digest::try_from(format!("sha256:{}", "a".repeat(64)).as_str()).unwrap();
        let digest_file = package_root.join(ocx_store::file_structure::DIGEST_FILENAME);
        ...
        ocx_util::fs::symlink::update(&package_root, &current_path).unwrap();
    ...
    async fn installed_current_digest_dangling_symlink_returns_none() {
        ...
        ocx_util::fs::symlink::update(&nonexistent, &current_path).unwrap();
