...
fn synthesize_descriptor_manifest(descriptor_bytes: &[u8]) -> (Vec<u8>, ocx_oci::Digest, ocx_oci::Digest) {
    let layer_digest = ocx_oci::Algorithm::Sha256.hash(descriptor_bytes);
...
fn unwrap_resolve_env_error(error: crate::Error) -> PackageErrorKind {
    if let crate::Error::ResolveFailed(package_errors) = error {
        ...
        return PackageErrorKind::Internal(crate::Error::ResolveFailed(Vec::new()));
    }
...
mod tests {
    ...
    use ocx_store::{file_structure::FileStructure, file_structure::PackageDir, file_structure::PackageStore};
