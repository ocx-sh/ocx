...
use crate::{composer, error::PackageError, error::PackageErrorKind, tasks::resolve::ResolvedChain};
use ocx_index::IndexOperation;
use ocx_package::{metadata, metadata::ValidMetadata};
...
pub struct RepositoryConflict {
    ...
    pub digests: Vec<ocx_oci::Digest>,
}
    ...
    pub async fn inspect_all(
        ...
        if packages.len() == 1 {
            let result = self.inspect(&packages[0], platform, options).await.map_err(|kind| {
                crate::error::Error::InspectFailed(vec![PackageError::new(packages[0].clone(), kind)])
            })?;
        ...
        super::common::drain_package_tasks(&packages, tasks, crate::error::Error::InspectFailed).await
    }
}
...
fn project_surface(nodes: &[ClosureNode], self_view: bool) -> Surface {
    ...
    for node in nodes.iter().filter(|node| admitted_on_surface(node, self_view)) {
        ...
        if composer::carrier_crosses(metadata::Entrypoints::IMPLICIT_VISIBILITY, node.is_root, self_view) {
            entrypoints.extend(
                node.entrypoints
                    .iter()
...
mod spec_tests {
    ...
    use crate::test_support::manifest_source::{ConcurrencyProbe, FakeManifestSource};
    ...
    async fn inspect_default_image_index_returns_candidates() {
        ...
        let result = mgr
            .inspect(&tagged_id(), ocx_oci::Platform::any(), options)
            .await
            .unwrap();
    ...
    async fn inspect_unknown_tag_is_policy_blocked_offline() {
        ...
        let err = mgr
            ...
            .unwrap_err();
        match err {
            PackageErrorKind::Internal(crate::Error::OciIndex(ocx_index::error::Error::PolicyResolutionBlocked {
                policy,
                ..
            })) => assert_eq!(
                policy, "offline",
    ...
    async fn inspect_default_unrecognised_token_is_shown_verbatim_though_composing_it_refuses() {
        ...
        let error = ocx_package::metadata::template::TemplateResolver::new(dir.path(), &contexts)
            .resolve(TOKEN_VALUE)
            .expect_err("composing the same value must refuse");
        assert!(
            matches!(
                &error,
                ocx_package::metadata::template::TemplateError::UnknownToken { token, .. }
    ...
    /// `ocx_oci::Digest::try_from` inside the walker's own manifest/config
    ...
    /// A `Bundle` config JSON carrying an `env` array — the interface-env
    /// summary test's variant of [`closure_config`]. Each `env` element is a
    /// full `Var` wire object (`{key, type, value, visibility}`).
    fn closure_config_with_env(
        dependencies: serde_json::Value,
        entrypoints: serde_json::Value,
    ...
    fn local_index_at(dir: &TempDir) -> LocalIndex {
        LocalIndex::new(LocalConfig {
            index_store: ocx_index::IndexStore::new(dir.path().join("index")),
        })
    }
    ...
    async fn walk_closure_three_node_chain_topo_order_and_through_edge_composition() {
        ...
        assert_eq!(closure.interface.binaries.len(), 1);
        assert_eq!(closure.interface.binaries[0].0, root_pinned);
        assert_eq!(closure.interface.binaries[0].1.as_str(), "root-tool");
        assert_eq!(closure.interface.entrypoints.len(), 1);
        assert_eq!(closure.interface.entrypoints[0].0, root_pinned);
        assert_eq!(closure.interface.entrypoints[0].1.as_str(), "root-ep");
        assert!(
            closure.interface.binaries_complete,
            "root declares binaries and is the only admitted node"
        );
    }

    /// The interface-env summary mirrors the composer's two-gate admission:
    /// a node's env var reaches the summary iff (1) the node is
    /// interface-admitted AND (2) the var itself is interface-visible. Root
    /// vars gate on the var only; a sealed dep contributes nothing even for
    /// its interface-visible vars; private vars never cross.
    #[tokio::test(flavor = "multi_thread")]
    async fn walk_closure_interface_env_gates_by_node_and_entry_visibility() {
        let dir = TempDir::new().unwrap();

        // Interface dep: node admitted; its interface var crosses, its private
        // var does not.
        let mid_config = closure_config_with_env(
            serde_json::json!([]),
            serde_json::json!({}),
            serde_json::json!([
                {"key": "MID_HOME", "type": "constant", "value": "${installPath}", "visibility": "interface"},
                {"key": "MID_SECRET", "type": "constant", "value": "x", "visibility": "private"},
            ]),
        );
        let (source, mid_digest) = register_closure_node(FakeManifestSource::default(), &mid_config);
        let mid_id = closure_pinned("mid", &mid_digest);

        // Sealed dep: node NOT admitted — even its interface-visible var is
        // excluded (node gate).
        let sealed_config = closure_config_with_env(
            serde_json::json!([]),
            serde_json::json!({}),
            serde_json::json!([
                {"key": "SEALED_HOME", "type": "constant", "value": "${installPath}", "visibility": "interface"},
            ]),
        );
        let (source, sealed_digest) = register_closure_node(source, &sealed_config);
        let sealed_id = closure_pinned("sealed", &sealed_digest);

        let root_config = closure_config_with_env(
            serde_json::json!([
                closure_edge(&mid_id, "interface", "mid"),
                closure_edge(&sealed_id, "sealed", "sealed"),
            ]),
            serde_json::json!({}),
            serde_json::json!([
                {"key": "ROOT_PATH", "type": "path", "value": "${installPath}/bin", "visibility": "interface"},
                {"key": "ROOT_PRIVATE", "type": "constant", "value": "y", "visibility": "private"},
            ]),
        );
        let (mgr, root_pinned, root_metadata) = seed_and_load_root(&dir, source, &cdigest("1"), &root_config).await;
        ...
        .await
        .expect("walk must succeed with all nodes present locally");

        let keys: Vec<&str> = closure.interface.env.iter().map(|(_, var)| var.key.as_str()).collect();
        assert!(keys.contains(&"ROOT_PATH"), "root's interface var is exposed: {keys:?}");
        assert!(
            keys.contains(&"MID_HOME"),
            "an interface-admitted dep's interface var crosses: {keys:?}"
        );
        assert!(
            !keys.contains(&"ROOT_PRIVATE"),
            "a private var never reaches the interface surface: {keys:?}"
        );
        assert!(
            !keys.contains(&"MID_SECRET"),
            "an admitted dep's private var still does not cross: {keys:?}"
        );
        assert!(
            !keys.contains(&"SEALED_HOME"),
            "a sealed dep contributes nothing, even its interface-visible var: {keys:?}"
        );

        let (root_path_owner, root_path_var) = closure
            .interface
            .env
            .iter()
            .find(|(_, var)| var.key == "ROOT_PATH")
            .expect("ROOT_PATH present");
        assert_eq!(*root_path_owner, root_pinned, "env var is attributed to its declarer");
        assert_eq!(
            root_path_var.kind,
            ocx_package::metadata::env::modifier::ModifierKind::Path,
            "the path modifier kind is carried through"
        );
    }

    /// The private surface is the self-execution projection, matching
    /// `ocx env --self` (`composer`): node admission by `has_private()` (a
    /// private-only dep is admitted to private alone, a public dep crosses both),
    /// but env crossing is asymmetric. The ROOT's own vars follow the surface
    /// axis (`has_private` here), while a DEPENDENCY contributes only its
    /// interface-side vars — a dep's own `private` var never crosses the edge, so
    /// it appears on neither surface. An interface-only root var never reaches
    /// the private surface.
    #[tokio::test(flavor = "multi_thread")]
    async fn walk_closure_private_surface_matches_self_execution_composition() {
        let dir = TempDir::new().unwrap();

        // Private dep: has_private but NOT has_interface — admitted to the
        // private surface only. Its private var crosses on the private axis.
        let private_config = closure_config_with_env(
            serde_json::json!([]),
            serde_json::json!({}),
            serde_json::json!([
                {"key": "PRIV_HOME", "type": "constant", "value": "${installPath}", "visibility": "private"},
            ]),
        );
        let (source, private_digest) = register_closure_node(FakeManifestSource::default(), &private_config);
        let private_id = closure_pinned("priv", &private_digest);

        // Public dep: crosses both axes.
        let public_config = closure_config_with_env(
            serde_json::json!([]),
            serde_json::json!({}),
            serde_json::json!([
                {"key": "PUB_PATH", "type": "path", "value": "${installPath}/bin", "visibility": "public"},
            ]),
        );
        let (source, public_digest) = register_closure_node(source, &public_config);
        let public_id = closure_pinned("pub", &public_digest);

        let root_config = closure_config_with_env(
            serde_json::json!([
                closure_edge(&private_id, "private", "priv"),
                closure_edge(&public_id, "public", "pub"),
            ]),
            serde_json::json!({}),
            serde_json::json!([
                {"key": "ROOT_PRIVATE", "type": "constant", "value": "y", "visibility": "private"},
                {"key": "ROOT_IFACE", "type": "constant", "value": "z", "visibility": "interface"},
            ]),
        );
        let (mgr, root_pinned, root_metadata) = seed_and_load_root(&dir, source, &cdigest("1"), &root_config).await;
        ...
        let private_keys: Vec<&str> = closure.private.env.iter().map(|(_, var)| var.key.as_str()).collect();
        let interface_keys: Vec<&str> = closure.interface.env.iter().map(|(_, var)| var.key.as_str()).collect();

        // Private surface: the root's own private var and the public dep's
        // public var (public crosses both axes). The private dep's own private
        // var does NOT appear — a dependency only crosses its interface side,
        // even onto the parent's self surface (matches `ocx env --self`).
        assert!(
            private_keys.contains(&"ROOT_PRIVATE"),
            "root's private var is on the private surface: {private_keys:?}"
        );
        assert!(
            !private_keys.contains(&"PRIV_HOME"),
            "a dependency's own private var never crosses the edge, so it is absent from the parent's private surface: {private_keys:?}"
        );
        assert!(
            private_keys.contains(&"PUB_PATH"),
            "a public var crosses the private axis too: {private_keys:?}"
        );
        assert!(
            !private_keys.contains(&"ROOT_IFACE"),
            "an interface-only var never reaches the private surface: {private_keys:?}"
        );

        // Interface surface: the private-only dep is NOT interface-admitted.
        assert!(
            !interface_keys.contains(&"PRIV_HOME"),
            "a private-only dep contributes nothing to the interface surface: {interface_keys:?}"
        );
        assert!(
            interface_keys.contains(&"PUB_PATH"),
            "a public var crosses the interface axis: {interface_keys:?}"
        );
        assert!(
            !interface_keys.contains(&"ROOT_PRIVATE"),
            "a private var never reaches the interface surface: {interface_keys:?}"
        );
    }
    ...
    async fn walk_closure_conflicts_repo_two_digest_interface_pair_reports_entry() {
        ...
        assert_eq!(conflict.repository, ocx_oci::Repository::from(x_id.as_identifier()));
    ...
    async fn walk_closure_conflicts_repo_two_digest_sealed_edge_excluded() {
        ...
        assert!(
            closure
                ...
                .repositories
                .iter()
                .all(|c| c.repository != ocx_oci::Repository::from(x_id.as_identifier())),
    ...
    async fn walk_closure_dep_image_index_incompatible_platform_is_feature_mismatch_not_not_found() {
        ...
        let glibc_platform: ocx_oci::Platform = "linux/amd64+libc.glibc".parse().expect("valid platform grammar");
    ...
    async fn walk_closure_diamond_via_image_index_and_direct_pin_collapses_to_one_node() {
        ...
        assert!(
            closure
                ...
                .repositories
                .iter()
                .all(|c| c.repository != ocx_oci::Repository::from(direct_child_id.as_identifier())),
    ...
    /// `inspect` with `closure: false` must perform zero new writes to the
