...
/// The `[records]` section shape, owned by the config layer because `Config`
/// carries it as a field. Re-exported here so this module's own vocabulary
/// stays complete: the recording subsystem reads the section, the config layer
/// parses it, and only one of those two may depend on the other.
