...
impl Shell {
    ...
    pub fn remove_list_element(
        ...
        let raw = match separator {
            None => ocx_util::path::strip_one_quote_pair(value.as_ref()),
            Some(_) => value.as_ref(),
        };
        if raw.is_empty() {
...
pub fn is_emittable(entry: &ocx_package::metadata::env::entry::Entry) -> Result<(), &'static str> {
...
impl TryInto<clap_complete::Shell> for Shell {
    ...
    fn try_into(self) -> Result<clap_complete::Shell, Self::Error> {
        match self {
            ...
            _ => Err(error::Error::UnsupportedClapShell(self)),
        }
    }
}
...
mod tests {
    ...
    fn run_batch(body: &str) -> Option<String> {
        ...
        let _guard = ocx_util::fs::DropFile::new(&path);
    ...
    fn folded(existing: &str, value: &str, separator: &str) -> String {
        ocx_util::list::append_unique(existing, value, separator)
    }
    ...
    fn export_path_empty_value_is_a_commented_no_op() {
        ...
        assert_eq!(
            ocx_util::path::move_to_front(std::ffi::OsStr::new("/a:/b"), std::ffi::OsStr::new("")),
    ...
    fn move_to_front(existing: &str, value: &str) -> String {
        ocx_util::path::move_to_front(std::ffi::OsStr::new(existing), std::ffi::OsStr::new(value))
            .to_string_lossy()
            .into_owned()
    }
    ...
    fn remove_segment(existing: &str, value: &str) -> String {
        ocx_util::path::remove_segment(std::ffi::OsStr::new(existing), std::ffi::OsStr::new(value))
            .to_string_lossy()
            .into_owned()
    }
    ...
    fn folded_removal_inverts_append_unique() {
        ...
        for (existing, value, separator) in [
            ...
            let applied = ocx_util::list::append_unique(existing, value, separator);
            assert_eq!(
    ...
    fn live_emit_message_prints_byte_exactly_to_stderr() {
        ...
        let coloured = format!(
            "ocx: {} {} {}",
            ocx_console::Theme::new(true).ok("+JAVA_HOME"),
            ocx_console::Theme::new(true).tag("~PATH"),
            ocx_console::Theme::new(true).alert("-PYENV_ROOT")
        );
