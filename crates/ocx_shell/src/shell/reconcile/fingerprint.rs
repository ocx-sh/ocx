...
mod watch_path_tests {
    ...
    fn watch_paths_carry_the_global_tier_without_a_project_c019() {
        ...
        assert!(paths.contains(&file_structure.state.managed_config().snapshot_file()));
    }
    ...
    fn watch_paths_under_no_config_re_derive_the_system_tier_and_nothing_below_it() {
        ...
        assert!(
            paths.contains(&ocx_config::loader::ConfigLoader::system_path()),
            "the system tier still loads under OCX_NO_CONFIG, so it stays watched; got: {paths:?}"
        );
        for pruned in ocx_config::loader::ConfigLoader::user_path()
            .into_iter()
            .chain(ocx_config::loader::ConfigLoader::home_path())
        {
            assert!(
