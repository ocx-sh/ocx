...
impl PackageSign {
    ...
    async fn resolve_override_token(&self, identifier: &oci::Identifier) -> anyhow::Result<Option<Zeroizing<String>>> {
        ...
        #[cfg(not(unix))]
        if self.identity_token_file.is_some() {
            return Err(anyhow::Error::from(SignError::new(
                identifier.clone(),
...
mod tests {
    ...
    ///
    /// This is the only test that reaches the `#[cfg(not(unix))]` arm, and it
    /// is what keeps `make_sign_cmd` live on Windows — the arm was previously
    /// unreachable from any test, so a regression to "silently skip the check"
    /// would have compiled and shipped.
    #[cfg(not(unix))]
    #[tokio::test]
