...
use clap::Parser;
use ocx_lib::project::{DEFAULT_GROUP, ProjectConfig, ProjectLock, declaration_hash, registry::ProjectRegistry};
...
impl Pull {
    pub async fn execute(&self, context: crate::app::Context) -> anyhow::Result<ExitCode> {
        ...
        // Register the lock path in the per-user project registry so that
        // `ocx clean` retains packages pinned by this project even when the
        // project has never run `ocx lock` (pure-pull workflow). Non-fatal:
        // a failure here logs at WARN and does not abort the pull.
        let ocx_home = context.file_structure().root().to_path_buf();
        let registry = ProjectRegistry::new(&ocx_home);
        if let Err(e) = registry.register_if_present(&lock_path).await {
            ocx_lib::log::warn!(
