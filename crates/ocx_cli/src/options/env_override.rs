...
/// The repeatable `--env KEY[:TYPE]=VALUE` per-invocation override.
///
...
pub struct EnvOverride {
    /// Set an environment variable for this invocation, as `KEY[:TYPE]=VALUE`.
    ///
...
impl EnvOverride {
    ...
    /// [`ModifierKind`] — `constant` (replace) or `path` (prepend). Omitted, it
    /// is `constant`, so a plain `KEY=VALUE` means exactly what it always did.
    ...
    /// type marker, and a Windows value like `C:\tools\bin` is untouched
    /// because only that segment is inspected.
    ///
    ...
    /// additive), when the declared `TYPE` names no modifier, when the key is
    /// outside the POSIX environment-name grammar, or when a key matches the
    /// reserved `OCX_*` / `__OCX_*` namespace — the same gate the file form
    /// applies, so the flag cannot be the way in.
    ///
