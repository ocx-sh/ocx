...
fn build_embed(summary: &RunSummary) -> DiscordEmbed {
    ...
    for version in &summary.versions {
        ...
        fields.extend(render_version_block(version, summary.push_job_url.as_deref()));
    }
...
/// Build the embed description — the free-form text rendered between title
/// and fields. Surfaces the canonical OCI identifier (e.g. `ocx.sh/shfmt`)
/// so readers see *what* the embed is about without scrolling the title.
fn build_description(summary: &RunSummary) -> Option<String> {
    if summary.target.trim().is_empty() {
        return None;
    }
    Some(format!("`{}`", summary.target))
}

/// Build the thumbnail URL from a `GITHUB_REPOSITORY` value (e.g. `owner/repo`).
///
/// Returns `None` when the env var is unset. Points at `main` branch's
/// `logo.png` — the OCX in-tree mirror convention (see `mirrors/<name>/logo.png`).
/// Discord renders no thumbnail when the URL 404s, so callers can skip a
/// file-existence probe. The hardcoded filename will become configurable
/// once the `pipeline describe` `catalog:` block lands (handover Unit 1).
fn build_thumbnail(github_repo: Option<&str>) -> Option<DiscordEmbedThumbnail> {
    ...
    Some(DiscordEmbedThumbnail {
        url: format!("https://raw.githubusercontent.com/{repo}/main/logo.png"),
    })
}

/// Render one version's outcome as three inline fields: Platform / Status / Detail.
///
/// Pushed platforms come first (✅), failed ones after (❌). The Status column
/// carries the markdown link to the responsible job — green rows link to
/// `push_job_url`; red rows link to the failure's own `job_url` (test matrix
/// leg for `test_failed` / `missing_junit`, push job for `push_error` /
/// `missing_bundle`, both stamped upstream by `pipeline push`). The Detail
/// column stays plain text so the data is greppable even when Discord
/// markdown rendering is off (e.g. mobile share previews).
fn render_version_block(version: &VersionSummary, push_job_url: Option<&str>) -> Vec<DiscordEmbedField> {
...
/// Wrap a status glyph in a markdown link when a job URL is available.
fn link_status(glyph: &str, url: Option<&str>) -> String {
    match url.map(str::trim).filter(|s| !s.is_empty()) {
        Some(u) => format!("[{glyph}]({u})"),
...
mod tests {
    ...
    #[test]
    fn build_embed_red_row_status_carries_markdown_link_when_job_url_present() {
        ...
        let status = embed
            .fields
            .iter()
            .find(|f| f.name == "Status" && f.inline)
            .expect("Status column must be present");
        assert!(
            status.value.contains(&format!("[❌]({job_url})")),
            "Status column must wrap ❌ in a markdown link to job_url: {}",
            status.value,
        );
    }

    #[test]
    fn build_embed_red_row_status_renders_plain_when_job_url_absent() {
        ...
        assert!(
            ...
            "Status without job_url must stay plain: {}",
            status.value,
        );
        assert!(status.value.contains('❌'));
    }

    #[test]
    fn build_embed_green_row_status_links_to_push_job_url() {
        // Green rows pick up summary.push_job_url so readers can navigate to
        // the push job logs.
        ...
        assert!(
            status
                .value
                .contains("[✅](https://github.com/ocx-sh/mirror-shfmt/actions/runs/2/job/3)"),
            "Green Status must link to push_job_url: {}",
            status.value,
        );
    }

    #[test]
    fn build_embed_green_row_status_plain_when_push_job_url_absent() {
        ...
        let status = embed
            ...
            .find(|f| f.name == "Status" && f.inline)
            .expect("Status column must be present");
        assert!(!status.value.contains("]("), "Green Status without URL stays plain");
        assert!(status.value.contains('✅'));
    }

    #[test]
    fn build_embed_detail_column_stays_plain_text() {
        // Detail column is plain regardless of job_url presence — the link
        // belongs to Status. This pins down the shape after the
        // "Status carries the link" refactor.
        let job_url = "https://github.com/ocx-sh/mirror-shfmt/actions/runs/42/job/7";
        let summary = red_summary_with_failure("3.7.0", "linux/amd64", "test_failed", Some(job_url.to_string()));
        let payload = build_payload(&summary);
        let embed = &payload.embeds[0];
        ...
        assert!(
            ...
            "Detail must not carry markdown links — Status carries them: {}",
            detail.value,
        );
    ...
    fn build_embed_push_error_row_uses_stamped_push_job_url() {
        ...
        // `pipeline push` (from `OCX_MIRROR_JOB_URL`). The Status column
        // therefore renders a link even though no per-platform matrix job
        // exists for the push step.
        ...
        let detail_idx = embed
            .fields
            .iter()
            .position(|f| f.name == "Platform · 3.7.1")
            .expect("expected per-version block for 3.7.1");
        let status = &embed.fields[detail_idx + 1];
        let detail = &embed.fields[detail_idx + 2];

        assert_eq!(status.name, "Status");
        assert!(
            status.value.contains(&format!("[❌]({push_url})")),
            "push_error Status must link to push job: {}",
            status.value,
        );
        assert_eq!(detail.name, "Detail");
        assert!(detail.value.contains("push_error"));
        assert!(!detail.value.contains("]("), "Detail must stay plain");
    }
    ...
    fn build_thumbnail_attaches_url_when_github_repository_is_set() {
        ...
        assert_eq!(
            thumb.url, "https://raw.githubusercontent.com/ocx-sh/mirror-shfmt/main/logo.png",
            "thumbnail URL must point at the repo's logo.png on main",
        );
    }
    ...
    #[test]
    fn build_embed_description_surfaces_target_identifier() {
        let summary = make_all_green_summary();
        let payload = build_payload(&summary);
        let embed = &payload.embeds[0];
        let desc = embed
            .description
            .as_deref()
            .expect("description must be populated from summary.target");
        assert!(
            desc.contains("ocx.sh/shfmt"),
            "description must surface canonical identifier ocx.sh/shfmt: {desc}",
        );
    }

    #[test]
    fn build_embed_description_omitted_when_target_empty() {
        let mut summary = make_all_green_summary();
        summary.target = "   ".to_string();
        let payload = build_payload(&summary);
        let embed = &payload.embeds[0];
        assert!(embed.description.is_none(), "empty target must omit description");
    }
}
