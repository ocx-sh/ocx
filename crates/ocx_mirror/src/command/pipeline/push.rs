...
async fn evaluate_junit(
    ...
    if !missing_reasons.is_empty() {
        ...
        let job_url = read_job_url_sidecar(junit_dir, version, platform_slug, container_ids).await;
...
/// Read the `junit-{V}-{P}-{C}.url` sidecar emitted by the test matrix.
///
/// The workflow's matrix legs write the leg's `html_url` next to the junit XML
/// so the push job can thread per-leg links into `run-summary.json`. Each
/// container has its own sidecar; we return the first non-empty value because
/// every container in a leg shares the same job URL.
async fn read_job_url_sidecar(
    junit_dir: &Path,
    version: &str,
    platform_slug: &str,
    container_ids: &[String],
    ...
    for container_id in container_ids {
        let url_path = junit_dir.join(format!("junit-{version}-{platform_slug}-{container_id}.url"));
        if let Ok(contents) = tokio::fs::read_to_string(&url_path).await {
...
<testsuites>
  <testsuite name="ocx-mirror.shfmt.{slug}.{cid}" tests="2" failures="0" errors="0" skipped="0" timestamp="2026-05-13T10:00:00Z" time="1.0">
    ...
    // ── Sidecar job_url plumbing for the Discord embed ────────────────────
    //
    // The test matrix writes `junit-{V}-{P}-{C}.url` next to its junit XML.
    // The push job reads it inside evaluate_junit so the run-summary carries
    // a clickable link to the responsible matrix leg. Sidecar is optional —
    // greens and runs without the URL recording step both produce summaries
    // with job_url = None.

    fn write_url_sidecar(dir: &std::path::Path, version: &str, platform_slug: &str, container_id: &str, url: &str) {
        let name = format!("junit-{version}-{platform_slug}-{container_id}.url");
        std::fs::write(dir.join(&name), url).unwrap();
    }

    #[test]
    fn evaluate_junit_attaches_job_url_from_sidecar_for_test_failed() {
        let junit_dir = tempdir().unwrap();
        let version = "1.0.0";
        let slug = "linux_amd64";
        ...
        write_url_sidecar(junit_dir.path(), version, slug, "_native_", url);
    ...
    #[test]
    fn evaluate_junit_attaches_job_url_from_sidecar_for_missing_junit() {
        let junit_dir = tempdir().unwrap();
        let version = "1.0.0";
        let slug = "linux_amd64";
        let url = "https://github.com/ocx-sh/mirror-shfmt/actions/runs/42/job/11";

        // No junit XML — only the URL sidecar. The matrix recorded its URL
        // before the test step crashed, so the link is still actionable.
        write_url_sidecar(junit_dir.path(), version, slug, "ubuntu_2404", url);
    ...
    #[test]
    fn evaluate_junit_omits_job_url_when_sidecar_absent() {
        let junit_dir = tempdir().unwrap();
        let version = "1.0.0";
        let slug = "linux_amd64";

        // Failing junit but no `.url` sidecar — push runs that never recorded
        // a URL (legacy workflow templates) must still produce a usable
        // PlatformFailure, just without the clickable link.
        write_junit(
            junit_dir.path(),
            version,
            slug,
        ...
        match decision {
            VpDecision::Red { platform_failure, .. } => {
                assert!(
                    ...
                    "missing sidecar must produce job_url=None"
                );
            }
            VpDecision::Green => panic!("failing JUNIT must yield Red"),
        }
    ...
    #[test]
    fn evaluate_junit_ignores_sidecar_for_green_decision() {
        // Green decisions don't carry a PlatformFailure, so the sidecar is
        // simply ignored. This pins down that the sidecar read never derails
        // a green run (e.g. permission denied on the URL file must not flip
        // a green into a red).
        let junit_dir = tempdir().unwrap();
        let version = "1.0.0";
        let slug = "linux_amd64";
        ...
        write_url_sidecar(
            ...
            slug,
            "_native_",
            "https://github.com/ocx-sh/mirror-shfmt/actions/runs/42/job/3",
        );
        ...
        assert!(
            matches!(decision, VpDecision::Green),
            "passing junit must yield Green even when a sidecar is present"
        );
    }
}
