...
fn parse_layout_tail(full: &str, tail: &str) -> Result<(crate::LayerLayoutSpec, Option<String>), LayerRefParseError> {
    ...
    let mut layout = crate::LayerLayoutSpec::default();
    ...
    for entry in tail.split(',') {
        ...
        match key {
            ...
            "prefix" => {
                ...
                let prefix = ocx_util::fs::path::RelativePath::parse(value).map_err(|source| {
...
impl std::str::FromStr for LayerRef {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if !looks_like_path {
            for media_type in ArchiveMediaType::ALL {
                for ext in media_type.extensions() {
                    ...
                    if let Some(hex_part) = ref_str.strip_suffix(&suffix)
                        && let Ok(digest) = crate::Digest::try_from(hex_part)
                    {
            ...
            if crate::Digest::try_from(ref_str).is_ok() {
...
mod tests {
    ...
    fn parse_digest_tar_gz() {
        ...
        match lr {
            LayerRef::Digest { digest, media_type, .. } => {
                assert!(matches!(digest, crate::Digest::Sha256(ref h) if h == &hex));
    ...
    fn parse_digest_sha512_with_ext() {
        ...
        assert!(matches!(
            lr,
            LayerRef::Digest { digest: crate::Digest::Sha512(ref h), .. } if h == &hex
        ));
    }
    ...
    fn display_digest_tar_gz() {
        ...
        let lr = LayerRef::Digest {
            digest: crate::Digest::Sha256(hex.clone()),
    ...
    fn from_str_layout_grammar() {
        ...
        assert!(
            source
                .and_then(|e| e.downcast_ref::<ocx_util::fs::path::PathEscapeError>())
                .is_some(),
