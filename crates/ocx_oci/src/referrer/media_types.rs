...
/// Both name the same document, and
/// `ocx_lib::oci::attest::predicate::sbom_predicate_type_uri` (still in
/// `ocx_lib`) maps them to one predicateType.
...
//
// The `created` key is [`crate::annotations::CREATED`] — an existing
...
///
/// Written by `ocx_lib::oci::attest::pipeline::AttestPipeline` (still in
/// `ocx_lib`); this is what tells an attestation referrer from a signature
/// referrer in a listing, without fetching the blob.
...
/// `ocx_lib::oci::simplesigning` (still in `ocx_lib`).
...
/// tag-only shape; see `crate::verify::attestation_sidecar`.
