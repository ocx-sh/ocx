        ...
        config
            ...
            .extend(
                self.extra_roots
                    .der()
                    .iter()
                    .map(|der| crate::native::oci_client::client::Certificate {
                        encoding: crate::native::oci_client::client::CertificateEncoding::Der,
                        data: der.to_vec(),
                    }),
            );
        config
...
impl ClientBuilder {
    ...
    pub fn progress(mut self, progress: ocx_console::progress::ProgressManager) -> Self {
...
impl ClientBuilder {
    ...
    /// consumed by [`build`](Self::build) into the transport's `crate::native::Client`,
...
mod tests {
    ...
    fn extra_ca_roots_are_appended_after_the_mozilla_set() {
        use crate::native::oci_client::client::CertificateEncoding;
        ...
        let roots = ExtraRoots::from_pem(bundle.as_bytes()).expect("two minted roots parse");
...
mod push_wire_tests {
    ...
    use crate::client::transport::{OciTransport as _, no_progress};
    ...
    fn stub_image(stub: &StubRegistry) -> crate::native::Reference {
        format!("{}/test/blob:latest", stub.address).parse().unwrap()
    }

    fn sha256_digest(data: &[u8]) -> crate::Digest {
...
mod read_timeout_tests {
    ...
    async fn read_stalled_blob_with_timeout(address: &str, read_timeout: Duration) -> std::io::Result<u64> {
        ...
        let mut stream = crate::client::transport::OciTransport::pull_blob_streaming(&transport, &image, &digest)
            .await
