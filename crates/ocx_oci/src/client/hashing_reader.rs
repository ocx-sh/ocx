...
impl DigestState {
    fn new(algorithm: crate::Algorithm) -> Self {
        match algorithm {
            crate::Algorithm::Sha256 => Self::Sha256(sha2::Sha256::new()),
            crate::Algorithm::Sha384 => Self::Sha384(sha2::Sha384::new()),
            crate::Algorithm::Sha512 => Self::Sha512(sha2::Sha512::new()),
        }
    }
    ...
    fn finalize(self) -> crate::Digest {
        match self {
            Self::Sha256(h) => crate::Digest::Sha256(hex::encode(h.finalize())),
            Self::Sha384(h) => crate::Digest::Sha384(hex::encode(h.finalize())),
            Self::Sha512(h) => crate::Digest::Sha512(hex::encode(h.finalize())),
        }
    }
}
...
mod tests {
    ...
    async fn known_vector_hello_world_matches_sha256() {
        ...
        let expected = crate::Algorithm::Sha256.hash(input.as_ref());

        let mut reader = HashingAsyncReader::new(&input[..], crate::Algorithm::Sha256);
        let mut buf = Vec::new();
    ...
    async fn sha512_known_vector_matches() {
        ...
        let expected = crate::Algorithm::Sha512.hash(input.as_ref());

        let mut reader = HashingAsyncReader::new(&input[..], crate::Algorithm::Sha512);
        let mut buf = Vec::new();
        tokio::io::AsyncReadExt::read_to_end(&mut reader, &mut buf)
            .await
            .unwrap();
        ...
        assert!(
            matches!(digest, crate::Digest::Sha512(_)),
    ...
    async fn sha384_known_vector_matches() {
        ...
        let expected = crate::Algorithm::Sha384.hash(input.as_ref());

        let mut reader = HashingAsyncReader::new(&input[..], crate::Algorithm::Sha384);
        let mut buf = Vec::new();
        tokio::io::AsyncReadExt::read_to_end(&mut reader, &mut buf)
            .await
            .unwrap();
        ...
        assert_eq!(byte_count, input.len() as u64);
        assert!(
            matches!(digest, crate::Digest::Sha384(_)),
            "finalize() must return digest in the algorithm passed at construction"
        );
    }
    ...
    async fn empty_stream_produces_sha256_of_zero_bytes() {
        ...
        let mut reader = HashingAsyncReader::new(&[][..], crate::Algorithm::Sha256);
        let mut buf = Vec::new();
        tokio::io::AsyncReadExt::read_to_end(&mut reader, &mut buf)
            .await
            .unwrap();
        ...
        assert_eq!(
            digest,
            crate::Digest::Sha256(EMPTY_SHA256.to_string()),
    ...
    async fn error_mid_stream_digest_covers_only_successful_reads() {
        ...
        let expected_partial = crate::Algorithm::Sha256.hash(b"abcde");

        let inner = FailAfter::new(data.clone(), fail_after);
        let mut reader = HashingAsyncReader::new(inner, crate::Algorithm::Sha256);
    ...
    async fn multi_chunk_reads_produce_correct_cumulative_digest() {
        ...
        let expected = crate::Algorithm::Sha256.hash(&data);

        let mut reader = HashingAsyncReader::new(&data[..], crate::Algorithm::Sha256);
    ...
    /// spec §HashingAsyncReader contract: finalize() returns (crate::Digest, u64) tuple
    #[tokio::test]
    async fn finalize_returns_digest_and_byte_count_tuple() {
        let input = b"specification test";
        let expected_digest = crate::Algorithm::Sha256.hash(input.as_ref());
        ...
        let (digest, byte_count): (crate::Digest, u64) = reader.finalize();
        assert_eq!(digest, expected_digest);
        assert_eq!(byte_count, input.len() as u64);
    }
}
