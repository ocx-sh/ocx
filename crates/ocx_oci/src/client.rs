...
use crate::media_type::{
    ACCEPTED_MANIFEST_MEDIA_TYPES, MEDIA_TYPE_OCI_IMAGE_INDEX, MEDIA_TYPE_OCI_IMAGE_MANIFEST, MEDIA_TYPE_PACKAGE_V1,
...
impl LayerCounts {
    pub fn record(&mut self, outcome: LayerPushOutcome) {
        match outcome {
...
impl Client {
    ...
    #[cfg(any(test, feature = "__testing"))]
    pub fn with_test_mirror(mut self, upstream: &str, host: &str, path_prefix: &str) -> Self {
        self.mirrors = MirrorMap::new([(
            upstream.to_string(),
            crate::client::mirror_map::ParsedMirror {
    ...
    pub async fn list_tags_or_empty_addressed(
        ...
        match self.list_tags_addressed(identifier, addressing).await {
            Err(ClientError::RepositoryNotFound(_)) => Ok(Vec::new()),
            other => other,
        }
    }
    ...
    pub async fn fetch_manifest_digest_addressed(
        ...
        self.transport()
            .ensure_auth(&ref_, crate::RegistryOperation::Pull)
            .await
    ...
    pub async fn merge_platform_into_index(
            ...
            Ok((blob, digest_str)) => {
                ...
                match existing {
                    crate::Manifest::Image(_) => {
                        ...
                        let entry = crate::ImageIndexEntry {
                    ...
                    crate::Manifest::ImageIndex(index) => index,
                }
            }
        ...
        index.manifests.push(crate::ImageIndexEntry {
            media_type: MEDIA_TYPE_OCI_IMAGE_MANIFEST.to_string(),
    ...
    async fn pull_layer_with_caps(
        ...
        let blob_compression = crate::media_type::compression_for(&layer.media_type).ok_or_else(|| {
    ...
    pub async fn push_manifest_and_merge_tags(
        ...
        let (_manifest, manifest_data, manifest_sha256, layer_counts) =
            self.push_multi_layer_manifest(identifier, layers, manifest).await?;
        ...
        let primary_tag = identifier.tag_or_latest().to_string();
    ...
    pub async fn push_multi_layer_manifest(
                ...
                async move {
                    ...
                    match layer {
                        LayerRef::File {
                            ...
                            Ok::<(crate::Descriptor, LayerPushOutcome), ClientError>((
                                crate::Descriptor {
        ...
        let layer_descriptors: Vec<crate::Descriptor> = layer_results
            .into_iter()
    ...
    pub async fn push_blob(
        ...
        self.transport()
            .push_blob(&image, data, digest, transport::no_progress())
            .await?;
        Ok(())
    }
    ...
    pub async fn pull_blob_to_file(
        ...
        let image = self.read_reference(identifier, addressing);
        self.transport().pull_blob_to_file(&image, digest, path).await
    }
    ...
    pub async fn fetch_single_layer_artifact(
        ...
        let image_manifest = match manifest {
            crate::Manifest::Image(m) => m,
            crate::Manifest::ImageIndex(_) => return Err(ClientError::UnexpectedManifestType),
        };
...
mod tests {
    ...
    use super::*;
    use crate::media_type::{MEDIA_TYPE_OCI_EMPTY_CONFIG, MEDIA_TYPE_PACKAGE_METADATA_V1};

    use std::sync::Mutex;

    /// The non-layer half of a package manifest, as a caller hands it to
    /// `push_multi_layer_manifest` / `push_manifest_and_merge_tags`.
    ///
    /// The real publish path builds this from `Info::manifest_builder`, which
    /// serializes `package::metadata::Metadata` into the config blob. Nothing
    /// under `oci/` may name that type (boundary
    /// `oci_client_does_not_import_workflow`), and no test here asserts on the
    /// config blob's bytes — the frozen package-manifest goldens live beside
    /// the chain that produces them, at `package::info`. So the config payload
    /// is a stand-in of the same shape.
    fn package_manifest() -> ManifestBuilder {
        ManifestBuilder::new()
            ...
            .expect("stand-in package config serializes")
    }
    ...
    fn make_image_manifest(config_digest: &str, layer_digest: &str) -> crate::ImageManifest {
        ...
        crate::ImageManifest {
            ...
            layers: vec![crate::Descriptor {
                media_type: crate::media_type::MEDIA_TYPE_TAR_GZ.to_string(),
    ...
    fn serialize_manifest(manifest: &crate::Manifest) -> (Vec<u8>, String) {
    ...
    async fn fetch_manifest_raw_bytes_retains_verbatim_bytes_for_tag_identifier() {
        ...
        assert!(
            matches!(parsed, crate::Manifest::Image(_)),
    ...
    async fn fetch_manifest_raw_bytes_rejects_a_manifest_served_under_another_digest() {
        let requested = crate::Manifest::Image(crate::ImageManifest::default());
        let (_requested_bytes, requested_digest) = serialize_manifest(&requested);
        let served = crate::Manifest::Image(crate::ImageManifest {
    ...
    /// newer writer added (here `subject`, which `crate::ImageIndex` does not
    ...
    async fn fetch_manifest_success() {
        let manifest = crate::Manifest::Image(make_image_manifest("sha256:cff", "sha256:1a0e"));
        let (manifest_data, digest_str) = serialize_manifest(&manifest);
        ...
        assert_eq!(digest.to_string(), digest_str);
        assert!(matches!(fetched, crate::Manifest::Image(_)));
    }
    ...
    async fn pull_manifest_unexpected_manifest_type() {
        ...
        let manifest = crate::Manifest::ImageIndex(index);
        let (manifest_data, digest_str) = serialize_manifest(&manifest);
    ...
    async fn pull_manifest_accepts_any_media_types() {
        ...
        let manifest = crate::Manifest::Image(m);
        let (manifest_data, digest_str) = serialize_manifest(&manifest);
        let digest_hex = digest_str.strip_prefix("sha256:").unwrap();
        let id = test_pinned(digest_hex);
    ...
    fn setup_manifest_and_blob(
        ...
        let oci_manifest = crate::Manifest::Image(manifest);
    ...
    async fn pull_blob_returns_raw_bytes() {
        ...
        // Round-trip parse confirms the bytes are intact. Deliberately typed as
        // raw JSON rather than `package::metadata::Metadata`: nothing under
        // `oci/` may name that type, and the byte-equality assertion above is
        // what actually pins the payload.
        let parsed: serde_json::Value = serde_json::from_slice(&bytes).expect("returned bytes must parse as JSON");
        let _ = parsed;
    }
    ...
    async fn fetch_single_layer_artifact_wrong_artifact_type_errors() {
        ...
        let manifest = crate::Manifest::Image(manifest_struct);
        let (manifest_data, digest_str) = serialize_manifest(&manifest);
    ...
    fn fork_digest_error_io_wrapping_maps_to_digest_mismatch_not_io() {
        ...
        let result: std::result::Result<(), ClientError> =
            crate::client::native_transport::map_fork_io_error_to_client_error(string_only_io_error);
        ...
        let result2: std::result::Result<(), ClientError> =
            crate::client::native_transport::map_fork_io_error_to_client_error(typed_mismatch);
    ...
    async fn streaming_invalid_tar_inside_valid_xz_wrapper_yields_internal_error() {
        ...
        let layer = crate::Descriptor {
            media_type: crate::media_type::MEDIA_TYPE_TAR_XZ.to_string(),
            digest: digest_str.clone(),
    ...
    async fn pull_layer_succeeds_when_zstd_trailer_arrives_in_final_chunk() {
        ...
        let id = test_pinned(digest_str.strip_prefix("sha256:").unwrap());
        let layer = crate::Descriptor {
            media_type: crate::media_type::MEDIA_TYPE_TAR_ZSTD.to_string(),
            digest: digest_str.clone(),
    ...
    impl crate::sealed::Sealed for InterruptingTransport {}
    ...
    impl super::OciTransport for InterruptingTransport {
        ...
        async fn fetch_manifest_digest(&self, _image: &crate::native::Reference) -> super::transport::Result<String> {
            unimplemented!()
        }
        ...
        async fn push_referrer_manifest(
            ...
            _manifest_bytes: &[u8],
            _media_type: &str,
        ) -> super::transport::Result<crate::Descriptor> {
        ...
        ) -> super::transport::Result<Vec<crate::Descriptor>> {
            unimplemented!("not needed for the streaming-interruption test")
        }
    ...
    mod merge_platform {
        ...
        fn platform(s: &str) -> crate::Platform {
            s.parse().unwrap()
        }
        ...
        fn read_pushed_index(data: &StubTransportData, tag: &str) -> crate::ImageIndex {
            ...
            let manifest: crate::Manifest = serde_json::from_slice(bytes).unwrap();
            match manifest {
                crate::Manifest::ImageIndex(idx) => idx,
        ...
        async fn fresh_tag_creates_new_index() {
            ...
            let entry_plat: crate::Platform = index.manifests[0].platform.clone().unwrap().try_into().unwrap();
        ...
        async fn existing_index_adds_platform() {
            ...
            let existing_bytes = serde_json::to_vec(&crate::Manifest::ImageIndex(existing)).unwrap();
            let existing_digest = crate::Algorithm::Sha256.hash(&existing_bytes).to_string();
            data.write()
                .manifests
        ...
        async fn a_tag_carrying_one_platform_twice_comes_back_carrying_it_once() {
            let data = StubTransportData::new();
            let id = test_identifier("3.28");
            let duplicated = |digest: &str, size: i64| crate::ImageIndexEntry {
                media_type: MEDIA_TYPE_OCI_IMAGE_MANIFEST.to_string(),
                digest: digest.to_string(),
                size,
                platform: Some(platform("linux/amd64").into()),
            ...
            let amd64: Vec<&str> = index
                ...
                .filter(|entry| {
                    entry
                        .platform
                        .clone()
                        .and_then(|p| crate::Platform::try_from(p).ok())
        ...
        async fn existing_image_manifest_upgrades_to_index() {
            ...
            let id = test_identifier("3.28");
            let image_manifest = crate::ImageManifest {
                config: crate::Descriptor {
            ...
            let manifest = crate::Manifest::Image(image_manifest);
            let manifest_bytes = serde_json::to_vec(&manifest).unwrap();
            let manifest_digest = crate::Algorithm::Sha256.hash(&manifest_bytes).to_string();
        ...
        fn seed_index_with_annotations(
            ...
            let bytes = serde_json::to_vec(&crate::Manifest::ImageIndex(existing)).unwrap();
            let digest = crate::Algorithm::Sha256.hash(&bytes).to_string();
            data.write()
                .manifests
        ...
        fn source_annotation(url: &str) -> BTreeMap<String, String> {
            BTreeMap::from([(crate::annotations::SOURCE.to_string(), url.to_string())])
        }
        ...
        async fn supplied_annotations_land_on_a_fresh_index() {
            ...
            assert_eq!(
                index
                    .annotations
                    .as_ref()
                    .and_then(|a| a.get(crate::annotations::SOURCE))
                    .map(String::as_str),
        ...
        async fn supplied_annotations_merge_over_the_existing_ones() {
            ...
            seed_index_with_annotations(
                ...
                Some(BTreeMap::from([
                    (
                        crate::annotations::SOURCE.to_string(),
                        "https://example.invalid".to_string(),
                    ),
                    (crate::annotations::VENDOR.to_string(), "OCX".to_string()),
                ])),
            );
            let client = stub_with_capture(&data);
            let id = test_identifier("3.28");
            ...
            assert_eq!(
                annotations.get(crate::annotations::SOURCE).map(String::as_str),
                Some("https://github.com/ocx-sh/ocx"),
                "the supplied key overwrites",
            );
            assert_eq!(
                annotations.get(crate::annotations::VENDOR).map(String::as_str),
                Some("OCX"),
    ...
    mod push_keep_tag_tests {
        ...
        fn index_with_entry(digest: &str, platform: crate::Platform) -> crate::Manifest {
            crate::Manifest::ImageIndex(crate::ImageIndex {
                schema_version: crate::INDEX_SCHEMA_VERSION,
                media_type: Some(MEDIA_TYPE_OCI_IMAGE_INDEX.to_string()),
                artifact_type: None,
                manifests: vec![crate::ImageIndexEntry {
        ...
        async fn pushes_namespaced_sha256_keep_tag_with_the_same_bytes() {
            ...
            let digest_ref = id
                .without_tag()
                .clone_with_digest(crate::Digest::Sha256(hex.clone()))
                .canonical_reference();
            data.write()
                .manifests
        ...
        async fn a_sha512_digest_writes_no_keep_tag() {
            ...
            let digest_ref = id
                .without_tag()
                .clone_with_digest(crate::Digest::Sha512(hex.clone()))
                .canonical_reference();
            data.write()
                .manifests
                .insert(digest_ref.to_string(), (b"platform manifest".to_vec(), digest.clone()));
    ...
    mod ensure_auth {
        ...
        async fn push_multi_layer_manifest_authenticates_with_push() {
            ...
            let layers = [LayerRef::Digest {
                digest: crate::Digest::try_from(layer_digest).unwrap(),
                media_type: crate::ArchiveMediaType::TarGz,
                layout: crate::LayerLayoutSpec::default(),
                mount_from: None,
            }];

            let _ = client
                .push_multi_layer_manifest(&test_identifier("1.0"), &layers, package_manifest())
                .await;
            let calls = auth_calls(&data);
    ...
    mod multi_layer_digest_resolve {
        ...
        async fn digest_layer_uses_supplied_media_type_tar_xz() {
            ...
            let layers = [LayerRef::Digest {
                digest: crate::Digest::try_from(layer_digest).unwrap(),
                media_type: crate::ArchiveMediaType::TarXz,
                layout: crate::LayerLayoutSpec::default(),
                mount_from: None,
            }];
            let (manifest, _bytes, _digest, counts) = client
                .push_multi_layer_manifest(&test_identifier("2.0.0"), &layers, package_manifest())
                .await
            ...
            assert_eq!(
                ...
                crate::media_type::MEDIA_TYPE_TAR_XZ,
        ...
        async fn digest_layer_not_found_in_registry_errors() {
            ...
            let layers = [LayerRef::Digest {
                digest: crate::Digest::try_from(missing_digest).unwrap(),
                media_type: crate::ArchiveMediaType::TarGz,
                layout: crate::LayerLayoutSpec::default(),
                mount_from: None,
            }];
        ...
        async fn unknown_file_extension_is_rejected() {
            ...
            let err = client
                .push_multi_layer_manifest(&test_identifier("1.0.0"), &layers, package_manifest())
                .await
    ...
    mod mount_reuse {
        ...
        async fn mixed_layer_list_produces_correct_counter_breakdown() {
            ...
            let layers = [
                ...
                LayerRef::Digest {
                    digest: crate::Digest::try_from(verified_digest).unwrap(),
                    media_type: crate::ArchiveMediaType::TarGz,
                    layout: crate::LayerLayoutSpec::default(),
                    mount_from: None,
                },
    ...
    mod cascade_order {
        ...
        fn platform() -> crate::Platform {
            "linux/amd64".parse().expect("platform parses")
        }
        ...
        async fn annotations_land_on_the_primary_tag_and_every_cascade_tag() {
            ...
            for tag in ["1.2.3", "3", "latest"] {
                ...
                let manifest: crate::Manifest = serde_json::from_slice(bytes).expect("index parses");
                let crate::Manifest::ImageIndex(index) = manifest else {
    ...
    mod transport_reference {
        ...
        fn test_digest(hex_seed: char) -> crate::Digest {
    ...
    mod mirror_routing {
        ...
        use crate::client::MirrorMap;
        use crate::client::mirror_map::ParsedMirror;
        use crate::client::transport::{OciTransport, ProgressFn, Result as TransportResult};
        ...
        impl RecordingTransportData {
            ...
            fn record(&self, method: &'static str, image: &crate::native::Reference) {
                self.0
                    .lock()
                    .unwrap()
        ...
        impl OciTransport for RecordingTransport {
            async fn ensure_auth(
                ...
                _operation: crate::RegistryOperation,
            ) -> TransportResult<()> {
            ...
            async fn fetch_manifest_digest(&self, image: &crate::native::Reference) -> TransportResult<String> {
            ...
            async fn push_referrer_manifest(
                ...
                manifest_bytes: &[u8],
                media_type: &str,
            ) -> TransportResult<crate::Descriptor> {
            ...
            async fn list_referrers(
                ...
                _subject_digest: &crate::Digest,
                _artifact_type: Option<&str>,
            ) -> TransportResult<Vec<crate::Descriptor>> {
            ...
            async fn push_blob_from_path(
                ...
                crate::client::push_blob_buffered(self, image, path, digest, on_progress).await
            }
        ...
        async fn head_blob_routes_through_mirror() {
            let (client, data) = mirrored_client();
            let id = identifier_with_tag("1.0");
            let digest = crate::Digest::Sha256(digest_hex('a'));
            let _ = client.head_blob(&id, &digest).await;
        ...
        async fn fetch_layer_blob_capped_routes_through_mirror() {
            let (client, data) = mirrored_client();
            let id = identifier_with_tag("1.0");
            let digest = crate::Digest::Sha256(digest_hex('f'));
    ...
    fn canonical_reference_only_used_in_allowed_files() {
        ...
        // copies, and the copy below lost the freshness half — see
        // `workspace_rs_files`.
        let rs_files = workspace_rs_files();
        ...
        assert_allow_list_is_live("T-arch-A1", ALLOWED_SUFFIXES, &rs_files);
    }
    ...
    fn native_reference_direct_construction_restricted_to_seams() {
        ...
        assert_allow_list_is_live("T-arch-G1", ALLOWED_SUFFIXES, &rs_files);
    }

    /// Every `.rs` file in the workspace, floored on what the walk **read**.
    ///
    /// `!rs_files.is_empty()` is not a floor. The prior scope of both gates
    ...
    /// `crates/*/src` on disk must appear in the scan, so a root that narrows to
    /// one crate reds **naming the crates it stopped reading**, and a count that
    /// drifts as files are added or deleted never enters into it.
    fn workspace_rs_files() -> Vec<std::path::PathBuf> {
        ...
        let crates_root = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .expect("crates/ocx_oci has a parent")
            .to_path_buf();

        // The oracle, floored in turn: a `crates_root` that resolves to nothing
        // yields an empty expectation, which every scan satisfies. This repo has
        // had more than one member since before either gate existed.
        let expected: BTreeSet<String> = fs::read_dir(&crates_root)
            .unwrap_or_else(|error| panic!("crates root {} is unreadable: {error}", crates_root.display()))
            .flatten()
        ...
        assert!(
            expected.len() > 1,
            "only {} crate(s) with a `src/` found under {} — the oracle for this scan has itself \
             collapsed, and every corpus satisfies an empty expectation",
            expected.len(),
            crates_root.display()
        );
        ...
        collect(&crates_root, &mut rs_files);

        let scanned: BTreeSet<String> = rs_files
            .iter()
            .filter_map(|path| path.strip_prefix(&crates_root).ok())
            .filter_map(|relative| relative.components().next())
            .map(|component| component.as_os_str().to_string_lossy().into_owned())
            .collect();
        let unread: Vec<&String> = expected.difference(&scanned).collect();
        assert!(
            unread.is_empty(),
            "the source scan read no file from {} of {} workspace crate(s) — it is not walking the \
             workspace, so every gate built on it is clean over a corpus it does not cover: {:?}",
            unread.len(),
            expected.len(),
            unread
        );
    ...
    /// Every allow-list entry must name a file the scan actually read.
    ///
    ...
    fn assert_allow_list_is_live(gate: &str, allowed: &[&str], rs_files: &[std::path::PathBuf]) {
        ...
        assert!(
            stale.is_empty(),
            "{gate}: allow-list entr(ies) name a file that is not in the scan:\n  {}\n\n\
             Re-point the entry at where the file moved, or drop it.",
            stale.join("\n  ")
        );
    }
    ...
    fn client_rs_production_reference_construction_count_pinned_to_seams() {
        ...
        let this_file = Path::new(manifest_dir).join("src/client.rs");
    ...
    mod cascade_normalization {
        ...
        async fn repush_same_platform_different_feature_order_produces_one_entry() {
            ...
            let first_platform = crate::Platform::Specific {
                os: crate::OperatingSystem::Linux,
                arch: crate::Architecture::Amd64,
                variant: None,
            ...
            let second_platform = crate::Platform::Specific {
                os: crate::OperatingSystem::Linux,
                arch: crate::Architecture::Amd64,
                variant: None,
    ...
    mod auth_wire_tests {
        ...
        async fn a_warm_ensure_auth_issues_no_wire_requests() {
            ...
            client
                .ensure_auth(&identifier, crate::RegistryOperation::Pull)
                .await
                .unwrap();
        ...
        async fn another_repository_or_operation_gets_its_own_exchange() {
            ...
            client
                .ensure_auth(&registry.identifier("test/one"), crate::RegistryOperation::Pull)
                .await
                .unwrap();
            client
                .ensure_auth(&registry.identifier("test/two"), crate::RegistryOperation::Pull)
                .await
                .unwrap();
            client
                .ensure_auth(&registry.identifier("test/one"), crate::RegistryOperation::Push)
                .await
                .unwrap();
        ...
        async fn concurrent_cold_ensure_auth_shares_one_token_exchange() {
            ...
            for _ in 0..CALLERS {
                ...
                handles.push(tokio::spawn(async move {
                    client.ensure_auth(&identifier, crate::RegistryOperation::Pull).await
                }));
            }
        ...
        async fn three_repositories_share_one_challenge_probe() {
            ...
            for repository in ["test/one", "test/two", "test/three"] {
                client
                    .ensure_auth(&registry.identifier(repository), crate::RegistryOperation::Pull)
                    .await
                    .unwrap();
            }
