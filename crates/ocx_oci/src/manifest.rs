...
mod tests {
    ...
    fn make_index(entries: Vec<(&str, &str, i64)>) -> Manifest {
        let manifests = entries
            ...
            .map(|(platform_str, digest, size)| {
                ...
                let native: crate::native::Platform = platform.into();
            ...
            .collect();
        Manifest::ImageIndex(crate::ImageIndex {
            schema_version: 2,
            media_type: None,
            artifact_type: None,
            manifests,
    ...
    fn parse(bytes: &[u8]) -> crate::ImageIndex {
    ...
    fn has_platform_image_manifest_returns_false() {
        let manifest = Manifest::Image(crate::ImageManifest::default());
        let platform: Platform = "linux/amd64".parse().unwrap();
        assert!(!has_platform(&manifest, &platform));
    }
