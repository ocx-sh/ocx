...
impl ReferenceManager {
    ...
    fn back_ref_path(
        ...
        content_path: &Path,
        forward_path: &Path,
    ) -> std::result::Result<PathBuf, crate::file_structure::PackageDirError> {
        Ok(self
            .file_structure
            .packages
    ...
    pub fn link_dependency(
        ...
        dependency_digest: &ocx_oci::Digest,
    ) -> std::result::Result<(), crate::file_structure::PackageDirError> {
    ...
    fn create_forward_dep_ref(
        ...
        std::fs::create_dir_all(&deps_dir).map_err(|e| ocx_util::error::FileError::new(deps_dir.clone(), e))?;
    ...
    pub async fn broken_refs(&self) -> Result<Vec<PathBuf>> {
        ...
        for obj in &package_dirs {
            ...
            if !ocx_util::fs::path_exists_lossy(&refs_dir).await {
                continue;
            }
    ...
    pub async fn link_blobs<'a>(
        ...
        tokio::fs::create_dir_all(&refs_blobs)
            .await
            .map_err(|e| ocx_util::error::FileError::new(refs_blobs.clone(), e))?;
...
async fn check_refs_dir(refs_dir: &Path, expected_content: &Path) -> Result<Vec<PathBuf>> {
    let mut entries = tokio::fs::read_dir(refs_dir)
        .await
        .map_err(|e| ocx_util::error::FileError::new(refs_dir, e))?;
    let mut broken = Vec::new();
...
mod tests {
    ...
    fn make_digest(n: u32) -> ocx_oci::Digest {
        ocx_oci::Digest::Sha256(format!(
    ...
    fn link_creates_forward_symlink_and_back_ref() {
        ...
        assert!(ocx_util::fs::symlink::is_link(&back_ref));
    ...
    fn link_updates_forward_and_moves_back_ref_on_relink() {
        ...
        assert!(ocx_util::fs::symlink::is_link(&back_ref_for(&content_b, &forward)));
    ...
    fn link_tolerates_missing_old_content_on_relink() {
        ...
        let gone = root
            ...
            .join("gone")
            .join("content");
        ocx_util::fs::symlink::create(&gone, &forward).unwrap();
    ...
    fn link_replaces_stale_back_ref_at_target_location() {
        ...
        ocx_util::fs::symlink::create(root.join("nowhere"), &back_ref).unwrap();
    ...
    fn link_is_race_free_under_concurrent_identical_calls() {
        ...
        for round in 0..8 {
            ...
            assert_eq!(std::fs::read_link(&forward).unwrap(), content);
            assert!(ocx_util::fs::symlink::is_link(&back_ref_for(&content, &forward)));
        }
    }
    ...
    fn unlink_removes_forward_symlink_and_back_ref() {
        ...
        assert!(!ocx_util::fs::symlink::is_link(&forward));
    ...
    async fn broken_refs_detects_missing_forward_symlink() {
        ...
        ocx_util::fs::symlink::create(&ghost_fwd, &back_ref).unwrap();
    ...
    async fn broken_refs_detects_forward_pointing_to_wrong_content() {
        ...
        ocx_util::fs::symlink::create(&content_a, &forward).unwrap();
        ...
        ocx_util::fs::symlink::create(&forward, &back_ref).unwrap();
    ...
    fn make_blob(root: &Path, registry: &str, digest: &ocx_oci::Digest) -> std::path::PathBuf {
    ...
    async fn link_blobs_tolerates_eexist_when_target_matches() {
        ...
        ocx_util::fs::symlink::create(&blob_path, &link_path).unwrap();
    ...
    async fn link_blobs_updates_stale_symlink_target() {
        ...
        ocx_util::fs::symlink::create(&stale_target, &link_path).unwrap();
    ...
    async fn link_blobs_missing_blob_file_creates_dangling_symlink() {
        ...
        let link_path = refs_blobs.join(&ref_name);
        assert!(
            ocx_util::fs::symlink::is_link(&link_path),
    ...
    fn link_dependency_creates_forward_ref() {
        ...
        assert!(ocx_util::fs::symlink::is_link(&dep_path));
