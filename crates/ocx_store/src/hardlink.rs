...
pub fn create(source: impl AsRef<std::path::Path>, link: impl AsRef<std::path::Path>) -> Result<()> {
    ...
    hard_link_or_err(source, link).map_err(|error| FileError::new(link, error))?;
    Ok(())
}
...
pub(crate) fn create_in_existing_parent(
    ...
    let source = source.as_ref();
    let link = link.as_ref();
    std::fs::hard_link(source, link).map_err(|error| FileError::new(link, error))?;
    Ok(())
}
...
pub fn update(source: impl AsRef<std::path::Path>, link: impl AsRef<std::path::Path>) -> Result<()> {
    ...
    if link.exists() || ocx_util::fs::symlink::is_link(link) {
        std::fs::remove_file(link).map_err(|error| FileError::new(link, error))?;
    }
    create(source, link)
}
...
mod tests {
    ...
    fn create_fails_if_link_exists() {
        ...
        let FileError { cause: io_err, .. } = result.unwrap_err();
        assert_eq!(
            io_err.kind(),
    ...
    /// wrapping the IO failure with path context (`FileError`).
    #[test]
    fn create_returns_error_when_source_missing() {
        ...
        if let Err(FileError { path, .. }) = result {
    ...
    fn create_errors_on_cross_device() {
        ...
        let FileError { cause: io_err, .. } = err;
        assert_eq!(
            io_err.kind(),
