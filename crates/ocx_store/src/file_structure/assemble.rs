...
use ocx_util::fs::path::LayerPlacement;
...
pub enum AssembleError {
    ...
    /// The destination prefix resolved through a symlink, or the walk that
    /// checks for one failed.
    #[error(transparent)]
...
async fn expand(
    ...
    let mut entries = tokio::fs::read_dir(src).await.map_err(|e| file_error(src, e))?;

    while let Some(entry) = entries.next_entry().await.map_err(|e| file_error(src, e))? {
...
async fn prepare_destination(sources: &[&Path], dest_content: &Path) -> Result<()> {
    ...
    if !ocx_util::fs::path_exists_lossy(dest_content).await {
        if let Some(parent) = dest_content.parent()
            && !ocx_util::fs::path_exists_lossy(parent).await
        {
            return Err(file_error(
                dest_content,
        ...
        tokio::fs::create_dir(dest_content)
            .await
            .map_err(|e| file_error(dest_content, e))?;
    }
...
async fn process_directory(req: SpawnRequest, ctx: WalkContext) -> Result<AssemblyStats> {
    ...
    for source in &src_dirs {
        ...
        let mut entries = tokio::fs::read_dir(src_dir).await.map_err(|e| file_error(src_dir, e))?;

        while let Some(entry) = entries.next_entry().await.map_err(|e| file_error(src_dir, e))? {
            let prev = entries_seen.fetch_add(1, Ordering::Relaxed);
            if prev + 1 > max_entries {
                return Err(file_error(
                    &dest_root,
            ...
            let file_type = entry.file_type().await.map_err(|e| file_error(&src_path, e))?;
            ...
            if file_type.is_dir() {
                ...
                let size = entry.metadata().await.map_err(|e| file_error(&src_path, e))?.len();
                merged.entry(file_name).or_default().push(Contributor {
                    layer_idx,
                    strip,
            ...
            match tokio::fs::create_dir(&dest_path).await {
                ...
                Err(e) if e.kind() == std::io::ErrorKind::AlreadyExists => {
                    let existing = tokio::fs::symlink_metadata(&dest_path)
                        .await
                        .map_err(|e| file_error(&dest_path, e))?;
                    if !existing.is_dir() {
                        return Err(file_error(
                            &dest_path,
                            std::io::Error::other(AssemblyError::LayerOverlap(overlap_kind(contributors))),
...
async fn handle_symlink_entry(
    ...
    #[cfg(unix)]
    {
        ...
        ocx_util::fs::symlink::create(&target, dest_path)?;
...
mod tests {
    ...
    async fn preserves_relative_symlink_same_directory() {
        ...
        ocx_util::fs::symlink::create(std::path::Path::new("libfoo.so.1.2.3"), &link_in_layer).unwrap();
    ...
    async fn preserves_relative_symlink_cross_directory() {
        ...
        ocx_util::fs::symlink::create(std::path::Path::new("../lib/real.so"), &link_in_layer).unwrap();
    ...
    async fn preserves_absolute_external_symlink() {
        ...
        ocx_util::fs::symlink::create(&external_file, &link_in_layer).unwrap();
    ...
    async fn preserves_chained_symlinks() {
        ...
        // c → real
        ocx_util::fs::symlink::create(std::path::Path::new("real"), root.join("layer/content/c")).unwrap();
        // b → c
        ocx_util::fs::symlink::create(std::path::Path::new("c"), root.join("layer/content/b")).unwrap();
        // a → b
        ocx_util::fs::symlink::create(std::path::Path::new("b"), root.join("layer/content/a")).unwrap();
    ...
    async fn preserves_broken_symlink_verbatim() {
        ...
        ocx_util::fs::symlink::create(std::path::Path::new("does_not_exist"), &link_in_layer).unwrap();
    ...
    async fn absolute_symlink_survives_dest_rename() {
        ...
        let link_in_layer = root.join("layer/content/abs_link");
        ocx_util::fs::symlink::create(&external, &link_in_layer).unwrap();
    ...
    async fn errors_when_source_content_missing() {
        ...
        if let Err(AssembleError::File(FileError { cause: io_err, .. })) = result {
            assert!(
                matches!(
                    io_err.kind(),
            ...
            panic!("expected AssembleError::File wrapping an IO error");
        }
    }
    ...
    async fn errors_on_file_collision_across_layers() {
        ...
        match err {
            AssembleError::File(FileError { path, cause: io_err }) => {
                assert_eq!(
                    io_err.kind(),
            ...
            other => panic!("expected AssembleError::File, got {other:?}"),
        }
    }
    ...
    async fn errors_on_type_mismatch_dir_vs_file() {
        ...
        match err {
            AssembleError::File(FileError { cause: io_err, .. }) => {
                assert_eq!(
                    io_err.kind(),
                    std::io::ErrorKind::AlreadyExists,
    ...
    async fn windows_file_symlink_errors_unsupported() {
        ...
        if let AssembleError::File(FileError { cause: io_err, .. }) = &err {
            ...
            panic!("expected AssembleError::File wrapping AssemblyError");
        }
    }
    ...
    async fn walker_rejects_escaping_symlink_into_dest_root() {
        ...
        ocx_util::fs::symlink::create(Path::new("../../../../etc/passwd"), &link_in_layer).unwrap();
    ...
    async fn ml_symlink_overlap() {
        ...
        let src_a = make_dir(&root, "layer_a/content");
        make_dir(&root, "layer_a/content/lib");
        ocx_util::fs::symlink::create(std::path::Path::new("target_a"), root.join("layer_a/content/lib/link")).unwrap();
        let src_b = make_dir(&root, "layer_b/content");
        make_dir(&root, "layer_b/content/lib");
        ocx_util::fs::symlink::create(std::path::Path::new("target_b"), root.join("layer_b/content/lib/link")).unwrap();
    ...
    async fn ml_type_mismatch_dir_vs_symlink() {
        ...
        let src_b = make_dir(&root, "layer_b/content");
        ocx_util::fs::symlink::create(std::path::Path::new("some_target"), root.join("layer_b/content/foo")).unwrap();
    ...
    fn uniform_strip(count: usize, strip: u8) -> Vec<LayerPlacement> {
        (0..count)
            .map(|_| LayerPlacement {
                strip,
                prefix: ocx_util::fs::path::RelativePath::default(),
            })
            .collect()
    }
    ...
    fn rel(spec: &str) -> ocx_util::fs::path::RelativePath {
        ocx_util::fs::path::RelativePath::parse(spec).expect("test prefix parses")
    }
    ...
    fn assembly_error_message(err: &AssembleError) -> String {
        let AssembleError::File(FileError { cause: io_err, .. }) = err else {
            panic!("expected AssembleError::File wrapping an AssemblyError, got {err:?}");
        };
        let assembly = io_err
            .get_ref()
    ...
    async fn layout_refuses_symlinked_prefix_ancestor() {
        ...
        ocx_util::fs::symlink::create(&outside, dest.join("share")).unwrap();
