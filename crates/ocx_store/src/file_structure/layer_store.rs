...
impl LayerStore {
    ...
    pub async fn list_all(&self) -> Result<Vec<LayerDir>> {
        ...
        ocx_util::fs::DirWalker::new(self.root.clone(), classify_layer_dir)
...
fn classify_layer_dir(dir: &Path, _depth: usize) -> ocx_util::fs::WalkDecision<LayerDir> {
    if dir.join("content").is_dir() {
        if super::cas_path::is_valid_cas_path(dir) {
            return ocx_util::fs::WalkDecision::leaf(LayerDir { dir: dir.to_path_buf() });
        }
        log::warn!("Skipping content/ dir not matching CAS layout: {}", dir.display());
        return ocx_util::fs::WalkDecision::skip();
    }
    ocx_util::fs::WalkDecision::descend_skip(LAYER_SKIP_NAMES)
}
