...
impl PackageStore {
    ...
    pub async fn list_all(&self) -> Result<Vec<PackageDir>> {
        ...
        ocx_util::fs::DirWalker::new(self.root.clone(), classify_package_dir)
...
fn package_dir_for_content(path: &Path) -> std::result::Result<PathBuf, PackageDirError> {
    ...
    if canonical.file_name() == Some(std::ffi::OsStr::new("content")) {
        canonical
            .parent()
            .map(|p| p.to_path_buf())
            .ok_or(PackageDirError::PathInvalid(canonical))
    } else {
        Ok(canonical)
    }
}
...
pub async fn record_origin(pkg: &PackageDir, identifier: &ocx_oci::PackageRef) -> Result<()> {
    ...
    if tokio::fs::try_exists(&marker)
        .await
        .map_err(|e| ocx_util::error::FileError::new(marker.clone(), e))?
    {
        return Ok(());
    }
...
fn classify_package_dir(dir: &Path, _depth: usize) -> ocx_util::fs::WalkDecision<PackageDir> {
    if dir.join("content").is_dir() {
        if super::cas_path::is_valid_cas_path(dir) {
            return ocx_util::fs::WalkDecision::leaf(PackageDir { dir: dir.to_path_buf() });
        }
        log::warn!("Skipping content/ dir not matching CAS layout: {}", dir.display());
        return ocx_util::fs::WalkDecision::skip();
    }
    ocx_util::fs::WalkDecision::descend_skip(PACKAGE_SKIP_NAMES)
}
...
mod tests {
    ...
    fn metadata_for_content_follows_symlink() {
        ...
        ocx_util::fs::symlink::create(&content, &link).unwrap();
