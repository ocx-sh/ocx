...
- Add [`[records]`](https://ocx.sh/docs/reference/configuration#keys-records) — write one JSON [exec-time resolution record](https://ocx.sh/docs/reference/execution-records) per invocation to an operator-designated directory immediately before a tool starts, naming every package digest that composed the environment plus the resolved executable. Configurable by `[records]` config, [`OCX_RECORDS_DIR`](https://ocx.sh/docs/reference/environment#ocx-records-dir) / [`OCX_RECORDS_NAME`](https://ocx.sh/docs/reference/environment#ocx-records-name), and `--records-dir` / `--records-name` on `ocx run` and `ocx package exec` (not `ocx launcher exec`, which inherits the sink via forwarded env only). A SYSTEM-scope `[records]` policy locks the whole block and can require the invocation to fail closed (exit 74 for an unwritable sink, exit 78 for a sink substituted with a symlink) rather than run unrecorded. Each published record file is owner-only (mode `0600` on Unix) and is not `fsync`'d before publish, so a host power loss can lose the most recent ones *(record)*
- The record's field names describe exactly what each value carries: `process` has no `args` (a command line can carry secrets a central, often fleet-aggregated sink must not receive — `process.executable` still names the binary), and adds `process.arch` (the running ocx binary's own architecture, not the host machine's) and `process.user.id` alongside the caller-controlled `process.user.name`; `resolution.registries` (was `indexSources`) names the content registries resolved packages carry, not the index endpoint a version choice was looked up through; `resolution.mirrors` is `host -> {registry?, index?}` so a mirror entry covering both roles keeps both endpoints; `scope.lock.declarationDigest` (was `digest`) names what it hashes — the lock's declaration hash, not the resolved closure; `sh.ocx.binaries`/`sh.ocx.entrypoints` are JSON arrays, not comma-joined strings; and two project-tier-only annotations, `sh.ocx.binding` and `sh.ocx.group`, name the `ocx.toml` binding and group a package was selected under *(record)*
...
- `ocx package select` no longer wires `current` to a candidate the host cannot run *(cli)*: resolving a package already in the store did not record which platform the resolution selected, so the host-runnable gate that already suppressed the `current` symlink on `ocx install -p <foreign-platform>` saw nothing to act on and `select -p <foreign-platform>` wired it anyway. Both paths now stamp the selected platform, so the two commands agree.
- Refuse `[records] required = true` when no `dir` is configured at any tier *(record)*: a block carrying only that line — the plainest way an operator writes "recording is mandatory" — resolved to a policy with nowhere to write, and the launch path skips a non-recording policy before the fail-closed posture is ever applied, so every invocation on the host ran unrecorded with exit 0 and no warning. It is now a configuration error (exit 78) raised where the configuration is read. A SYSTEM-scope `[records]` block with neither `dir` nor `required` still resolves cleanly — that is an operator locking recording *off* for the host, not asking for it.
- Stop a symlinked or unreadable `/etc/ocx/config.toml` from silently dropping operator policy *(config)*: the system tier was filtered with the same best-effort semantics as the user tiers, so a symlink (or any non-absent stat failure) was skipped with a warning and the invocation continued with none of the sections locked there — an operator symlinking the file at a fleet-managed config took the whole fleet out of a locked [`[records]`](https://ocx.sh/docs/reference/execution-records) sink without a failure anywhere. A system file that exists and cannot be consulted is now a configuration error (exit 78), on the `OCX_NO_CONFIG=1` path too. Absence stays a silent skip, and the user/`$OCX_HOME` tiers keep best-effort discovery.
- Refuse the maintainer-preview recording exemption under a fail-closed policy *(record)*: `ocx package test` and `ocx patch test` write no execution record, and a package that declares an entrypoint carries that exemption across the `ocx launcher exec` re-entry through the scratch pkg-root it was baked with. Because the exemption is claimed by path placement and both scratch roots live in the invoking user's own `$OCX_HOME`, copying an installed package tree there and invoking the launcher against it was an opt-out any caller could take — and the exempt path skipped the `[records]` fold entirely, so a SYSTEM-locked policy was never consulted. Under [`required = true`](https://ocx.sh/docs/reference/configuration#keys-records-required) no exemption is now granted: the launch is refused with exit 74 naming the `[records]` policy. Previews still run unrecorded wherever no policy demands otherwise. **Consequence**: on a host carrying a fail-closed policy, `ocx package test` and `ocx patch test` do not run.
- Stop `OCX_NO_CONFIG=1` from silently defeating a SYSTEM-scope config lock *(config)*: the flag used to prune the system tier wholesale, so a SYSTEM-scope `/etc/ocx/config.toml` policy stopped applying under it instead of staying enforced. `OCX_NO_CONFIG=1` now still loads the system file, filtered to only its locked sections — a locked [`[registry]`](https://ocx.sh/docs/reference/configuration#keys-registry), [`[registries.<name>]`](https://ocx.sh/docs/reference/configuration#keys-registries), [`[mirrors]`](https://ocx.sh/docs/reference/configuration#keys-mirrors), [`[patches]`](https://ocx.sh/docs/reference/configuration#keys-patches), or [`[records]`](https://ocx.sh/docs/reference/configuration#keys-records) section now applies even under this flag. Only ordinary (unlocked) system/user/`$OCX_HOME` configuration and the `[managed]` tier are pruned, matching the flag's documented hermetic-CI intent.
