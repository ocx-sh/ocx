...
- `$OCX_HOME/init.<shell>` static PATH-prepend entrypoint written by `ocx shell init`. Sourced by non-interactive shells (CI `bash --norc`, `bash -c`) so the global `current` tool set is visible without the per-prompt hook. Contains a self-contained POSIX upward `ocx.toml` walk that suppresses the PATH-prepend when a project is in scope (strict isolation). *(shell)*
