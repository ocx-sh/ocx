...
- An index-indirected package resolves its physical transport address from the local index copy *(cli)*: only the remote index answered the `repository` pointer, so with no network — and no `--offline` flag — `ocx run`/`env`/`pull` exited 69 on a `GET https://index.ocx.sh/config.json` against a fully warm store, and under `--offline` the lookup returned nothing at all, which reads as "no rewrite" and reported the *logical* identifier (`ocx.sh/<ns>/<pkg>`) as its own transport — indistinguishable from a genuinely registry-backed package. Every committed root document already carries the answer, so the local copy now supplies it whenever no configured source can. A reachable index source still answers first: its address has passed the SSRF pre-flight, and reading the local copy ahead of it would skip that floor on every warm-cache resolve while saving nothing.
