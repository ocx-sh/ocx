...
- `ocx starlark-lsp` (hidden, internal, unstable) provides LSP-based completion and hover for `ocx.*` and `expect.*` APIs in editors that support the Language Server Protocol. Not shown in `--help`; wire format carries no stability promise. *(cli)*
