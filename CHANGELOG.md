...
- `mirror.yml` `tests` entries accept one of three mutually-exclusive forms: `command` (shell command, as before), `script` (path to a `.star` file resolved against the mirror repo), or `script_inline` (inline Starlark as a YAML block scalar). The generated CI workflow runs `command` via `ocx package test -- …`, `script` via `--script <path>`, and `script_inline` piped to `--script -`. *(mirror)*
- `ocx package test --script <PATH|->` runs an embedded [Starlark](https://github.com/bazelbuild/starlark) test script against a locally materialized package instead of exec'ing a trailing command. The script has access to the `ocx.*` host API (run, env, platform, file ops) and the `expect.*` assertion API (`ok`, `eq`, `ne`, `true`, `false`, `contains`, `matches`, `fail`). Sandbox bounds script-driven file access to a scratch directory; binaries spawned via `ocx.run` run with normal host privileges. `--script -` reads the script from stdin. Exit codes: 0 all expectations passed, 1 expectation failure, 64 usage/missing script, 65 syntax or type error, 74 I/O failure. `--format json` emits a structured `ScriptRunReport` envelope via the existing output path. *(cli)*
- `ocx lsp` (hidden, internal, unstable) provides LSP-based completion and hover for `ocx.*` and `expect.*` APIs in editors that support the Language Server Protocol. Not shown in `--help`; wire format carries no stability promise. *(cli)*
...
- `entrypoints` field in package metadata. Publishers declare named launchers (e.g. `cmake`, `ctest`); `ocx install` generates per-platform launchers under `<package>/entrypoints/<name>` (POSIX) and `<package>/entrypoints/<name>.cmd` (Windows) that delegate to `ocx launcher exec` against a baked package root. Each entry has a single field — `name` — and the launcher resolves it against the composed `PATH` from the package's `env` block at exec time. *(package)*
...
- `ocx shell hook` (stateful prompt-hook entry point) and `ocx shell direnv` (stateless direnv-style export generator) emit shell exports for the resolved project toolchain. The fingerprint env var `_OCX_APPLIED` keeps the prompt cheap by skipping unchanged invocations. *(shell)*
- `ocx shell init <SHELL>` prints a per-shell init snippet that wires `ocx shell hook` into Bash `PROMPT_COMMAND`, Zsh `precmd`, Fish `fish_prompt`, and Nushell `pre_prompt` hooks. *(shell)*
- `ocx generate direnv` writes a `.envrc` file wiring `ocx shell direnv` into [direnv](https://direnv.net/), watching `ocx.toml` and `ocx.lock` for re-evaluation. *(cli)*
...
- `ocx update --check` verifies the candidate lock would match the predecessor and exits without writing. Mirrors `lock --check` for the partial-resolve flow: exit 0 on match, 65 when an advisory tag has moved upstream for any selected or preserved entry, 78 when the predecessor lock is absent. *(cli)*
...
The project registry on-disk format (`$OCX_HOME/projects.json`) is at schema v1; `EntryOnDisk.config_path` is required. The feature has not shipped, so there is no migration path from earlier in-flight schemas.
