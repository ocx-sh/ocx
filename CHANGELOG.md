...
- Make a configured index authoritative for its whole registry *(oci)* **BREAKING**: if `[registries."<ns>"]` sets `index`, only packages that index holds are discoverable through it. A reference the index has no root for is now a hard miss (exit 79) instead of falling through to the plain OCI registry the index points at — the fall-through is what let a flat `ocx.sh/<tool>` name resolve past the index and so past its yank and deprecation gate. The refusal names the index that was consulted and both ways forward: announce the package there, or take the namespace off the index with `index = ""`. An index outage is still reported as an outage, never as an absent package. The index's `config.json` `name_segments` declaration is gone with the fall-through it existed to describe; it is now an unknown key clients ignore. See [Indices](https://ocx.sh/docs/in-depth/indices#public-index-declared-names).
