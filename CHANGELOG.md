...
- `--global` toolchain tier on `add`, `remove`, `lock`, `upgrade`, `pull`, `run`, and `install`. `--global` re-targets the project file to `$OCX_HOME/ocx.toml` and is mutually exclusive with `--project` (exit 64). `install --global <pkg>` is sugar: add to the global file + re-lock + install + select. Strict isolation: `run` and `exec` are hermetic and never read the global file without `--global`. *(project)*
=======
...
- `ocx package` command group with `install`, `uninstall`, `select`, `deselect`, `exec`, `env` — OCI-tier package primitives moved under this group. *(cli)*
...
=======
- `EntrypointNameCollision` structured error at `package install --select` / `package select` and at consumption time (`ocx env`, `ocx package exec`) when two packages in the same visible closure declare the same entrypoint `name`. Surfaces with exit code 65 (`DataError`); recovery: use `package deselect` on one package before selecting the other. *(package-manager)*
- Synthetic `PATH ⊳ <pkg-root>/entrypoints` entry emitted per visible package with a non-empty `entrypoints` array, so generated launchers are reachable through `ocx env` / `ocx package exec` without manual PATH wiring. *(env)*
- Windows-only synthetic `PATHEXT ⊳ .CMD` prepend on `ocx env` and auto-injected by `ocx package exec` so generated `.cmd` launchers are discoverable when the host shell's `PATHEXT` does not already include `.CMD`. Consumer-boundary commands (`package install`, `package select`) emit a stderr warning when `PATHEXT` is missing `.CMD`. *(cli)*
>>>>>>> 9b296687 (feat(cli)!: toolchain CLI taxonomy + global activation via env exporter)
...
- **Breaking:** Root `ocx install`, `ocx select`, `ocx deselect`, `ocx uninstall`, and `ocx exec` commands removed — they exit 64 (`UsageError`). Use the `ocx package` group instead: `ocx package install`, `ocx package select`, `ocx package deselect`, `ocx package uninstall`, `ocx package exec`. *(cli)*
