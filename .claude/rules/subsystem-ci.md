...
- **The floor is `CRATES_RULE_TARGETS` (78), the script's default, and no lane raises it.** It is a *reader* floor — it separates "the reader parsed nothing" from "the build was clean" — and 78 is the only number for this with a home (`scripts/bazel_gate_proofs.py`, re-asserted by `prove_counts`). Spelling the live `//...` count here would be a second copy of `STAGE_FLOORS["stage-4"]`, in a place nothing re-measures. The tighter reads of the same graph block two steps away: `bazel:build:drift` floors at 82 (`DRIFT_TARGET_FLOOR`), `bazel:tag:guard` at 358 (`STAGE_FLOORS["stage-4"]`, its default stage).
...
| Gate | Platform scope |
|---|---|
| `task verify` (root) | **No Windows or macOS leg.** `.verify:lint` runs `rust:format:check` directly, not `rust:verify`, and `.verify:build-test`'s `rust:clippy:check` is a Linux-only Bazel pass (`platforms: [linux]`), so a macOS or Windows `task verify` runs no clippy. |
