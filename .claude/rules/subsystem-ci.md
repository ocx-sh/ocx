...
**A known ceiling, at roughly half its headroom.** junit2otlp reads stdin with a 1 MB-per-line scanner, and pytest writes its report as a single line — 545,175 bytes for the full acceptance suite as of the commit that added this. Past the ceiling the tool exits 1 with `bufio.Scanner: token too long` and pushes nothing, which both routes report as a warning; it never truncates. The suite floor only rises and a red run's failure text inflates the report further, so the gap is not permanent. The fix, when it trips: put newlines into the stream (`sed 's/<testcase/\n&/g'` ahead of the pipe — inter-element whitespace is ignorable in XML, and a literal `<testcase` cannot occur inside text content because it would be escaped there), or take a junit2otlp with a larger buffer. Nextest's report is not exposed: it is already 8,286 lines with a 242-byte maximum.
