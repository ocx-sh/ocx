...
| Command | Purpose | Key Flags |
|---------|---------|-----------|
| `package announce` | Observe an owner-curated tag set and publish the rebuilt entry into the index (write locally or open/update a fork pull request) | `--package`, `--tags`/`--tags-file`/`--refresh`, `--out`/`--fork`, `--index-repo`, `--yank`/`--unyank`/`--yank-reason` |
