...
- **`lsp` internal subcommand**: hidden from `--help` (`#[command(hide = true)]`). Wire ABI is `ocx lsp` (stdio LSP server). INTERNAL and UNSTABLE — not part of the user-facing CLI surface, name/wire format carry no stability promise. Documented only in authoring/IDE docs, never in the command-line reference. Implementation lives in `crates/ocx_cli/src/command/lsp.rs`.
