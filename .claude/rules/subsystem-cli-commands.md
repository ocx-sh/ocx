...
| **Low-level registry** | `package pull`, `package push`, `package copy`, `package description push`, `package description pull`, `package create`, `package receipt`, `index update/sync/list/catalog`, `login`, `logout` | OCI identifiers | Never |
...
| `update [-g GROUP]... [NAME...]` | Re-resolve advisory tags in lock against the LIVE registry by default (update-family verb: writes `ocx.lock` only, never tag pointers; `--remote` redundant-but-accepted; `--frozen` caps at snapshot, unknown tag exit 81); whole file (no args) or a scoped subset by name/group (reuses `resolve_lock_touched`: named bindings re-resolve, rest carried forward verbatim; scoped needs a predecessor lock, exit 78 if absent; refuses drifted `ocx.toml`, exit 65; unknown group/name, exit 64). ADR `adr_toolchain_update_family.md` | `-g/--group`, `--check`, `--pull/--no-pull`, `-v/--verbose` (plain rendering only — lists the bindings that held still; `--format json` unaffected) |
| `exec [-g GROUP]... [NAME...] -- ARGV...` (visible alias `x`) | Spawn child with composed toolchain env. No `--self`: the self view is package vocabulary and drops a package's own `entrypoints/` from `PATH`, so it composes a strictly worse toolchain. Declares `--pinned`/`--no-pinned` as the CLI tier of the `pinned` ladder (see `subsystem-cli.md` "Toolchain Activation") | `-g/--group`, `--clean`, `--env`, `--pinned/--no-pinned` |
| `env [--shell[=NAME]] [--ci[=PROVIDER]] [--pinned \| --no-pinned]` | Composed toolchain env; output via root `--format` (default plain); `--shell[=NAME]` = eval-safe; `--ci` = CI sink (later-step); installs on miss by default (`--no-pull` opts out → offline local probe; missing tool → stderr warn + omit, exit 0); JSON also carries `binaries`/`entrypoints`/`integrations` admitted-claim attribution arrays — `integrations` is the payload-carrying `{namespace, package, payload}` shape, one row per (package, namespace) pair, never merged (never in `--shell`/`--ci` output). Declares `--pinned`/`--no-pinned` as the CLI tier of the `pinned` ladder, same as `exec` | `-g/--group`, `--env`, `--shell[=NAME]`, `--ci[=PROVIDER]`, `--export-file`, `--pull/--no-pull`, `--pinned/--no-pinned` |
| `pull` | Pre-warm package store from `ocx.lock`; re-saves lock to advance mtime for direnv re-fire (skipped under `--dry-run`). **Also renders the toolchain home** (C-045) — `shells/default/bin` trampolines (reached as `active/bin`) for the default group, `links/<group>/<entry>` links for every group the render covers, with `-g` narrowing that coverage (the one command whose `-g` narrows the render). A render outcome is never `pull`'s exit code: a skip warns on stderr and the command still reports its pull result. `--dry-run` reports the render delta and heals nothing. Declares no `--pinned` (RUL-72) | `--dry-run`, `-g/--group`, `-p/--platform`, `--lazy-mode` |
...
| Command | Purpose | Key Flags |
|---------|---------|-----------|
| `package announce <NAMESPACE>/<NAME>` | Observe an owner-curated tag set and publish the rebuilt entry into the index — write locally (`--out`), or open/update a pull request (GitHub) or merge request (GitLab) from a fork (`--fork`) or from a branch on the index repo itself (neither flag; needs push access, verified up front, exit 80 naming repo + permission). Coordinates are `[HOST/]NAMESPACE/PROJECT` — host selects a self-hosted instance (GHES `/api/v3`, GitLab `/api/v4`), namespace may nest (GitLab groups; GitHub refuses nesting). Forge inferred for github.com/gitlab.com/no-host, `--forge` REQUIRED for a self-hosted host (never probed — a wrong guess sends the credential to the wrong API), exit 64. `--transport git` writes over a temporary clone instead of the REST API (GitLab only), and a forge capability that transport needs but the credential lacks exits 86. The package is a **positional**; the old `--package` flag is hidden, deprecated, warns once on stderr and is removed in 0.7 (naming it both ways is exit 64) | `<NAMESPACE>/<NAME>` (positional, required), `--tags`/`--tags-file`/`--tags-from-registry`/`--refresh`, `--out`/`--fork` (both optional), `--index-repo`, `--forge github\|gitlab`, `--transport api\|git`, `--yank`/`--unyank`/`--yank-reason` |
| `package claim <NAMESPACE>/<PACKAGE>` | Claim ONE package before its first `package announce` — the claimed unit is the package root `p/<ns>/<pkg>.json`, never the namespace prefix, so every package needs its own claim run: render the index entry, resolve the owners it records, and open a pull or merge request against the index repo — or write it locally with `--out`. Owners default to the CI environment's user variables, else the identity behind the credential; `--owner` replaces that list rather than adding to it, and a bot identity is refused at 64. A re-claim adds the caller's owners (union by forge id) and reports `updated`, or `unchanged` with nothing to add; refuses with 65 only on `RootNameMismatch` / `RepositoryMismatch`. Every claim writes the `__ocx.desc` description (readme + logo blobs), and a re-claim refreshes it. `--transport git` writes over a temporary clone instead of the REST API (GitLab only; refused on GitHub, with `--fork` and with `--out`, each 64), and a forge capability the transport needs but the credential lacks exits 86. Argv refusals, all 64: `--out` ⟂ `--fork`; `--fork` must name the same host as `--index-repo`; `--forge` REQUIRED for a self-hosted host (never probed — a wrong guess sends the credential to the wrong API) and GitHub refuses a nested namespace; `--upstream-repository-url` must be an `http`/`https` URL with **no embedded credentials**, since it reaches the committed root verbatim and a forwarded `CI_REPOSITORY_URL` would publish its job token (the refusal names the flag and the rule, never the value) | `--repository` (required), `--owner` (repeatable, `LOGIN` or `LOGIN:ID`), `--upstream-org`/`--upstream-repository-url`/`--upstream-disclaimer`, `--index-repo`, `--forge github\|gitlab`, `--transport api\|git`, `--fork`, `--out` |
...
| `package cascade repair [--dry-run] [--tags-file PATH] <id>...` | Recompute and PUT the whole alias index for every tag the fold disagrees with — batched, concurrent writes, preflighted against missing child manifests. `--dry-run` previews the same plan with zero registry writes. Exit 0 once every attempted registry write succeeded (remaining index staleness is `announce`'s job, not a failure here); 65 when a finding remains after the run (including an alias refused for lacking new content); 64 usage | `--dry-run`, `--tags-file` |
...
| `package exec PKGS... -- CMD` (visible alias `x`) | Run command with package env (hermetic); `--rm` collects each composed package afterwards unless an install symlink, a project or global `ocx.lock` pin, or a site-patch companion still holds it (reachability decides, never "did this run pull it") — stderr only, child exit code never overridden | `--clean`, `-p`, `--self`, `--env`, `--rm` |
...
| `package receipt BUNDLE` | Print the build receipt `create` wrote beside a bundle (`platform`/`identifier`, each key absent when not recorded) via root `--format`; 79 when absent, 65 when malformed | — |
| `package create PATH` | Bundle directory into archive; `--bin-scan`/`--no-bin-scan` fill or verify the `binaries` claim. `PATH` may be an archive instead of a directory under `--extract` | `-o`, `-m`, `-l`, `-j`, `--force`, `--bin-scan`/`--no-bin-scan`, `--extract`, `--strip-components N` (implies `--extract`; an extraction that yields nothing is exit 65) |
| `package push -i ID LAYERS...` | Publish archive to registry | `-i`, `-c`, `-n`, `-m`, `-p`, `--build-timestamp`, `--ci-annotations[=github\|gitlab]` (derive `image.source`/`.revision`/`.created`/`.version` from the CI env onto every index the push writes; bare = autodetect, exit 64 when undetectable; `.version` strips the variant prefix and is absent, not an error, for a non-version tag; `created` always stamped, the other three absent when their variable is unset/blank; explicit `--annotation` wins), `--default` (boolean — let the pushed tag's own variant also own the bare version track; index write only, no re-upload; cascades the bare track under `-c`; a tag with no variant is a usage error, exit 64, not a silent no-op), `--keep-tag/--no-keep-tag` (default on — pushes `__ocx.keep.<algorithm>-<hex>` per platform manifest, registry-side deletion safety net, skipped when the tag would exceed 128 chars; `index.ocx.sh` ignores it), `--tags-file` (append pushed + cascade tags to a scratch file `package announce --tags-file` can consume), `--sign` (sign each pushed platform manifest inline, never the index — keyless default, `--key` for a key pair), `--sbom PATH` (attest a CycloneDX SBOM against the pushed digest), `--signature-format bundle\|simplesigning\|both` and `--key`/`--rekor-upload`/`--no-rekor-upload` (apply to both `--sign` and `--sbom`'s inline signing; usage error exit 64 without either) |
| `package copy SOURCE` | Promote a published package to another registry/repository. Leaf manifests + blobs + referrers copied verbatim (digest preserved, so signatures and lock pins survive); the target index is merged **per platform**, never byte-copied; rolling tags recomputed against the **target**. Per-platform report rows: `added`/`unchanged`/`replaced`/`kept (not in source)` | `--to` (host rewrite) \| `-i/--identifier` (mutually exclusive; required for a digest source), `-p/--platform` (filter for a tag source, **required declaration** for a digest source), `-c/--cascade`, `--keep-tag`/`--no-keep-tag` (default on), `--referrers`/`--no-referrers` (default on; exit 84 without the Referrers API), `--description`, `--annotation`, `--dry-run` |
...
| `package sign IDENTIFIER` | Sigstore sign via OCI Referrers, keyless by default | `-p/--platform` (optional — narrows into one platform of an index; error when the resolved object isn't one), `--fulcio-url`, `--rekor-url`, `--identity-token-file`, `--identity-token-stdin`, `--no-tty`, `--no-cache`, `--signature-format bundle\|simplesigning\|both`, `--key` (keyless default; `awskms`/`gcpkms`/`azurekms`/`hashivault`/`k8s` recognized+refused exit 85; unparseable exit 64), `--rekor-upload`/`--no-rekor-upload` (keyless always on, `--no-rekor-upload` there is exit 64; `--key` off by default), `--tags`/`--tags-file` (sweep indices instead of one reference; mutually exclusive with `-p`) |
| `package verify IDENTIFIER` | Sigstore verify via OCI Referrers, keyless by default | `-p/--platform` (optional — narrows into one platform of an index; error when the resolved object isn't one), `--certificate-identity` / `--certificate-oidc-issuer` (optional-when-a-`[trust.policy]`-matches; **both-or-neither**, one alone → exit 64), `--sigstore-trusted-root`, `--rekor-url`, `--offline`, `--no-cache` |
| `package attest --predicate FILE --type TYPE IDENTIFIER` | Attach a signed in-toto attestation (SBOM/provenance) as a cosign-bundle referrer, keyless by default | `--predicate` (required), `--type` (required; cyclonedx/spdx/spdxjson/slsaprovenance1/URI), `-p/--platform` (optional, same narrowing-modifier contract as `sign`), `--fulcio-url`, `--rekor-url`, `--identity-token-file`, `--identity-token-stdin`, `--no-tty`, `--no-cache`, `--signature-format bundle\|simplesigning\|both`, `--key`, `--rekor-upload`/`--no-rekor-upload` (same defaults and exit codes as `sign`), `--tags`/`--tags-file` (sweep indices; mutually exclusive with `-p`) |
...
| `package test -i ID LAYERS... -- CMD` | Materialise + exec locally (no registry) | `-i`, `-p`, `-m`, `--keep`, `-o`, `--self`, `--clean`, `--env`, `--junit PATH` (scripted runs only — no clap `requires = "script"`, since that would demand `--script` and the trailing `<COMMAND>` simultaneously; `command` instead lists `junit` in its `required_unless_present_any`, so a bare `--junit` parses through to a hand-written `validate_junit` check that gives the clean "--junit requires --script" message, while `conflicts_with = "command"` still catches `--junit` beside a command at parse time; truncates, never merges; one file per platform) |
...
- Both subcommands take one or more identifiers, logical (`ocx.sh/<ns>/<pkg>[:tag]`, resolved via `Index::route_for_dial` the same way `install` does) or physical (bare registry path); a digest-pinned identifier is a usage error (exit 64) — there is no tag graph to diff against a fixed digest.
...
- `repair --tags-file <PATH>` writes the tags this run re-pointed or created, unioned with every tag an index finding names (newline-separated, `parse_tags_file`-compatible; empty file when nothing changed, which is the usual `--dry-run` result — a preview wrote nothing, so it hands on only its index findings, never its plan) — the follow-up publish step is `package announce --tags-file <PATH>` (union semantics, so it also picks up an alias that was never committed at all).
...
| Command | Purpose | Key Flags |
|---------|---------|-----------|
| `self setup [VERSION]` | Bootstrap specified or latest ocx, write env shims, add managed activation block to shell profiles, and register the ocx install `bin` dir plus `$OCX_HOME/toolchain/active/bin` on the **session** PATH (global tier only). `--toolchain-activate MODE` additionally writes `activate` into `$OCX_HOME/ocx.toml` | `--no-modify-path`, `--profile PATH`, `--toolchain-activate MODE`, `--dry-run`, `--force` |
