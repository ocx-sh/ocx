...
- `repair --announce-tags <PATH>` writes the tags this run re-pointed or created (newline-separated, `parse_tags_file`-compatible; empty file when nothing changed) — the follow-up publish step is `package announce --tags-file <PATH>` (union semantics, so it also picks up an alias that was never committed at all).
