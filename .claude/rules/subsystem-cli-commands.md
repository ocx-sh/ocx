...
| Command | Purpose | Key Flags |
|---------|---------|-----------|
| `self setup [VERSION]` | Bootstrap specified or latest ocx, write env shims, add managed activation block to shell profiles, and register the ocx install `bin` dir plus `$OCX_HOME/toolchain/bin` on the **session** PATH (global tier only). `--toolchain-activate MODE` additionally writes `activate` into `$OCX_HOME/ocx.toml` | `--no-modify-path`, `--profile PATH`, `--toolchain-activate MODE`, `--dry-run`, `--force` |
