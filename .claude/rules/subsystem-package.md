...
| `metadata/authoring.rs` | `AuthoringMetadata`/`AuthoringBundle` — sidecar (authoring) superset of `Metadata`: dep digests optional, per-dep `platforms` pin maps, bundle `platforms` target set; `to_published(platform)` projection strips sidecar fields by construction; `AuthoringError` (65). ADR `adr_dependency_manifest_pinning.md` |
| `metadata/authoring/dependency.rs` | `AuthoringDependency` (digest-optional identifier + `platforms: Option<BTreeMap<String, Digest>>`, keys are canonical-grammar `Platform` strings), `pin_for(platform)` projects via `project::lookup_host_leaf` — the same `select_best`/`is_compatible` relation `Index::select` uses (D1, `adr_platform_model_unification.md`), not a tiered lookup; `AuthoringDependencies` validated collection |
| `dependency_pinning.rs` | `pin_dependencies(metadata, index, declared_platform)` — the `ocx package create` compile step: resolves tag-only deps to platform MANIFEST digests via `Index::fetch_candidates(Resolve)` (never index digests — GC hazard) for the single declared platform (D5 — no bundle-level target set); `DependencyPinningError` (NotFound 79 / DataError 65 / transparent index cause → 81) |
...
| `metadata/template.rs` | `TemplateResolver` — parse + resolve `${installPath}` and `${deps.NAME.installPath}` placeholders; `Usage` enum (`Environment`/`EntryPointArgs`) + `AllowedTokens { deps: bool }` capability gate (gate-before-regex, ADR D6); `disallowed_dep_token(s)` helper identifies `${deps.*}` tokens; `TemplateError` variants including `DisallowedToken { token }` |
| `metadata/validation.rs` | `ValidMetadata` newtype wrapping `Metadata` after cross-field checks (dep name uniqueness, `${deps.*}` references resolve to declared deps); `validate_entrypoint_args` (rejects `${deps.*}` and unknown placeholders in `args` elements, publish-time, pure syntax); `first_unknown_placeholder(value)` helper shared by env and args validation |
...
Metadata (tagged enum: "type" = "bundle")
  └─ Bundle { version: V1, strip_components: Option<u8>, env: Env, dependencies: Dependencies, entrypoints: Entrypoints, binaries: Option<Binaries> }
