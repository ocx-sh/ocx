...
| `metadata/authoring/dependency.rs` | `AuthoringDependency` (digest-optional identifier + `platforms: Option<BTreeMap<lock_key, Digest>>`), `pin_for(platform)` 3-tier lookup (exact → base → any, via `project::lookup_host_leaf`); `AuthoringDependencies` validated collection |
| `metadata/authoring/target_platforms.rs` | `TargetPlatforms` non-empty duplicate-free platform set (serialized as canonical platform strings) — the push fan-out set |
| `dependency_pinning.rs` | `pin_dependencies(metadata, index, declared_platform)` — the `ocx package create` compile step: resolves tag-only deps to platform MANIFEST digests via `Index::fetch_candidates(Resolve)` (never index digests — GC hazard), derives + embeds the target set; `DependencyPinningError` (NotFound 79 / DataError 65 / transparent index cause → 81) |
