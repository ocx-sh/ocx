...
- Full evidence and the per-crate reasoning: `.agents/research/rust-ecosystem.md`, which is gitignored and local to whoever ran the research -- deliberately not a link, because it does not ship with the repository. That file carries its own 54-rule `ECO-*` set — a **separate namespace** from the `DEP-*` rules above, which is why these were renumbered rather than kept as `ECO-*`. An `ECO-nn` citation always means the research file; a `DEP-nn` citation always means this one.
