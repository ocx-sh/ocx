...
- `disable-model-invocation: true` for action skills with side effects (deploy, release, config mutation). Owner may unlock one deliberately when the skill's own workflow is the safety rail (`swarm-plan`, `swarm-execute`, `codex-adversary`, `commit`) — record it in `_EXPECTED_DISABLE_MODEL_INVOCATION` and `MODEL_INVOCABLE_ACTION_SKILLS` in `.claude/tests/test_ai_config.py` the same commit
