...
| `tasks/render_toolchain.rs` | Whole-directory reconcile of a toolchain home — `.gitignore`, one `bin/` trampoline per exposed name (default group only; Windows writes `<name>.exe` plus its `<name>.exec` sidecar), one `<group>/<entry>/` directory link per selected group, then a prune of what the tree no longer declares. Fixed step order, each position forced by the next: dry-run returns first (C-049); refuse a symlinked home and ensure the root **before** the case-fold probe (a probe-first order made a first-ever `ocx pull` skip forever) and before `ensure_gitignore` (which `create_dir_all`s with the ambient umask); then one `lock_scoped` over the home for the whole body — a lock timeout is C-050's **skip, never an error**; then the probe, the render, the best-effort `projects/` ledger registration, and the stamp last (C-048) |
