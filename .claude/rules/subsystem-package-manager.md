...
| `tasks/resolve.rs` | `resolve()`, `resolve_all()`, `resolve_env(packages, self_view: bool)` — index + env resolution; `self_view` bool selects interface surface (`false`) or private surface (`true`) via `composer::compose`; `AdmittedClaims { binaries: Vec<(PinnedIdentifier, BinaryName)>, entrypoints: Vec<(PinnedIdentifier, EntrypointName)>, integrations: Vec<(PinnedIdentifier, IntegrationEntry)> }` (renamed in place from `AdmittedBinaries` — no alias, internal structure has no stability) + `resolve_env_with_attribution(packages, self_view, scope)` — sibling accessor to `resolve_env_with_patch_boundary` (unchanged signature) that additionally surfaces the admitted-set `binaries`/`entrypoints`/`integrations` claim attribution consumed by `ocx env` / `ocx package env`. ADR `adr_declared_binaries_metadata.md` §4, `adr_package_integrations.md` C-013. **A patch companion contributes its `integrations` on the same array**, attributed to the companion's own `PinnedIdentifier` — a companion is a package loaded into the environment and gets no carrier-specific exception (C-017, reversed 2026-08-10). The companion projection cache and `SitePatchSet` both carry `CompanionContribution<E>` (env entries + integrations; `E` is bare while cached, provenance-paired once admitted under a base). The `integrations_cross(self_view)` gate is threaded INTO `build_site_patch_set` and `composer::compose_companion` rather than applied once after the fact, so a companion's integrations payload is never resolved on a surface that would discard it — resolution asserts every `${deps.*}` dependency content directory exists, and gating first keeps a `--self` composition from hard-erroring over a value that surface never carries. Rows are deduped a second time by `(PinnedIdentifier, namespace)` **across the whole composition**, seeded from the base's own rows before companions fold in, because base and companion contributions come from separate `compose` calls that each dedup only within themselves. `admitted_binaries`/`admitted_entrypoints` stay discarded for a companion — PATH-shaped claims the overlay never puts on PATH. |
...
| `tasks/prepare_lazy.rs` | `prepare_lazy(package, platform) -> PreparedLazy { shim, closure, advisories }` — the **producer** half of lazy loading. Resolves the pinned digest through the ambient index chain, stages the closure's config blobs, walks the metadata-only closure with the SAME `common::walk_closure_nodes` `inspect --closure` uses, derives the interface-surface name set (`binaries` claims ∪ entry points, admitted through the shared `inspect::admitted_on_surface` + `composer::carrier_crosses` algebra), writes one generated launcher per name, and publishes the tree into `ShimStore` by a single atomic rename. **No content is downloaded and no package directory is created.** A sibling task, never a cut in `pull.rs` — so a tool that later materializes is byte-identical to one that was never deferred. Publication is all-or-nothing and lock-free (staged tree lands by `rename`; a losing racer converges because the same pinned identifier yields byte-identical bodies), and `bin/` — not `digest` — is the completeness marker, written last, matching what the GC walker classifies on. Idempotent: an already-published tree is returned as-is, and a tree `ocx clean` collected is regenerated. Two refusals: `ShimNamesNotEnumerable` (a node claims neither `binaries` nor entry points) and `ShimNameInvalid` (an entry-point name that is not a valid `BinaryName`). A claimed name equal to `ocx` is **admitted on purpose** (C-024) — a project may pin its own `ocx`, so it renders like any other name and the self-resolution hazard is closed on the trampoline's own re-entry path instead. `ShimNameNotClaimed` belongs to the consuming half (`ocx launcher shim`), not here. `advisories` is C-015's warning-only classifier (`tasks/lazy_advisory.rs`), run **only** for a deferred tool and returned rather than logged so a composing caller can serialize it |
| `tasks/materialize_lazy.rs` | The **consumer** half: what `ocx launcher shim` runs on a first invocation. Materializes through `find_or_install_all` — which creates **no candidate symlink**, so `ocx package uninstall` has nothing to remove afterwards — then re-resolves the name on the materialized `PATH` and execs it. Never touches `ShimStore`, so the shim tree survives materialization and the next compose still emits its slot |
| `tasks/toolchain_names.rs` | `exposed_names(closure, policy) -> BTreeMap<BinaryName, NameOwner>` — the **name-ownership algebra**, one answer to "what names does this closure expose, and who owns each", shared by `ocx env`, `inspect --closure`, `prepare_lazy` and the rendered `bin/`; never a directory scan. Interface surface only. A collision never refuses or warns: the last node walked wins and the losers land on the winner's `NameOwner::shadowed`. `NotEnumerablePolicy` picks `Refuse` (a deferred tool has no fallback) or `Skip` (a render is a projection of declared metadata). `fold_case_insensitive(names)` is the separate **pure** C-025 collapse the renderer applies on a case-insensitive filesystem — ASCII case, never Unicode; the winner is the greatest `walk_index`, never the greatest key; no I/O and no `cfg!`, so it unit-tests deterministically on every host |
| `tasks/render_toolchain.rs` | Whole-directory reconcile of a toolchain home — `.gitignore`, the `active -> shells/default/` symlink healed by observed kind (C-081), one `shells/default/bin/` trampoline per exposed name (default group only; Windows writes `<name>.exe` plus its `<name>.exec` sidecar), one `links/<group>/<entry>/` directory link per selected group, then a prune of what the tree no longer declares. Depth 1 stays the closed set `{.gitignore, active, links, shells}` (C-071) — no user-supplied name ever lands there. Fixed step order, each position forced by the next: dry-run returns first (C-049); refuse a symlinked home and ensure the root **before** the case-fold probe (a probe-first order made a first-ever `ocx pull` skip forever) and before `ensure_gitignore` (which `create_dir_all`s with the ambient umask); then one `lock_scoped` over the home for the whole body — a lock timeout is C-050's **skip, never an error**; then the probe, the `active` heal (after `shells/default/bin` exists, before any trampoline is written), the render, the best-effort `projects/` ledger registration, and the stamp last (C-048) |
| `tasks/lazy_advisory.rs` | `classify_lazy_advisories(package, metadata) -> Vec<LazyAdvisory>` — pure, I/O-free classifier over already-loaded metadata. Three variants: `UndeclaredBinaries` (claim is `None`, not `Some([])`), `InstallPathRootedNonPathVar` (a `constant`/`list` value interpolating `${installPath}`), `CombinedPathValue` (a `path` value concatenating a package-rooted segment with anything else). **Warning only, never a decision** — nothing downstream may steer on one |
...
| `mutate.rs` | `commit_and_render(guard, lock, ToolchainRender)` — the **commit-then-render** sequencing `ocx add`, `ocx remove`, `ocx lock` and `ocx update` all share (C-054). One function rather than four call sites because ADR D4 makes "the four re-render" a behavioural contract, not CLI convenience — encoding it in `ocx_cli` would put it out of reach of every non-CLI consumer. The render covers the **whole** home, never the `-g` selection (that scopes resolution; `ocx pull` is the command whose `-g` narrows the render), and it takes no `dry_run` (RUL-54: `--dry-run` stays `ocx pull`-only). A render failure never rolls the commit back (RUL-53) — the lock is the declaration, the tree only its materialisation. Also `skipped_render_warnings(report)` and `default_group_roots(lock, platform)` |
| `composer.rs` | Two-env composition: `compose(roots, store, self_view: bool) -> ComposeOutput` (flat iteration over each root's pre-built TC with cross-root dedup, surface-gated via the shared predicates below); `emit_package_vars` resolves **every** declared var in declaration order regardless of crossing status (D8, resolve-then-gate) — a crossing var via `EnvResolver::resolve`, a non-crossing one via `resolve_without_emit_assertions` — accumulating a `declared_before: SelfEnvScope<Entry>` scope so `${self.env.KEY}` can reference an earlier var whether or not that earlier var itself crosses the surface; only a crossing var's resolved entry is pushed onto the emitted `entries`; **the surface algebra `dep_admitted(effective, self_view)` + `carrier_crosses(vis, is_root, self_view)` is the single source of truth for what a surface contains — `compose` AND `package_manager::tasks::inspect::project_surface` both route every admission / crossing decision through it, so `ocx env`/`--self` and `ocx package inspect --closure` can never disagree** (the env-asymmetry class of bug came from inspect re-deriving the env rule). Recursive definition (module comment): `surface(P, axis)` = P's own carriers with `vis.has(axis)` ∪ interface surfaces of deps whose edge reaches the axis; below the root only a dep's INTERFACE surface crosses (Algorithm v3 step 5), edge composition precomputed by `through_edge`/`merge` (`ResolvedPackage::with_dependencies`). Flattened: `dep_admitted` = `has_private()` under `self_view` else `has_interface()` (root always admitted — depth 0); `carrier_crosses` = root's carriers on the surface axis, a dep's only their `has_interface()` side on either surface. Carriers with no declared visibility carry an implicit one on their metadata type: `Entrypoints::IMPLICIT_VISIBILITY` = INTERFACE (launchers are consumer-facing; gates the claim AND the synth-`entrypoints/` PATH push in `emit_root_path_block`/`emit_dep_path_block`, so a claim can never contradict PATH — root's launchers absent under `--self`), `Binaries::IMPLICIT_VISIBILITY` = PUBLIC (raw executables serve consumers and the package's own shims). `self_view` only selects which surface is emitted — never membership; `ComposeOutput { entries, admitted, admitted_binaries: Vec<(PinnedIdentifier, BinaryName)>, admitted_entrypoints: Vec<(PinnedIdentifier, EntrypointName)> }` — the latter two are zero-extra-I/O projections of `.binaries()`/`.entrypoints()` off metadata already loaded during the same admission walk, admitted iff the owning package was admitted to this compose call AND the carrier crosses under its implicit visibility (root packages at depth 0, dep packages iff their TC entry passes the active surface gate — same algebra already gating `entries`); `check_entrypoints(roots, store)` (interface-projection collision gate over 1..N roots, reports all N owners); `check_repo_digest_conflicts(roots, self_view)` (**fatal** version-conflict gate — same `registry/repo` resolved to ≥2 distinct digests on the active surface → `Err(DependencyError::Conflict { repository, identifiers })`, exit 65; two tags → same digest tolerated; sealed/private edges excluded; called by `compose`). The diagnostic `deps` command instead calls the non-fatal `warn_repo_digest_conflicts` so the conflicting tree stays inspectable — both share `collect_repo_digest_conflicts`. ADR `adr_declared_binaries_metadata.md` §4 Decision A. `ComposeOutput.admitted_integrations: Vec<(PinnedIdentifier, IntegrationEntry)>` is a fourth such array, but its admission rule is NOT visibility-derived like the other three: `integrations_cross(self_view) = !self_view` gates the carrier — interface-surface only, at every depth, no `is_root` parameter — a **structural** deviation from the algebra above (no `Visibility` value can express it; proof in `adr_package_integrations.md` §4.1), while the edge term stays algebraic and unchanged (`dep_admitted(effective, /* self_view = */ false)`, i.e. only interface-reaching deps ever contribute, on either surface). Payloads are interpolated with the DECLARING package's own `${installPath}` via the already-built `dep_contexts` map — zero extra I/O, same as the other two, and an unrecognized token inside a payload is refused (exit 65) the same as everywhere else in metadata, not passed through. `project_surface` in `tasks/inspect.rs` routes through `integrations_cross` the same way it routes through `dep_admitted`/`carrier_crosses`, so `ocx env`/`--self` and `ocx package inspect --closure` cannot disagree here either. Both `compose` and the patch-companion path (`compose_companion`) route through the private `compose_gated(roots, store, self_view, collect_integrations)` — the one function that actually collects `admitted_integrations`, in its dep and root loops; `integrations_cross` is evaluated once by the caller (`compose`'s own `self_view`, or `resolve_env_with_attribution` for the companion overlay) and threaded down as a plain boolean, never re-derived per collection site. The suppression input changes only what is COMPUTED, never what the surface CONTAINS when the gate is on — with `collect_integrations = true`, `compose_gated`'s output is byte-identical to an ungated `compose` call for the same root set. ADR `adr_package_integrations.md` C-011, C-012, C-017, §4. |
...
`install_companion` (`tasks/patch_discovery.rs`) owns the companion tier's binding: a recorded pin (`FileStructure::patch_companion_path` — see `subsystem-file-structure.md`) is pulled by digest with no tag resolve; a miss resolves through `index().remote_view()` — `ChainMode::Remote` + `LocalWritePolicy::ReadOnly`, mode-INDEPENDENT by design — pulls that digest pinned, then records it. Mode-independence is the #293 fix: patches float, so a companion must resolve even when the ambient chain is `ChainMode::Frozen`; a sync needs the live answer for the same reason. `--offline` is gated before any view is built (`PatchError::PolicyBlocked`, exit 81 when the companion is required). Compose reads the same binding through `companion_pin` (snapshot ▸ record) in `tasks/resolve.rs`. `resolve_site_patch_roots(platform, scope)` takes a `PatchRootScope`: `Recorded` (record only — `ocx patch freeze`, so a freeze snapshots live state instead of its own snapshot) or `RecordedAndSnapshot` (record ∪ the active snapshot's companion and descriptor pins — `clean`, because compose resolves snapshot-first and GC must retain what compose will read; a record-only root set collects the companion a snapshot-driven build still composes). There is no fallback to a local-index tag pointer for the same repository — that pointer is package-tier and belongs to whoever named the package.

**Zero bytes under `index/`, not just no tag pointer.** Both companion pulls run on `PackageManager::read_only_view()` (`LocalWritePolicy::ReadOnly`). Resolving no tag is not sufficient: a `tag@digest` pull skips `commit_root_tag` but under `Full` still `persist_dispatch`es the image index the digest names into the companion repository's own `p/<repo>/o/<algo>/<hex>.json`, and the blob-store `AbsentDispatch` recovery self-heals one back the other way. `ReadOnly` closes both. Content is unaffected — `stage_and_link_chain_blobs` still writes every chain blob to `$OCX_HOME/blobs` and ref-links it from the installed package, which is where compose and GC read it back from. Both companion readers (`find_companion_local`, `resolve_site_patch_roots`) therefore go through the one `companion_manifest_index(manager)` helper in `tasks/resolve.rs`: no sources + `ChainMode::Offline` + the attached `fs.blobs` content store + `read_only_view` — digest-addressed, network-free, digest-verified, and incapable of re-creating the directory the pin move emptied. Unit: `patch_discovery::tests::companion_install_writes_nothing_into_the_local_index_home`; acceptance: `test/tests/test_frozen.py`, `test/tests/test_patches.py` (`assert_no_index_footprint`).

Two consequences of "the record is what freeze and GC read":

- A pin that came from the SNAPSHOT is written back into the record after the pull (`backfill_companion_pin` → `PatchTagMap::write_tag_if_absent`), fill-in only. A machine that only ever installs under `OCX_PATCH_SNAPSHOT` would otherwise hold an empty record, and a freeze there rewrites the snapshot as `companions: {}` while `clean` collects the package. Never an overwrite: a recorded digest is the LIVE binding and only a sync may advance it. The backfilled value is the snapshot's platform-manifest digest rather than the top digest — both read back, because the readers platform-select only when the manifest at the pin turns out to be an image index.
- **A snapshot key is `registry/repository:tag`, not `registry/repository`.** One repository named at two tags is two companions — the record already stores them per tag inside one file, and the overlay composes each separately (`same_companion_repo_at_two_tags_projects_both_versions`) — so a repository-only key made a freeze drop one of them and the snapshot-driven build compose the survivor twice. Both ends of the scheme go through `patch::snapshot::{companion_key, companion_key_identifier}` (inverses, pinned by `companion_key_round_trips`), and `resolve_companion_pinned` keeps the companion's advisory tag on the `PinnedIdentifier` so a freeze has one to key on; the package store still locates it by registry + digest. `SnapshotVersion` has one variant; an older file is refused (`PatchError::UnsupportedSnapshotVersion`, exit 65) naming `ocx patch freeze` — no second parse path, the snapshot is derived state.
- `RecordedAndSnapshot` only bites when the invocation itself carries a snapshot (`context.rs` loads one from `OCX_PATCH_SNAPSHOT` — the sole selector). A plain `ocx clean` after an `ocx patch sync` can therefore still collect a companion that only the outdated snapshot names. That gap is left open rather than closed here: reading the CWD's `patches.snapshot.json` unconditionally would make machine-global GC roots depend on the working directory, and the consistent alternative — every ledger-registered project's snapshot, the way lock roots already work — is a design decision, not a patch.
...
| `compose_roots(requests, platform, materialization, concurrency)` | **Depends** | `ComposeRoots` | The one entry point for both halves of the lazy split, used by every env-composing command. Per request, in **request order**: `LazyMode::Never` → materialized under the caller's own `Materialization` policy, unchanged; `LazyMode::Always` → `prepare_lazy` generates (or converges on) the shim tree and the root carries a `DeferredComposition`. Returns `{ roots, advisories, omitted }`. **The result does not depend on content-cache state** — a deferred request composes the same root warm or cold, which is what makes `ocx env` byte-stable. Under `Materialization::LocalOnly` (`--no-pull`) a request whose content (eager) or metadata (deferred) is not local lands in `omitted` and the caller warns; under every other policy it is an error. Warn-and-omit is the CALLER's decision, not `prepare_lazy`'s: only the caller knows whether this invocation promised to reach the network |
| `prepare_lazy(package, platform)` | No | `PreparedLazy` | Generates one deferred tool's shim tree; see the module map row above |
| `clean(dry_run, force)` | N/A | `CleanResult` | Removes unreferenced objects + stale temps; `force=true` bypasses project registry. Walks `shims/` too — a shim is retained by a lock pin and collected when `--force` drops the project roots |
| `check_update(identifier, throttle, probe)` | N/A | `UpdateCheckResult` | Generic update-check for any identifier. Throttle: `None` = 24h, `Some(ZERO)` = bypass, `Some(d)` = custom. `probe`: `TagProbe::Index` lists tags through `self.index()` (ChainMode-aware — honours `--offline`/`--frozen`/`--remote` + `OCX_INDEX`); `TagProbe::Remote` lists live from the sources **through that same configured chain** via `self.index().remote_view()` — a `ChainMode::Remote` + `LocalWritePolicy::ReadOnly` view, so it ignores the ambient ChainMode and can never move a pin (offline → `Skipped(Offline)`, gated before the view is built). Routing through the chain is load-bearing: `ocx.sh/ocx/cli` is a **logical** name the published index routes to a physical repository, so a bare registry tags-API probe answers for the literal name and caps the release list (the v0.5.0 always-up-to-date bug). Returns `Skipped` on throttle short-circuit (no state touch). |
...
| `self_update()` | N/A | `SelfUpdateResult` | Checks via `self_check_update(ZERO, TagProbe::Remote)` (lists live through the configured index chain for the newest release, matching the `self setup` bootstrap and the auto-check; `--offline` → `Skipped(Offline)`) then installs via `install_all(candidate=false, select=true)` if newer. Bootstrap mode no longer short-circuits — install always proceeds. Returns `Installed { from: Option<String>, to }` on success (`from` is `None` when subprocess version query fails), `AlreadyUpToDate` if current, `Skipped(SkippedReason)` on soft failure. Wraps check errors in `Error::SelfCheckFailed`. |
