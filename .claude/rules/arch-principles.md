...
| `adr_self_setup.md` | `ocx self setup` — bare-binary install complement to the install script: bootstrap + env shim write + managed RC-block (`# >>> ocx v1 <hash> >>>`) in user shell profiles; `ExitCode::DirtyRcBlock` (82) for user-edited blocks; `ocx self update` refreshes shims post-swap (4C). **Amended 2026-09-10 by `adr_self_update_handoff.md`**: 4C's post-swap refresh is deleted — `self update` pulls without selecting and spawns the new binary's own `ocx self setup --handoff`, which performs the select. |
| `adr_self_update_handoff.md` | **(Accepted 2026-09-10)** `ocx self update` stops enumerating setup phases: it pulls with `select=false`, then spawns the newly pulled binary as `ocx self setup <tag>@sha256:<hex> --handoff`, whose phase 1 performs the select — so an update applies version N's setup contract, not N−1's. Select-last is the commit point (a failed setup leaves the machine byte-identical). The verdict is the **canonicalized `current` symlink**, never the child's exit code, since phase 1 selects before phases 2–5 can fail; new `Pulled { handoff: Option<HandoffFailure> }` exits 75. Hidden `--handoff` threads to `apply_target`'s existing `heal_only` and is passed unconditionally (heal, never introduce). `--no-modify-path` and `--profile`/`--no-profile` now persist as `[shell] modify_path` / `[shell] profiles`, written only when explicitly given; `profiles` absent = auto-detect, `[]` = no blocks, a set list replaces detection and never unions. `toolchain-dir` → `toolchain_dir`. **The hand-off takes effect only from the update *after* the one installing it** — the installing parent is still the old binary running the old flow. |
...
| `adr_project_toolchain_links.md` | **(Proposed)** Stable addressing for composed toolchains: `.ocx/toolchain/<group>/<entry>` link tree next to its `ocx.toml` (global: `$OCX_HOME/toolchain/` store, outside the GC graph), lock-entry links only targeting the package root in both materialization states (deps and shim slots stay digest-pinned), per-site resolver `install_path` override, heal-before-emit on the composing emit paths, junctions on Windows (ARCH-4c `ReferenceManager` carve-out); following/pinned as an `EnvScope::Project` field (`--pinned` on `env`+`exec`, parity-safe). Consumer matrix: dereference→link, exec-with-composed-env→launcher, ambient exec→link-tree bin dir. Independent of `adr_shell_env_overhaul.md` (joins its `owned_prefixes`). **Amended 2026-09-04 by `adr_toolchain_activation.md`**: `bin/` moves inside `toolchain/` (`bin` reserved as group + tool name); `[toolchain] dir`/`links` replaced by `config.toml` root-level `toolchain_dir` + `ocx.toml` `pinned`; `ocx pull` renders the home; consumer-matrix row 3 gains trampolines |
| `adr_toolchain_activation.md` | **(Accepted 2026-09-05, amended 2026-09-08 — closed depth-1 tree)** Toolchain activation for every context the per-prompt hook cannot reach. `<home>/toolchain/` renders `shells/default/bin/<name>` launcher **trampolines**, reached through the `active` link as `active/bin/<name>` (default group; per-package closure env; POSIX home derived via `readlink -f "$0"`, with only `links/<group>/<entry>` baked **as a single-quoted literal** — `LauncherSafeString` guards a single-quoted context only — plus a new strict charset validator on group names and `[tools]` keys; Windows `.exe` + the frozen `.shim` sidecar) beside the links ADR's tree, now rooted at `links/<group>/<entry>/`. `ToolchainStore` is the **global** `FileStructure` field; a project home is the `ToolchainHome` value from `project::resolve_toolchain_home`. `ocx.toml` keys `activate = env\|bin\|none` (default `env`) and `pinned`, resolved by a generic `Ladder<T>` (`OCX_TOOLCHAIN_ACTIVATE`/`OCX_TOOLCHAIN_PINNED` are the **weakest** tier, not overrides; keys stay bare inside `ocx.toml`, noun-qualified outside it, and `ocx self setup --toolchain-activate` persists the mode); `config.toml` root-level `toolchain_dir` (project-only, keyed `<dir>/<project-key>/`, refused unless it resolves inside `$HOME`/`%USERPROFILE%` or `$OCX_HOME`, and refused when it names a filesystem/system prefix or is not owner-owned and non-world-writable). `ocx pull` renders always (`shells/default/bin` = default group, `links/` = every group in the lock; `--dry-run` reports the delta and performs **no heal**); `add`/`remove`/`lock`/`update` re-render; consent is NOT materialization. Render writes a **stamp** under `state/` covering the launcher names, bodies **and the `links/<group>/<entry>` link targets**; the hook emits the *project's* `<home>/toolchain/active/bin` only in `bin` mode, only when that stamp matches and the lock-only link heal passes, and always behind `ocx_install_bin_path`; the two **global** session entries are desired in every mode, since `owned_prefixes` makes an omitted segment a deleted one. Name set = one shared algebra returning per-name ownership (four consumers: `ocx env`, `inspect --closure`, `prepare_lazy`, the rendered launcher directory) over the metadata-declared `binaries` ∪ `entrypoints` claims of the roots **and every interface-admitted dependency** — never a directory scan, no post-pull `bin_scan` fallback; **no denylist and no refusal of any kind**, including for `ocx` itself (the shipped `ShimNameShadowsOcx` refusal is relaxed here, since a project may pin its own ocx), every collision last-walked-wins at debug plus `ocx inspect` — no WARN tier. `ocx self setup` registers `ocx_install_bin_path` then `$OCX_HOME/toolchain/active/bin` at session level per platform with per-format encoding refusals (Windows `HKCU\Environment` as `REG_EXPAND_SZ` + idempotent merge + `SendMessageTimeoutW`, never `setx`, `%` refused; Linux `environment.d` co-primary with `~/.profile`, newline/`$` refused; macOS LaunchAgent `RunAtLoad` plist 0644 carrying a **composed** literal read from `launchctl getenv PATH`, XML-escaped) and gains `--toolchain-activate`, a new writer of the global `ocx.toml`. Amends `adr_project_toolchain_links.md`; joins `adr_shell_env_overhaul.md` via consent-scoped `owned_prefixes` additions **Addendum 2026-09-08 (C-071…C-084)**: depth 1 of the toolchain home is a **closed set of tree-own names** — `{.gitignore, active, links/, shells/}` — so every user-supplied name lives under `links/<group>/<entry>` and no group can collide with a tree-own directory by construction. `active` is a symlink to `shells/default/`; `bin()` is the PATH-facing accessor (`<root>/active/bin`) and `shell_bin()` the physical render target (`<root>/shells/default/bin`). The `bin` **name reservation is deleted** — a group or tool may be called `bin` again. Consumers read `toolchain_bin` from `ocx shell state` rather than joining `/bin` onto `toolchain_home` |
| `system_design_toolchain_activation.md` | **(Draft)** Companion design record to `adr_toolchain_activation.md` — C4 context/container/component levels, the four flows (render at `pull`; trampoline exec incl. deferred materialization; hook import per `activate` × `pinned`; session-PATH registration per platform), sidecar + launcher-body data contracts, Rust type signatures, failure-mode/self-heal table, NFR detail. No trade-off matrix (that lives in the ADR) |
...
| `adr_announce_diverged_branch_rebuild.md` | A `Diverged` announce branch under an open pull request is `BranchState::Stale`, not `Live` — the root's shape is rebuilt on the index base with the branch's tag delta carried forward (base entry wins on a shared key, for yank governance) and the ref repointed with `RefUpdate::Reset`; `Forge::pull_request_mergeability` is a tripwire on the unchanged path only, `Conflicting` → `PullRequestUnmergeable` (exit 65). Restates the #228 invariant as "no tag announced into an open pull request is ever lost", not "the branch's commit chain is never rewritten" |
| `adr_exec_resolution_record.md` | Exec-time resolution record (#214) — a pre-exec JSON record per launch frame (`ocx exec` / `package exec` / `launcher exec` / `launcher shim`), naming the resolved package closure plus the resolved leaf executable, written to an operator-designated `[records]` sink; `pkg:oci` purl + in-toto `ResourceDescriptor` identity, fail-closed under a SYSTEM-scope lock, two records per entrypoint or deferred-tool invocation joined by content digest. |
...
| Same, but for an append-only destination where a chosen target name must never be replaced (`PersistOutcome::Occupied` hands the tempfile back so the caller retries under a fresh name; a device+inode identity check against the occupant — not link count — resolves the NFS lost-reply `EEXIST`) | `utility::fs::persist_temp_file_noclobber` | `utility/fs.rs` (used by the execution-record sink) |
