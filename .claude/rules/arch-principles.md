...
| `adr_exec_resolution_record.md` | Exec-time resolution record (#214) — a pre-exec JSON record per launch frame (`ocx run` / `package exec` / `launcher exec`), naming the resolved package closure plus the resolved leaf executable, written to an operator-designated `[records]` sink; `pkg:oci` purl + in-toto `ResourceDescriptor` identity, fail-closed under a SYSTEM-scope lock, two records per entrypoint invocation joined by content digest. |
