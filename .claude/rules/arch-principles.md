...
| `adr_toolchain_activation.md` | **(Accepted 2026-09-05)** Toolchain activation for every context the per-prompt hook cannot reach. `<home>/toolchain/` renders `bin/<name>` launcher **trampolines** (default group; per-package closure env; POSIX home derived via `readlink -f "$0"`, with only `<group>/<entry>` baked **as a single-quoted literal** — `LauncherSafeString` guards a single-quoted context only — plus a new strict charset validator on group names and `[tools]` keys; Windows `.exe` + the frozen `.shim` sidecar) beside the links ADR's `<group>/<entry>/` tree. `ToolchainStore` is the **global** `FileStructure` field; a project home is the `ToolchainHome` value from `project::resolve_toolchain_home`. `ocx.toml` keys `activate = env\|bin\|none` (default `env`) and `pinned`, resolved by a generic `Ladder<T>` (`OCX_TOOLCHAIN_ACTIVATE`/`OCX_TOOLCHAIN_PINNED` are the **weakest** tier, not overrides; keys stay bare inside `ocx.toml`, noun-qualified outside it, and `ocx self setup --toolchain-activate` persists the mode); `config.toml` root-level `toolchain-dir` (project-only, keyed `<dir>/<project-key>/`, refused unless it resolves inside `$HOME`/`%USERPROFILE%` or `$OCX_HOME`, and refused when it names a filesystem/system prefix or is not owner-owned and non-world-writable). `ocx pull` renders always (`bin/` = default group, links = every group in the lock; `--dry-run` reports the delta and performs **no heal**); `add`/`remove`/`lock`/`update` re-render; consent is NOT materialization. Render writes a **stamp** under `state/` covering `bin/` names, bodies **and the `<group>/<entry>` link targets**; the hook emits the *project's* `<home>/toolchain/bin` only in `bin` mode, only when that stamp matches and the lock-only link heal passes, and always behind `ocx_install_bin_path`; the two **global** session entries are desired in every mode, since `owned_prefixes` makes an omitted segment a deleted one. Name set = one shared algebra returning per-name ownership (four consumers: `ocx env`, `inspect --closure`, `prepare_lazy`, rendered `bin/`) over the metadata-declared `binaries` ∪ `entrypoints` claims of the roots **and every interface-admitted dependency** — never a directory scan, no post-pull `bin_scan` fallback; **no denylist and no refusal of any kind**, including for `ocx` itself (the shipped `ShimNameShadowsOcx` refusal is relaxed here, since a project may pin its own ocx), every collision last-walked-wins at debug plus `ocx inspect` — no WARN tier. `ocx self setup` registers `ocx_install_bin_path` then `$OCX_HOME/toolchain/bin` at session level per platform with per-format encoding refusals (Windows `HKCU\Environment` as `REG_EXPAND_SZ` + idempotent merge + `SendMessageTimeoutW`, never `setx`, `%` refused; Linux `environment.d` co-primary with `~/.profile`, newline/`$` refused; macOS LaunchAgent `RunAtLoad` plist 0644 carrying a **composed** literal read from `launchctl getenv PATH`, XML-escaped) and gains `--toolchain-activate`, a new writer of the global `ocx.toml`. Amends `adr_project_toolchain_links.md`; joins `adr_shell_env_overhaul.md` via consent-scoped `owned_prefixes` additions |
