...
| `adr_live_env_reload.md` | **(Proposed)** Typed three-way shell-env reconciler (desired/current/`__OCX_ENV_STATE` ledger), per-prompt hook + `ocx` wrapper via `self activate`, `[shell]` config section, consent stamps for untouched projects; replaces the direnv dependency; amends `handshake_toolchain_cli.md` §2 `shell hook` deletion |
