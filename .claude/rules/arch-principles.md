...
| Per-prompt shell-env reconciliation | `crates/ocx_lib/src/shell/reconcile.rs` | Typed, provenance-tagged three-way planner (desired D / current C / private `__OCX_ENV_STATE` ledger L); driven by `ocx self activate --reconcile`, read by `ocx shell state` (`adr_shell_env_overhaul.md`) |
...
| `shell/` (`shell.rs`, `shell/reconcile.rs`, `shell/hook.rs`, `shell/coexistence.rs`) | `Shell` export helpers (`export_path`/`export_constant`/`export_list`/`remove_list_element`/`unset`/`emit_message`) + `conventions::emit_lines` — shell-specific export gen; env-var key validation delegated to the shared `env::is_valid_env_key` (also used by `ci/`); `reconcile.rs` is the typed three-way per-prompt planner (`adr_shell_env_overhaul.md`); `hook.rs` emits the per-shell prompt-hook registration + `ocx` wrapper; `coexistence.rs` detects a live direnv/mise session to yield to. The root `ocx shell hook`/`init`/`env` **commands** are removed — the per-prompt hook itself is not; it rides `ocx self activate`'s hidden `--reconcile` arm | `ocx env`, `ocx package env`, `ocx direnv export`; consumed by `ocx self activate --reconcile` (applies) and `ocx shell state` (reports) |
