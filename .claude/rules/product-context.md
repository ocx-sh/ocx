...
| 8 | Declarative env + tool-config metadata | Multi-package env composition without shell scripts; a `integrations` block on any dependency also composes vendor-namespaced, non-env configuration (editor extensions, devcontainer blocks, language-server settings) across the whole dependency closure, each row attributed to its declaring package, concatenated never merged — the deliberate point of divergence from devcontainer's `customizations` key, which merges within one file and has no closure to walk |
...
| 12 | Keyless Sigstore signing (v1 scope) | Sign + verify via OCI Referrers, identity-pinned `[[trust.policy]]`, offline/auto-verify. Real Sigstore throughout: public-good trust root over TUF, standard Rekor SET + Merkle proof (verification delegated to `sigstore-rs`), cosign 3.x bundle interop, and a self-hostable stack (Fulcio/Rekor/TesseraCT/dex) the acceptance suite runs against. Rekor v2 and DSSE attestations deferred — see signing.md § Deferred to Future Work |
...
ocx package verify -p linux/amd64 my/cmake:3.28 \        # Keyless verify (self-hosted stack: --sigstore-trusted-root or [trust.sigstore])
...
- **Workspace**: 18 `ocx_*` tier crates + `crates/ocx_cli` (CLI); the mirror tool lives in its own repo ([ocx-sh/ocx-mirror](https://github.com/ocx-sh/ocx-mirror))
- **Default registry**: `ocx.sh` (configurable via `OCX_DEFAULT_REGISTRY`)
- **Platform support**: linux/amd64, linux/arm64, darwin/amd64, darwin/arm64, windows/amd64, windows/arm64 (native); wasip1/wasm, wasip2/wasm (WebAssembly, explicit `--platform` only). `SUPPORTED_PAIRS` in `crates/ocx_oci/src/platform.rs` is the source of truth — a pairing outside it is refused at parse
