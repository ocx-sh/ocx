...
- `ocx shell state` — **new** (ADR Decision 10, C-050) — read-only per-prompt-reconciliation diagnostics: decoded ledger, per-scope applied state, fingerprint status, and the enumerated reason a shell is inert. Root `--format json` for machine-readable output. **Never eval-able** — no line is valid `export`/`set`/`$env.` syntax in any arm. Exit 0 in every reportable state (including inert/corrupt-ledger/over-cap/yielded); exit 74 only if `$OCX_HOME` is unreadable. Not a stamp writer — a named non-member of the six-writer consent-stamp allowlist
