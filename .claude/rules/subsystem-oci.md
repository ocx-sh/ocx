...
| `oci/endpoint.rs` | Sigstore endpoint SSRF/scheme validation — `validate_sigstore_url` (the boundary for `--rekor-url`/`--fulcio-url`), the default public Rekor endpoint constant (`DEFAULT_REKOR_URL`), and the shared timeouted `sigstore_http_client()` reused by all five Sigstore call sites (Fulcio CSR, Rekor upload, ambient OIDC token exchange, verify pipeline, the public-good TUF trust-root fetch), whose DNS resolver replays the guard's pinned addresses so no dial re-resolves a name the guard already judged. Seeds its extra CA roots from `tls::sigstore_roots()` at first build — the process-wide set `tls::install_sigstore_roots` installs from `Context::try_init` — so every one of the five call sites, TUF fetch included, trusts an operator's `extra_ca_certs`. A peer of `oci/sign` and `oci/verify`, not owned by either |
...
| `oci/verify/trust_root.rs` | Trust-root loading: Sigstore `TrustedRoot` JSON (`--sigstore-trusted-root`, pinned Rekor key), cache rebuild; public-good root over TUF (`SigstoreTrustRoot::new_with_client`, dialing through `sigstore_http_client()` after `resolve_sigstore_url` pins `SIGSTORE_TUF_URL` — so extra CA roots and the proxy reach the TUF fetch, ocx#470). There is no CA-PEM loader — a bare Fulcio CA carries no CT log key, so the pipeline refuses it |
...
The published `name_segments` declaration is no longer read for jurisdiction. Its only job there was
to interpret a missing root as "fall through"; with the fall-through gone it bought a `config.json`
fetch plus a 404 per declined name and decided nothing. `IndexFormatConfig` still carries the field —
serde still parses it, and OCX's own writer omits it rather than guessing a value, so a full mirror
copy of a site's `config.json` stays byte-identical — but no client-side branch consults it to route a
resolve; the live site's `name_segments` key is now an unmodelled sibling this client tolerates and
ignores (`live_index_wire.rs` pins that forward-compat against captured production bytes).
...
and never a security control: the yank gate, the dispatch-object digest verify, and the terminal
stop above delegate nothing to it.
...
(`command/index_common.rs`, behind both `ocx index update` and `ocx index sync`) walks a package's
index sources and selects the one whose `jurisdiction` is not `Outside` before calling it — the same
verdict a resolve routes by. A name that source declines now fails the refresh instead of silently
deriving a root from the registry underneath.
...
- Supported pairs — `SUPPORTED_PAIRS` in `oci/platform.rs` is the source of truth, enforced by `validate_pair` at the only two sites that build a `Specific` from separate parts (`FromStr`, `TryFrom<native::Platform>`); the cross product of the two enums is *not* the legal set. Native: linux/amd64, linux/arm64, darwin/amd64, darwin/arm64, windows/amd64, windows/arm64. Wasm: wasip1/wasm (WASI 0.1 modules), wasip2/wasm (WASI 0.2 components) — no host reports either OS, so they are reachable only through an explicit `--platform`, and they get no automatic binary scan (`bin_scan::claim_name` returns `None`, so `binaries` stays empty).
...
- **The SSRF guard is route-aware behind an HTTP proxy (`guard_destination`, `ProxyRules`, ocx#407/ocx#323).** `proxy_rules()` (`oci/ssrf.rs`) is the **single** process-wide reader of `HTTPS_PROXY`/`HTTP_PROXY`/`ALL_PROXY`/`NO_PROXY` and the OS proxy settings, read once via `LazyLock` and shared by `ssrf` and `endpoint` — never re-derive proxy policy from env at a second call site. `guard_destination` is the route-aware replacement for `resolve_and_validate` at every guard site: on `Route::Proxied` it skips the DNS pre-flight (the proxy resolves the destination, not OCX) but still refuses a forbidden IP literal typed straight into an index root's pointer; on `Route::Direct` it is `resolve_and_validate` verbatim. **Both** resolver hooks — `GuardedResolver` (registry dial, this file) and `PinnedResolver` (Sigstore dial, `oci/endpoint.rs`) — admit the process's own configured proxy host via `ProxyRules::is_proxy_host` with a plain `lookup_host` and no range judgement, closing ocx#323 without weakening the floor on any other unpinned name. Tests inject `ProxyRules::new(Matcher::builder()...build())` for a hermetic matcher; never mutate `HTTPS_PROXY`/`NO_PROXY` env vars in a test (`set_var` is unsafe in Rust 2024 and racy under `cargo test`).
...
- **Submodule at `external/sigstore-rs/`** patched fork of `sigstore` (base tag `v0.14.0`, branch `ocx/integration`). One upstreamable commit per change, DCO-signed (`git commit -s`, subject `<area>: <summary>`), tested the crate's way (`rstest` + `#[tokio::test]`). Format with `cargo fmt --manifest-path external/sigstore-rs/Cargo.toml -- --config max_width=100 --check` — ocx's `rustfmt.toml` would otherwise leak `max_width=120` in; add no fork `rustfmt.toml`. Its cargo does not run from a nested agent worktree (the outer checkout's workspace claims the path): use a detached `git worktree` of the fork outside the ocx tree.
