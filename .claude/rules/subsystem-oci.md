...
- **`physical_reference` is source-first, local-second — deliberately NOT the local-first shape of `fetch_manifest`/`fetch_blob`.** Those read digest-verified *content*; this reads a *pointer* into remote-controlled data (a copied index tree is a supported distribution mechanism, A2), and the layer pull that consumes it runs on the shared, unguarded `PackageManager` client. `ChainedIndex::physical_reference` walks the sources, holds a **transport outage** (`is_source_outage` — `IndexHttpFailed` only, a closed set so a new error class fails closed), then falls back to `LocalIndex::physical_reference` (the committed root's `repository`), re-raising the held error when no local root exists. Any other source error is a **refusal**, not an outage, and propagates immediately — an `SsrfError` answered around by the local root would be a guard that fires and is then discarded. That fallback is what makes `--offline` (no sources at all) and a no-network Default resolve answer the PHYSICAL address instead of `Ok(None)` — which `resolve_transport_pinned` reads as "no rewrite" and turns into the logical identifier. **There is no `ChainMode::Offline` early return** in that method: ahead of the local read it would reproduce that exact silent fallback; after it, it is unreachable.
- **The local answer is NOT a rare fallback — it is the ordinary answer for a flat `ocx.sh/<tool>` name.** `candidate_sources` skips a source declaring `Jurisdiction::Outside`, and the live `index.ocx.sh` declares `Outside` for every flat name (41 of 44 fleet repositories), so on a plain online run with no flags the source loop never executes and the local root is the only answer. `ChainedIndex::guard_local_physical` therefore applies the SSRF floor to that answer with the **same** `trusted_hosts` set `OcxIndex` used (`IndexImpl::trusted_hosts`, keyed on registry ownership — one config value, never a second notion). Two carve-outs: a root naming the identifier's own registry is not a *rewrite* (every derived root is that shape; guarding it would refuse private registries that predate indices and whose namespace carries no source to hold an exemption), and `ChainMode::Offline` skips it (offline builds **no** OCI client, so nothing can follow the answer, and no sources, so `trusted_hosts` is unreachable). A DNS **lookup failure** on a genuine DNS name is tolerated — that is the steady state of the warm-store-no-network case, and refusing there re-raises the exit-69 bug — but an address-shaped host that fails to resolve (a bracketed IPv6 authority) stays fail-closed. Residual: the pull client itself still has no `GuardedResolver`, so the resolve→connect rebinding window is open for any target the pre-flight admitted.
