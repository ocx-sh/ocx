...
| `oci/verify/trust_root.rs` | Trust-root loading: Sigstore `TrustedRoot` JSON (`--trusted-root`, pinned Rekor key), cache rebuild; public-good root over TUF (`SigstoreTrustRoot`). There is no CA-PEM loader — a bare Fulcio CA carries no CT log key, so the pipeline refuses it |
