...
- **`physical_reference` is source-first, local-second — deliberately NOT the local-first shape of `fetch_manifest`/`fetch_blob`.** Those read digest-verified *content*; this reads a *pointer* into remote-controlled data, and only the `OcxIndex` answer has passed the SSRF pre-flight (the layer pull that consumes it runs on the shared, unguarded `PackageManager` client). `ChainedIndex::physical_reference` walks the sources, holds any error, then falls back to `LocalIndex::physical_reference` (the committed root's `repository`), re-raising the held error when no local root exists. That fallback is what makes `--offline` (no sources at all) and a no-network Default resolve answer the PHYSICAL address instead of `Ok(None)` — which `resolve_transport_pinned` reads as "no rewrite" and turns into the logical identifier. **There is no `ChainMode::Offline` early return** in that method: ahead of the local read it would reproduce that exact silent fallback; after it, it is unreachable. Known gap: the local fallback answer is not SSRF-pre-flighted (`resolve_and_validate` needs DNS, unavailable in the offline path it exists to serve, and `trusted_hosts` is per-`OcxIndex`) — reachable only when no configured source can answer.
