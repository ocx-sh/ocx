...
| `rust:build` / `test/taskfile.yml`'s `.build-binaries` | Both build `ocx` (and, for `.build-binaries`, `ocx_shim`) under root `Cargo.toml`'s `[profile.test-bin]` (`inherits = "release"`, `incremental = true`) with `--features ocx/__testing`, into `target/test-bin/` — the one profile they share, so `task verify` (which runs both) compiles it once rather than twice. `[profile.test-bin]` carries the whole Stage-0 latency win: `.build-binaries` 53.9 s → 30.1 s after a one-crate edit, T1 114.2 s → 88.5 s. Not combined with sccache (0 hits after an edit against an incremental profile); CI's `Swatinem/rust-cache` sets `CARGO_INCREMENTAL=0`, so the lever is local-only. |
