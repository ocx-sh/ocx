...
5. **Subsystem env overrides go on task, not include.** Most project toolchain env (OCX_HOME, etc.) comes from direnv (`.envrc` → `ocx direnv export`) locally and from the `setup-ocx` action in CI -- taskfiles do not set these. `OCX_INDEX` is the one exception: the root taskfile's `env:` block pins it to the committed `.ocx/index/` index copy for every task in this repository, so CI and non-interactive `task` invocations resolve against the committed index collection without depending on direnv. direnv itself never exports `OCX_INDEX` ambiently -- a whole-collection pin at the shell level would redirect every index operation in that shell, including host-scope ones, which is deliberately out of scope; an interactive shell outside `task` falls back to the machine-local `$OCX_HOME/index/` unless `--index`/`OCX_INDEX` is set explicitly. When a task genuinely needs a different value, override with per-task `env:`.
