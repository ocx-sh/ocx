...
| `rust:test:loop-guard` | The `inert` guards of `crates/ocx_test_support/tests/workspace_structure.rs`: no loop inside test code has a body that can neither fail nor be observed — no call, macro, assignment, `.await`, `?` or wildcard-free `match`. Reds naming file, line and keyword. Its witness fixture must still yield `for`, `while` and `loop`, so a blind scanner reds instead of reporting a clean workspace. Gates nothing on its own (`rust:test:workspace-structure` and the `verify:scoped` manifest route already reach it); it is the per-batch entry point. |
| `rust:test:workspace-structure` | Every case of `crates/ocx_test_support/tests/workspace_structure.rs` under nextest, in `task verify` phase 2 after `bazel:test:unit`. The Bazel target `--skip`s 23 of them (they walk `crates/*/src` of crates it does not depend on), so without this step they ran on `verify-deep.yml`'s macOS and Windows legs alone. |
