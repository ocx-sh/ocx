...
| LINT-16 | Commit a per-file, per-lint-code baseline (`clippy-warn-baseline.json`, one integer per `<file>::<code>`). CI fails if any single key's live count exceeds its entry; decreases are committed; a code reaching 0 everywhere moves into `[workspace.lints.clippy]` and leaves the baseline in the same commit. The same argument twice: per-code rather than aggregate, because one lint's improvement would mask another's regression; per-file rather than per-code, because one file's improvement masks another's the same way, and a refactor that moves code across file or crate boundaries produces exactly that shape. Per-file keys also make the baseline the record of *which* files carry a backlog, which is the only evidence that distinguishes a file that was fixed from a file that was never linted. | Counts from `cargo clippy --workspace --all-targets --message-format=json`, grouped by the primary span's file and `.message.code.code`, diffed against the committed file; no key appears in both it and `[workspace.lints.clippy]` | MUST |
