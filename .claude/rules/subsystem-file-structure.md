...
| `state/projects/<key>/consent.json` | **Per-project shell-activation consent stamp** (`adr_shell_env_overhaul.md` Decision 2). `<key>` = the same `name_for_path(dunce::canonicalize(canonicalize(<project file>).parent()))` derivation the `projects/` ledger uses. `StateStore::project_state_dir(key)` / `consent_stamp_file(key)` / `project_state_root()`. Written by the consent-stamp allowlist — the toolchain-tier mutation commands (`add`/`remove`/`lock`/`update`/`run`/`pull`), plus `ocx init` for the project it creates and the explicit `ocx shell allow` — never by `ocx shell state` or `ocx self activate`, which are read-only. Membership is **suppressible, not conditional**: `OCX_NO_CONSENT` (and `--no-consent` on `init`/`pull`/`exec`, which outranks it) makes an allowlist member write nothing on a given invocation, resolved once in `app::project_context::record_activation_consent_over` and nowhere else — the set of commands that *may* write is unchanged, which is what A-29 constrains. `ocx shell allow` is outside that resolution by construction: it calls `project::consent::record` directly, so the explicit human gesture is never suppressible. Replaced atomically via `write_bytes_atomic`, never edited in place. **GC is the one exception to "`state/` is not walked by `ocx clean`" below**: swept iff the stamp deserializes at an understood `v` AND a pre-removal re-probe proves its own recorded `project_dir` is definitively absent — never derived from the `projects/` ledger, which the stamp does not consult. See `arch-principles.md`'s **State** glossary entry for the one-exception framing. |
...
`adr_oci_referrers_signing_v1.md` Amendment 3):
