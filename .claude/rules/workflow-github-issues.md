...
Rules for creating and managing GitHub issues. **Never create issues without user approval.**

**Tool preference**: Prefer MCP tools `mcp__github__*` (e.g., `mcp__github__create_issue`, `mcp__github__list_issues`, `mcp__github__get_issue`, `mcp__github__update_issue`) for structured, typed access. Fallback: `gh issue <subcommand>` — still valid when MCP lacks parity (custom JSON projections, label management edge cases) or when a parallel session already uses `gh`.
