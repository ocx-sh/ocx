...
| Entry-point visibility levels (sealed/public/interface) | Not in issue scope; env-var visibility already covers this axis. **Partially addressed** by [`adr_visibility_two_axis_and_exec_modes.md`](./adr_visibility_two_axis_and_exec_modes.md): `Var.visibility` (entry-axis: private/public/interface) shipped in v2 schema; exec-mode dispatch (`--mode=consumer/self/full`) controls which entries each exec context sees. Full entry-point-level visibility (hiding specific launchers from consumers) remains deferred. |
