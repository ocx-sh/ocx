...
| **A-10** | Nothing. The record does not state which gate compiles the Windows arms | **Local and CI coverage differ, and the difference is worth writing down.** `task check:windows-cfg` is `cargo check -p ocx_shim` for the two MSVC targets and nothing else — deliberately — so **no local gate type-checks `ocx_lib`'s `cfg(windows)` arms**: the renderer's Windows publish path, the session-PATH writers, `Env::pathext`. On CI they are covered, by the **deep** tier only: `verify-deep.yml`'s `build` matrix runs `cargo nextest run --workspace --target=x86_64-pc-windows-msvc --profile ci --locked` natively on `windows-latest`, and the same command against `aarch64-apple-darwin` on `macos-latest`, so a `#[cfg(windows)]` unit test there is **compiled and executed** and is not an unchecked green. **Corrected frequency.** This row first pointed at a Windows job of `verify-basic.yml`, running *"for every pull request to `main`"*; ruling D5 deleted that job, and basic now has no Windows or macOS leg at all. Deep's matrix carries the draft guard and `on.pull_request.branches` is `[main]`, so the arms are compiled on **non-draft** pull requests into `main`, the merge queue, and pushes to `main` — never a draft, never a pull request into `announce/**`. The widening spike **stays dropped**: what falsified it was that *some* CI gate compiles and runs these arms, which still holds, and the two residual gaps are accepted rulings D5 and D10 recorded in `subsystem-ci.md`, not an open hole | R-W45, D-V34; `taskfiles/rust.taskfile.yml`, `.github/workflows/verify-deep.yml`, `.claude/rules/subsystem-ci.md` |
...
| **A-16** | **A-2** re-justifies putting `ocx_install_bin_path` **first** on `PATH`: *"a user typing `ocx` at a prompt must reach the installed binary and not a repository's"*. **D-5**, **D-6**, **C-060** and **RUL-87** all state that order | **The order is reversed, by owner decision (2026-09-07).** Front to back the three session directories are now the project's `<home>/toolchain/bin`, then `$OCX_HOME/toolchain/bin`, then `ocx_install_bin_path` — *project pin ▸ global pin ▸ the installed binary*. The three ordering rules in force were **cyclic**: install-ahead-of-project (A-2), project-ahead-of-global (RUL-87) and the owner's requirement that a global toolchain pinning `ocx` be reachable at all. Any two hold; all three cannot. A-2's ground was dropped because it contradicts **D-4**, which removed the `ShimNameShadowsOcx` refusal precisely so a toolchain may pin its own `ocx` — an ordering that keeps the installed binary in front leaves such a pin rendered and permanently unreachable, so D-4 granted a capability the ordering then denied. A-2's *second* ground survives untouched: the bare-word `ocx` a trampoline bakes when `trampoline_ocx_binary`'s ladder declines is now defended where the resolution happens rather than by ordering — `resolve_command_excluding` drops every trampoline directory from its **lookup copy** of `PATH`, and `is_ocx_trampoline` re-checks the resolved answer independently. **All three emitters moved together** — the per-prompt reconciler (`SessionPath::entries`), the login stream (`activation_lines`) and the OS session-level registration (`setup::session_path_directories`, folded verbatim by the Windows registry, `environment.d` and LaunchAgent writers) — because a partial change leaves the three PATH surfaces disagreeing about where the installed binary sits. **One thing beyond the ordering**: the emitted `ocx` **wrapper function** now names `$OCX_HOME/toolchain/bin/ocx` when that file exists, falling back to the install path. The wrapper is a shell function of the same name, so it shadows `PATH` outright; left on the install path it would have re-introduced the old precedence at exactly the interactive prompt a pin is typed at, and the reorder would have been observable only in scripts, `sh`, `ssh host cmd`, git hooks and CI. Resolved once at login rather than per call, so C-045's absolute-binary rule is untouched and no shell dialect grows a `command ocx`. **The hook body deliberately does not follow**: `self activate --reconcile` runs every prompt speaking the private `__OCX_ENV_STATE` ledger protocol, and a version-skewed or broken pin there would break the prompt of every shell rather than one command | `SessionPath`, `activation::desired_entries`, `ocx_cli::command::self_group::activate::{activation_lines, wrapper_binary_path}`, `setup::session_path_directories`; tests `c060_rul62_the_resulting_path_leads_with_project_bin_then_global_bin_then_install_bin`, `c060_the_login_stream_puts_the_toolchain_bin_in_front_of_the_install_bin`, `c060_the_wrapper_follows_a_global_ocx_pin_and_falls_back_to_the_install`, `the_session_path_directories_put_the_toolchain_bin_dir_ahead_of_the_install_one`, `test/tests/test_toolchain_activate.py::{test_bin_mode_path_leads_project_then_global_then_install,test_a_bare_ocx_lookup_resolves_the_projects_ocx_pin,test_a_bare_ocx_lookup_resolves_the_global_ocx_pin}` |
...
exercised only by `verify-deep.yml`'s `build` matrix Windows leg — per non-draft pull request into
`main`, not per pull request (A-10). Rejected, but kept named: it is the
