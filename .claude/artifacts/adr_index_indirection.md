...
| 2026-07-26 | builder (opus) | **F1/H amended — jurisdiction is index-DECLARED, not client-guessed.** F1's `config.json` gains an optional `name_segments` (`NonZeroU32`): the slash-separated segment count a package name must have within the namespace, counted on `Identifier::repository()`. `index.ocx.sh` publishes `2`, restating its root schema's `^ocx\.sh/<ns>/<pkg>$`. A name the index declares it cannot express is outside that source's jurisdiction — no request is issued for it and its silence decides nothing, so the chain falls through to the registry. **Absence = serves every name = the prior behaviour verbatim**, so a private index that declares nothing is never narrowed by the client. No `format_version` bump: the field is purely additive and ignorable (F1 already promises unknown sibling fields are ignored), while bumping to `2` would make every deployed ocx raise `UnsupportedIndexFormat` against the index and convert a resolution bug into a total namespace outage. `name_segments` is explicitly **not** a security control — an older client ignores it, and the yank gate, dispatch-object verify and terminal stop delegate nothing to it. **Fail-closed:** only a successfully-read config that positively declares a name inexpressible moves it out of jurisdiction; a 404, malformed, unsupported-version or unreachable `config.json` keeps the source authoritative (an index outage must never silently downgrade a namespace to plain OCI). H's component model gains the outcome type: `Jurisdiction::{Authoritative,FallThrough,Outside}` on a distinct async trait method consulted **before** a source is asked, so "outside jurisdiction" is structurally unobservable as the `Ok(None)` of a fetch and `LocalIndex::refresh_tags`'s derived-source switch cannot misread it. `IndexImpl::is_authoritative_for` is deleted. Provenance is split off as the synchronous, no-I/O `serves_registry(&str)`: local subtree layout (catalog vs `p/` enumeration) is per-source and never per-name, so `ChainedIndex::kind_for_registry` is now the primitive, `kind_for` delegates with `id.registry()`, and the `Identifier::new_registry("_", registry)` placeholder is gone. `ChainedIndex::candidate_sources` is the single consult point (six source loops); `LocalIndex::sync_catalog` step 3 skips a self-contradicting catalog row with a warn rather than aborting before the catalog + ETag commit; `ocx index update`'s source routing replaces its independent namespace-equality guess with the same question. Motivation: index.ocx.sh's two-segment root schema made every flat name (41 of 44 repositories on the ocx.sh registry, including ocx's own toolchain) unresolvable once the index was configured, because a 404 under an authoritative source is a terminal stop. |
| 2026-07-26 | builder (opus) | **F1/H corrected — the declaration interprets a MISS, it does not gate the request.** The amendment above let `name_segments` decide *whether* to ask, which re-opened the yank bypass from the other side: the count rides an unsigned, CDN-cacheable file on the same channel as the roots, so a template bug, a stale edge, or a compromise scoped to that one file could narrow a namespace and make the client stop asking about a name the index does hold a yanking root for — the yanked build then installs through the plain-OCI catch-all, exit 0. `OcxIndex::jurisdiction` now fetches the root for a name the declaration rejects: root found ⇒ `Authoritative` (the evidence overrules the declaration), root 404 ⇒ `Outside` (the flat-name fall-through, the only thing the declaration decides), root fetch error ⇒ `Authoritative` (fail-closed). A wrong `name_segments` can therefore bypass nothing. Cost: one root `GET` that 404s per declined name, memoized with the hits (`SourceCacheInner.roots: Option<Arc<IndexRoot>>`) — one request per name per process. The `Outside` verdict logs at `warn!`, not `debug!`: leaving the verified two-hop path is rare and deliberate. Also: `LocalIndex::sync_catalog` step 3's `refresh_tags` is now non-fatal (warn + `continue`) — ordinary publish skew (a catalog regenerated ahead of its roots) made its `?` abort before the step-4 catalog + ETag commit, pinning the ETag forever and silently stranding every other moved package on every later `ocx index update`. |
| 2026-07-26 | builder (opus) | **Non-fatal is not "adopt anyway" — a skipped row keeps its entry and holds the ETag back.** The amendment above made `sync_catalog` step 3 warn-and-continue, but step 4 still merged the failed row's FETCHED catalog value and still committed the new ETag. On disk that left `p/<key>.json` at the OLD bytes with `c/index.json[key]` at the NEW digest — the root/catalog straddle A2 forbids — and both continuations dead-ended: `IndexStore::read_root` self-heals the catalog back and serves the STALE root, so a tag yanked in the new one resolves and installs; and with the ETag committed the next sync answers `304` (or diffs NEW against NEW), so the row is never retried. Both skips (jurisdiction-declined and `refresh_tags` failure) now collect into a `stale` set that step 4 reads twice: the fetched entry is not merged, and the ETag is passed as `None` so the previous validator survives and the next `ocx index update` re-diffs and retries. Healthy rows still land — the failure vetoes nothing. Also: `resolve_root` raises `IndexHttpFailed` on a `304` answering its unconditional `GET` instead of folding it into the 404 miss (only a confirmed 404 may settle a name as `Outside`, and the miss is memoized), and the `Outside` verdict logs at `debug!` again — it is the steady state for 41 of the 44 `ocx.sh` repositories and fires per chain consult, not per name. |
| 2026-07-29 | builder (opus) | **H amended, and the two amendments above superseded — a configured index is AUTHORITATIVE FOR ITS WHOLE REGISTRY (owner decision 2026-07-28, [ocx-sh/ocx#251](https://github.com/ocx-sh/ocx/issues/251): *"if an index is set, only these packages in the index should be discoverable"*).** The `name_segments` declaration and the `Ok(None) => Jurisdiction::Outside` arm it fed are **deleted**. `OcxIndex::jurisdiction` collapses to `serves_registry(id.registry())` — `Authoritative` for its own registry, `Outside` for another, decided with **no I/O**; `Outside` now means only "foreign registry". `IndexFormatConfig` carries `format_version` alone (the live site's surviving `name_segments` key is an unknown sibling the client ignores — `live_index_wire.rs` pins that forward-compat, and `FormatVersionState::Confirmed` drops its payload). Rationale: once a miss under an authoritative source is terminal, asking the declaration first buys a `config.json` fetch plus one 404 per declined name and decides nothing — the whole mechanism existed to turn that 404 into a fall-through. Consequence, accepted and deliberate: a package absent from the index is unresolvable through it. This closes the yank/deprecation bypass the fall-through opened (a flat `ocx.sh/<tool>` resolved straight off the registry, past the index, past the gate) and converts a *availability* dependency the index already had into a **completeness** dependency it did not. **Unconditional — no config gate**: the escape hatch is the documented `[registries."<ns>"] index = ""` kill switch (an empty base URL is not a kind marker), plus the `[mirrors."<ns>"]` registry pin that suppresses a compiled-in default; a second opt-out would be a parallel mechanism for the same job. **The error is the deliverable**: `oci::index::error::Error::NotInIndex { identifier, namespace, base_url }`, raised from `ChainedIndex::fetch_and_persist_chain`'s authoritative clean-miss arm when the source can name itself (`IndexImpl::index_base_url`, new; `None` for a plain registry keeps the plain terminal `Ok(None)`), classified `ExitCode::NotFound` (79) so `case $?` contracts are untouched. It names the base URL rather than only the namespace because the effective base is merged across the compiled-in default, the managed tier, `[registries."<ns>"] index` and the `[mirrors."<host>"] index` role. **The clean-miss and error arms stay separate** — `Ok(None)` = "the index answered no" → `NotInIndex`; `Err` = "the index could not be read" → the transport error verbatim; merging them would turn an index outage into a confident wrong answer, guarded by `an_index_outage_is_never_reported_as_a_missing_package`. Fail-closed is unchanged and now structural: no config state (absent, malformed, unsupported, unreachable) can move the verdict because the verdict no longer reads the config, and the following `resolve_root` raises the real error. Also dropped: `sync_catalog` step 3's jurisdiction skip (unreachable — a catalog row is always in its own source's registry; the `refresh_tags` failure arm covers the same publish-skew outcome), and `ocx index update`'s reroute-to-registry for a declined name (it now asks the owning index and fails loudly rather than deriving a root the index never gave). **Sequencing is not optional**: the branch is held behind (1) porting the ~41 flat `ocx-contrib/mirror-*` specs to two-segment names, (2) announcing ocx's own eight-package toolchain, (3) announcing the fleet. The bootstrap path (`setup-ocx`, `ocx self setup`) stays index-free. |
| 2026-09-06 | builder (opus) | **C2 gains a third grow shape — a TAGLESS routing root on a digest-addressed resolve ([#424](https://github.com/ocx-sh/ocx/issues/424)).** Grow-on-resolve was tag-keyed by construction: `fetch_and_persist_chain`'s branch requires `identifier.tag().is_some() && identifier.digest().is_none()`, so a lock-driven (digest-addressed) resolve grew nothing and the local copy never learned the `repository` pointer. Every later invocation re-asked the source for it — one `GET /p/<ns>/<pkg>.json` per package, forever, on any machine that only ever runs `pull` and `exec` against a committed lock (a fresh clone, a CI checkout). **What is written, field by field:** the fetched root's package-level fields — `repository` above all, plus any sibling field the document carries — and `tags: {}`. **No tag pointer is among them**, and that is the correctness argument, not a convenience: a digest pull learns *where this package's content lives*, it never learns *which digest a tag names*, because it never asked the index that question. Writing a tag there would fabricate a mapping nobody resolved. **Invariant 1 holds unchanged** — a published root is never auto-refreshed: `RootScope::Routing` writes only when this machine holds **no** committed root (`merge_root` returns `None` otherwise), so a package already snapshotted is left byte-identical and a routing *migration* remains `ocx index update <pkg>`'s (`RootScope::Package`) alone. **Write policy per caller:** non-frozen pull/exec → `Full` + `ChainMode::Default`, writes; `--frozen` → refused on the mode, writes **nothing at all** — the *existence* of `p/<ns>/<pkg>.json` is itself a snapshot of the physical address a later invocation routes by, so a tagless root is still a snapshot and `--frozen` promises the invocation creates none (consequence accepted: a frozen invocation against a cold index may still dial); `ocx update` → `NoTag`, excluded; `ocx package inspect` → `ReadOnly`, excluded. `NoTag` was considered as the carrier and does not fit: it is a *view-level* policy meaning "resolve without committing a tag pointer", and the grow branch it gates requires `Full`, so routing the digest path through it writes nothing rather than a tagless root — the axis needed is *what this commit contains*, which is `RootScope`'s. Cost: one extra `GET /p/<repo>.json` per package per machine (`fetch_root_document` memoizes misses only), paid once because the local root answers afterwards. |
| 2026-09-06 | builder (opus) | **Correction to the row above: the write policy was never the gate, and the row's per-caller list read as if it were.** That list enumerated the callers whose *view* excludes them (`ocx update` → `NoTag`, `ocx package inspect` → `ReadOnly`) and the mode that excludes `--frozen`, and stopped there — as though every remaining caller were a materializing one. It is not. The write lived inside `ChainedIndex::physical_reference`, and **four of that method's five non-test callers resolve a logical name without materializing anything**: `ocx package cascade check` (`command/package_cascade.rs`), and the `verify` / `sign` / `attest` pipelines (`oci/{verify,sign,attest}/pipeline.rs`) — each of which asks for a physical address to reach a *registry*, never to fill the local store. All four run on the ordinary `default_index()`: `LocalWritePolicy::Full`, `ChainMode::Default`. Under the row above they all wrote a routing root, and `test_package_cascade.py::test_j10_logical_check_reports_index_staleness` — which asserts a pure read leaves zero local index bytes — caught it. **The gate is now placement, not policy:** `record_routing_pointer` is called from `resolve_transport_pinned` (`package_manager/tasks/resolve.rs`), the single resolve that exists in order to materialize, and `physical_reference` is write-free again. The three conditions from the row above still hold inside the recorder (`Full`, not `Frozen`, `SourceKind::Published`), joined by a fourth — a local copy that already answers returns early, so a warm machine spends no request discovering it has nothing to repair. Policy remains a necessary condition; it was never a sufficient one, because a policy gate asks *how this caller was configured* when the question is *what this caller is doing*. |
