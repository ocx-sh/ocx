...
- `__ocx.desc` — the description artifact (`package/tag.rs:30`). Already excluded.
- `sha256.<hex>` **canonical tags** — pushed by default (`adr_index_indirection.md` Decision E) and
  ...
  re-pushes it under the digest-named tag with `MEDIA_TYPE_OCI_IMAGE_MANIFEST` (`client.rs:393-398`).
...
- no existing manifest → start a fresh empty `ImageIndex` (`client.rs:322-331`);
- an existing **bare** `Manifest::Image` → **wrap it** into an `ImageIndex` as a descriptor rather than push
  alongside it (`client.rs:296-318`);
- either way, add the platform entry (`client.rs:343-351`) and push with `MEDIA_TYPE_OCI_IMAGE_INDEX`
  (`client.rs:360-364`).
