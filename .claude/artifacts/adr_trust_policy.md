...
## Amendment (2026-08-02, owner ruling): a system-tier pin fixes the specificity level

Amends **D3** and **D4**. As shipped, specificity beat tier authority *within* the operator
set: the three `config.toml` tiers pool, then pure most-specific-wins picks the winner — so
a lower tier (in particular the untrusted `[managed]` payload, which merges above the user
config) could displace a system-tier `ghcr.io/acme/*` pin for one package by declaring
`scope = "ghcr.io/acme/tool"` with its own identity. D3 closed that hole only against the
project `ocx.toml`, not against the tiers pooled into the operator set itself.

**Ruling.** A system-scope policy is *locked*, and a locked policy that matches the target
pins the winning specificity level: the winner set becomes every matching policy — locked or
not — at that exact level. Statable contract: *a system-tier trust policy pins the
specificity level for the scopes it matches; lower tiers may join the ANY-of set at that
level but can never outbid it.* When no locked policy matches, D4's global most-specific-wins
is unchanged.

The level is pinned rather than the lower tiers excluded because cross-tier ANY-of at equal
specificity is the fleet key-rotation capability (worked example B) — excluding lower tiers
would break rolling a new signer out through `[managed]`.

**Mechanism.** The existing `lock_as_system()` + `#[serde(skip)] system_locked` provenance
marker already used by `[registry]` / `[registries.<n>]` / `[mirrors]` / `[patches]` /
`[managed]`, applied unconditionally (`[trust]` has no opt-out field) and per
`[[trust.policy]]` entry rather than per section — trust policies array-append, so the
section does not survive the fold but the entries do. `serde(skip)` is the security boundary:
a payload writing `system_locked = true` parses as an unknown key and is dropped.

**Residual — the lock bounds substitution and denial, not admission.** A lower tier (user
`config.toml`, `$OCX_HOME`, or the untrusted `[managed]` payload) may still declare the pin's
*own* scope with an identity of its choosing. It joins the ANY-of set at the pinned level, and
`verify_policies` (`crates/ocx_lib/src/oci/verify/identity.rs`) returns `Ok(())` on the first
policy whose identity **and** issuer both match — so an artifact signed by that identity
verifies, for every package under the pinned prefix. Write access to any of those three
surfaces is therefore trust-equivalent to the system tier for **adding** an accepted signer.
What the lock guarantees is narrower than it reads: no lower tier can narrow a grant to one
package, and the system-tier identity can never be displaced or excluded. Accepted
deliberately — closing admission means dropping cross-tier ANY-of, which is the fleet
key-rotation capability this ruling is built on (worked example B). Operators needing
admission closed must hold the lower tiers' write surfaces at the same trust level as
`/etc/ocx/config.toml`.
