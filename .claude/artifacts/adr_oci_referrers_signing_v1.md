...
- **2026-08-02 (rebase onto main, owner ruling):** Adds Amendment 9 — the JSON error envelope is CLI-wide, not sign/verify-scoped. The §"JSON error envelope" opening sentence is superseded; shape, stability contract, kind inventory and `context` catalog are unchanged. Records the report-then-fail suppression latch and the `--quiet` interaction.
- **2026-06-07 (rebase onto main):** Rebasing slice-1 onto current `main` introduced `ExitCode::DirtyRcBlock = 82` (from `adr_self_setup.md`), which collides with this ADR's original numbering. To resolve, `RekorUnavailable` moved 82 → **83** and `ReferrersUnsupported` moved 83 → **84**. Per the append-only convention the body below (taxonomy table §Exit-Code, variant inventory, scenarios) retains the original 82/83 numbers; the **canonical** exit-code source is now `crates/ocx_lib/src/cli/exit_code.rs` and `quality-rust-exit_codes.md` (both updated, in lockstep). No other mapping changed: `RekorUnavailable=83`, `ReferrersUnsupported=84`, `DirtyRcBlock=82` (main-owned), `OfflineBlocked=81`.
...
### Amendment 9 — the JSON error envelope is CLI-wide, not sign/verify-scoped (2026-08-02)

**Supersedes:** §"JSON error envelope (Codex finding #9 + Spec A1, A4, A7 + C-S1-1 frozen v1 shape)" — its opening sentence only ("All `--format json` errors in signing / verify commands produce exactly this shape"). The shape, the stability contract, the `error_kind` inventory and the `context` catalog are unchanged.

**Background.** The envelope was specified inside the sign/verify slice, so the ADR describes it as a sign/verify feature. What shipped is a single renderer at the CLI's outermost error boundary (`crates/ocx_cli/src/app.rs`): **any** command that fails under `--format json` and has not already written a stdout document gets the envelope. `canonical_command_name` maps every `Command` variant to a frozen string, so `ocx add`, `ocx index update`, `ocx package announce` and the rest emit envelopes too.

**Decision: keep it CLI-wide; the ADR text was narrower than the design.** A JSON consumer that gets a structured envelope from `package verify` but bare stderr from `add` has to implement two error paths for one `--format json` contract — which contradicts Product Principle #1 (backend-first, machine-consumable output). Scoping the renderer back to two commands would be a deliberate inconsistency, not a smaller surface.

**Consequences for the frozen v1 contract:**

- The `command` field's legal values are the full canonical-name table in `canonical_command_name`, not just `package sign` / `package verify`. Adding a command adds a value (minor); renaming or removing one is a `schema_version` major, exactly as for `error.kind`.
- The `error.kind` inventory above remains the closed set for the *signing* kinds. Errors from other commands classify through the same `ClassifyErrorKind` route; a command whose error type contributes a new kind bumps `schema_version` under the existing rule.
- **Report-then-fail suppression.** A command that already emitted its stdout document (e.g. `package push --announce-file` with an unwritable path) does **not** get an envelope appended — two concatenated JSON documents on stdout is not parseable by any consumer. `Api` latches whether a report reached stdout; the envelope is the stdout document only for failures that reported nothing.
- **`--quiet` interaction.** `Api::report` returns before latching under `--quiet`, so `--quiet --format json` prints nothing on success and the envelope on failure. Intentional: quiet suppresses success reporting, not error reporting.
...
> **Superseded in part — see Amendment 9.** The envelope is emitted for **every** command under `--format json`, not only signing / verify. Everything else in this section (shape, stability contract, kind inventory, `context` catalog) stands as written.
