...
[ "$http" = 200 ] || { echo "VERDICT: FAIL — registry did not serve the object by digest"; exit 1; }
[ "$served_digest" = "$OBJ" ] || { echo "VERDICT: FAIL — registry returned a different digest (conversion)"; exit 1; }
cmp "$SP/committed.json" "$SP/served.json" || { echo "VERDICT: FAIL — bytes differ"; exit 1; }
