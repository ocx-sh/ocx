...
| # | Decision |
|---|---|
| C1 | v1 "what's new" = **explicit owner-curated tags**. Registry-scan mode = future (`--sync`, not now). **Amendment (owner, 2026-07-27):** shipped as `--tags-from-registry`, not `--sync` — it groups with the `--tags*` family (sibling of `--tags`/`--tags-from-file`), not with the index's separate `reconcile` operation. No longer future; see C5. |
...
| C3 | `--tags` = full **replace** (reference-impl parity: curated set is the universe; absent tag = deleted). `--tags-from-file` (renamed from `--tags-file` — the `-file` suffix is reserved for output sinks per `subsystem-cli.md` Flag Naming Convention) = **additive union** with the committed root — documented deviation from replace-only `regenerate()`; deletion happens only via explicit `--tags`. |
| C4 | **Update, don't overwrite**: when our own announce branch already exists on the fork, regenerate from the **branch head**, not main — sequential announces accumulate into the one open PR. Fixes the racing-CI edge (announce #2 force-push silently dropping announce #1's still-unmerged tag). Safe because index CI re-derives every claim. **Amendment (owner, 2026-07-24, sol F2):** branch updates are fast-forward-only CAS (`force:false`) with one re-read-and-retry on failure — closes the concurrent-announce lost-update window (a dropped yank is not re-derived by CI). **Amendment (owner, 2026-09-04, #399):** "branch head" narrows to a branch that is genuinely live — a `Diverged` branch under an open pull request is `Stale`, so the root's *shape* comes from the index base while the branch's tag delta is carried forward and the ref is repointed with `RefUpdate::Reset` (including on the non-fast-forward retry, which then re-reads the base, not the branch head); see [`adr_announce_diverged_branch_rebuild.md`](./adr_announce_diverged_branch_rebuild.md). |
| C5 | `--refresh`: re-observe every tag already in the committed root **plus** file additions — catches moved digests (`latest`, cascades) without becoming registry-scan. Universe stays owner-curated. **Amendment (owner, 2026-07-27):** `--tags-from-registry` (C1) is the separate mode that deliberately does registry-scan — enumerating the physical repository's tags at the owner's own explicit invocation, additive-union only (D1/D2 in `adr_announce_publisher_surface.md`). It does not reinstate the index-bot registry enumeration FP-2 removed (`ocx-sh/index` `adr_fork_pr_announce.md` — FP-2 lives in the index repo, not this one): authority stays with the publisher, who triggers it by hand, not with automated index tooling. `--refresh` itself is unchanged. |
| C6 | **Unchanged ⇒ no-op**: if serialized root is byte-identical to the committed root AND no new CAS files → skip commit, skip PR, exit 0, report `status: "unchanged"` (vs `"updated"` + PR URL). No warnings — benign state. Works because `regenerate()` preserves `observed` timestamps for unmoved digests and the serializer is deterministic. Reference bot gets the same short-circuit (Track B — `cli/announce.py:218-267` currently always commits). **Amendment (owner, 2026-07-24, sol F1):** in fork mode, an unchanged run whose branch is ahead of the upstream base additionally ensures an open PR exists (read-only check; opens one if absent) — recovery for the commit-succeeded/PR-open-failed double fault. Unchanged with no branch divergence stays a pure no-op. **Amendment (owner, 2026-09-04, #399):** the byte comparison always reads the *base's* raw bytes (never a merged root), and on a `Stale` branch an unchanged run is no longer benign — mergeability is consulted at that one call site and a `Conflicting` open pull request raises `PullRequestUnmergeable` (exit `DataError` 65) instead of reporting `unchanged`; see [`adr_announce_diverged_branch_rebuild.md`](./adr_announce_diverged_branch_rebuild.md). |
...
- **Commit-verify hook mechanics**: the PreToolUse hook blocks ANY bash invocation whose command string contains `git commit` unless a fresh verify mark exists — including chains that would have run `task verify` first. Sequence as two separate Bash calls: (1) `task verify` (it writes the mark; hand-mark only via `task verify:mark`), (2) the `git commit`. The mark is bound to the HEAD it was written on, so re-mark before each commit.
