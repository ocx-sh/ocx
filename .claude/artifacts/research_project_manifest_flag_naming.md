...
and `OCX_CONFIG_FILE` for the tier-config TOML (`config.toml`). That namespace is
...
|---|---|---|
| Matches existing `--config FILE` / `OCX_CONFIG_FILE` pattern | ✓ direct mirror | ✗ different shape |
...
**`--project <FILE>` with `OCX_PROJECT_FILE` env var and `OCX_NO_PROJECT` kill switch.**
...
| Empty-string escape | `OCX_CONFIG_FILE=""` → unset | `OCX_PROJECT_FILE=""` → unset |
...
| 2 | Empty-string escape | `OCX_PROJECT_FILE=""` → unset | Parity with `OCX_CONFIG_FILE` |
| 3 | Precedence | `--project` > `OCX_PROJECT_FILE` > CWD walk | Matches `--config` / `OCX_CONFIG_FILE` / tier discovery |
...
- `OCX_PROJECT_FILE=<valid>` → loads the file
- `OCX_PROJECT_FILE=""` → treated as unset (falls through to CWD walk)
- `OCX_NO_PROJECT=1` → skips CWD walk + env-var path
- `OCX_NO_PROJECT=1` + `--project <valid>` → still loads (CLI beats kill switch)
- `OCX_NO_PROJECT=1` + `OCX_PROJECT_FILE=<valid>` → env-var path still loads
  (mirrors `OCX_NO_CONFIG=1` + `OCX_CONFIG_FILE` behavior)
- Precedence: `--project` beats `OCX_PROJECT_FILE` beats CWD walk
...
- Subprocess env override: `OCX_PROJECT_FILE=fixture/ocx.toml ocx exec -g default -- cmd`
