...
- **The project's `<home>/toolchain/bin` is emitted only in `bin` mode**, in front of the two global entries: the project dir, then `$OCX_HOME/toolchain/bin`, then `ocx_install_bin_path` (**superseded 2026-09-07 by `adr_toolchain_activation.md` A-16**, which put the install directory last so a toolchain that pins `ocx` can win; this record originally said the install directory led). A trampoline reached through the session PATH runs with `OCX_BINARY_PIN` unset, so its `${OCX_BINARY_PIN:-ocx}` falls back to a bare PATH lookup — ocx's own binary directory ahead of both trampoline dirs means that lookup can never land in a repository-controlled directory. In `env` mode the project dir is rendered and not emitted, and the real-dir prepends land ahead of everything, which is why a trampoline never runs in an activated shell.
...
1. **A directory of executables on a session PATH.** Bounded by: names come from a lock the user pinned to digests; **superseded 2026-09-07 by A-16** — `ocx_install_bin_path` now comes **last**, so a bare `ocx` resolves a toolchain pin where one exists (D-4 admits the name deliberately); the bare-name residual is bounded instead by `resolve_command_excluding`, which drops every trampoline directory from its lookup copy of PATH, and by the independent `is_ocx_trampoline` re-check. **Deliberately *not* bounded by a name denylist** — a project may pin its own `ocx` and shadow the name for itself, so nothing is refused (owner decision D-4, residual R4).
