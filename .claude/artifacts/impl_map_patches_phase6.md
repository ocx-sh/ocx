...
- `PatchPublishArgs` clap struct: `--descriptor`, root-global vs template sub-path target, `--companion <pkg> <archive>`, `--cascade`.
...
Also new: `PatchTestArgs` (`<base-id>`, `--descriptor`, `--script | -- cmd`).
