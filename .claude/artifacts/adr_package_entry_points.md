...
**Research artifact:** [`research_launcher_patterns.md`](./archive/research_launcher_patterns.md)
...
> **⚠ Superseded by `adr_visibility_two_axis_and_exec_modes.md`.** The launcher body shape below reflects the original design. The v2 implementation uses a `file://` URI scheme and embeds `--mode=self`. This is a One-Way Door commitment — the literal string `--mode=self` and the `file://` URI are frozen in every launcher on disk. See the two-axis ADR for the rationale and the current wire format.

```sh
#!/bin/sh
# Generated by ocx at install time. Do not edit.
exec ocx exec --mode=self 'file:///path/to/package/content' -- "$(basename "$0")" "$@"
```

Single-quoted URI — no shell expansion of the baked path. `$(basename "$0")` extracts the entry-point name from the launcher filename.

**Windows launcher body (≤15 lines):**

> **⚠ Superseded by `archive/adr_visibility_two_axis_and_exec_modes.md`.** Same note as Unix above.

```bat
@ECHO off
SETLOCAL DisableDelayedExpansion
ocx exec --mode=self "file://C:\path\to\package\content" -- "%~n0" %*
```

`DisableDelayedExpansion` prevents `!` characters in paths from triggering delayed expansion. `%~n0` extracts the entry-point name from the launcher filename.
...
- The `target` field in metadata is **not** the launcher file; it is the binary the launcher's `ocx exec` resolves via env. The launcher always calls `ocx exec --mode=self 'file://...' -- <name> "$@"` — the `target` shows up in metadata only to assert publish-time that the binary the name refers to actually exists.
...
**For existing installs when upgrading `ocx`:** Installed packages do not retroactively gain launchers. The next `ocx install` generates launchers for newly-installed packages that declare `entrypoints`. For users who want launchers for already-installed packages: `ocx uninstall <pkg> && ocx install <pkg> --select`.

No `ocx migrate` or `ocx repair` command v1. A first-class `ocx install --reinstall <pkg>` flag (drop candidate symlink, re-pull, re-emit launcher set) is tracked at [ocx-sh/ocx#71](https://github.com/ocx-sh/ocx/issues/71).
...
- [PRD](./archive/prd_package_entry_points.md)
- [Research](./archive/research_launcher_patterns.md)
- [Discover](./archive/discover_package_entry_points.md)
...
`ocx exec 'file://<...>' -- "${0##*/}" "$@"` (Unix) or
`ocx exec "file://<...>" -- "%~n0" %*` (Windows). *(Spelling corrected 2026-09-06 — D-V3:* the Unix form read `"$(basename "$0")"`. The **value** is unchanged; the extraction is now the `${0##*/}` shell builtin, because forking `basename` resolved it through the ambient `PATH`.*)* The `file://` scheme +
...
Both the POSIX `.sh` launcher and the Windows `.cmd` launcher pass the script's own basename (which resolves to the entry-point name) as the first argument after `--`. `ocx exec` uses this to determine which binary to invoke inside the package's content tree. The `-- <name> <user-args>` separator + argv shape is preserved by both launcher formats. Changing the separator or the position of `<name>` requires regenerating every installed launcher.

**The stable surface is the argv shape, not the extraction.** *(Clarified 2026-09-06 — D-V3.)* This section previously spelled the POSIX side as `"$0"` and the body as `"$(basename "$0")"`. The POSIX bodies now use the `${0##*/}` builtin — same value, no forked `basename` resolving through the ambient `PATH` — and the change is byte-visible in every generated launcher, so it rides its own commit subject and its own golden-test update. It is **not** a change to this frozen contract: `ocx exec` still receives `-- <entry-point name> <user args>`.
...
### (d) Presentation argspec strings `--log-level=off`, `--color=never`, `--format=plain` (2026-05-01 amendment) {#stable-presentation-argspec}

The launcher templates bake the literal substrings `--log-level=off`, `--color=never`, and `--format=plain` ahead of `exec --self ...` on the inner `ocx` call. Their purpose is opacity — every installed launcher relies on the host `ocx` accepting these argspec strings so the entrypoint chain remains free of ocx's own log lines, JSON envelopes, and ANSI escape sequences. Renaming any of the flag names, dropping the `off` / `never` / `plain` value tokens, or reshaping the `--flag=value` form requires regenerating every installed launcher. This expands the One-Way Door wire-vocabulary surface (previously: `--self`) to four substrings.
...
| 2026-05-01 | Implementer (`feat/package-entry-points`) | Amendment: launcher template now invokes `${OCX_BINARY:-ocx}` (POSIX) / `IF DEFINED OCX_BINARY` branch (Windows) instead of bare `ocx`, and bakes `--log-level=off --color=never --format=plain` on the inner `ocx exec` call. Rationale: avoid coupling the launcher chain to whatever `ocx` resolves on `$PATH` (a stale or feature-missing binary breaks installed launchers — observed `ocx --version` failure when the user's `~/.local/bin/ocx` shadowed the dev binary), and keep entrypoints opaque to the surrounding tool — running `cmake --version` through a launcher must stream only cmake's output, never ocx's logs/JSON/ANSI. The outer ocx writes `OCX_BINARY` onto every subprocess env via `env::Env::apply_ocx_config`. New One-Way Door wire-vocabulary commitments under §Stable Surfaces: the literal argspec strings `--log-level=off`, `--color=never`, `--format=plain` must remain accepted by `ocx` indefinitely, alongside the existing `--self`, `file://` URI scheme, and `-- "$0"`/`"%~n0"` argv shape. Stdin is now always inherited by `ocx exec`'s child (the `--interactive` flag was removed); matches shell exec semantics. |
| 2026-05-15 | Implementer (`refactor/entrypoint-map-shape`) | **Addendum — convert `entrypoints` from array of structs to map keyed by name.** Wire shape changes from `"entrypoints": [{"name": "cmake"}, {"name": "ctest"}]` to `"entrypoints": {"cmake": {}, "ctest": {}}`. Rationale: with `target` already gone, the array index identity duplicated the `name` field; a map matches the `[dependencies.X]` / Compose `services:` / Actions `jobs:` idiom, makes per-entry future fields (aliases, description, platform gating) land naturally in the value object, and gives intra-package uniqueness via JSON object key semantics. `Entrypoints` now wraps `BTreeMap<EntrypointName, Entrypoint>` with a custom `MapAccess` `Deserialize` that rejects duplicate keys with `EntrypointError::DuplicateName` (`serde_json` defaults to silently last-wins on duplicate object keys). `Entrypoint` becomes an empty struct (`{}` value today) so future per-entry fields land additively. JSON schema for the field becomes `type: object` with `additionalProperties: <Entrypoint>` and `propertyNames` carrying the slug regex + length cap. Cross-package collision detection in `composer.rs::check_entrypoints` is unaffected — it walks the interface-projected TC and now iterates `Entrypoints::names()`. **One-Way Door consequences**: (a) `Bundle.version` stays `V1` — the registry wipe scheduled for 0.3.0 (per drop-target addendum) covers this change too; (b) reverting to the array shape would break every published bundle's wire format. **Stable Surfaces unchanged**: `file://` URI scheme, argv shape, `${OCX_BINARY}` pinning, presentation argspecs all preserved. |
| 2026-05-14 | Implementer (`refactor/entrypoint-drop-target`, commit `e84ec1dd`) | **Addendum — drop `Entrypoint.target`.** `Entrypoint` schema shrinks from `{ name, target }` to `{ name }`. Dispatch resolves the name against the composed `PATH` from the package's `env` block at launcher exec time via `Env::resolve_command` in `crates/ocx_lib/src/env.rs`. Rationale: at architect review the `target` field was found to be dead weight at runtime — `launcher exec` (`crates/ocx_cli/src/command/launcher/exec.rs`) already resolved via PATH, never reading `target`. The publish-time validator (`validate_entrypoints`, `check_target_*`, `check_no_unknown_tokens`, `check_launcher_safe`) was the only consumer; ~150 lines of validator + ~10 dedicated tests dropped. Mirror `metadata.json` files at `mirrors/cmake/` already declared the correct `env.PATH` (e.g. `${installPath}/CMake.app/Contents/bin` on darwin) so `name`-only dispatch resolves the binary without restating intent in `target`. Tension 1 cases (rename aliases `python → python3.13`, wrapping with mandatory args, dep-relative re-exports) are not in the corpus today; future support is additive on the `{ name }` shape. **One-Way Door consequences**: (a) the registry must be wiped before 0.3.0 ship — `Bundle.version` stays `V1` because the user accepted the wipe; (b) future entrypoint shape changes must be additive (`#[serde(default)]` fields) on the `{ name }` struct; reintroducing `target` would require an `EntrypointV2`. **Stable Surfaces unchanged**: `file://` URI scheme, argv shape, `${OCX_BINARY}` pinning, presentation argspecs all preserved. The launcher body templates are byte-stable per existing golden tests; only the metadata schema and publish-time validator shrank. Sibling validator `validate_env_tokens` (env-value `${deps.*}` resolution) is untouched. |
| 2026-09-06 | WP-13b (`adr_toolchain_activation.md` implementation) | **Addendum — POSIX launcher bodies resolve the invoked name with `${0##*/}`, not `$(basename "$0")`.** All three POSIX bodies changed in one work package: the two shipped ones (`unix_launcher_body`, `unix_shim_body`) and the new toolchain trampoline. Rationale: `basename` is an external command, so the old form performed a **bare-name `PATH` lookup on every launcher invocation** — a repository-controlled `PATH` could supply the `basename` a launcher then runs (CWE-426). `${0##*/}` is a POSIX parameter expansion, evaluated by the shell with no process spawn and no lookup, and yields the identical value. **Stable Surfaces are unaffected:** §(b) still freezes the argv shape `-- <entry-point name> <user args>` — only the extraction spelling changed, and §(b) is clarified inline to say so. §(a) `file://` scheme, §(c) `ocx exec` positional acceptance, `${OCX_BINARY:-ocx}` pinning and the presentation argspecs all stand. Every generated launcher's bytes change, so the golden tests move with the commit and the break is announced in that commit's subject. The two `$(basename "$0")` occurrences inside the already-superseded pre-`file://` body blocks are left as historical text. |
