...
| Phase 7 (shell trio) | 5 days | `shell hook`, `shell direnv`, `shell init` |
...
3. **direnv** — run `ocx generate direnv` to write an `.envrc`. Works with stock direnv; the resulting `.envrc` is `eval "$(ocx --offline shell direnv)"` plus a `watch_file` for `ocx.toml` and `ocx.lock`.
...
The shell hook (`ocx shell hook`, `ocx shell direnv`) does NOT make network calls, does NOT install anything, does NOT modify symlinks, and does NOT read any file outside the `OCX_CEILING_PATH` walk. It reads `ocx.toml` + `ocx.lock` + the object store's install sentinels, and emits `export` lines. That's it. This boundary is enforced by tests that run the hook in an offline sandbox.
