...
`ocx --format json pull` already prints digests, and `ocx.lock` guarantees `pull` and `exec` resolve identically. But that binds *pull-time* state. Between pull and exec:

- config or index can change;
- a floating tag can move (the OCI tier has no lock at all);
- `exec` / `package exec` **auto-install missing packages on demand**, so an exec can materialise a package that no prior `pull` ever saw.
...
| Frame | Has | Lacks |
|---|---|---|
| Outer (`exec` / `package exec`) | full logical identity (purl, registry, repo, tag, digest), composed env, closure, what the user asked for | the leaf binary — its `executable` is the launcher |
...
| **L3** | Record only at the launcher | Rejected — misses `exec` / `package exec` of a non-entrypoint binary entirely, and loses logical identity everywhere. |
...
ocx exec ──┐
ocx pkg    │
  exec     ├─ resolve packages ──► Vec<Arc<InstallInfo>>   (F3: identity + digest + closure)
ocx        │                       ComposeOutput           (F2: admitted_binaries/entrypoints)
 launcher  │                       Vec<Entry>              (F2: per-entry provenance absent)
   exec    │
ocx        │
 launcher──┘                                 │
   shim  (added post-ADR, see 2026-09-04     ▼
          amendment — same seam, fourth site)
...
**Why the emit is a sibling call at each launching frame, not a field on `ChildEnv`.** The first draft put it inside `apply_child_env`, which would have guaranteed no future command could forget it. That is impossible: `Env::resolve_command` runs *after* `apply_child_env` (it resolves against the env that call just built), and the resolved executable is the record's highest-value field. So the record cannot be assembled at the `ChildEnv` seam. `ChildEnv` stays exactly as it is; the emit is a sibling call at every launching frame — four as of the 2026-09-04 amendment, `ocx launcher shim` having joined the original three — and the plan owes an acceptance test that every one of them produces a record.
...
ocx exec --records-dir /var/log/ocx/records --records-name '{time}-{pid}.json' -- cmake --build build
...
The same honesty applies on the read side: **a v1 record is unsigned and the sink authenticates no writer**, so any principal who can write to the sink can author a file that parses as a record. On a shared sink the residual integrity signals are the record's `0600` mode and its file ownership, cross-checked against the kernel-sourced `process.user.id` — `process.user.name` comes from `$USER` and is informational only. Signing is a v2 question that belongs with the in-toto `Statement` wrapper, not here.
...
pub struct ExecutionRecord {
    ...
    pub frame: Frame,                  // Exec | PackageExec | LauncherExec | LauncherShim + identity quality
...
| `os.type` | nested under `host.os.type`; enumerates `macos` | top-level `os.type`; enumerates `darwin`, `linux`, `windows`, … | **OTel** | The two disagree on both nesting and member spelling — ECS's own `os` field-set docs carry the `macos`/`darwin` mismatch. OCX follows OTel: flat `os.type`, OTel's member spelling. |
| parent pid | `process.parent.pid` (nested) | `process.parent_pid` (flat) | **ECS** | Consistent with the rest of the block, and the nested form leaves room for more parent fields without a rename. Lets a consumer reconstruct call trees — `make` → `ocx exec` → tool — from records alone. |
...
| `process.arch` | *(no process-level field; ECS's architecture field is `host.architecture`)* | `host.arch` — vocabulary: `amd64`, `arm32`, `arm64`, `ia64`, `ppc32`, `ppc64`, `s390x`, `x86`, and OTel permits values beyond this list | **OTel's vocabulary, ocx's placement** | An OCX extension inside ECS's `process` block, borrowing OTel's `host.arch` value vocabulary — the vocabulary ECS lacks at the process level — but scoped to the **process**, not the host machine. See the next subsection for why a machine-level arch field does not exist at all. |
...
|---|---|---|
| `process.arch` | what architecture is *ocx itself* running as | OTel's `host.arch` vocabulary (= OCI arch names; OTel permits values beyond this list) |
...
2. **`uri` is a purl and carries the digest as its version.** `pkg:oci/<name>@sha256:<hex>?arch=<arch>&repository_url=<registry>/<full-repository-path>&tag=<tag>`. Type is `oci`, **not** an invented `pkg:ocx` — OCX packages *are* OCI artifacts. `repository_url` is the registry **plus the full repository path**, repeating the name segment (`docker.io/library/debian` for name `debian` is the `oci` type's own canonical example) — never registry-only. Qualifiers always serialize in **alphabetical order** (`arch`, `repository_url`, `tag`), never authored order, because that is what the purl builder emits; assert on parsed qualifiers, never on a literal string.
3. **The colon in the purl version is emitted UNENCODED** (`sha256:3f7a…`, not `sha256%3A3f7a…`), and this is settled, not a house style against the spec. [ECMA-427](https://ecma-international.org/publications-and-standards/standards/ecma-427/) §5.4 says the colon shall not be percent-encoded, and the `oci` type's own official test suite gives `sha256:` bare in every canonical form. Trivy — the only real `pkg:oci` producer at scale — and Red Hat's security-data guidelines both emit unencoded too, and a literal colon parses correctly under both spec-correct and naive parsers. Documented so nobody "fixes" it later.
...
9. **Key casing is flat lowercase in the borrowed blocks, camelCase in the envelope.** Both surviving borrowed vocabularies (ECS, OTel's arch enum) are flat lowercase; the envelope matches the provenance cluster (in-toto, SLSA, CycloneDX, SPDX). The first draft's mixed snake/camel wart came from OTel's object form and is gone with it. **Amended 2026-09-04:** a same-day rename of the envelope to snake_case was applied and reverted. The record follows in-toto's JSON style because it *is* an in-toto-style document; OCX's own snake_case convention governs OCX-only JSON, not this record.
...
11. **`[mirrors]` endpoints are recorded with their userinfo stripped.** A `user:password@` prefix on a `[mirrors]` `registry` or `index` value is a working credential — `parse_url` keeps it inside the host and the transport interpolates that host back into every request — and embedding one in a remote-repository URL is a mainstream Artifactory/Nexus idiom OCX accepts without comment. `resolution.mirrors` reports the endpoint with that span removed, by the same authority rule `parse_url` uses (scheme split on `://`, authority to the first `/`, userinfo to the last `@` within it); an endpoint carrying none is recorded byte-for-byte. This is the `args` row of "The process/host/os split, field by field" applied to a second field: the sink is operator-collected and routinely fleet-aggregated, which is exactly what turns one leaked secret into many. Refusing userinfo at the config parser instead would close the same exposure for logs and the forwarded `OCX_MIRRORS` payload too, but that is a config-surface break and a separate decision.
12. **Two field classes, and only one can fail the invocation.** The record must never fail an invocation because an *environmental* detail could not be determined.
...
### 1. `ocx exec` — project tier, full closure
...
```json
{
  ...
  "executable": {
    ...
    "sh.ocx.package": "pkg:oci/cmake@sha256:3f7a2b9c5d1e8f04a6b3c7d2e9f1a5b8c4d6e0f2a3b7c9d1e5f8a0b2c4d6e8f0?arch=amd64&repository_url=index.ocx.sh%2Focx%2Fcmake"
  },
  "scope": {
    "tier": "project",
    "cleanEnv": false,
  ...
  "packages": [
    {
      "name": "cmake",
      "uri": "pkg:oci/cmake@sha256:3f7a2b9c5d1e8f04a6b3c7d2e9f1a5b8c4d6e0f2a3b7c9d1e5f8a0b2c4d6e8f0?arch=amd64&repository_url=index.ocx.sh%2Focx%2Fcmake",
    ...
    {
      "name": "ninja",
      "uri": "pkg:oci/ninja@sha256:8b1c4d7e0f3a6b9c2d5e8f1a4b7c0d3e6f9a2b5c8d1e4f7a0b3c6d9e2f5a8b1c?arch=amd64&repository_url=index.ocx.sh%2Focx%2Fninja",
    ...
    },
    {
      ...
      "uri": "pkg:oci/libstdcxx-runtime@sha256:c0d3e6f9a2b5c8d1e4f7a0b3c6d9e2f5a8b1c4d7e0f3a6b9c2d5e8f1a4b7c0d3?repository_url=index.ocx.sh%2Focx%2Flibstdcxx-runtime",
...
```json
{
  ...
  "executable": {
    "sh.ocx.provenance": "ocx-package",
    "sh.ocx.kind": "binary",
    "sh.ocx.package": "pkg:oci/solver@sha256:aa11bb22cc33dd44ee55ff6677889900aabbccddeeff00112233445566778899?arch=amd64&repository_url=internal.corp.example%2Fsolver"
  },
  "scope": {
    "tier": "package",
    "cleanEnv": true,
  ...
  "packages": [
    {
      "name": "solver",
      "uri": "pkg:oci/solver@sha256:aa11bb22cc33dd44ee55ff6677889900aabbccddeeff00112233445566778899?arch=amd64&repository_url=internal.corp.example%2Fsolver&tag=2024.3",
...
3. [ ] Emit call at every launching frame, **after** `Env::resolve_command` (F15); `ChildEnv` unchanged. Classify the resolved executable as `ocx-package` / `external` by store-root containment.
...
- `process.parent.pid` recorded so call trees (`make` → `ocx exec` → tool) reconstruct from records alone.
- Environmental fields are best-effort: an undetectable hostname, architecture or username omits a key, never fails the invocation (format rule 12).
...
## Amendment (2026-09-04) — rebase onto main

Between acceptance (2026-07-27) and this rebase, main renamed `ocx run` to `ocx exec` (alias `x`), added a fourth launching frame for lazy/deferred tools, and landed a signing/attestation/trust-policy stack this ADR predates. A max-tier conformance review (`conform-{cli,config,resolution,signing}.md`) enumerated the drift; this amendment records the owner-confirmed resolutions. Vocabulary, the frame count, and every `ocx run` mention are corrected in place throughout the sections above; everything else lands here rather than rewriting settled prose.

1. **`frame.command` wire vocabulary now tracks main's own CLI names verbatim**: `"exec"`, `"package exec"`, `"launcher exec"`, `"launcher shim"` — space-separated, not hyphenated. The record has never shipped, so the frozen-string rule protecting the error envelope's deprecated `"run"` spelling does not bind it — there is no released consumer to protect. The hidden deprecated `ocx run` spelling records `"exec"` too, not `"run"`: truthful about what executed, and one fewer dead value once 0.7 deletes the deprecated arm.

2. **A fourth launching frame, `ocx launcher shim`, joined the original three.** It is the frame a deferred (`lazy-mode = always`) tool's first invocation runs through to download its content, and — like the entrypoint-launcher split — it produces a second record joined to the outer frame by content digest. Unlike `ocx launcher exec`'s synthetic identity, the shim receives the pinned identifier it was built for, so its frame identity is `complete`, not `degraded`. `resolution.autoInstalled` is set in the shim frame, since that is where the pull happens. Documented in `website/src/docs/reference/execution-records.md`'s new "The launcher shim frame" section; every "three call sites" / "three frames" clause above is corrected to describe every launching frame, current count four.

3. **New/changed record fields**, all additive under format rule 7 (no `schemaVersion` bump):
   - `resolution.noVerify` (bool) — whether `OCX_NO_VERIFY` disabled the install-time signature gate for this invocation. The env-tier opt-out only: the per-command `--no-verify` flag is a one-shot user choice that never reaches a launching frame, so no launching frame can report it. Policy, not outcome: verification happens at install on a cold path, not at exec, and the record never claims a package was actually verified. Free to add — `OcxConfigView` already carries the field, and `resolution_block` already reads three of its four sibling booleans off the same struct.
   - `resolution.frozen`'s doc comment is corrected to state its true scope: `--frozen` freezes package-tier resolution only. A patch companion in force at the same time still resolves live and pins independently; freezing the patch tier is `ocx patch freeze` plus `OCX_PATCH_SNAPSHOT`, a separate act this field does not report.
   - `sh.ocx.composition: "deferred"` on a `packages[]` entry whose root composed lazily and carries no content on disk at record-build time (`root.deferred().is_some()`) — the same "omit, never fabricate" contract every best-effort field already follows, made explicit for a case F3 did not anticipate.
   - `executable["sh.ocx.kind"]` gains a third value, `"shim"`, for a resolved executable that is a generated shim launcher rather than an entrypoint launcher or a real leaf binary. `sh.ocx.provenance` stays `"ocx-package"` for a shim target — the store-containment test is extended to the shim namespace (`$OCX_HOME/shims/`, a sibling of the three CAS tiers), so a lazily-composed tool is not misreported as `"external"`.

4. **`sh.ocx.resolved-from: "tag"` means "a tag was in the request," never "this identity can move."** The reference doc's original prose conflated the two. The local index is the package tier's lock and is never updated automatically, so a committed pin does not move under a later invocation naming the same tag — only the first resolution of a tag the index has not yet seen walks the registry. The marker fires on every path that reaches a pinned identifier via a tag (a store hit, a first-time tag resolve, a patch-companion pin), three of which are not floating in any sense; it answers "was this identity reached by name," not "could it still move." Corrected in `website/src/docs/reference/execution-records.md` — no wire field changed.

5. **`resolution.autoInstalled` is now derived from the package manager's own compose outcome**, not a call-site diff. Main moved composition into `PackageManager::compose_roots`, which takes a `Materialization` *policy* (`Install`/`LocalOnly`/`Symlink`) and returns `ComposeRoots { roots, advisories, omitted }` with no per-root outcome. `ComposeRoots` gains a `pulled: Vec<Identifier>` field, populated only inside the `Install` branch — the one branch that can materialize anything — and `resolution.autoInstalled = pulled`. This covers the ordinary compose frames and the `ocx launcher shim` frame (item 2) with one mechanism, since `materialize_deferred` calls the same `find_or_install_all` internally.

6. **§5 "Why there is no in-toto Statement in v1" — the consumer table's `cosign verify-attestation` row is stale; the verdict is not.** Main's `oci/attest/pipeline.rs` now builds exactly the DSSE-signed Statement over an OCI image subject that row said was missing. The surviving reason is unchanged from the two structural blockers §5 always led with: an execution has no subject and no manifest to attach a referrer to, and exec cannot afford a Fulcio round-trip on every invocation. **No Statement in v1 stands.** If a signed wrapper is ever wanted, the shape is a separate, additive, off-exec-path verb (e.g. `ocx record attest`) operating on already-written records, reusing `statement::build` and the existing signer unchanged — never wrapping at exec time.

7. **D5 ("lifts into `resolvedDependencies` without a translator") needs one qualification.** The record names the **logical** identity — purl `repository_url` is the logical registry plus repository. Main's own signing and attestation pipeline names the **physical** repository as its subject (`physical.repository()`), and publishes no purl at all. The *shape* still lifts with no reshaping — `ResourceDescriptor` is unchanged — but the *identity string* does not match ocx's own attestation subject without a lookup through the same logical→physical mapping `resolution.registries`'s doc comment already describes. The digest join (§7, item 8 below) is unaffected and remains exact regardless.

8. **§7 "chain them, do not couple them" is now backed by main's own enforcement, and the promised docs section has shipped.** `attest::statement::binds_subject` hardcodes the `sha256` algorithm and ignores `subject[].name` entirely, so main itself makes the digest the only correct join — not merely the convenient one this ADR assumed. `website/src/docs/reference/execution-records.md`'s new "Chaining to an SBOM or attestation" section shows the record-digest → `ocx package sbom` / `ocx package verify --attestation` chain, closing Implementation Plan item 9's deferral.

9. **The risk table's own escalation trigger has fired.** The row "If the managed tier ever grows teeth (signing, attestation), write a threat model then and run `/security-auditor`" was written against a managed tier with no teeth. Signing, DSSE attestation, `[[trust.policy]]` and a system-locked `[trust.sigstore]` have since landed, the last distributed through the managed tier itself. This ADR's "no threat model, so no security framing" passages are still individually defensible — the `[records]` SYSTEM lock guards against accident, not malice, independent of what else the managed tier now carries — but the premise that triggered the deferral has changed. Whether to run that audit now is an open owner decision, not resolved by this amendment.

10. **`{time}`-style filename placeholders were re-examined against `adr_interpolation_token_grammar.md` and kept unchanged.** That ADR's claim-all rule covers exactly two `${…}` surfaces — package-metadata `env` values and entrypoint `args` — and does not reach config-tier templates. `[records] name`'s `{…}` grammar is not a stray precedent either: `[patches] path`'s `{registry}/{repository}` default is the existing config-tier template with the identical brace grammar, and it survived the same unification untouched. No change.

11. **The `[records]` SYSTEM-lock placement (outside the fold) is a documented divergence, kept as designed, per owner call.** Main introduced two lock placements for the same "a SYSTEM file locks this section" shape: `[[trust.policy]]` locks per-entry at resolution time, and `[trust.sigstore]` locks inside its own `merge` with the SYSTEM tier's fold adopting the flag — the shape `RegistryDefaults::merge` also uses. `[records]`'s clamp sits outside the fold, matching the `[patches]`/`[managed]` whole-section-assignment precedent instead, for the reason already in this ADR's Configuration section: embedding the rule in `merge` would make every field's precedence conditional and unreadable. Two placements now exist for the same lock shape; this ADR keeps its own rather than converging on `SigstoreTrust::merge`'s.

12. **`[records] dir` from a config file is anchored at the declaring file's own directory**, the same rule `[trust.sigstore] trusted_root` follows — `/etc/ocx/config.toml` and `$OCX_HOME/config.toml` each mean their own value regardless of the process's working directory. `--records-dir` / `OCX_RECORDS_DIR` are **not** anchored this way and stay resolved against the process's working directory, like every other CLI flag and env var path in OCX. A repository's own project-tier `ocx.toml` cannot set `[records]` at all — the section is stripped there the same way `[shell]` is, closing the same class of hole `fold_project_tier`'s doc comment names: a repository consenting to its own audit-trail redirection on checkout. Documented in `website/src/docs/reference/execution-records.md`.

---

## Amendment (2026-09-04) — wire shape extensions

Two new envelope fields and a third package role, settled the same day as the rebase amendment above.

1. **`resolution.insecureRegistries`** — an array of registry hosts among this resolution's `registries` that were declared plain-HTTP (`[registries.<name>] insecure`, or a host named in `OCX_INSECURE_REGISTRIES`). **Absent** when none were. A security-material fact about where the recorded bytes actually came from — the same "omit rather than assert a negative" rule every optional `resolution.*` field already follows. Additive under format rule 7; no `schemaVersion` bump.

2. **Patch companions join `packages[]`, and a patch-tier freeze gets its own field.** A patch companion admitted onto `PATH` for this invocation is a `ResourceDescriptor` entry like any other — `name`, `uri`, `digest`, `annotations` — carrying `sh.ocx.role: "companion"` alongside the existing `"root"` / `"dependency"` values, listed **after** every root and every dependency. `resolution.patchSnapshot: { "sha256": "<hex>" }` names the patch snapshot pinned for this invocation (`ocx patch freeze` / `OCX_PATCH_SNAPSHOT`), **absent** when patch companions resolved live with no frozen pin. This does not change `resolution.frozen`'s scope, already corrected in the rebase amendment above (item 3) — that field remains the package-tier freeze only, and a patch snapshot in force is an independent fact carried by its own field. Both additive under format rule 7.

**Extras on a descriptor are annotations only, never a new top-level field, because `packages[]` must lift into SLSA `resolvedDependencies` unchanged.** `sh.ocx.role: "companion"` is exactly the same move as the existing `"root"` / `"dependency"` values — a value inside the `annotations` map, `ResourceDescriptor`'s frozen `name`/`uri`/`digest`/`annotations` shape untouched. This is why the patch-companion addition costs nothing structurally: a consumer that already understood `sh.ocx.role` for roots and dependencies needs no new parsing path for a companion, and D5's "lifts into `resolvedDependencies` without a translator" claim keeps holding exactly as before.
...
- **Adopted:** [purl spec](https://github.com/package-url/purl-spec) / [ECMA-427](https://ecma-international.org/publications-and-standards/standards/ecma-427/) (`pkg:oci` type) · [in-toto attestation spec](https://github.com/in-toto/attestation) (`ResourceDescriptor`) · [ECS process fields](https://www.elastic.co/docs/reference/ecs/ecs-process) (`process.executable`, `process.args`) · [OTel `host.arch`](https://opentelemetry.io/docs/specs/semconv/registry/attributes/host/) (OCI arch vocabulary; OTel permits values beyond the standard list) · [Conftest/OPA](https://github.com/open-policy-agent/conftest) (the consumer)
