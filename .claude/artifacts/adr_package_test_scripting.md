...
> **Superseded (2026-06-04):** Phase B+C (`ocx lsp`) was implemented — the
> `ocx-sh/starlark-rust` fork carried dotted-member completion / hover /
> param-name completion — then **removed**. The hidden subcommand never
> reached a working dotted-completion UX in editors (no `.` completion trigger
> character; trailing-dot parse fragility means a mid-typed `obj.` fails to
> parse, so members never surface), demand was low (few hand-authored mirror
> scripts), and the LSP was the sole reason for the vendored fork — its
> maintenance cost (rebasing on upstream, no upstream-merge path) outweighed
> the benefit. The starlark family now comes from crates.io and `ocx lsp` is
> gone. Phase A below (vscode-bazel syntax highlighting) stands.
