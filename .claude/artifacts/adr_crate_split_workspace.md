...
| `ocx_python` | PEP 751 lock → OCX package translation: lock parse, wheel selection/repack, env composition | moved from ocx-mirror (`crates/ocx_python`), not from `ocx_lib` | `ocx_oci`, `ocx_package` | ecosystem | ocx-mirror translates `source.type: pylock`/`pypi` specs through it; ocx-dist is its planned second writer of the same registry namespace. `ocx` does not link it (added 2026-09-23) |
...
| `ocx_cli` (pkg `ocx`) | Application layer: argv, context, commands, reports, subscriber setup, and *all* error-to-exit-code classification | unchanged + `exit/` (the relocated ladder) + tracing init | every crate above but `ocx_python` | interface (CLI surface) | unchanged |
