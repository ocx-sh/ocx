...
tag names ──► package::version::variant_names   (Rust, ONE derivation)
                 ...
                 └─► announce::pipeline::apply_variants ──► root["variants"]
                                                              │
tag names ──► version_order.variant_names (Python) ───────────┤ must agree
                 └─► regenerate() ──► PackageRoot.variants ───┘
                                          │
                                          └─► render._catalog_entry (READS, never re-derives)
