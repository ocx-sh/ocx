...
Superseded by [adr_project_lock_inplace.md](./adr_project_lock_inplace.md) (2026-05-03).
...
| 2026-05-03 | worker-doc-writer (Stream C) | Superseded by `adr_project_lock_inplace.md`. Audit confirmed: (a) readers never take a lock — atomic open+parse on data file; (b) no shared-mode use exists — only exclusive locks are taken, so the sentinel was over-engineered for the actual usage profile; (c) writers commit via tempfile + atomic rename, dropping the lock immediately after rename, making orphan-inode-after-rename a non-issue; (d) `init_project` is not covered by the lock helper — tempfile-rename onto a missing target plus `symlink_metadata` existence check handles that race. Consequence: `.ocx-lock` sentinel file eliminated; project mutex now an in-place exclusive `flock(2)` on `ocx.toml` itself via `acquire_project_lock`. |
