...
edge-case register plus 6 adjacent rows the register marks "not stated", in 52
resolutions (A-39 through A-43 close review findings rather than register rows; A-44 records an
owner decision; **A-45 through A-52** record the deltas between this specification and the code
that shipped out of the [#339](https://github.com/ocx-sh/ocx/pull/339) closeout, two of them —
A-49 and A-50 — recording a review finding *rejected* rather than adopted). Every code claim the register makes was re-verified against the
...
**Shipped, and a correction to [ocx-sh/ocx#352](https://github.com/ocx-sh/ocx/issues/352)'s
premise.** The "one shared helper" requirement is now met literally:
`register_project_dir_best_effort` calls `consent::canonical_project_dir` through
`spawn_blocking` rather than deriving the directory itself. **But the divergence the issue
predicts never existed.** #352 says the ledger keys on `registry.rs`'s form and the stamp on
`consent.rs`'s, so on Windows the two could disagree on a `\\?\`-prefixed path. They could
not: `ProjectRegistry::register` re-canonicalises its argument through `dunce::canonicalize`
before computing `name_for_path`, so both keys were already `dunce(std_canonicalize(config)
.parent())` and agreed on every platform. `register_project_dir_best_effort`'s local
derivation was passed straight to `register` and used for nothing else but a warning message.
So this was **duplication, not divergence** — worth removing as a drift hazard, never a latent
defect. The acceptance check #352 specifies ("red before: the two helpers diverge on that
input") therefore has **no reachable red** and was deliberately not written; a green that
cannot go red is the defect this branch exists to refuse, and closing an issue with one would
have been the same error in a new costume. What shipped instead is
`a30_best_effort_registration_keys_off_the_shared_canonicalizer` — the first coverage that
function has ever had — proved red by returning the config path itself from the closure, which
lands the ledger link under the key for `ocx.toml` instead of the project directory.
...
### A-44 — The ocx home toolchain is always consented

Closes: an owner decision, recorded here so nobody re-derives it. Not a register row.

**Question.** Consent (Decision 4, C-024…C-033) is scoped to projects. Does any of it —
a `[shell.consent]` grant, its absence, a refused stamp, a more specific project config —
reach the global tier?

**Decision.** **No, and it never may.** The ocx home toolchain — `$OCX_HOME/ocx.toml`, its
`[env]`, and every package it locks — composes unconditionally on every prompt. There is no
consent gate in front of it, no path grant it needs, no namespace grant it can lose, and no
`[shell.consent]` entry (or absence of one) that suppresses it. This holds while a project
config is present and refused, and while the reconciler has yielded to direnv or mise.

**Owner's rationale, verbatim in substance.** The global tier is controlled by the user, by
definition. `$OCX_HOME/ocx.toml` is not clone-authored text arriving from a repository the
user has not read — it is the user's own file, edited by the user, on the user's machine.
Consent exists to stop *someone else's* checkout putting itself PATH-front on `cd`; asking
the user to consent to their own home directory answers a question nobody asked and turns
the global toolchain into a second thing that can silently stop working.

**Where the code says it.** Three seams, all of which must keep saying it.

*Composition.* The caller resolves `resolve_global_pinned_env` and hands the entries to
`crate::activation` as `SessionInput::global`, **before** the project walk, the lock read,
`consent_evidence` or `evaluate_with_stamp` are reached. (It resolves in `ocx_cli` rather than
inside `activation` because the login exporter owns it — `--env` overrides and group selection are
argv concerns a prompt never has; `session` cannot produce the global tier itself.) Every
subsequent arm — no project, yielded, inert, activated — returns the same `Outcome`, and
nothing clears `global`. `desired_entries` unions global first (C-018), and A-37's yield
narrows `desired` *to* the global scope, which is the same rule seen from the other side.

*The write seam.* `consent::record_in` refuses to stamp the ocx root at all, identified by
device+inode through `same_dir` rather than by path bytes. Without it, `ocx --global lock`
wrote `state/projects/<key-for-$OCX_HOME>/consent.json` — see the correction on the
global-tier sweep carve-out in [`adr_shell_env_overhaul.md`](./adr_shell_env_overhaul.md).

*The sweep.* `ocx clean`'s consent sweep classifies an ocx-root stamp as invalid by *identity*
rather than by liveness, so a stamp a pre-guard binary already wrote is collected. Liveness
alone cannot collect it: the ocx root is always `DirProbe::Present`, so the ordinary
departed-project path retains it forever.

**Test hook.** `a44_the_ocx_home_never_owns_a_consent_stamp` (the write seam, red when the
`same_dir` early return is deleted) · `a44_consent_sweep_collects_a_stamp_recording_the_ocx_home`
(the sweep, red when every candidate re-probes, with a live project's stamp in the same run so a
sweep that deleted everything would fail too) · `test/tests/test_shell_reconcile.py::test_the_global_toolchain_composes_
regardless_of_project_consent` — a refused clone with a global `[env]` present must still
emit the global constant, both with no `[shell.consent]` at all and with one granting a
different directory. Red state: add `outcome.global.clear()` to `compose`'s inert arm and the
test reds on the missing global export while every other consent row stays green.

**Verdict.** Changes no prior decision; makes explicit a scope boundary A-26, A-39 and A-40
each assumed without stating. Any future change that gates the global tier is a defect
against this resolution, not a design choice.

**Implementation deltas — the PR [#339](https://github.com/ocx-sh/ocx/pull/339) closeout**

### A-45 — The per-prompt sequencing lives at `crate::activation`; `shell/` imports no `project`

Closes: [ocx-sh/ocx#343](https://github.com/ocx-sh/ocx/issues/343)'s placement half — a
module-layout decision the plan got wrong, not a register row.

**Question.** `plan`, `capture_priors`, `current_fingerprint` and the `Ledger` codec are
each pure and each testable alone. What they do not say is the **order**: resolve the
global tier, evaluate consent over the walk's project, compose only what consent
authorized, diff that against the live environment, record the result. That order is
normative — C-018's capture ordering, C-028's read-before-consent bound, A-11's determinacy
rule — and it lived in `ocx_cli`, where `ocx shell state` had to re-derive it to explain it.
Where does it go when it is lifted into the library?

**Decision.** **The crate root — `crates/ocx_lib/src/activation.rs`, module `crate::activation`
— never under `shell/`.** `shell/reconcile.rs` keeps the three pure pieces and nothing else:
the carrier format (`ledger.rs`), the planner (`plan.rs`), the fingerprint (`fingerprint.rs`).
Both callers share one derivation: `ocx self activate --reconcile` runs `activation::session`
and renders its `Outcome`; `ocx shell state` runs `activation::evaluate_consent` and reports
the `ProjectConsent` it produced. The command crate keeps argv, rendering and I/O.

`Context`-free by construction: every input is a plain field on `SessionInput`, so nothing in
the moved code can reach for ambient CLI state and no `Context::try_init` enters `ocx_lib`.

**Rationale.** The plan's `shell/reconcile/session.rs` needed no top-level `mod` line, which
is exactly what made it look free. It was not: `project::consent` reads
`shell::coexistence::Observation` and `shell::reconcile::ScopeId`, and the sequencing reads
`project::consent`. Placing it under `shell/` closes a `shell` ⇄ `project` cycle — and a
`use` cycle does not compile across a crate boundary, so it is a blocker for the planned
`ocx_lib` split rather than a style preference — a **third** one, alongside the
`sign` ⇄ `verify` cycle ([#313](https://github.com/ocx-sh/ocx/issues/313)) and the HTTP
transport layer's missing owner ([#324](https://github.com/ocx-sh/ocx/issues/324)). Those
two are siblings under ARCH-16, not this cycle: closing this one does not close either. No other
file under `shell/` reaches for `project`, so the cycle was that one file alone. At the crate
root the sequencing may depend on both, and both stay independent of it. Sequencing is
application-layer anyway; `shell/` is shell-emit vocabulary.

**Test hook.** `shell_does_not_import_project` (`activation.rs`) walks every `.rs` under
`src/shell/` plus `shell.rs` and asserts none imports `crate::project`. It carries its own
non-vacuity twin — the same scanner must still find that import in `activation.rs`, which is
the file the offending imports moved into, and the walk must have found more than one source
— so a green cannot come from the needle having stopped matching. Red state: move any
`use crate::project::…` back under `shell/`.

**Verdict.** Overrules the plan's file placement for [#343](https://github.com/ocx-sh/ocx/issues/343)
and adopts everything else about it. The `ocx_cli` seam the issue set out to delete is gone
either way; the crate root is the only placement that also leaves `ocx_lib` splittable.

### A-46 — A deferred diagnostic is announced on the prompt that caused it, once

Closes: the carrier gap A-21 left open — it specified the channel, not the repetition rule.

**Question.** A-21 gives deferred diagnostics a real emit channel. Nothing there says how
often one prints. Every message in `Outcome::messages` states a *condition* — direnv manages
this directory, the `[env]` channel is withheld, the managed tier dropped a consent payload —
and a condition that has not moved is not news. Neither verdict a message rides with is
cacheable: an `Activate` verdict never is (C-007), and a yield hangs off an env sentinel `fp`
does not fold. So what stops the direnv line printing before every prompt for the shell's
whole life?

**Decision.** The carrier gains **`messages_fp`** (C-002): 16 hex characters of SHA-256 over
the emitted messages, length-prefixed per message the way `reconcile::fingerprint`'s own fold
is, so `["ab", "c"]` and `["a", "bc"]` do not collide. An empty list digests to the **empty
string**, not to SHA-256 of nothing, so "this prompt had nothing to say" is spelled the way
`Ledger::empty` spells it and costs no carrier bytes. A message set is emitted only when
`next.messages_fp != previous.messages_fp`. The summary line is exempt — it is already a delta
between the two ledgers and is `None` on a prompt that changed nothing.

**Additive per A-04**: `#[serde(default, skip_serializing_if = "String::is_empty")]`, so it is
absent from the wire when empty, moves neither `v` nor the envelope tag, and a binary that
does not know it re-prints once. It **survives into the over-cap marker** beside `fp` — an
over-cap shell would otherwise re-announce every deferred diagnostic on every prompt, which is
the exact state the field exists to stop.

**Rationale.** 16 hex characters decide whether to *re-print* a line, never whether to *trust*
one, so the carrier budget outweighs the remaining collision margin — the direnv-yield line
alone would spend 70 bytes stored literally, the `[env]` hint another 150. This is the same
"already announced on the prompt that did it" rule `over_cap`'s marker already follows, and the
same treatment the summary line gave itself by being a delta.

**Test hook.** Drop the `next.messages_fp != previous.messages_fp` guard in `message_lines`
and a two-prompt fixture in a direnv directory prints the yield line twice. For the over-cap
half, drop `messages_fp` from `Ledger::encode`'s marker and the same fixture re-announces
under an over-cap carrier while the under-cap one stays quiet.

**Verdict.** Extends A-21 rather than changing it; A-21's channel is unaltered and only its
repetition is now specified.

### A-47 — `tiers` is truncated at decode, at eight

Closes: the unbounded-array residual in A-13 / A-33.

**Question.** A-13 puts the config-tier candidate paths in the carrier so a grant added to any
tier expires the cached verdict. `MAX_CARRIER_BYTES` bounds the envelope, not the array inside
it — and `tiers` is the one field that turns array length into **per-prompt syscalls**: every
path in it becomes a `stat` in `watch_paths`' set, on every prompt, and the list is carried
forward for the shell's whole life. A carrier the user is invited to `unset` (C-012) can equally
be hand-set. What bounds it?

**Decision.** `Ledger::decode` truncates `tiers` to **`MAX_RECORDED_TIERS = 8`**. Truncated,
not rejected — the same direction as A-02's forged-constant strip and for the same reason: the
rest of the record is still usable, and refusing a whole carrier over one oversized field
discards a live shell's revert set.

**Rationale.** ~12 KiB of short JSON strings fits ~2400 paths inside the 16 KiB envelope, so
without a bound a hand-set carrier buys ~2400 `stat` calls per prompt, permanently. Eight is
headroom over the five `ConfigLoader::load_with_local_view` can actually emit — the three
discovered candidates (system, user, home) plus `OCX_CONFIG` and `--config`. Nothing legitimate
is ever truncated, so A-13's guarantee that a grant added to any tier expires the cached verdict
is untouched.

**Test hook.** Encode a ledger carrying `MAX_RECORDED_TIERS * 100` tiers, assert the pre-decode
value holds all 800 and the decoded one holds exactly eight. Red state: delete the `truncate`
call. The first assertion is the non-vacuity half — without it a decode that dropped `tiers`
entirely would also read as green.

**Verdict.** Extends A-13 and A-33; changes neither. Both still quantify over the candidate
list rather than the surviving one.

### A-48 — The recorded `dir` is bounded before it ever names a file on disk

Closes: the C-007 rule (a) residual A-11's probe opened.

**Question.** A-03 and C-007 rule (a) make `key` and `dir` **advisory identity labels**: both
re-derived from the CWD walk every prompt, neither may construct a path. A-11's determinacy
probe breaks that rule by design — it builds `dir/ocx.toml` and stats it. The carrier is
user-writable by design (C-012 invites `unset`). What bounds the one exception?

**Decision.** The recorded `dir` reaches the filesystem in **exactly one place**,
`activation::walk_is_indeterminate`, and it is validated there rather than trusted:

1. `dir.is_absolute()` — A-30 makes an honest one canonical, hence absolute; anything else is
   forged or corrupt and takes the fail-safe answer.
2. `OCX_NO_PROJECT=1` reverts regardless — a deliberate instruction is not a failed probe.
3. `cwd.starts_with(dir)` — the recorded directory must still be an ancestor-or-self of the
   live CWD.
4. One `symlink_metadata` on `dir/ocx.toml`, which must report a regular file.

So a carrier can name **no path outside the process's own working-directory ancestry**, and
even inside it selects no value, re-grants no consent and names no revert. The probe takes
`Option<&Path>`, not a `&Ledger`: that is all it reads, and the caller hands it across a
`spawn_blocking` hop where two `Option<PathBuf>`s cross for free and a `Ledger` would deep-copy
both scopes' entries and priors on every revert. `ProjectScope`'s own doc comment now states
this as the single sanctioned exception, with these bounds; everywhere else A-03 holds verbatim.

**Rationale.** The absolute check is not defensive padding — it closes a live hole. `""` is in
the forged set the carrier tests already enumerate, every path `starts_with("")`, and
`Path::new("").join("ocx.toml")` is **relative**. An empty `dir` therefore turned the probe into
*"does the process CWD hold an `ocx.toml`"* — a question the walk has already answered — and a
hand-set carrier could pin the recorded scope for the shell's whole life from any directory that
happens to hold one. The direction is retention of an already-applied scope, never a revert, so
the failure was silent.

**Test hook.** `walk_is_indeterminate(Some(Path::new("")), None)` and
`walk_is_indeterminate(Some(Path::new("../../../../etc")), None)` must both be `false`, while an
absolute recorded dir with no CWD is `true`; and an unrelated absolute dir must be `false` from
outside it and `true` only from inside. Red state: delete the `is_absolute` guard and the empty-
dir case flips to `true` while every other row stays green.

**Verdict.** Adopts C-007 rule (a) unchanged and narrows A-11: the probe is not merely "one
`stat`", it is one `stat` inside the CWD's own ancestry on an absolute path.

### A-49 — There is no fourth summary mark; the `(false, false)` arm cannot arise

Closes: a review finding, **falsified**. Recorded so it is not re-found.

**Question.** The summary line marks each touched variable `+` (this prompt starts setting
it), `~` (both ledgers record it, the value changed) or `-` (the previous ledger recorded it
and the next does not). A review pass read the `(false, false)` arm — a key neither ledger
names — as a reachable fourth state wanting a glyph of its own, and proposed one.

**Decision.** **No fourth mark. The arm is unreachable through the shipped caller**, and folds
in with `-` only to keep the match total. `plan` and `next_ledger` run the same A-10 admission
predicate over the same `Outcome`, so every key `sets` can name is in `after`; and every key
`removes` or `restores` can name comes either from the previous ledger — so it is in `before` —
or from `repair_owned_segments`' key set, which is drawn from exactly those two places. The
lost-ledger repair (C-006) therefore **rides the `+` of the key D still declares** and never
earns a token of its own.

**Rationale.** The finding was reasonable from the arm alone; it is false against the caller.
The proposed glyph was reverted. What was actually wrong was a comment that described the arm
as merely "folded in", which reads as a deliberate collapse of two reachable states rather than
as totality over an unreachable one — so the comment was corrected and the *reachable* behaviour
pinned by a test, instead of shipping a mark for a state that cannot occur.

**Test hook.** `the_lost_ledger_repair_rides_the_key_it_repairs`: an empty ledger, a stale
ocx-owned segment still on `PATH`, and D declaring the current one. The first assertion pins
that the fixture genuinely repairs — without it the mark assertion proves nothing — and the
second pins `ocx: +PATH`. Red state: give `plan` an owned prefix that does not cover the stale
segment and the repair stops firing, emptying `removes`.

**Verdict.** **Rejects the finding.** A reviewer who re-derives it from `plan.rs` alone has
read the arm and not the caller.

### A-50 — A stale or missing lock retains the scope and says so, with no plan

Closes: two review findings that disagreed. Records which shipped, and why the other is wrong.

**Question.** A consenting project whose `ocx.lock` is absent or whose `declaration_hash` no
longer matches `ocx.toml` raises `SessionError::{LockMissing, StaleLock}`. Propagating it exits
65 with an empty stdout and a stderr the emitted hook discards — so a user edits `ocx.toml`,
forgets `ocx lock`, and from then on every prompt silently stops tracking the project with no
line printed. Both findings agreed that is wrong. They disagreed on the fix. One would push the
sentence onto `outcome.messages` and let the prompt plan normally; the other would keep the
scope and report the error as text.

**Decision.** **The retain-the-scope fail-safe shipped.** The prompt emits the sentence and
**no plan and no checkpoint**; the carrier is re-emitted **unchanged but for `messages_fp`**
(A-46), so the line is said once rather than before every prompt until the user runs
`ocx lock`. `session`'s `Err` contract is "emit nothing at all" — the applied environment is
left exactly as it stands. Withholding the checkpoint is deliberate and matches the
indeterminate walk: the stamp stays stale so the next prompt retries rather than latching on a
lock the user is about to regenerate.

**Rationale.** The rejected alternative is not merely noisier — it is wrong in direction. A
plan built from a `desired` that is missing the project scope **reverts** it, which is exactly
the mid-`git checkout` teardown the `Err` contract exists to prevent, and exactly the direction
A-11 refuses for an indeterminate walk. Building a fresh ledger instead of carrying `previous`
forward would drop the project scope this path is refusing to tear down, leaving the *next*
prompt no record of it to revert. A stale lock is a transient state of the user's own making;
the fail-safe is to hold still and say so.

**Test hook.** Two prompts against a consenting project with its lock removed: the first emits
the sentence, the second does not, and `$PATH` is unchanged across both. Red state: build the
refusal ledger with `Ledger::empty()` instead of `announcing(previous, …)` and the project
scope is gone from the carrier by the second prompt.

**Verdict.** **Adopts the fail-safe finding; rejects the plan-and-announce one.** A future pass
that "fixes" this back into a normal plan re-introduces the teardown.

### A-51 — A project switch names the project now in effect

Closes: the summary-line ambiguity a two-project transition leaves.

**Question.** Leaving `/w/a` for `/w/b` retires one project's contributions and applies
another's, so every variable both touch comes back as a bare `~`. `ocx: ~PATH` says a variable
moved without saying what moved it. Entering from a project-free prompt, or leaving to one, is
not ambiguous — the `+` and `-` marks already give the direction.

**Decision.** The summary line ends with ` (<project-dir-name>)` **only when both ledgers name
a project and the two differ**. Not on entering, not on leaving, not when only one side has a
project. Only the recorded directory's final component is rendered, and nothing is ever built
from it — the recorded `dir` is advisory (C-007 rule (a), A-03) and A-48 governs its one
sanctioned use as a path, which this is not.

**Rationale.** Naming the project unconditionally would put a suffix on the line every first
prompt in every project prints, for a transition the marks already describe. The switch is the
one case they cannot.

**Test hook.** `a_project_switch_names_the_project_now_in_effect` pins `ocx: ~PATH (p2)` on a
switch, with a non-vacuity twin pinning a bare line on first entry. Red states: drop the
`left.dir == entered.dir` check and the entry case grows a `(p1)`; drop `switched_project`
entirely and the switch case loses its `(p2)`.

**Verdict.** Adds to the summary grammar; changes no mark. No user-facing page documents the
mark grammar, so nothing downstream drifts —
`website/src/docs/in-depth/shell-integration.md` names `ocx: +PATH` and `ocx: -PATH` only as
what its three recordings show, and none of the three is a project switch.

### A-52 — The reconcile budget is measured, and it moves with its evidence

Closes: the provenance defect under the C-044 latency gate.

**Question.** The reconcile Δ budget was amended 2 ms → 3 ms on 2026-08-26 against eight
known-good runs. Those runs were taken on a bench arena whose every fixture root took
`local_root`'s `NotFound` arm, so `composer::compose` ran over an **empty root set**: the
measurement contained the lock walk and the consent source-set derivation and zero metadata
reads, `${…}` interpolation, closure walk or entry building. The reconciler's dominant term was
simply absent from the number that bounds it. What follows?

**Decision.** The budget is **re-derived, not turned**, and the ADR stops carrying the figure.
The live source is `test/bench/shell_latency.py`: `RECONCILE_BUDGET_MS` is the budget,
`_WORST_KNOWN_GOOD_RECONCILE_MS` is the measurement it is drawn through, and both carry their
own provenance in the file. The mechanism that keeps them honest is a self-check asserting
`0.10 <= (budget - worst) / worst <= 0.45`, so the two **cannot move independently**: too tight
and the budget is inside the measured cost of working code, too loose and it stops bounding
anything. A better-or-worse measurement — a CI runner, a further optimisation — re-derives
both together.

**Rationale.** The gate was not red; 3 ms was never breached. The defect is *provenance* — a
budget whose evidence had been proven not to measure the thing it bounds, sitting in the assert
that guards it. That is the same defect the arena fix closed one layer down, and correcting one
without the other would have left the record asserting a number it could not support. Every
other bench fixture states its input as an offset from the budget, which keeps them correct when
it moves and is exactly what would let the budget drift to 50 ms with all of them still green;
the headroom assert is the one check that would not.

**A second, larger correction followed, and it was not provenance.** The same arena had never
staged a `$OCX_HOME/ocx.toml` either, so both global reads failed fast and the **global tier was
absent from the measurement in its entirety** — `resolve_global_pinned_env` composes it on every
non-stat-only prompt, unconditionally, under A-44. Seeding it needed more than a lock and a
materialised package: the global tier resolves through `manager.find`, which walks the manifest
chain first, so a lock plus a package composed **0 of 7** until the config blob and image
manifest were published into the arena's blob cache. The two tiers are gated **separately**; a
sum would let a healthy project tier carry a global tier that resolved nothing.

That correction was **forced by a real red**, not by a falsified premise under a passing gate:
the measured Δ exceeded the standing budget outright. It is the one budget move in this sequence
that the ordinary meaning of the gate demanded. It also revealed the cost the contract had been
quietly excluding — a prompt on a host with both tiers populated costs roughly twice what the
project tier alone suggested, and the added tier's per-tool figure is the same order as the
project tier's own, which is what distinguishes the reconciler's work from a fixture artifact.

**No figure is restated here on purpose**, and the reason is now demonstrated rather than
asserted: every figure written into this entry has gone stale within the hour. Read
`RECONCILE_BUDGET_MS` and `_WORST_KNOWN_GOOD_RECONCILE_MS` in `test/bench/shell_latency.py`,
which carry their own series and provenance. The measurements to date are WSL2-only and say so;
a CI runner re-derives both together, as the headroom assert requires.

**Test hook.** `self_check` in `test/bench/shell_latency.py` — the headroom assert above, plus
the `--expect-fail-gate` needle cases that hold the fault injection's sizing rule against the
live budget. Red state: move `RECONCILE_BUDGET_MS` without moving
`_WORST_KNOWN_GOOD_RECONCILE_MS`.

**Verdict.** Supersedes the 2026-08-26 decision-log entry's **number** and adopts its whole
argument — floor + work, absolute Δ over a ratio, the two budgets kept separate. The startup
budget still does not move.
