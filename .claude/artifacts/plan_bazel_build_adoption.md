...
| DX-97 | **Stage 4 lands as a cached lane, and A4's abort condition is breached and overruled.** Owner decision, executed on `wp/accept-bazel`: `task bazel:test:accept` runs `bazel test //test:all --local_test_jobs=1` in `task verify` phase 2 and in `verify-deep.yml`'s acceptance job, replacing the direct `task test:parallel` call in both. Until now nothing ran the 181 `sh_test` targets at all — both callers ran pytest directly, so stage 4 was neither cached nor reported through Bazel. **Three wall clocks, one tree, one host, one compose stack, measured in this order:** `task test:parallel --force` **160 s** (pytest's own line: `3723 passed, 156 skipped, 5 xfailed in 154.46s`); the cold `task bazel:test:accept` **1421 s** (`Executed 181 out of 181 tests`, 3884 cases against `test/SUITE_FLOOR` 3884); the same task on an unchanged tree **1 s** (`Executed 0 out of 181 tests: 181 tests pass`, bazel's own `Elapsed time: 0.272s`). **The cold run is 8.9x `task test:parallel`, i.e. +788 % against A4's +50 % ceiling — the abort condition is met and the owner overruled it**, for the reason A4 itself predicted: 181 serial `uv run pytest` processes, each paying interpreter start, `conftest` import and a compose-stack probe, against one `-n auto --dist loadgroup` run. The mitigation is the caching this row's other half is about: an edited module executes **5** of 181, a docs-only change executes **0** (0.23 s), and a rebuilt `test/bin/ocx` executes **all 181** (1425 s). *(The 5 is post-review: it was 1 when this row was written, and 1 was the defect — the four modules that read every sibling's source were being served a cached PASS over a tree they had never read. Measured after the fix on `//test:test_install //test:test_smoke_coverage //test:test_bench_smoke //test:test_session_path`: a marker deleted from `test_install.py` re-executed 2 of those 4 — the module and its sweeper — and left the other two cached, where the same edit on the pre-fix graph re-executed 0 of 2.)* Both directions are shown, so neither number is the kind that could be true of a check that never ran. One further measurement, because the warm number would otherwise only be about one server: after that binary-swap run, with the binary restored, `bazel test //test:all` reported `Executed 0 out of 181 tests` in **2 s** — and the local action cache cannot have served it, because it holds one entry per action and the mutated run had just rewritten every one, so `--disk_cache` did, which is the tier a fresh CI server has. **What the reversal cost, stated rather than absorbed:** `external` and `local` come off the targets (`local`'s `no-remote` half suppresses the `--disk_cache` hit on a warm *fresh server* — measured, `scripts/bazel_accept_proofs.py --prove-s015` now carries a five-row table and `acc_no_sandbox` is the added row), `no-sandbox` replaces them, and the real input set is declared through a new `//test:suite_inputs` filegroup whose exclusion list was derived by snapshotting `test/` around a suite run rather than guessed (243 paths moved: 236 `__pycache__`, 4 `.pytest_cache`, 2 `bin/`, 1 `results/junit.xml`) — plus, post-review, `bench/**` (minus its generated `results/`) and `taskfile.yml`, both read by acceptance modules and both absent from the first pass, and per-module `extra_data` for the five modules that read their siblings' source. **Of the two compensating controls the ADR demanded, exactly ONE runs in a lane — which is which is the thing to read off this row.** **LIVE, in `task verify` phase 2 and in `verify-basic.yml`'s `smoke` job:** `bazel:tag:guard`, advanced to **stage 4** (`//...`, floor 332) so the acceptance package is in its universe at all. Its blanket "a `no-sandbox` target carries the cache-excluding tag its kind needs" clause is **narrowed, not holed** — an acceptance target is credited without `external` only while `//test:docker-compose.yml` and `//test:suite_anchor` are in its input closure, shown red at the target and red at the shared group (all 181 at once, the case a one-hop reader would miss), and refused for an impostor outside `//test:`. Three clauses were added when a review found that first pass incomplete: (i) an acceptance target carrying any of `external`, `local`, `no-cache`, `no-remote` or `no-remote-cache` now **reds** rather than being excused — shown red for each of the first three, and before it, re-adding one silently killed every disk-cache hit on CI with nothing anywhere reddening, because the run-side reader of that fact is invoked by no lane; (ii) an acceptance target whose own source sweeps its SIBLING modules must declare them, with the sweeping set **derived** from each module's `ast` rather than hardcoded, so a sixth sweeper reds (the review named two; the derivation found five); and (iii) `//test:suite_inputs` is itself floored — 9 required directories, 11 required files, a 156-label count — because the per-target clause survives any narrowing *inside* the group, which is exactly how `bench/**` and `test/taskfile.yml` stayed undeclared while every gate stayed green. **MANUAL, invoked by no lane and no task:** the binary-digest half, A4's red half 3 proper — `task -d test bazel:tags` then `python3 scripts/bazel_accept_proofs.py --check-s015 --warm <unchanged-tree BEP> --mutated <binary-swap BEP> --tags test/results/bazel-acceptance-tags.json --built-binary target/release/ocx --binary-under-test test/bin/ocx`. It inverted with the contract and now takes two BEPs, the warm one and a binary-swap one; it needs a rebuilt binary and a warm run, which is why it is a command and not a gate. **The escape hatch is `NOCACHE=1`, not `--force`.** The first cut read `{{.CLI_FORCE}}`, which is also what `task verify --force` sets — and this repository tells everyone to pass `--force`, so every full gate would have run `--nocache_test_results` and the caching this row is about would have been unreachable from the one command that runs the lane: 1421 s serial on every run for a cache nothing would ever hit. `task verify` trusts the declared inputs, which is what declaring them was for, and `bazel:tag:guard` is what keeps the declaration honest on every run; `task bazel:test:accept NOCACHE=1` re-executes all 181, and a release lane, when one exists, is where that belongs as a matter of course. **Two residual deviations, both named rather than closed:** the lane is `//test:all` and not `//test/...`, because `bazel test` builds the non-test targets a pattern expands to and `//test/...` reaches `//test/doc_scripts`' 79 cast and GIF genrules plus `:gif_check`; and ~20 modules read `target/release/ocx_schema`, `crates/**`, `website/src/docs/**` or the `//test/doc_scripts` package across a package boundary and are therefore undeclared — `test/bazel.bzl`'s docstring lists each with its stale-green risk, bounded by `--remote_upload_local_results=false` and the acceptance job holding only the cache's read credential, so no wrong entry leaves the host that made it. |
