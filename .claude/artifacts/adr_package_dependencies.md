...
> **⚠ Superseded for runtime dispatch by `adr_visibility_two_axis_and_exec_modes.md` Tension 2.** The runtime no longer hard-codes the consumer path — it dispatches via `ExecMode::includes()`. `ExecMode::Consumer` applies `is_consumer_visible()` (public + interface), `ExecMode::Self_` applies `is_self_visible()` (public + private), and `ExecMode::Full` applies `is_visible()` (all non-sealed). The `resolve_env()` signature now takes an `ExecMode` parameter. The propagation algebra and `resolve.json` persistence described above are unchanged.
