...
## Amendment (2026-07-20, owner review — supersedes the D2/D3 wire shape below)

Owner review after the first cut reshaped the surface. Where the sections below
describe `--deps`, a flat `closure` array, and a sibling `interface_surface`
object, the shipped contract is:

1. **Flag:** `--deps` → `--closure` (pre-1.0, renamed cleanly, no alias).
2. **Wire:** one `closure` OBJECT — `closure.deps` (the transitive dependencies
   in transitive-closure order, **root excluded**; each carries
   `effective_visibility`, tri-state `binaries`, `entrypoints`, own
   `dependencies`), `closure.surface.{interface,private}`, and
   `closure.conflicts`.
3. **Two surfaces, not one — computed by the SAME code as `ocx env --self`.**
   The single `interface_surface` becomes a pair
   `Surface { binaries, entrypoints, env, binaries_complete }`, built by
   `project_surface(nodes, self_view)`. **Every admission / crossing decision
   is delegated to the shared surface algebra in `composer`** — the exact code
   the runtime env command uses — so a surface here EQUALS what `ocx env`
   (`self_view=false`, interface/consumer) / `ocx env --self`
   (`self_view=true`, private/self) composes. The algebra (module comment on
   `composer.rs`): `surface(P, axis) = {carriers of P: vis.has(axis)} ∪
   ⋃{interface_surface(D): edge has(axis)}` — recursion below the root always
   takes a dep's INTERFACE surface (Algorithm v3 step 5), edge composition
   precomputed by the `through_edge`/`merge` fold
   (`ResolvedPackage::with_dependencies`, shared). Flattened as two
   predicates: `dep_admitted(effective, self_view)` (edge-union term) and
   `carrier_crosses(vis, is_root, self_view)` (carrier term; `is_root` =
   recursion depth 0). Every carrier kind routes through `carrier_crosses`
   under its visibility: env vars their publisher-declared one, entry points
   `Entrypoints::IMPLICIT_VISIBILITY` (= INTERFACE — launchers are
   consumer-facing, the package's own runtime bypasses them and calls `bin/`;
   gates BOTH the synth-`entrypoints/` PATH push and the
   `admitted_entrypoints` claim, so a claim can never contradict PATH),
   binaries claims `Binaries::IMPLICIT_VISIBILITY` (= PUBLIC — raw
   executables serve consumers and the package's own shims; ADR
   declared_binaries §4 admission preserved). `self_view` only SELECTS which
   surface is emitted — membership comes from visibility alone, no
   per-kind structural rules. History: third pass (2026-07-20) killed
   inspect's re-derived env rule; fourth pass (2026-07-21) fixed the
   unconditional root-entrypoints claim contradicting the PATH gate but
   expressed it as a structural predicate `entrypoints_on_surface(is_root,
   self_view)`; fifth pass (owner directive 2026-07-21) replaced that with
   the implicit-visibility formulation — behaviorally identical
   (`carrier_crosses(INTERFACE, is_root, self_view) ≡ !(is_root &&
   self_view)`) but derived from the algebra, and any future dispute over a
   carrier's surface membership is a one-constant change.
   `binaries_complete` is the one inspect-only add-on. Regressions:
   `composer.rs::tests::compose_root_entrypoints_are_interface_only` (lib),
   `inspect.rs::tests::walk_closure_private_surface_matches_self_execution_composition`
   (lib) and
   `test_package_inspect_closure.py::test_inspect_closure_surface_equals_env_and_env_self`
   (acceptance — asserts inspect's surfaces equal `ocx package env` / `env --self`
   byte-for-byte on the `deps-app` shape, incl. a dependency's private var absent
   from both).
4. **Plain tree:** one `closure` branch — a FLAT `deps` list (each dep once, TCO
   order, short-name label + whole-identifier digest span + visibility tag; no
   root repetition, no `(*)` marker) plus a `surface` branch with `interface` +
   `private` sub-branches. The former nested re-rooted tree and its diamond
   `(*)` dedup are gone.
5. **Read-only index contract (NEW).** Inspect no longer grows the permanent
   local index. It resolves through `PackageManager::read_only_view` → a
   `ChainedIndex` `LocalWritePolicy::ReadOnly` (no dispatch object, no tag
   pointer, no AbsentLeaf self-heal); content still warms the GC-able blob cache.
   The index is deployment-managed and outside GC (`adr_index_indirection.md`
   B1), so an inspect-time write would be permanent pollution. See
   `subsystem-oci.md` "LocalWritePolicy". Regression: `test_inspect_no_index_growth.py`,
   `chain_refs_tests::read_only_view_resolves_but_writes_no_dispatch_object_or_tag_pointer`.

The remainder of this ADR is the original proposal, retained for rationale.
