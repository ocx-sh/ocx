...
#### Option 5B: `ocx shell init <shell>` with a prompt hook + `ocx hook-env` + `ocx generate direnv`

A three-tier activation stack:

1. **Direct:** `ocx exec [--group <name>] -- cmd` — hermetic, primary, always works.
2. **Shell hook:** `ocx shell init bash` (run once) writes a `PROMPT_COMMAND` that calls `ocx hook-env` on every prompt. `ocx hook-env` reads the nearest `ocx.toml`, compares against the previously-exported state, and prints `export`/`unset` lines. It **never installs** — if a tool in the lock is not in the object store, it prints `# ocx: cmake 3.28 not installed; run 'ocx pull' to fetch` to stderr and skips exporting that tool's vars.
3. **direnv:** `ocx generate direnv` writes an `.envrc` of the form `eval "$(ocx --offline shell-hook)"` plus `watch_file ocx.toml ocx.lock`. `ocx shell-hook` is the non-stateful variant — same output as `hook-env` but without the previous-state diff.

**PATH deduplication:** `ocx hook-env` tags every export with a sentinel env var `_OCX_APPLIED="group1:group2:..."` containing a serialized fingerprint of what it last applied. On next invocation, if the fingerprint changed, emit `unset` for everything previously applied before emitting the new exports. This is the mise / direnv convention; it is battle-tested.
...
| Pros | Cons |
|------|------|
| Covers all three modes of use (hermetic exec, interactive shell, direnv) without duplicating logic — `hook-env`, `shell-hook`, and `ocx exec`'s env composition all reuse `resolve_env()` | Three commands (`hook-env`, `shell-hook`, `shell init`) — discoverability friction |
...
|---|---|---|---|---|
| `ocx hook-env` | Prompt-hook entry point | Yes (diffs against `_OCX_APPLIED`) | stdout (eval'd by shell) | No (called from prompt) |
| `ocx shell-hook` | One-shot export generator | No (stateless) | stdout (eval'd by direnv) | No (called from `.envrc`) |
...
| 5. Activation model | 5B — `ocx exec` + `ocx hook-env` (prompt hook) + `ocx shell-hook` (direnv) + `ocx shell init <shell>` |
...
- Six new commands in v1 (`ocx lock`, `ocx update`, `ocx hook-env`, `ocx shell-hook`, `ocx shell init`, `ocx shell profile generate`). We accept this because each has a distinct, non-overlapping contract documented above.
- `profile.json` deprecation is a two-release migration users have to follow. Mitigated: v1 still supports it with a one-line warning; no forced migration.
...
- `lock.rs`, `update.rs`, `hook_env.rs`, `shell_hook.rs`, `shell_init.rs`, `shell_profile_generate.rs`.
...
ocx hook-env [--shell SHELL]         Prompt-hook export generator (stateful)
ocx shell-hook [--shell SHELL]       Direnv export generator (stateless)
...
ocx generate direnv                  Writes .envrc that eval's shell-hook
```
...
7. [ ] **Phase 7 — Shell hook trio.** `hook-env`, `shell-hook`, `shell init`. Reuses `ProfileBuilder` for per-shell syntax generation.
8. [ ] **Phase 8 — `ocx generate direnv`.** Writes `.envrc`.
9. [ ] **Phase 9 — Home-tier `ocx.toml` + profile deprecation notice.** `profile.json` keeps working; `ocx shell profile generate` added; `ocx shell profile load` emits a deprecation note.
...
- [ ] `hook-env` never initiates a network call (assert with an offline sandbox test).
- [ ] `hook-env` correctly removes previously-applied exports when the group fingerprint changes.
...
| Env var | `OCX_CONFIG_FILE` | `OCX_PROJECT_FILE` |
...
**G2. Env var `OCX_PROJECT_FILE`.** Same semantics as `--project`. Empty string (`OCX_PROJECT_FILE=""`) is treated as unset, matching the `OCX_CONFIG_FILE=""` escape hatch.

**G3. Kill switch `OCX_NO_PROJECT=1`.** Skips CWD walk *and* `OCX_PROJECT_FILE`. Does NOT prune an explicit `--project` flag — the CLI is trusted caller intent, matching the existing `OCX_NO_CONFIG=1` + `--config <FILE>` behavior covered by `loader.rs` test `no_config_with_cli_flag_still_loads`.

**G4. Precedence.** `--project` > `OCX_PROJECT_FILE` > CWD walk. The ceiling path (`OCX_CEILING_PATH`, Amendment F) only bounds the CWD walk. Explicit paths escape it — that is the whole point of the flag.

**G5. Symlink policy.**
- Explicit paths (`--project`, `OCX_PROJECT_FILE`) follow symlinks (trusted caller intent, matching `--config`).
- CWD walk rejects symlinks at the discovery step (matching tier-config discovery).

**G6. Basename.** Any filename is accepted via `--project` / `OCX_PROJECT_FILE` (matches Cargo `--manifest-path`; fixtures and integration tests need this). The CWD walk still looks for the literal name `ocx.toml` (unchanged from Amendment F).

**G7. File-not-found exit code.** `--project <missing>` or `OCX_PROJECT_FILE=<missing>` → `NotFound` (79). Distinct from `ConfigError` (78), which is reserved for parse/schema failures on files that exist. Matches how `--config` handles missing explicit paths today.
...
**G9. Non-`NotFound` I/O on explicit paths.** `--project <path>` / `OCX_PROJECT_FILE=<path>` where the `path` exists but the OS surfaces a non-`NotFound` error kind (permission denied, path-is-not-a-directory, stale NFS handle, EIO) → `Error::Io` → `IoError` (74). Silently succeeding would be a correctness hole: the explicit path is resolved for its later-phase side effects, so a phantom `Ok(Some(path))` that never materializes a readable file would mask the failure until a downstream consumer stumbles into it. Explicit G7 symmetry: missing file → 79 (NotFound), unreadable file → 74 (IoError); the two codes are distinguishable to automation.
...
**Rationale.** The existing tier-config triple (`--config` / `OCX_CONFIG_FILE` / `OCX_NO_CONFIG` / `""` escape hatch) is already the OCX contract for "explicit file path overrides discovery." Duplicating that shape verbatim means users, tests, and future loaders inherit one mental model, not two. Divergence from uv/PDM's `--project <DIR>` convention is a deliberate trade-off in favor of internal consistency with OCX's own `--config`.
...
| 2026-04-19 | Design session (sion) | Amendment G added: explicit project-file escape hatch (`--project <FILE>` + `OCX_PROJECT_FILE` + `OCX_NO_PROJECT`) mirroring the existing tier-config triple one-to-one |
