...
| Tiebreaker between multiple matching entries | Manifest array index order (publisher controls; standard OCI convention) |
...
| `os_version` matching | **Strict equality when the candidate declares one** (`other.os_version.is_some() && other.os_version != self.os_version` → no match). `Platform::current()` never populates `os_version`, so a version-bearing candidate stays fail-closed (unselectable), preserving the pre-`can_run` strict-`matches` behaviour. Future version-range work supersedes this via a new ADR. |
| Versions (e.g. glibc ≥ 2.17) | **Deferred**. Preferred future path: encode in `os.version` (spec-blessed). Today's `libc.musl`/`libc.glibc` stay valid. |
