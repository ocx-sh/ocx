...
**Exit code:** 83 (`RekorUnavailable` — new variant, distinct from 69 so CI scripts can branch on "retry later" vs "network broken").

**Note:** On verify-side, the same 83 fires if Rekor cannot be reached during SET validation.
...
**Exit code:** 84 (`ReferrersUnsupported` — registry lacks OCI Distribution Spec v1.1 Referrers API; dedicated exit code per ADR decision S1-F).
...
| FR-2 | On GHCR or similar (no Referrers API), `ocx package sign` exits 84 with `ReferrersUnsupported` — no fallback tag written | `test_sign_referrers_unsupported_exits_84` |
...
| FR-12 | Rekor unreachable on verify exits 83 with `rekor_unavailable` | `test_verify_rekor_unavailable_exits_83` |
...
| FR-17 | Capability cache respects a flat 6h TTL (CI/interactive split deferred to v2 per ADR Amendment 6) | `test_capability_cache_ttl` |
