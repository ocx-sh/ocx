...
| `ocx self setup` (`[shell]` toggles, `OCX_EXTRA_CA_CERTS` persistence, the `[managed]` seed fence) | Every `$OCX_HOME/config.toml` read-modify-write goes through `config::edit::edit` (`edit_text` for the fence, whose `rc_block` state machine works on lines, re-run under the lock after the fetch): `lock_scoped(locks, "config-edit", $OCX_HOME, "config.toml", 5 s)` under `$OCX_HOME/locks`, then bounded read → `toml_edit` closure → render → `write_bytes_atomic`. The file is atomic-rename-replaced, so its inode rotates — the Locking Policy's `lock_scoped` row, never a sidecar (ocx#468). | Two concurrent invocations serialize; the second reads the first's document, so both edits land. An unchanged render writes nothing (an absent file stays absent). | Lock timeout: 75 `TempFail` (`EditError::Locked`, the lock-timeout row below). Over `MAX_CONFIG_SIZE` before or after the edit: 78 `ConfigError` (`EditError::TooLarge`). Unparseable / wrong-shaped file: 74, file left byte-identical. `--dry-run` takes neither the directory nor the lock. |
