...
- Git's own `commit-msg` hook (a prek shim → `scripts/commit_gate.py`; `task git:hooks` installs it) blocks commits without fresh verify mark. When blocks, run `task verify:scoped --force` (or `task verify`; never `--no-verify`) — both write the mark themselves. Only when a passing verify is in hand and just rebase / cherry-pick context changed since:
  ```sh
  task verify:mark
  ```
