...
- `worker-doc-reviewer` — always at max (doc drift at scale is default failure mode); model per resolved `--doc-reviewer` overlay (`sonnet` default; `haiku` only via explicit override — see `overlays.md` doc-reviewer axis)
