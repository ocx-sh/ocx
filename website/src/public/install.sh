...
modify_shell_profile() {
    ...
    if [ "$_ocx_home" = "$HOME/.ocx" ]; then
        ...
        _source_line='. "$HOME/.ocx/env"'
    else
        _source_line=". \"$_env_path\""
    fi
...
main() {
    ...
    _old_version=""
    _bin_path="${_ocx_home}/installs/ocx.sh/ocx/current/bin/ocx"
    if [ -x "$_bin_path" ]; then
...
export_github_path() {
    local _install_path="${OCX_HOME:-$HOME/.ocx}/installs/ocx.sh/ocx/current/bin"
