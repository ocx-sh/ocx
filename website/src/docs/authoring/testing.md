...
Full reference for every typed value and every host function is in the [Script Host API reference][ref-script-host-api].
...
For `.star` syntax highlighting in VS Code, add the [vscode-bazel][vscode-bazel] extension to your workspace — it provides `.star` file syntax highlighting.
