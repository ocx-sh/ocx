...
The full identifier rules — required registry component, identifier syntax, the "no version ranges" decision — live in the [dependencies reference][reference-dependencies]. The short version: every dependency is a fully-qualified `registry/repo:tag@sha256:...` string, registry mandatory, digest mandatory, tag advisory.
