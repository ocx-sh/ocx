...
## Toolchain Activation {#toolchain-activation}
...
`--clean` controls only stage 1 — what the child process inherits from the parent shell. It is not what makes the package-composed set (stage 2) reproducible. That comes from the resolver's scope: package env values are computed from `ocx.lock` and the resolved digests alone, identically with or without `--clean`. Project `[env]` (stage 4) is the opposite case by design — it is the user's own file, deliberately allowed to read ambient state, and is excluded from the lock's `declaration_hash` for exactly that reason. `OCX_HOME` is the one exception to "stage 1 is stripped under `--clean`" — it is forwarded set-always via `apply_ocx_config` regardless of `--clean`, alongside `OCX_BINARY_PIN`, so a generated launcher's re-entry resolves the same store the parent did instead of falling back to `~/.ocx`. The narrower statement is that `apply_ocx_config` writes the resolution-affecting `OCX_*` set onto the child in either mode: `OCX_HOME` and `OCX_BINARY_PIN` always, and every other member — including the ambient-only [`OCX_DEFAULT_REGISTRY`][env-default-registry], [`OCX_INSECURE_REGISTRIES`][env-insecure-registries], [`OCX_NO_PROJECT`][env-no-project] and [`OCX_NO_CONFIG_REFRESH`][env-no-config-refresh] — whenever this invocation resolved one. `--clean` strips what the *shell* contributed, never what ocx itself decided.
:::
...
[asdf-shim-loop]: https://github.com/asdf-vm/asdf/issues/2166
...
<!-- configuration -->
[config-activate]: ./configuration.md#project-config-activate
[config-pinned]: ./configuration.md#project-config-pinned
...
[env-no-config-refresh]: ./environment.md#ocx-no-config-refresh
