...
## ocx-mirror Pipeline {#ocx-mirror-pipeline}

These variables are used by the `ocx-mirror pipeline` subcommand family and by the [GitHub Actions][github-actions-docs] workflows generated by `ocx-mirror pipeline generate ci`. They are not read by the `ocx` binary itself.

### `OCX_MIRROR_RELEASE_TAG` {#ocx-mirror-release-tag}

The `ocx` release tag used by generated workflow jobs to download the `ocx` binary from [GitHub Releases][github-releases] of the `ocx-sh/ocx` repository. Sourced from `mirror.yml: ocx_mirror.release_tag` by the workflow renderer.

```sh
OCX_MIRROR_RELEASE_TAG=v0.7.2
```

Used on host legs (Linux glibc, macOS, Windows) to pick the correct release asset. The tag is also used as a cache key for the downloaded binary.

**Scope:** Generated workflow jobs `prepare`, `test` (native legs), `push`, `notify`.

### `OCX_MIRROR_REV` {#ocx-mirror-rev}

Full 40-character git SHA used to `cargo install` the `ocx-mirror` binary. Sourced from `mirror.yml: ocx_mirror.rev`. When both `OCX_MIRROR_REV` and `OCX_MIRROR_RELEASE_TAG` are set in `mirror.yml`, `rev` wins for the `cargo install` path.

```sh
OCX_MIRROR_REV=abc123def0123456789012345678901234567890
```

**Scope:** Generated workflow jobs `discover`, `prepare`, `push`, `notify`.

### `OCX_MIRROR_SOURCE` {#ocx-mirror-source}

Override for where to install `ocx-mirror` from. Supports three URI forms:

| Form | Example | Meaning |
|------|---------|---------|
| `path:<dir>` | `path:/workspace/ocx` | Local Cargo workspace — `cargo install --path` |
| `git+https://<url>?rev=<sha>` | `git+https://github.com/ocx-sh/ocx?rev=abc123` | Explicit git rev |
| `git+https://<url>?branch=<name>` | `git+https://github.com/ocx-sh/ocx?branch=main` | Branch HEAD |

When set, overrides the `cargo install` source derived from `OCX_MIRROR_REV`. Not used in production workflows — intended for development and integration testing.

**Scope:** Generated workflow jobs that install `ocx-mirror`.

### `OCX_BINARY_OVERRIDE` {#ocx-binary-override}

When set, generated workflow test legs and the `push` job use the file at this path as the `ocx` binary instead of downloading a release artifact. The file must be a compiled `ocx` binary for the host platform (`linux-gnu`, macOS, or Windows).

```sh
OCX_BINARY_OVERRIDE=/tmp/ocx-test-build/ocx
```

This is the integration-test override knob. Setting it skips the `gh release download` step and uses a locally-built binary. No other step changes.

**Scope:** Generated workflow jobs on host/native legs.

### `OCX_BINARY_OVERRIDE_MUSL` {#ocx-binary-override-musl}

When set, generated workflow Linux container legs use the `.tar.xz` archive at this path as the `ocx` musl binary instead of downloading a release artifact. The archive must be a `tar.xz` containing an `ocx` binary compiled for `linux-musl`.

```sh
OCX_BINARY_OVERRIDE_MUSL=/tmp/ocx-musl.tar.xz
```

This is the container-leg counterpart of `OCX_BINARY_OVERRIDE`. The archive is consumed by the per-leg ephemeral Dockerfile (`ADD ocx /usr/bin/ocx`) — switching the source tar is sufficient to override the binary for every container image without changing the build path.

**Scope:** Generated workflow jobs on Linux container legs.

### `DISCORD_WEBHOOK_URL` {#discord-webhook-url}

The [Discord][discord] webhook URL used by `ocx-mirror pipeline notify`. This variable is never set directly in `mirror.yml` — the `mirror.yml` `notify.discord.webhook_secret` field holds the name of the [GitHub Actions secret][github-actions-secrets] that carries this value, and the generated workflow exposes it via `env:`.
...
The actual URL is stored as a repository secret in [GitHub Actions][github-actions-docs]. The `webhook_secret` name in `mirror.yml` must match the secret name exactly.

::: warning Never hardcode the URL
`ocx-mirror pipeline generate ci` rejects any `mirror.yml` where `notify.discord.webhook_secret` contains a URL (matching `https?://` or `discord.com`). This prevents live webhook URLs from being committed to the repository.
:::

**Scope:** `ocx-mirror pipeline notify` job.
