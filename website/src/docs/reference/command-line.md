...
| [`--pinned`](#arg-pinned), [`--no-pinned`](#arg-pinned) | — | Top tier of the [`pinned` resolution ladder](#arg-pinned-ladder). `--pinned` composes the digest roots `ocx.lock` names; `--no-pinned` composes the rendered `links/<group>/<entry>` links. Neither takes a value, and passing both is last-wins. | *(inherit from `ocx.toml` / `OCX_TOOLCHAIN_PINNED`)* |
| `--env <NAME\|KEY[:TYPE[:SEP]]=VALUE>` | — | Set an environment variable for this invocation only. Repeatable; later occurrences win over earlier ones for the same key. Splits on the **first** `=`, so `--env FOO=a=b` yields `FOO` → `a=b`. Only the segment before that first `=` is checked for a `:TYPE[:SEP]` qualifier — an environment variable name can never contain `:`, so a Windows-style value with its own colon (`--env PATH:path=C:\tools\bin`) is read correctly, and `--env FOO:constant=a=b` sets `FOO` to `a=b`. `TYPE` is `constant` (replaces, the default when omitted), `path` (prepends), or `list` (appends) — the same three kinds [`[env]`][config-project-env] uses. `SEP` qualifies `list` only: the string a `list` contribution is joined to the existing value with (`--env GODEBUG:list:,=gctrace=1`); omitted, the key inherits whatever separator another contributor already declared, or a single space if none did — see [Env Composition][env-composition-list]. A relative `path` value resolves against the **current directory** the flag was invoked from, not the project root [`[env]`][config-project-env] resolves against: a checked-in file must mean the same thing from any subdirectory, while a flag is composed by whatever script invokes `ocx`, and the current directory is the one base that script can compute. Highest-precedence stage: wins over ambient, package, patch, and project/group [`[env]`][config-project-env] (see [Project Environment][env-composition-project-env]). A bare `--env NAME`, with no `=` and no `:TYPE`, passes the value through: the invoking process's own `NAME` becomes a `constant` entry, exactly as `--env NAME="$NAME"` would, so a calling script names what may travel instead of spelling every value out. A `NAME` the invoking process does not set contributes nothing and is not an error; a `NAME` set to the empty string forwards the empty string. It is the form that makes such an allowlist usable under `--clean`, where the child environment otherwise starts empty. A `TYPE` written without a value (`--env FOO:path`) is rejected -- only the plain bare form passes a value through. So are a `TYPE` that names no modifier or is empty, a `SEP` that is empty, contains `=`, contains a newline or carriage return, qualifies a non-`list` type, or edges a `list` value, an invalid variable name, and an `OCX_*`/`__OCX_*` key in either form. All exit 64. | — |
...
| 64 | `--` missing; empty argv; empty `-g` segment; no `ocx.toml` found; unknown `-g` group; unknown binding NAME; ambiguous NAME across groups with conflicting identifiers; `--global` combined with `--project`; an `--env` `TYPE` written without a value (`--env FOO:path` -- only a plain bare `--env NAME` passes a value through); an `--env` `TYPE` that names no modifier or is empty (`--env X:bogus=v`, `--env X:=v`); or `--env` sets an `OCX_*`/`__OCX_*` key. (OCX remaps clap's default exit 2 to 64.) |
...
- `--env <NAME|KEY[:TYPE[:SEP]]=VALUE>`: Set an environment variable for this invocation only, same grammar as [`ocx exec --env`](#exec). A relative `path` value resolves against the directory ocx runs in, which under direnv is the directory holding `.envrc`.
...
- `--env <NAME|KEY[:TYPE[:SEP]]=VALUE>`: Set an environment variable for this invocation. Repeatable; appended last in the report's `env` array, matching where it lands in composition.
- `-h`, `--help`: Print help information.
...
|---|---|
| `PATH` | The directory whose project to consent to. Defaults to the current directory. The upward walk from `PATH` is the one a shell prompt makes, so this consents to exactly the project a prompt would activate. A [`--project`](#arg-project) or [`--global`](#global-flag) selector still takes precedence. |

**Exit codes**

| Condition | Exit code |
|---|---|
| The stamp was written | 0 |
| No `ocx.toml` governs `PATH` | 64 |
| `PATH` names the ocx home — the global toolchain is always consented and never carries a stamp | 64 |

Grants that live in configuration instead — a directory under `[shell.consent] paths`, an organisation under `[shell.consent] namespaces` — are edited in `config.toml`, not here. [`ocx shell state`](#shell-state) reports which of the three is in effect.
...
| Argument | Description |
|---|---|
| `PATH` | The directory whose project to revoke. Defaults to the current directory, resolved by the same walk [`ocx shell allow`](#shell-allow) uses. |
...
| `--transport <TRANSPORT>` | How the request is written: `api` opens it through the forge's REST API, `git` clones the index repository and creates it from a single authenticated push. `git` is GitLab-only, and is the only way a GitLab CI job token can open a merge request — see [`claim`](#package-claim) for the same flag's full contract. | `api` |
...
| A digest-pinned identifier, or any tag that does not name a node in the version graph — junk, a reserved tag such as a keep tag, a bare variant name, or a version nothing published (`:9.99`) | 64 |
| An I/O error writing `--tags-file` — a missing parent directory or permission denied. `error.kind` is `io_error` with **no** `error.detail`; a script must branch on `error.kind` for this one | 74 |
...
- `<PATH>`: Path to the directory to bundle — or, under `--extract`, to an archive whose extracted tree is bundled instead.
...
| A bare `--ci-annotations` followed by a layer argument that parses as a CI provider name (`github`, `gitlab`, and their aliases) — the space form the flag's grammar cannot attach | 64 |
...
- `--fulcio-url <URL>`: [Fulcio][fulcio] CA endpoint (override for private deployments), for the keyless signing `--sign` or `--sbom` performs. Defaults to `[trust.sigstore].fulcio_url`, else `https://fulcio.sigstore.dev`. Keyless-only: alongside `--key` it is a usage error (exit 64) rather than a flag that quietly does nothing. A usage error (exit 64) without `--sign` or `--sbom`.
...
`--annotation` values are written verbatim: it never derives one for you. Omitting both annotation flags writes nothing and leaves whatever the index already carries untouched, so an earlier annotation survives a later plain push. Supplying a key overwrites that one key and leaves the rest of the index's annotations alone.

`--ci-annotations[=PROVIDER]` is the one exception, and it is opt-in. It fills the four keys a pipeline already knows the answers to — `source`, `revision`, `created`, and `version` — from the CI environment, and an explicit `--annotation` on the same key still wins. Three of the four are a line of `--annotation` each in any pipeline; `version` is the one that is not, because with `--identifier` omitted the tag comes from the [build receipt](#package-create-receipt) and this push is what resolves it, so a wrapper cannot name it until after the push has already happened.
...
| `--junit <PATH>` | — | Write a [JUnit XML][junit-xml-format] report for the scripted run to `PATH`. Requires `--script`; cannot be combined with a trailing command (a usage error, exit 64). Parent directories are created; an existing file is **truncated, never merged** — one invocation is one identifier, one platform and one script, so give each platform its own path and let CI glob them ([GitLab `artifacts:reports:junit`][gitlab-unit-test-reports] does not merge them — it keys each suite on the job name, so each matrix leg is its own suite; the GitHub test-reporter actions do merge on their side). Written whenever the scripted run is reached, including a red run and an unreadable `--script` path; a failure resolving the package beforehand writes none. The JSON envelope on stdout is unchanged; the exit code is not — a report that cannot be written exits 74 even after an otherwise green run (see the `--script` exit codes below). The `<testsuite>` is named for the identifier and the `<testcase>` for the platform, so a glob of per-platform files carries a distinct case per platform under a shared suite name — see [Testing locally][authoring-testing] for how GitLab and GitHub differ in what they do with that glob. A failure carries the script's `file` and `line` — taken from the Starlark diagnostic rather than parsed back out of its message — unless the script came from `--script -` (stdin), where there is no source file and both attributes are omitted. Captured stdout/stderr are attached on every run that produced output. | — |
...
| 64 | Usage error — both `--script` and `-- CMD` supplied; neither supplied; script file not found or unreadable. An argv fault wins over a `--junit` path that cannot be written: the report write is best effort there, logged as a warning, so the exit code and the message still name the argv fault |
| 65 | Script syntax, type, or arity error |
| 74 | I/O error — stdin read failure (`--script -`), scratch directory creation failure, or a `--junit` report that could not be written (an unwritable path exits 74 even when every expectation passed; the stdout envelope still reports the run's own result) |
...
- `--env <NAME|KEY[:TYPE[:SEP]]=VALUE>`: Set an environment variable for this invocation, surfaced in the report's `env` array. Repeatable. Per-invocation only — this command still reads no `ocx.toml`; for a project's declared environment use [`ocx inspect`](#inspect).
- `-h`, `--help`: Print help information.
...
| An I/O error reading `--readme` — missing file, permission denied, or a directory. `error.kind` is `io_error` with **no** `error.detail`; a script must branch on `error.kind` for this one | 74 |
...
| `--env <NAME\|KEY[:TYPE[:SEP]]=VALUE>` | — | Set an environment variable for this invocation only. Repeatable; later occurrences win over earlier ones for the same key. Splits on the **first** `=`, so `--env FOO=a=b` yields `FOO` -> `a=b`. `TYPE` is `constant` (replaces, the default when omitted), `path` (prepends), or `list` (appends); `SEP` qualifies `list` only (`--env GODEBUG:list:,=gctrace=1`) and, if omitted, inherits whatever separator another contributor to the key already declared, or a single space if none did. A relative `path` value resolves against the **current directory**. Applied last, so it overrides every package-declared variable. This is a per-invocation override, not project configuration -- it does **not** make this command read `ocx.toml`. A bare `--env NAME`, with no `=` and no `:TYPE`, passes the value through: the invoking process's own `NAME` becomes a `constant` entry, exactly as `--env NAME="$NAME"` would, so a calling script names what may travel instead of spelling every value out. A `NAME` the invoking process does not set contributes nothing and is not an error; a `NAME` set to the empty string forwards the empty string. It is the form that makes such an allowlist usable under `--clean`, where the child environment otherwise starts empty. A `TYPE` written without a value (`--env FOO:path`) is rejected -- only the plain bare form passes a value through. So are a `TYPE` that names no modifier or is empty, a `SEP` that is empty, contains `=`, contains a newline or carriage return, qualifies a non-`list` type, or edges a `list` value, an invalid variable name, and an `OCX_*`/`__OCX_*` key in either form. All exit 64. See the `PATH` override warning under [`ocx exec`](#exec). | — |
...
[env-ocx-signing-key]: ./environment.md#ocx-signing-key
