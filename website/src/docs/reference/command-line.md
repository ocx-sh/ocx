...
`clean` holds a store-wide GC lock for the sweep, and `install`/`pull` hold a shared lock while they stage objects, so concurrent OCX commands on the same `$OCX_HOME` are serialized automatically — an install never loses an object to a concurrent `clean`. As a second guard, objects newer than [`OCX_GC_GRACE_SECONDS`][env-ocx-gc-grace-seconds] (default 600) are always retained.
...
Entry-point launchers generated by `ocx install` call the internal `ocx launcher exec '<pkg-root>' -- <argv0> [args...]` subcommand, not `ocx exec`. That subcommand validates the package root, forces the self view internally, and executes the resolved entrypoint. See the [Entry Points][entry-points] guide for the launcher ABI. On Windows, the native `.exe` shim makes this call without routing through `cmd.exe`, closing the `%*` argument-injection surface for default resolution.
:::
