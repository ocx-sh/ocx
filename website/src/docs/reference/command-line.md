...
`--identifier` names the local test subject and stays required). The file is opened only when
something is missing, so an invocation that states everything never touches it.
