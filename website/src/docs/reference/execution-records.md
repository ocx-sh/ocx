...
`sh.ocx.resolved-from` answers a narrower question than its name suggests: it says a tag was present in the resolved *request*, not that the identity remains free to move. [The local index is never updated automatically][in-depth-indices-local] — once a tag's digest is committed there, a later invocation naming the same tag reads that committed pin rather than re-walking the registry, so most invocations carrying this marker are re-reading an already-fixed digest, not one still in motion. Only the first resolution of a tag the index has not yet seen genuinely walks the registry to learn a new digest; a patch companion's tag is kept for the same reason and is resolved by digest, never re-walked. A package installed ten minutes ago from a tag is still tag-derived, so it still carries the marker regardless of elapsed time or cache state — that stability is deliberate, not an oversight: tying the marker to cache state would make it blink in and out between two identical invocations, which an audit record must not do. A policy that wants "was anything pulled just now" reads `autoInstalled`; one that wants "was this identity reached by name rather than digest" reads `sh.ocx.resolved-from` — neither answers "could this identity still move," which depends on whether the index has committed a pin for it, a fact this record does not carry.

Had the identifier been digest-pinned instead, **both** would be absent and the purl would carry no `tag` qualifier — a tag appears only when one was actually resolved, never synthesized. A project-tier `ocx exec` is the same case for a different reason: `ocx.lock` stores no tags at all, so a lock-resolved package never carries the marker.
...
**Any `user:password@` userinfo is stripped from both endpoints before they reach the record.** A mirror URL with an embedded credential — a mainstream Artifactory and Nexus remote-repository idiom that OCX accepts — is recorded as the bare endpoint it rewrites to, so the field keeps what an auditor needs (which host the traffic went to) without publishing the credential into every record on the fleet. This is the same reasoning that keeps the invoked command line out of `process`. The credential still works: stripping it here changes what is recorded, not what is sent.

`resolution.insecureRegistries` lists which of `resolution.registries`' hosts were declared plain-HTTP for this resolution — an [`insecure = true`][config-registries-insecure] entry, or a host named in [`OCX_INSECURE_REGISTRIES`][env-ocx-insecure-registries]. It is a security-relevant fact about where the recorded bytes actually came from, and it follows the same "state a fact, never assert its absence" rule as every other optional `resolution.*` field: present, listing every plain-HTTP host, only when at least one of this resolution's registries was insecure; **absent** when none were.

`resolution.frozen` mirrors [`--frozen`][arg-frozen], and is scoped exactly as narrowly as that flag: `--frozen` freezes **package-tier** resolution only, so a patch companion in force during the same invocation still resolves live and pins independently under `$OCX_HOME/state/patch-companions/` — freezing the patch tier is a separate, deliberate act ([`ocx patch freeze`][cmd-patch-freeze] plus `OCX_PATCH_SNAPSHOT`). `resolution.frozen: true` says the package closure could not have added a new pin during this run; it says nothing about whether the patch tier was frozen too.

`resolution.noVerify` is `true` when [`OCX_NO_VERIFY`][env-ocx-no-verify] disabled the install-time signature gate for this invocation — the env-tier opt-out only. The per-command `--no-verify` flag is a one-shot user choice that never reaches a launching frame, so no launching frame can report it. It is a **posture**, never an outcome: verification happens at install, not at exec, and a warm store serves a cache hit without consulting the verifier at all, so the record never claims a package *was* verified — only whether the gate that would have covered it was switched off for this run. Reading `noVerify: false` says nothing disabled the gate; whether any individual package actually triggered it is an install-time fact this record does not carry.

A project-tier `ocx exec` record carries two annotations no OCI-tier `package exec` record can: `sh.ocx.binding` names the `ocx.toml` binding the package was selected under, and `sh.ocx.group` names which `[group.<name>]` supplied it (`default` for the top-level `[tools]` table). Its `scope` block also differs — `tier` is `"project"`, and `lock` names the `ocx.lock` the closure was resolved through:
...
| `process.pid`, `process.working_directory`, `host.name` | [ECS][ecs-process] and [OTel][otel-host] agree | Both standards name these fields the same way |
...
A record answers "which exact digests ran." It says nothing about what is *inside* those digests — that is a separate question, and [`ocx package sbom`][cmd-package-sbom] answers it. The two chain through a key the record already carries, with no format change on either side:

```
execution record  →  "these exact digests ran"
        │ packages[].digest.sha256 is the join key
        ▼
ocx package sbom <digest>  →  "and here is what is inside them"
```

The join is deliberately a digest, never a name or a purl. [`ocx package sign`][cmd-package-sign] and [`ocx package attest`][cmd-package-attest] bind every signature and attestation to the **manifest digest alone** — the verification pipeline hardcodes the `sha256` algorithm and ignores the subject's `name` field entirely, so a name- or purl-keyed join would match on a field the pipeline never treats as binding. `packages[].digest.sha256` in a record is that same digest, so it is already the correct join key, not merely a convenient one.

To confirm what ran was also what was approved, read the digest out of a record and hand it to [`ocx package sbom`][cmd-package-sbom] or [`ocx package verify --attestation`][cmd-package-verify-attestations]:

```sh
digest=$(jq -r '.packages[0].digest.sha256' /var/log/ocx/records/*.json)
ocx package sbom "registry.example/cmake@sha256:${digest}" --summary
ocx package verify --attestation -p linux/amd64 "registry.example/cmake@sha256:${digest}"
```

## Two records per launcher invocation {#execution-records-frames}

An entrypoint launcher re-enters OCX (`ocx exec cmake -- cmake …` resolves `cmake` against the composed `PATH`, hits the package's generated launcher, and that launcher calls the hidden [`ocx launcher exec`][cmd-launcher-exec] subcommand). Each of the two frames records — deliberately, not as a defect — because they see different halves of the truth:
...
|---|---|---|
| `executable["sh.ocx.kind"]` | `shim` — nothing has downloaded yet | `binary` — the now-materialized leaf executable |
...
```json
"resolution": {
  "patchSnapshot": { "sha256": "e2f4a6b8c0d1e3f5a7b9c1d3e5f7092c4d6e8f0a2b4c6d8e0f2a4c6e8f0a2c4e" }
}
```

The key is **absent** when no patch snapshot is in force, rather than null or empty — a run resolving patch companions live, with no frozen pin, carries no `patchSnapshot` key at all.

`resolution.frozen` does not cover the patch tier — see [above][execution-records-format]: it names the package-tier freeze only, and a patch companion pinned by `resolution.patchSnapshot` can be in force during a run where `resolution.frozen` is `false`, or vice versa. The two freezes are independent facts, each carried by its own field.
...
**A record is not signed, and the sink is not authenticated.** Any principal who can write to the sink directory can create a file that parses as a record and claims any package digest, any executable, and any user name. On a single-writer sink that costs nothing. On a shared sink, the residual integrity signals are the file's own mode and ownership: OCX creates every record `0600` under the invoking user, and `process.user.id` comes from the kernel rather than the environment. A collector on a shared sink should reject a record whose owning uid disagrees with its `process.user.id`, and should treat `process.user.name` — read from `$USER`/`$LOGNAME` — as an informational display field throughout.
...
`dir` and `name` resolve through the usual four-layer fold — [`[records]`][config-records] config → [`OCX_RECORDS_DIR`][env-ocx-records-dir] / [`OCX_RECORDS_NAME`][env-ocx-records-name] → `--records-dir` / `--records-name` on [`ocx exec`][cmd-exec] or [`ocx package exec`][cmd-package-exec] — with the highest layer winning per field.

A relative `[records] dir` set in a config file is anchored at **the declaring file's own directory**, the same rule [`[trust.sigstore] trusted_root`][config-trust-sigstore] follows — so `/etc/ocx/config.toml` and `$OCX_HOME/config.toml` each mean their own value regardless of the process's current working directory. `--records-dir` and [`OCX_RECORDS_DIR`][env-ocx-records-dir] are **not** anchored this way and stay resolved against the process's working directory, matching every other CLI-flag and env-var path in OCX. A repository's own project-tier `ocx.toml` cannot set `[records]` at all — the section is stripped there the same way `[shell]` is, so a cloned repository can never redirect the audit trail for whoever builds it.
...
[ecs-os]: https://www.elastic.co/docs/reference/ecs/ecs-os
...
[cmd-patch-freeze]: ./command-line.md#patch-freeze
[cmd-lock]: ./command-line.md#lock
[cmd-package-sbom]: ./command-line.md#package-sbom
...
[execution-records-frames]: #execution-records-frames
