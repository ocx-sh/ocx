...
OCX's Rust workspace builds twice: once through `cargo`, and once through a second, parallel graph that [Bazel][bazel] walks to check the first one honestly. Eight gates run in two lanes. Seven of them run on every change; the eighth, the acceptance suite, runs on the deep lane only. Four read the graph itself — a pin check, a loading-phase sanity build, a dependency-drift comparison against `Cargo.toml`, and a sandbox-tag audit. Two lint what declares it: `bazel:lint` runs buildifier over every `BUILD.bazel` and `.bzl` file, and `bazel:mod:check` refuses a stale `MODULE.bazel.lock`. The last two run tests: `bazel:test:unit` runs the workspace's own unit tests, floored per target off the build event stream rather than a bare pass/fail, and `bazel:test:accept` runs the acceptance suite as one target per test module. `bazel:test:unit`'s targets include, one per library crate, a `rust_doc_test` — so the workspace's doctests run here too, never as a separate cargo step. `bazel:test:accept` is the one that is not on every change: `verify-basic.yml` — the only workflow a pull request fires — runs the other seven, and the acceptance gate lives in `verify-deep.yml`, which has no `pull_request` trigger. Locally `task verify` runs all eight. Together they catch the class of defect a green `cargo build` cannot see on its own: a `BUILD.bazel` target that quietly stopped matching the crate it describes.

The same graph backs three more checks. `rust:clippy:check` runs clippy as a cached Bazel aspect. `rust:doc:ratchet` does the same for rustdoc. Both compare their diagnostics against a per-file baseline instead of failing on `-D warnings`. `schema:generate` builds the JSON schemas from the `//crates/ocx_schema:schemas` genrule. So `task verify` runs none of `cargo build`, `cargo clippy`, `cargo doc` or `cargo test --doc`.
...
That gate also writes a [Build Event Protocol][bazel-bep] stream. Four tasks write one: `bazel:build:nobuild`, `bazel:test:unit`, `rust:clippy:check` and `rust:doc:ratchet`. Each writes to its own 0700 `mktemp -d` directory outside the checkout and removes it on exit. Only the `bazel:build:nobuild` stream is exported to `otel.ocx.sh`. Bazel serialises the whole client environment and every rc-file flag value into that stream, credentials included, so the directory is deleted whatever the build did.
