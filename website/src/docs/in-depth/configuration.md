...
None of that gates the setup-time re-sync described above. `ocx config setup` and `ocx self setup --managed-config` reconcile an already-adopted seed inside CI, non-interactively, and regardless of [`OCX_NO_CONFIG_REFRESH`][env-ocx-no-config-refresh] — the only things that skip it are [`--offline`][arg-offline]/[`OCX_OFFLINE`][env-offline], a digest-pinned seed (content-addressed, so nothing to converge toward), and an in-force [pause][cmd-config-update]. That re-sync is also best-effort rather than tick-like: a fetch failure keeps the existing snapshot and exits 0 instead of degrading silently to a no-op probe.
