...
- **No SCT / certificate-transparency check.** [Fulcio][fulcio]'s Signed Certificate Timestamp is not verified; nothing in the verify path checks CT-log inclusion for the leaf certificate. This mirrors the sign side: OCX's Fulcio client speaks the plain `publicKeyRequest` shape rather than the SCT-mandating one, because the in-repo fake stack cannot mint an SCT — so today's certificates carry none to check.
- **No certificate extension checks on the trust root.** `verify_cert_chain` only checks that the leaf's signature verifies against one of the trust root's public keys; it does not check `basicConstraints` (CA:true) or `keyUsage` (keyCertSign) on the signing certificate. Any P-256 certificate present in the trust root can therefore sign a leaf, not only ones marked as a CA.
