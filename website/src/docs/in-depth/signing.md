...
6. **The public-good root over TUF** — with no override, no configured root and no cache, `TrustRoot::load_embedded` fetches and verifies the [Sigstore TUF][sigstore-tuf] trust root through [sigstore-rs][sigstore-rs]'s `tough`-backed client, dialed over OCX's shared Sigstore HTTP client — the same one Fulcio and Rekor calls use, extra CA roots included — caching the TUF metadata under `$OCX_HOME/state/tuf/`. This is the default for packages signed against public Sigstore, and it needs network — `--offline` never reaches it.
