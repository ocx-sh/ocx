...
**Bazel.** Bazel 9.2.0 is the graph over the same crates cargo sees, and in `task verify` cargo remains only for fmt, deny and about/metadata: clippy (`rust:clippy:check`), the rustdoc ratchet (`rust:doc:ratchet`), schema generation, the build, unit tests and doctests are Bazel actions. Besides `rust:clippy:check`, `rust:doc:ratchet` and `rust:build`, `task verify` runs eight Bazel gates — `bazel:pin:check` in phase 1, then `bazel:build:nobuild` → `bazel:build:drift` → `bazel:tag:guard` → `bazel:lint` → `bazel:mod:check` → `bazel:test:unit` (doctests included) → `bazel:test:accept` in phase 2. `bazel:test:accept` closes the phase where `task test:parallel` used to: the acceptance suite as one `sh_test` target per `test/tests/test_*.py` module (172 today — check the live `ACCEPTANCE_MODULE_TARGETS` in `scripts/bazel_gate_proofs.py`, it only falls as modules move to `test/lint/` or are ported down), **with results cached** — so an unchanged tree executes only the uncached residual below, an edited module re-runs itself plus any module that imports it (`module_data`), and an edit to a shared input in `//test:suite_inputs` (`test/pyproject.toml`, `test/uv.lock`, `test/ocx.toml`/`ocx.lock`, `docker-compose.yml`, `zot-config.json`, `test/sigstore/**`, `test/src/**`, the `test/tests/` helpers) re-runs every target, as does a Rust edit that changes the bytes of the binary under test — Bazel's own `//crates/ocx_cli:ocx`, a `data` input of every target (`task test`'s `test/bin/ocx` is a copy of it for the xdist path; cargo no longer builds it). The targets **run concurrently** (`--local_test_jobs` on the task's command line, never an rc file) at xdist parity through the runner's host locks (`test/bazel.bzl` § Concurrency): every `xdist_group` a module names needs `module_slots` in `test/BUILD.bazel`, and a file only some modules read goes in their `module_data`, not `:suite_inputs` — `bazel:tag:guard` reds a missing slot and a missing read it can derive (reads no AST sees are hand-listed in its `HAND_DECLARED_READS`, and a file one read implies — root `ocx.lock` beside `ocx.toml` — in `IMPLIED_READS`; a module that launches `task` or names a `test/` path outside `:suite_inputs` reds `tag-hand-read-unreviewed` until it has a `HAND_DECLARED_READS` entry). The structural sweeps that used to force a sibling re-run on every edit (`test_smoke_coverage`, `test_no_crate_path_assertions`, `test_patch_global_slot`, and `test_doc_scripts_publish`'s sweep, now `test/lint/test_doc_scripts_publish_structure.py` — its acceptance module still exists, only the sweep moved) moved to the uncached T0 lint tier (`test/lint/`, `task test:lint:structure`; `subsystem-tests.md` § Verification tiers) instead of being swept into every acceptance run via `extra_data`, which is deleted — no caller was left once the sweeps moved. A module on `test/bazel.bzl`'s `UNCACHED_MODULES` still re-runs regardless of cache (one named residual today: `tests/test_windows_shim.py`). It carries `test/SUITE_FLOOR` and the skip/xfail ceilings with it, read off the per-case JUnit reports it copies into `target/bazel/accept/`. `task test:parallel` remains the local xdist convenience. Two others are A7's lint lines: `bazel:lint` is buildifier over every `BUILD.bazel` and `.bzl` file, decided on `//:buildifier.check`'s exit code, and `bazel:mod:check` is `bazel mod deps --lockfile_mode=error`, the one place that flag appears — `MODULE.bazel.lock` must already be fresh. `bazel` is the one tool a plain shell cannot reach: every call goes through `ocx exec bazel -- <cmd>`, including a bare `bazel query` you run by hand, or the gates exit 1 on their reader floors (DX-42). `.bazelversion` and `MODULE.bazel` are pinned and `bazel:pin:check` is the authority over both. None of those gates reads the *host*, which is where a first build actually fails: `task bazel:doctor` does that (toolchain pin, the `~/.bazelrc` host block, the libstdc++ link prerequisite, the cache reader credential, the caches), and `/init-bazel-config` is the same script plus the prompts it cannot answer itself. Host state belongs in `~/.bazelrc`, never `.bazelrc.user` — this machine carries a worktree per agent task and a per-checkout file has to be re-created in each one. The decision is [`adr_bazel_build_adoption.md`](./.claude/artifacts/adr_bazel_build_adoption.md); the ruleset is [bazel-quality.md](./.claude/rules/bazel-quality.md).
